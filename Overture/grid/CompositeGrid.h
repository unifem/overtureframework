...
class CompositeGridData:
  ...
  public:
    ...
    enum {
      THEinterpolationCoordinates = ENDtheGridCollectionData,         // bit 25
      THEinterpoleeGrid           = THEinterpolationCoordinates << 1, // bit 26
      THEinterpoleeLocation       = THEinterpoleeGrid           << 1, // bit 27
      THEinterpolationPoint       = THEinterpoleeLocation       << 1, // bit 28
      THEinverseMap               = THEinterpolationPoint       << 1, // bit 29
...
//    RealArray                    interpolationConditionLimit;
    ...
    RCVector<CompositeGrid>             domain;         // (overloaded)
#else
    ...
    ListOfCompositeGrid                 domain;         // (overloaded)
#endif // USE_STL
    ...
    // Here are the new interpolation data arrays that are serial arrays -- these hold the
    // interpolation data for the interpolation points on a given processor
    ...
    LocalInterpolationDataEnum localInterpolationDataState;  // inidicates which grids use local arrays
    ...
    IntegerArray interpolationStartEndIndexLocal;      // for interp. pts ordered by interpolee grid

    Partitioning_Type           partition;             // for interpolation arrays 031016
    ...
    //
    // surface stitching mesh
    //
    UnstructuredMapping *surfaceStitching;
...
// Return the number of possible multigrid levels supported by the current numbers of grid lines
...
//
    ...
    // assign overlap parameters such as interpolationWidth etc.
    int setOverlapParameters();
    int setOverlapParameters(CompositeGrid & cg);   // base parameters on this CompositeGrid.
...
//
    ...
    UnstructuredMapping * getSurfaceStitching() { return rcData->surfaceStitching; }
