...
class GenericGridCollectionData:
  ...
  public:
    enum {
      ...
      THEdomain                       = THEmultigridLevel  << 1, // bit 24 -- added 060815 *wdh*
      ENDtheGenericGridCollectionData = THEdomain          << 1, // bit 25
    ...
    RCVector<GenericGridCollection> domain;
    ...
    IntegerArray                    domainNumber;
#endif // USE_STL
    std::vector<aString> domainName;
    bool keepGridDistributionOnCopy;  // used to copy a grid collection to a different parallel distrubution.
    GridDistributionList gridDistributionList;
    LoadBalancer *pLoadBalancer;
    ...
    const aString & getDomainName(int domain) const;
...
//  Constants to be ORed to form the first argument of update() and destroy():
    enum {
      ...
      THEdomain          = GenericGridCollectionData::THEdomain,
...
//  Collections of grids having the same domain
    RCVector<GenericGridCollection>       domain;
...
//
    inline const Integer& numberOfDomains() const
      { return rcData->numberOfDomains; }
//
...
//  Specify whether to keep the parallel distribution when copying gridCollections.
