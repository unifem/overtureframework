...
int 
outputMovingGrids(CompositeGrid & cgNew, 
		  CompositeGrid & cgOld, 
		  const LogicalArray & hasMoved )
// =======================================================================================
// /Description:
//   This function will output the CompositeGrid's that were used in a call to movingUpate.
// These grids can be used for testing the movingUpdate function.
//
// ========================================================================================
{
  printF("*** outputing CompositeGrid's cg and cgOld for movingUpdate ****\n");
  display(hasMoved,"hasMoved");
  
  for( int gg=0; gg<=1; gg++ )
  {
    CompositeGrid & cg = gg==0 ? cgNew : cgOld; 
    aString gridFileName = gg==0 ? "movingGridNew.hdf" : "movingGridOld.hdf";
      
    CompositeGrid c; // this will hold the base grids

    if( cg.numberOfRefinementLevels()>1 )
    {
      // Remove refinement levels since these are not used in the movingUdate
...
//\begin{>>ogenUpdateInclude.tex}{\subsubsection{movingUpdate: optimized overlap computation for moving grids}}
int Ogen::
movingUpdate(CompositeGrid & cg, 
	      CompositeGrid & cgOld, 
	      const LogicalArray & hasMoved, 
...
// ========================================================================================
{
  ...
  return movingUpdateNew(cg,cgOld,hasMoved,option);
#else
...
//    debug=7;
  ...
  for( grid=0; grid<numberOfBaseGrids; grid++ )
  {
	...
	{
	  if( mask(ip(i,0),ip(i,1),i3) & MappedGrid::USESbackupRules ) 
	  {
	    mask(ip(i,0),ip(i,1),i3) ^= MappedGrid::USESbackupRules;  // exclusive or to turn off USESbackupRules
	  }
	}
      }
      else
	...
	for( int i=0; i<numberOfInterpolationPoints; i++ )
	{
	  if( mask(ip(i,0),ip(i,1),ip(i,2)) & MappedGrid::USESbackupRules ) 
	  {
	    mask(ip(i,0),ip(i,1),ip(i,2)) ^= MappedGrid::USESbackupRules;  // exclusive or to turn off USESbackupRules
	  }
	}
      }
...
---- */
  ...
  intArray *iInterp = new intArray [ numberOfBaseGrids ];
  intArray *interpNew = new intArray [ numberOfBaseGrids ];

  intArray ia2(maxInterp,3); // *** fix this
  ia2=0;
  intArray useBackupRules(maxInterp);  

  intArray interpolates(maxInterp);
  useBackupRules=FALSE;
  
  // These next lines also appear in classifyPoints
  assert( backupValues==NULL );
  backupValues = new intArray [ numberOfBaseGrids ];  // to hold backup values
...
// **  const int numberOfBaseGrids = cg.numberOfBaseGrids();
  ...
  for( grid=0; grid<numberOfBaseGrids; grid++ )
  {
    ...
    const realArray & center = !isRectangular ? c.center() : r;

    intArray & mask = c.mask();
    intArray & ia = iInterp[grid];
    
    // intArray & inverseGrid = cg.inverseGrid[grid];
    // realArray & rI = cg.inverseCoordinates[grid];   // may not exist

    intArray & interpolationPoint = cg.interpolationPoint[grid];
    ...
    i3 = c.extendedIndexRange(Start,axis3);  // default value for 2D
	...
	intArray iag[6];   // could *new* to be 2*nd
	realArray rg[6], xg[6];
        if( debug & 1 ) printf("updateBoundaryAdjustment for grid=%i grid2=%i\n",grid,grid2);
...
// 	endIndex++;
	...
	{ 
	  ...
	  {
	    if( numberOfDimensions==2 )
	      x(R,axis)=center(ia2(R,0),ia2(R,1),i3,axis);
	    else
...
// 	  }
	  ...
	  intArray ib;
          intArray m; m = interpolates(R)==0;   // ***** fix this ****
  ...
  if( !ok )
  {
	...
	if( numberOfDimensions==2 )
	{
	  ...
	  inverseGrid(ip(R,0),ip(R,1),i3)=cg.interpoleeGrid[grid];
	  for( axis=0; axis<numberOfDimensions; axis++ )
	    rI(ip(R,0),ip(R,1),i3,axis)=cg.interpolationCoordinates[grid](R,axis);
	}
	else if( numberOfDimensions==3 )
	{
	  inverseGrid(ip(R,0),ip(R,1),ip(R,2))=cg.interpoleeGrid[grid];
	  for( axis=0; axis<numberOfDimensions; axis++ )
	    rI(ip(R,0),ip(R,1),ip(R,2),axis)=cg.interpolationCoordinates[grid](R,axis);
	}
	else
	{
	  throw "error";
    ...
    for( grid=0; grid<numberOfBaseGrids; grid++ )
    {
      ...
      for( axis=0; axis<numberOfDimensions; axis++ )
      {
	periodic[axis]=c.isPeriodic(axis);
        period[axis]=c.gridIndexRange(End,axis)-c.gridIndexRange(Start,axis);
      }

      intArray & mask = c.mask();
      intArray & ia = iInterp[grid]; 
      intArray & ian = interpNew[grid]; 
	...
	for( int dir=0; dir<c.numberOfDimensions(); dir++ )
	{
      ...
      const realArray & center = !isRectangular ? c.center() : rI;
...
//                        i1,i2,i3,grid,grid2,j1,j2,j3,inverseGrid2(j1,j2,j3));
	    ...
	    int gridI=inverseGrid2(j1,j2,j3);
        ...
        for( axis=0; axis<3; axis++ )
	{
	  ...
	  if( !periodic[axis] )
	  {
	    jvMin[axis]=max(iv[axis]-width,c.extendedIndexRange(Start,axis));
	    jvMax[axis]=min(iv[axis]+width,c.extendedIndexRange(End  ,axis));
	  }
	  else
	  {
	    jvMin[axis]=max(iv[axis]-width,c.dimension(Start,axis));
	    jvMax[axis]=min(iv[axis]+width,c.dimension(End  ,axis));
	  }
	...
	else // grid2 < grid 
	{
	  ...
	  if( expand )
	  {
	    ...
	    // bool ptAdded=false;
	    intArray & mask2 = cg[grid2].mask();
	    intArray & ian2 = interpNew[grid2];
	    intArray & inverseGrid2 = cg.inverseGrid[grid2];
        ...
        if( !ptok && canDiscretize(cg[grid], iv) )
	{
	  if( debug & 1 ) 
	    printf(" ...pt (%i,%i,%i) grid=%i can be used as a discretization pt\n",i1,i2,i3,grid);
          ptok=true;
          cg[grid].mask()(ia(i,0),ia(i,1),ia(i,2))=MappedGrid::ISdiscretizationPoint;
  ...
  else
  {
    ...
    for( grid=0; grid<numberOfBaseGrids; grid++ )
    {
      ...
      intArray & interpoleeLoc = cg.interpoleeLocation[grid];
      const intArray & interpoleeGrid = cg.interpoleeGrid[grid];
      const realArray & interpolationCoord = cg.interpolationCoordinates[grid];
      
      cg.interpolationStartEndIndex=cgOld.interpolationStartEndIndex;
      cg.variableInterpolationWidth=cgOld.variableInterpolationWidth;
