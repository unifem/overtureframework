// This file automatically generated from cutHolesNewer.bC with bpp.
#include "Overture.h"
#include "Ogen.h"
#include "display.h"
...
int Ogen::
cutHolesNewer(CompositeGrid & cg)
// =======================================================================================================
...
// =======================================================================================================
{
    real time0=getCPU();
  //  info |= 4;
  // debug=7;   // *WDH* 2012/07/07

    if( debug & 1 || info & 4 )
    {
        printF("--- cutHolesNewer: START of parallel hole cutting ---\n");
    }
  ...
  // We plot any suspicious discretization points that are too close to hole points as 'orphan' points
    ...
    if( debug & 4 ) fprintf(plogFile,"cutting holes with physical boundaries...\n");
...
//  const real maxDistFactor=SQR(2.); // cut holes within 2*( cell diagonal length of cutee grid ) 
    const real maxDistFactor=SQR(3.); // cut holes within 3*( cell diagonal length of cutee grid ) // *************** TEMP ******
  ...
  // *************************
    ...
    if( debug & 8 )
    {
        for( int grid=0; grid<numberOfBaseGrids; grid++ )
        {
            ...
            fprintf(plogFile," myid=%i grid=%i, local bounds with ghost= [%i,%i][%i,%i][%i,%i]\n",
           	     myid,grid,mask.getBase(0),mask.getBound(0),mask.getBase(1),mask.getBound(1),
  ...
  //
  //  xb(side,axis,grid)  : holds grid points for the ENTIRE boundary - we assume this array fits on
  //                        on single processor (we will need to fix this eventually)
  // 
  // --------------------------------------------------------------------------------
...
#define xBoundary(side,axis,grid) (*pxBoundary(side,axis,grid))
    ...
    for( grid=numberOfBaseGrids-1; grid>=0; grid-- )
    {
        ...
        // Note: do not cut holes with singular sides
          ...
          // -- Iv[0..2] : index bounds on the face=(side,axis) of grid=grid includes ghost pts in tangential direction --
        	  getIndex(g.dimension(),I1,I2,I3); 
                    Iv[axis]=g.gridIndexRange(side,axis);  // boundary Index
...
//   #endif
  ...
  // **********************************************************************
    ...
    // const realArray & vertex = g.vertex();
        ...
        {
            isPeriodic[dir]=g.isPeriodic(dir)==Mapping::functionPeriodic;
        }
...
// 	  }
	  ...
	  // no: getBoundaryIndex(extendedGridIndexRange(g),side,axis,I1,I2,I3);   // note: use gridIndexRange
          ...
          // **********************************
            ...
            " mayCutHoles=%i mayInterpolate=%i isNew(grid)=%i isNew(grid2)=%i mapIntersects=%i\n",
            grid,(const char*)g.getName(),side,axis,grid2,(const char*)g2.getName(),
            cg.mayCutHoles(grid,grid2),cg.mayInterpolate(grid,grid2,0),isNew(grid), isNew(grid2),
		...
		//       side,axis,grid,shareFlag,grid2);
		  ...
		  // grid2 has the same share flag as this face of grid.
              ...
              //                     perform a periodic wrap of the points. 
            	      ...
            	      if( debug & 8 )
            	      {
            		displayMask(mask2,sPrintF("mask for grid2=%i before cut holes by grid=%i",grid2,grid),plogFile);
            	      }
              ...
              // --------------------------------------------------------------------------------------------
              //   Make a list of points on the cutting face of grid that are in the bounding box of grid2
              // no need to cut holes with points that already interpolate from this grid!
              // -------------------------------------------------------------------------------------------

                            RealArray & xb = xBoundary(side,axis,grid);

              // *new* way *wdh* 110627 -- do not extend boundingBox -- rather extend the cutMask
                            RealArray boundingBox;
            	      boundingBox=g2.localBoundingBox();
            	      if( debug & 2 )
            	      {
            		fprintf(plogFile," ########## cutHoles: use local bounding box: grid2=%i "
                  			"box=[%8.2e,%8.2e][%8.2e,%8.2e][%8.2e,%8.2e] ##########\n",
                  			grid2,boundingBox(0,0),boundingBox(1,0), boundingBox(0,1),boundingBox(1,1), 
                                                            boundingBox(0,2),boundingBox(1,2) );
            	      }

            	      
// 	      if( useLocalBoundingBoxes )
// 	      { // *wdh* 080930 
//                 boundingBox=g2.localBoundingBox();
// 		if( debug & 2 )
// 		{
// 		  fprintf(plogFile," ########## cutHoles: use local bounding box: grid2=%i "
//                           "box=[%8.2e,%8.2e][%8.2e,%8.2e][%8.2e,%8.2e] ##########\n",
// 		       grid2,boundingBox(0,0),boundingBox(1,0), boundingBox(0,1),boundingBox(1,1), boundingBox(0,2),boundingBox(1,2) );
// 		}
...
// 	      }
                            ...
                            real delta = .01*max( boundingBox(End,Rx)-boundingBox(Start,Rx) );
              // real delta = .2*max( boundingBox(End,Rx)-boundingBox(Start,Rx) );
                            for( int dir=0; dir<numberOfDimensions; dir++ )
              	      {
              		boundingBox(Start,dir)-=delta;
              ...
              // ----------------------------------------------------------------------------------
                            ...
                            intSerialArray cutMask(I1,I2,I3);  // in parallel we don't need to make this so big
            		...
            		if( numberOfDimensions==2 )
            		{
              		  ...
              		  {
                		    cutMask(i1,i2,i3)=
                                                        (xb(i1,i2,i3,axis1)>boundingBox(0,axis1) && xb(i1,i2,i3,axis1)<boundingBox(1,axis1)&&
...
// 	      }
              ...
              // Extend the cutMask outward by one line of points -- mark these points as cutMask==2
                            int i1m,i1p, i2m,i2p, i3m,i3p;
            	      if( numberOfDimensions==2 )
            	      {
            		if( axis==0 )
            		{ // The cutting boundary is i1=const
              		  FOR_3D(i1,i2,i3,I1,I2,I3)
              		  {
                		    i2m=max(I2Base,i2-1); i2p=min(I2Bound,i2+1);
                		    if( cutMask(i1,i2,i3)==0 && ( cutMask(i1,i2m,i3)==1 || cutMask(i1,i2p,i3)==1 ) )
                		    {
                  		      cutMask(i1,i2,i3)=2;
                		    }
              		  }
            		...
            		{ // The cutting boundary is i2=const
              		  FOR_3D(i1,i2,i3,I1,I2,I3)
              		  {
                		    i1m=max(I1Base,i1-1); i1p=min(I1Bound,i1+1);
                		    if( cutMask(i1,i2,i3)==0 && ( cutMask(i1m,i2,i3)==1 || cutMask(i1p,i2,i3)==1 ) )
                		    {
                  		      cutMask(i1,i2,i3)=2;
                		    }
              		  }
            		...
            		{ // The cutting boundary is i1=const
              		  ...
              		  {
                		    i2m=max(I2Base,i2-1); i2p=min(I2Bound,i2+1);
                		    i3m=max(I3Base,i3-1); i3p=min(I3Bound,i3+1);
                		    if( cutMask(i1,i2,i3)==0 && 
                                              ( cutMask(i1,i2m,i3m)==1 || cutMask(i1,i2 ,i3m)==1 || cutMask(i1,i2p,i3m)==1 || 
                                                  cutMask(i1,i2m,i3 )==1 ||                           cutMask(i1,i2p,i3 )==1 || 
                                                  cutMask(i1,i2m,i3p)==1 || cutMask(i1,i2 ,i3p)==1 || cutMask(i1,i2p,i3p)==1  ) )
                		    {
                  		      cutMask(i1,i2,i3)=2;
                		    }
              		  }
            		...
            		{ // The cutting boundary is i2=const
              		  FOR_3D(i1,i2,i3,I1,I2,I3)
              		  {
                		    ...
                		    i3m=max(I3Base,i3-1); i3p=min(I3Bound,i3+1);
                		    if( cutMask(i1,i2,i3)==0 && 
                                              ( cutMask(i1m,i2,i3m)==1 || cutMask(i1,i2 ,i3m)==1 || cutMask(i1p,i2,i3m)==1 || 
                                                  cutMask(i1m,i2,i3 )==1 ||                           cutMask(i1p,i2,i3 )==1 || 
                                                  cutMask(i1m,i2,i3p)==1 || cutMask(i1,i2 ,i3p)==1 || cutMask(i1p,i2,i3p)==1  ) )
                		    {
                  		      cutMask(i1,i2,i3)=2;
                		    }
              		  }
            		...
            		{ // The cutting boundary is i3=const
              		  FOR_3D(i1,i2,i3,I1,I2,I3)
              		  {
                		    i1m=max(I1Base,i1-1); i1p=min(I1Bound,i1+1);
                		    i2m=max(I2Base,i2-1); i2p=min(I2Bound,i2+1);
                		    if( cutMask(i1,i2,i3)==0 && 
                                              ( cutMask(i1m,i2m,i3)==1 || cutMask(i1,i2m,i3)==1 || cutMask(i1p,i2m,i3)==1 || 
                                                  cutMask(i1m,i2 ,i3)==1 ||                          cutMask(i1p,i2 ,i3)==1 || 
                                                  cutMask(i1m,i2p,i3)==1 || cutMask(i1,i2p,i3)==1 || cutMask(i1p,i2p,i3)==1  ) )
                		    {
                  		      cutMask(i1,i2,i3)=2;
                		    }
              		  }
            	      ...
            	      if( info & 4 )
            		display(cutMask,"cutMask (on cutting face ) 1=inside local bounding box of g2, 0=outside, 2=next to 1",
                  			plogFile,"%2i");
              ...
              // adjust boundary points on shared sides *** x is changed **
            		...
            		if( debug & 2 )
            		{
                                    fprintf(plogFile,"After inverseMap for boundary cutting points: (cut grid2=%i with grid=%i)\n",grid2,grid);
              		  for( int i=R1.getBase(); i<=r1Bound; i++ )
              		  {
            	      ...
            	      for( int dir=numberOfDimensions; dir<3; dir++ )
            	      {
              ...
              // ********************************************************************************************
            	      ...
            	      if( numberToCheck>0 )
            	      {
            		holeMask.redim(I1,I2,I3);     // in parallel we don't need to make this so big
            		holeMask=0;
            	      }
                            real dr=0.;
                  ...
                  // **** fix this -- local mask is not defined everywhere!
               		   ...
               		   if( numberOfWarningMessages<1 )
               		   {
                 		     numberOfWarningMessages++; 
                                          if( debug & 2 )
                   		       printF("Ogen::cutHoles: fix me for parallel and mixed-boundaries\n");
               		   }

                   // holeMask(i1,i2,i3)=1;  // point is inside.
                                      holeMask(i1,i2,i3)=cutMask(i1,i2,i3);  // point is inside.  (2 means pt is outside local bounding box)
                                    #endif
...
// 			       myid,numberCut,k1,k2,k3);
			      ...
			      // cutShare2(k1,k2,k3)=share;
                      			    ...
                      			    if( debug & 4 )
                        			      fprintf(plogFile,"---- Cutting a hole %i on grid2=%i at (%i,%i,%i) share=%i \n",
                              				      numberCut-1,grid2,k1,k2,k3,share);
		    ...
		    // If the previous point was inside 
              ...
              // In parallel we must check on all processors since there are calls to inverseMap and map 
            	      ...
            	      if( debug & 8 )
            	      {
            		displayMask(cg[grid2].mask(),
                      			    sPrintF("mask for grid2=%i after cut holes by grid=%i, before un-cut holes",
                            				    grid2,grid),logFile);
            	      }
            	      if( debug & 8 )
            	      {
            		displayMask(mask2,sPrintF("mask for grid2=%i after cut holes by grid=%i, before un-cut holes",
                                					  grid2,grid),plogFile);
            	      }
		...
		// ====== now double check the points we cut out  ==========
            ...
            // 		checkPointsMarkedAsHoles();
              ...
              // determine xB -- the closest pt on the boundary 
                            ...
                            if( debug & 4 )
                            {
                                fprintf(plogFile,"\n ***checkPointsMarkedAsHoles: grid=%i grid2=%i side=%i axis=%i Call mapS for xB numberCut=%i***\n\n",
                            ...
                            if( debug & 4 )
                            {
                                fprintf(plogFile,"\n *** checkPointsMarkedAsHoles: grid=%i grid2=%i side=%i axis=%i AFTER mapS for xB numberCut=%i***\n\n",
                                  	    grid,grid2,side,axis,numberCut);
                                fflush(plogFile);
                            }
            //   #ifdef USE_PPP
            ...
            // 		}
                            if( numberCut >0  )   // the remaining code in this section has no communication
                            {
                                if( debug & 4 )
                                {
                                    ...
                                    {
                              	fprintf(plogFile,"potential hole pt: grid2=%i i=%i ia=(%3i,%3i,%3i), r2=(%8.1e,%8.1e,%8.1e)\n",
                                    		grid2,i,ia(i,0),ia(i,1),ia(i,2),
            	  ...
            	  // *wdh* 011027 only change interp if this is a higher priority
                                	  if( inverseGrid(i1,i2,i3)<grid || mask2(i1,i2,i3)!=MappedGrid::ISinterpolationPoint) 
                                	  {
                                  	    ...
                                  	    for( int dir=0; dir<numberOfDimensions; dir++ )
                                            ...
                                            cutShare2(i1,i2,i3)=share;  // *wdh* 2012/03/17    -- is this correct here? or maybe 2 lines above?
                              	}
                              	else if( r2(i,axis)!=Mapping::bogus &&       // point was invertible
                                     		 ( fabs(r2(i,axis  )-.5) <= .5+boundaryEpsilon
                                       		   || (side==0 && r2(i,axis  )>=0. ) ||(side==1 && r2(i,axis  )<=1. )
                     ...
                     // the point (j1,j2,j3) was used to cut this hole -- we need the closest bndry pt.
                      ...
                      // this next search IS wrong in parallel if the inverse is distributed: 
                                            if( !map.usesDistributedInverse() )
                                            {
                                                ...
                                                if( false )
                                                {
                          // double check distToBndry
                                                    real distToBndry2=0.;
                                                    if( isRectangular )
                                                    {
                                              	for( int ax=0; ax<numberOfDimensions; ax++ )
                                                	  distToBndry2+= SQR( xv[ax]-XV(jv,ax) );
                                                    }
                                                    else
                                                    {
                                              	for( int ax=0; ax<numberOfDimensions; ax++ )
                                                	  distToBndry2+= SQR( xv[ax]-xb(j1,j2,j3,ax) );
                                                    }
                                                    fprintf(plogFile,"cutHoles:checkHolePointWithBogusInverse:binarySearchOverBoundary distToBndry=%8.2e distToBndry2=%8.2e\n",
                                                                    distToBndry,distToBndry2);
                                                }
                                            }
                                            else
                                            {
                        // check for the closest pt near jv 
                        // *wdh* 110629 - new way:
                                                if( true ) 
                                                {
                          // fprintf(plogFile,"cutHoles:checkHolePointWithBogusInverse: xb bounds=[%i,%i][%i,%i][%i,%i]\n",
                          //         xb.getBase(0),xb.getBound(0),xb.getBase(1),xb.getBound(1),xb.getBase(2),xb.getBound(2));
                          // Check for a nearer point on the boundary:
                          // *** NOTE: this has trouble with thin bodies since the point that cut the hole may be on
                          // the opposite side of the body!
                                                    distToBndry=REAL_MAX;
                                                    const int widthToCheck=2;  // look for closer pts within this many pts of the starting guess
                                                    const int k1a = max(j1-widthToCheck,xb.getBase(0));
                                                    const int k1b = min(j1+widthToCheck,xb.getBound(0));
                                                    const int k2a = max(j2-widthToCheck,xb.getBase(1));
                                                    const int k2b = min(j2+widthToCheck,xb.getBound(1));
                                                    const int k3a = max(j3-widthToCheck,xb.getBase(2));
                                                    const int k3b = min(j3+widthToCheck,xb.getBound(2));
                                                    int kv[3], &k1=kv[0], &k2=kv[1], &k3=kv[2];
                                                    for( k3=k3a; k3<=k3b; k3++ )
                                                    {
                                              	for( k2=k2a; k2<=k2b; k2++ )
                                              	{
                                                	  for( k1=k1a; k1<=k1b; k1++ )
                                                	  {
                                                  	    real dist=0.;
                                                  	    if( isRectangular )
                                                  	    {
                                                    	      for( int ax=0; ax<numberOfDimensions; ax++ )  // we could do better here - compute distMin directory for rectangular !
                                                    		dist+= SQR( xv[ax]-XV(kv,ax) );
                                                  	    }
                                                  	    else
                                                  	    {
                                                    	      for( int ax=0; ax<numberOfDimensions; ax++ )
                                                    		dist+= SQR( xv[ax]-xb(k1,k2,k3,ax) );
                                                  	    }
                                                  	    if( dist<distToBndry )
                                                ...
                                                { // old way 
                                                    ...
                                                    {
                                              	for( int ax=0; ax<numberOfDimensions; ax++ )
                                                	  distToBndry+= SQR( xv[ax]-XV(jv,ax) );
                                                    }
                                                    else
                                                    {
                                              	for( int ax=0; ax<numberOfDimensions; ax++ )
                                                	  distToBndry+= SQR( xv[ax]-xb(j1,j2,j3,ax) );
                                                    }
                                                }
                                            }
                      ...
                      // jv ---> jpv
                                            ...
                                            real re[3]={0.,0.,0.}, dx[3], det;
                                            for( int ax=0; ax<numberOfDimensions; ax++ )
                                                dx[ax]=x2(i,ax)-xvj[ax];
                    // define XR(m,n) xr((j1),(j2),(j3),(m)+(n)*numberOfDimensions)
                                        #define XR(m,n) xra[n][m]
                                        #ifdef USE_PPP
                                            if( debug & 4 )   // *TEMP* debug & 8
                                            {
                                              fprintf(plogFile,"cutHoles:checkHolePointWithBogusInverse: pt=(%i,%i,%i)\n",ia(i,0),ia(i,1),ia(i,2));
                                              fprintf(plogFile," grid2=%i is being cut by the boundary of grid=%i (side,axis)=(%i,%i)\n",grid2,grid,side,axis);
                                              fprintf(plogFile," The pt we are checking is xv=x2=(%8.2e,%8.2e,%8.2e) on grid2=%i. Closest pt on grid is jv=(%i,%i,%i).\n",
                                                              xv[0],xv[1],xv[2],grid2, jv[0],jv[1],jv[2]);
                                              fprintf(plogFile," The hole was cut with ia(%i,0:2)=(%i,%i,%i), xb(%i,%i,%i)=(%8.2e,%8.2e,%8.2e) \n",i,ia(i,4),ia(i,5),ia(i,6),
                                                 	   jv[0],jv[1],jv[2],xb(jv[0],jv[1],jv[2],0),xb(jv[0],jv[1],jv[2],1),(numberOfDimensions==2 ? 0. : xb(jv[0],jv[1],jv[2],2)));
                                              fprintf(plogFile," Closest pts on the boundary are jv=(%i,%i,%i), jpv=(%i,%i,%i) \n", jv[0],jv[1],jv[2], jpv[0],jpv[1],jpv[2]);
                                                fprintf(plogFile," distance to pt jv is distToBndry=%8.2e\n",distToBndry);
                                            }
                      // just evaluate xr on the boundary from the mapping 
                                            ...
                                            if( numberOfDimensions==2 )
                                            {
                                                ...
                                                else
                                                {
                                                    ...
                                                    re[0]=rr(0,0); re[1]=rr(0,1); re[2]=0.;
                                                    re[axis]+=.1*(2*side-1); // what is this ? 
                                                }
                                            }
                                            else
                                            {
                                                det = (xxr(0,0,0)*xxr(0,1,1)-xxr(0,0,1)*xxr(0,1,0))*xxr(0,2,2) +
                                                            (xxr(0,0,1)*xxr(0,1,2)-xxr(0,0,2)*xxr(0,1,1))*xxr(0,2,0) +
                                                            (xxr(0,0,2)*xxr(0,1,0)-xxr(0,0,0)*xxr(0,1,2))*xxr(0,2,1);
                                                if( det!=0. )
                                                {
                                                    det=1./det;
                                                    re[0]=rr(0,0) +
                                                                ( (xxr(0,1,1)*xxr(0,2,2)-xxr(0,2,1)*xxr(0,1,2))*dx[0]+
                                                        	      (xxr(0,2,1)*xxr(0,0,2)-xxr(0,0,1)*xxr(0,2,2))*dx[1]+
                                                        	      (xxr(0,0,1)*xxr(0,1,2)-xxr(0,1,1)*xxr(0,0,2))*dx[2] )*det;
                                                    re[1]=rr(0,1) +
                                                                ( (xxr(0,1,2)*xxr(0,2,0)-xxr(0,2,2)*xxr(0,1,0))*dx[0]+
                                                        	      (xxr(0,2,2)*xxr(0,0,0)-xxr(0,0,2)*xxr(0,2,0))*dx[1]+
                                                        	      (xxr(0,0,2)*xxr(0,1,0)-xxr(0,1,2)*xxr(0,0,0))*dx[2] )*det;
                                                    re[2]=rr(0,2) +
                                                                ( (xxr(0,1,0)*xxr(0,2,1)-xxr(0,2,0)*xxr(0,1,1))*dx[0]+
                                                        	      (xxr(0,2,0)*xxr(0,0,1)-xxr(0,0,0)*xxr(0,2,1))*dx[1]+
                                                        	      (xxr(0,0,0)*xxr(0,1,1)-xxr(0,1,0)*xxr(0,0,1))*dx[2] )*det;
                                                }
                                                else
                                                { // if the jacobian is singular
                                                    if( debug & 1 )
                                                        printf("cutHoles:WARNING: non-invertible point and jacobian=0. for estimating location\n");
                                                    re[0]=rr(0,0); re[1]=rr(0,1); re[2]=rr(0,2);
                                                    re[axis]+=.1*(2*side-1); // what is this ? 
                                                }
                                            }
                                            if( debug & 4 )// *TEMP* debug & 8
                                            {
                                                fprintf(plogFile," point jv -> rr=(%8.2e,%8.2e,%8.2e) -> xx=(%8.2e,%8.2e,%8.2e) est. re=(%8.2e,%8.2e,%8.2e)\n",rr(0,0),rr(0,1),rr(0,2), xx(0,0),xx(0,1),xx(0,2), re[0],re[1],re[2]);
                                            }
                      // compute the normal -- we don't always need this -- could compute below
                                            const real signOfJacobian= det>0. ? 1.*(2*side-1) : -1.*(2*side-1);  // sign of the jacobian
                                            const int ap1=(axis+1) % numberOfDimensions;
                                            real l2Norm;
                                            if( numberOfDimensions==2)
                                            {
                                                normal2v[axis1]=+xxr(0,axis2,ap1); 
                                                normal2v[axis2]=-xxr(0,axis1,ap1);
                                                l2Norm=(1-2*axis)*signOfJacobian/SQRT(SQR(normal2v[axis1])+SQR(normal2v[axis2]));
                                            }
                                            else if( numberOfDimensions==3 )
                                            {
                                                const int ap2=(axis+2) % numberOfDimensions;
                                                normal2v[axis1]=xxr(0,1,ap1)*xxr(0,2,ap2)-xxr(0,2,ap1)*xxr(0,1,ap2);
                                                normal2v[axis2]=xxr(0,2,ap1)*xxr(0,0,ap2)-xxr(0,0,ap1)*xxr(0,2,ap2);
                                                normal2v[axis3]=xxr(0,0,ap1)*xxr(0,1,ap2)-xxr(0,1,ap1)*xxr(0,0,ap2);
                                                l2Norm=signOfJacobian/SQRT(SQR(normal2v[axis1])+SQR(normal2v[axis2])+SQR(normal2v[axis3]));
                                            }
                      // normalize the normal to have length 1
                                            for( int ax=axis1; ax<numberOfDimensions; ax++ )
                                                normal2v[ax]=normal2v[ax]*l2Norm;
                        ...
                        // }
                                        ...
                                        #undef XR
                                            for( int ax=0; ax<numberOfDimensions; ax++ )
                                            {
                                                ...
                                                normal2v[ax]=normal(j1,j2,j3,ax);  // save the normal for later possible use
                                            }
                                        #endif
                    // *wdh* 110630
                                        for(int ax=0; ax<numberOfDimensions; ax++ ) 
                                        {
                                            if( isPeriodic[ax] )
                                            {
                                                re[ax]=fmod(re[ax]+1.,1.);  // map back to [0,1]
                                            }
                                        }
                    // ********************************** fix me *******************************
                    // 11/06/06 : wdh: this next section was un-commented
                                            if( debug & 4 )
                                            {
                      ...
                      // or  3. outside in the tangential direction.
                        ...
                        // *wdh* 990502 || re[axisp2]<=rBound(Start,axisp2,grid) || re[axisp2]>=rBound(End,axisp2,grid) )
                                            {
                                                if( debug & 4 )
                                                {
                                                    fprintf(plogFile,"cutHoles: non-invert pt: ia=(%i,%i,%i) est. r=(%7.2e,%7.2e,%7.2e) is",
                                              	...
                                              	fprintf(plogFile," outside in the tangential direction. No hole cut. \n");
                                                    fprintf(plogFile,"cutHoles:INFO: Uncutting hole point (%i,%i,%i) on grid2=%i\n",
                                                    	      ia(i,0),ia(i,1),ia(i,2),grid2);
                                                }
                                            ...
                                            {
                        // Compute the cosine of the angle between the normal at the boundary and dx, the vector between the
                        // potential hole point and the closest point on the boundary
                                                real cosAngle =0.;
                                                jpv[0]=j1; jpv[1]=j2; jpv[2]=j3;
                                                jpv[axis]=g.gridIndexRange(1-side,axis);  // opposite boundary
                                                for( int ax=0; ax<numberOfDimensions; ax++ )
                                                    cosAngle += normal2v[ax]*dx[ax]; //  (x2(i,ax)-xb(j1,j2,j3,ax));
                        // --- Compute dx2 = approx. distance between adjacent points on grid2 ---
                        ...
                        // *wdh* 080519 ipv[axisp2]=ia(i,axisp2); // ia(i,axisp2)+1; *wdh* 021230
                                                ...
                                                if( ipv[axis]<=g2.dimension(1,axis) )
                                                {
                                                    ...
                                                    {
                                              	for( int ax=0; ax<numberOfDimensions; ax++ )
                                                	  dx2+=SQR(center2(ia(i,0),ia(i,1),ia(i,2),ax)-center2(i1p,i2p,i3p,ax));  // this is ok in parallel 
                                                    }
                                                    else
                                                    {
                                              	iv[0]=ia(i,0), iv[1]=ia(i,1), iv[2]=ia(i,2);
                        ...
                        // 
                                                if( debug & 4 )
                                                {
                                                    fprintf(plogFile,"cutHoles: INFO non-invert pt (%i,%i,%i) on grid=%i by grid=%i r=(%8.2e,%8.2e,%8.2e)"
                                                    	      " distToBndry=%8.2e, dx2=%8.2e dx2*maxDistFactor=%8.2e\n",
                                                    	      ia(i,0),ia(i,1),ia(i,2),grid2,grid,re[0],re[1],re[2],sqrt(distToBndry),sqrt(dx2),sqrt(dx2*maxDistFactor) );
                          ...
                          //      iv[0],iv[1],iv[2],ipv[0],ipv[1],ipv[2],dvx2[0],dvx2[1],dvx2[2],axisp1,axisp2);
                                              	...
                                              	distToBndry>dx2*maxDistFactor  || distToBndry>maximumHoleCuttingDistanceSquared ) 
                                                {
                                              	...
                                              	if( cosAngle < maxCosAngle )
                                                	  ...
                                                	  fprintf(plogFile,"since distToBndry=%7.2e > dx*%3.1f=%7.2e j=(%i,%i,%i) jp=(%i,%i,%i)\n",
                                                      		  sqrt(distToBndry),sqrt(maxDistFactor),sqrt(dx2*maxDistFactor),j1,j2,j3,j1p,j2p,j3p);
                                                    }
                                                    mask2(ia(i,0),ia(i,1),ia(i,2))=ia(i,3);   // reset these values. Do not cut a hole.
                                                }
                                            }
                ...
                //   cutShare2(ia(R,0),ia(R,1),ia(R,2))=share;
            	      ...
            	      {
            		displayMask(cg[grid2].mask(),
                      			    sPrintF("mask for grid2=%i after cut holes by grid=%i, after un-cut holes",
                            				    grid2,grid),logFile);
            		displayMask(mask2,
                      			    sPrintF("mask for grid2=%i after cut holes by grid=%i, after un-cut holes",
                            				    grid2,grid),plogFile);
    ...
    real timeForBasicHoleCut = getCPU();
    if( info & 2 ) 
    {
        printF(" time for basic hole cut..................................%e (total=%e)\n",timeForBasicHoleCut-time0,timeForBasicHoleCut-totalTime);
    }
  ...
  // now double check the hole cutting
    ...
    real timeForCheckHoleCut = getCPU();
    if( info & 2 ) 
    {
        printF(" time for check hole cut..................................%e (total=%e)\n",timeForCheckHoleCut-timeForBasicHoleCut,
                        timeForCheckHoleCut-totalTime);
    }
  ...
  // ***** It seems that we can delete these sooner -- no need to save as "grid" changes *********
    ...
    if( debug & 8 )
    {
        for( grid=0; grid<numberOfBaseGrids; grid++ )
        {
            displayMask(cg[grid].mask(),sPrintF("mask after cut holes, grid=%i",grid),logFile);
        ...
        for( grid=0; grid<numberOfBaseGrids; grid++ )
            display(cg.inverseGrid[grid],sPrintF("inverseGrid after cutHoles, grid=%i",grid),logFile,"%3i");
    }
