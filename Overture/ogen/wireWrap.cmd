*************************************************************************
*
*   A lattice of 3D hollow cylinders with helical wires wrapped around them
*
* Usage: 
*         ogen [noplot] wireWrap [options]
* where options are
...
* 
* Examples: 
*    ogen noplot wireWrap -factor=1 -npins=1 -name="temp.hdf"  ( -> wireWrap1pinsi1.order2.hdf)
*    ogen noplot wireWrap.cmd -npins=3 -factor=1 -zb=2     ->wireWrap3pinsi1.order2.hdf
*    ogen noplot wireWrap.cmd -npins=3 -factor=1 -zb=10 -name="wireWrap3pinsl10i1.hdf"
*    ogen noplot wireWrap.cmd -npins=3 -factor=2 -zb=5   ( -> wireWrap3pins2i.order2.hdf )
* 
*    mpirun -np 2 ogen noplot wireWrap.cmd -factor=2 -npins=3 -interp=e
***************************************************************************
...
$nStretch=3.; 
$helixHeight = 10.*$pinRadius; # 1e8; # 10.;  # heliz rotates once over this vertical distance 
...
GetOptions("name=s"=> \$name,"zb=f"=>\$zb,"order=i"=>\$order,"factor=i"=> \$factor,"interp=s"=> \$interp,"npins=i"=> \$npins);
* 
...
* 
$suffix = ".order$order"; 
if( $name eq "" ){$name = "wireWrap" . "$npins" ."pins" . "$interp$factor" . $suffix . ".hdf";}
* 
* domain parameters:  
...
{ local($radius,$xc,$yc)=@_; \
  ...
  "    STRT:name $outerCyl\n" . \
  "    exit\n"; \
  $wireWrap="wireWrap$count";     \
  $wireWrapNames = $wireWrapNames . "   $wireWrap\n"; \
  $radHelix=$rad+$wireInnerRadius+$wireOffset*$ds; \
  $nrh=8; if( $interp eq "e" ){$nrh=$nrh+5;}; $deltaWireRadius=$nrh*$ds*.4; \
  $wireOuterRadius=$wireInnerRadius+$deltaWireRadius; \
  $helixRotations=($zb-$za)/$helixHeight; \
  $nThetah = int( (2.*$pi*$wireOuterRadius)/$ds+ 1.5 ); \
  $hfactor=1.2;  \
  $nzh=int( $hfactor*($zb-$za)/$ds+1.5 ); \
  $makeDiskCommands = $makeDiskCommands . \
  "  user defined 1\n" . \
  "    helical wire\n" . \
  "     * the wire is located $wireOffset grid lines outside the cylinder\n" . \
  "     x0,y0,rh: $xc,$yc,$radHelix\n" . \
  "    * \n" . \
  "    ra,rb: $wireInnerRadius $wireOuterRadius\n" . \
  "    za,zb: $za , $zb\n" . \
  "    hfreq,arg0: $helixRotations, 0\n" . \
  ...
  "      $nThetah $nrh $nzh\n" . \
  "    boundary conditions\n" . \
  "     -1 -1 $wireBC 0 $inflowBC $outflowBC\n" . \
  "    share\n" . \
  "      0  0 0 0 $inflowBC $outflowBC\n" . \
  "    mappingName\n" . \
  "      $wireWrap-unstretched\n" . \
  "  exit\n" . \
  "  stretch coordinates\n" . \
  "    Stretch r2:itanh\n" . \
  "    STP:stretch r2 itanh: layer 0 1. $nWireStretch 0 (id>=0,weight,exponent,position)\n" . \
  "    stretch grid\n" . \
  "    STRT:name $wireWrap\n" . \
  "    exit\n"; \
}
