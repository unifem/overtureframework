...
#ifndef LISTOFPARENTCHILDSIBLINGINFO_H
#define LISTOFPARENTCHILDSIBLINGINFO_H "ListOfParentChildSiblingInfo.h"
...
//==========================================================================
//	ListOfParentChildSiblingInfo Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class ParentChildSiblingInfo to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          ParentChildSiblingInfo::reference( const ParentChildSiblingInfo & t ) 
//          ParentChildSiblingInfo::breakReference()
//  that behave in the same way as A++ arrays.
//  In addition the assignment operator "=" must be deep copy.
//
//  This class keeps an array of pointers to objects of class ParentChildSiblingInfo. 
...
//
//     floatArray a(10),b(5);
//     ListOfParentChildSiblingInfo<floatArray> list,list2;
...
class ListOfParentChildSiblingInfo : public ReferenceCounting
{
  ...
  ListOfParentChildSiblingInfo(const ListOfParentChildSiblingInfo&, 
    const CopyType=DEEP  ); 

  ~ListOfParentChildSiblingInfo();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfParentChildSiblingInfo& operator=(const ListOfParentChildSiblingInfo&);
  ...
  void addElement( const ParentChildSiblingInfo & t, const int index ); // Add an object and reference to object t

  void addElement( const ParentChildSiblingInfo & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  ParentChildSiblingInfo& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const ParentChildSiblingInfo & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfParentChildSiblingInfo;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     ParentChildSiblingInfo **aList;		// Pointer to a list of pointers to object ParentChildSiblingInfo.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfParentChildSiblingInfo &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfParentChildSiblingInfo &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfParentChildSiblingInfo(*this, ct); }
};
