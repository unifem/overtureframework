...
ListOfParentChildSiblingInfo::
ListOfParentChildSiblingInfo( const ListOfParentChildSiblingInfo & X, const CopyType copyType )
{
  switch (copyType)
  {
  case DEEP:
...
void ListOfParentChildSiblingInfo::
initialize()
{
  rcData = ::new RCData;
  rcData->incrementReferenceCount();
...
operator=(const ListOfParentChildSiblingInfo &X)
{
  ...
  {
    cout << "ListOfParentChildSiblingInfo:ERROR in operator=" << endl;
...
void ListOfParentChildSiblingInfo::
reference( const ListOfParentChildSiblingInfo & list )
{
  if( rcData->decrementReferenceCount() == 0 )
    deleteStuff();  
  rcData=list.rcData;
...
breakReference()
{
  ...
  {
    ListOfParentChildSiblingInfo list = *this;  // makes a deep copy
...
//   Whoever calls this function is responsible for "newing" a ParentChildSiblingInfo and adding it to the list
// =================================================================================================
{
  ...
  {  
    rcData->listLength++;
//    rcData->aList[rcData->listLength++]= ::new ParentChildSiblingInfo;
//    assert(rcData->aList[rcData->listLength-1]!=NULL);
  ...
  if(listLength() >= rcData->memAlloc)
  {
    ...
    ParentChildSiblingInfo **aListTmp;

    aListTmp = ::new ParentChildSiblingInfo* [rcData->memAlloc]; // create an array of pointers
...
//  rcData->aList[index] = ::new ParentChildSiblingInfo;			// Put it at the desired location.
...
addElement( const int index )
{
  ...
  rcData->aList[index]= ::new ParentChildSiblingInfo;
  assert(rcData->aList[index]!=NULL);
...
void ListOfParentChildSiblingInfo::
addElement( const ParentChildSiblingInfo & t, const int index )
{
//  addElement( index );
...
// *wdh   rcData->aList[index]= ::new ParentChildSiblingInfo(t,SHALLOW);  // trouble with A++ arrays
// *****
  rcData->aList[index]= ::new ParentChildSiblingInfo;
...
void ListOfParentChildSiblingInfo::
addElement( const ParentChildSiblingInfo & t )
{
...
void ListOfParentChildSiblingInfo::
deleteElement( const ParentChildSiblingInfo &X)
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
    {
      ...
      return;
					...
					// Not there!
  cerr << "ListOfParentChildSiblingInfo: Object not found in list for deletion!" << endl;
...
ParentChildSiblingInfo& ListOfParentChildSiblingInfo::
...
int ListOfParentChildSiblingInfo::
getIndex(const ParentChildSiblingInfo & X) const
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
...
checkRange( const int index) const 
{
  ...
  {
    cerr << "ListOfParentChildSiblingInfo: List Index Out of Range!" << endl;
    ...
    Overture::abort("ListOfParentChildSiblingInfo: List Index Out of Range!");
  }  
}
