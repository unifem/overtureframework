...
C***FIRST EXECUTABLE STATEMENT  DGMRES
      IERR = 0
c* wdh on input ITER is the minimum number of iterations
      LGMR=ITER
c* wdh
C   ------------------------------------------------------------------
...
C
      IFLAG = 0
c* wdh on input LGMR is the minimum number of iterations
      minit=LGMR
...
C
     ...
     $    HES, JPRE) .NE. 0 .and. iter.gt.minit ) RETURN
c*wdh     $    HES, JPRE) .NE. 0) RETURN
      TEM = 1.0D0/R0NRM
...
C   -------------------------------------------------------------------
     ...
     $      KMP, LGMR, MAXL, MAXLP1, V, Q, SNORMW, PROD, R0NRM,
     $    HES, JPRE) .NE. 0 .and. iter.gt.minit ) GO TO 200
c*wdh      $      HES, JPRE) .NE. 0) GO TO 200
...
c*wdh DOUBLE PRECISION B(*), X(*), IA(*), JA(*), A(*), R(*), Z(*), DZ(*)
      DOUBLE PRECISION B(*), X(*),               A(*), R(*), Z(*), DZ(*)
      integer ia,ja
c*wdh DOUBLE PRECISION RWORK(*), IWORK(*), SB(*), SX(*), Q(*), V(N,*)
