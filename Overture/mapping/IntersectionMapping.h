...
//-------------------------------------------------------------
/// \brief  Define a Mapping by the intersection of two other mappings.
//-------------------------------------------------------------
...
public:
 ...
 protected:
  ...
  void reOrder(realArray & x, int & iStart, int & iEnd, periodicType & periodic, 
	       periodicType curveIsPeriodic[2], real epsX);
  void reduce(realArray & x, int & iStart, int & iEnd);

  int intersectWithCompositeSurface(Mapping & map1_, CompositeSurface & cs,
	  			  GenericGraphicsInterface *gi=NULL,
				  GraphicsParameters & params =Overture::defaultGraphicsParameters() );
