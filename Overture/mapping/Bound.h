...
// ==================================================================================
/// \brief Define a bound type as either a fraction or a real number.
// ==================================================================================
class Bound{
 protected:
  boundType bt;
  ...
  Fraction f;
 public:
  Bound( ){ bt=null; x=0.; f=Fraction(0,1); } // wdh 051018 -- add defaults to all variables to fix UMR's
  Bound( const real x0 ){ bt=realNumber; x=x0; f=Fraction(0,1); }
  Bound( const int i ){ bt=fraction; f=Fraction(i,1); x=0.; }
  Bound( const Fraction f0 ){ bt=fraction; f=f0; }
  virtual ~Bound(){}; //kkc 040414 added virtual to get rid of warnings
