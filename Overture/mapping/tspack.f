...
C
      ...
      REAL D11I, D12I, D22I, DEN, DI, DIM1, DX,
...
C
      ...
      PP = P
      IER = 1
      IF (NN .LT. 2  .OR.  PP .LE. 0.E0) RETURN
C
...
C
      ...
      REAL D11I, D12I, D22I, DEN, DI, DIM1,
...
C
      ...
      PP = P
      IER = 1
      IF (NN .LT. 3  .OR.  PP .LE. 0.E0) RETURN
C
C The forward elimination step consists of scaling a row by
C   the inverse of its diagonal block and eliminating the
...
C
      ...
      RETURN
      END
      REAL FUNCTION ENDSLP (X1,X2,X3,Y1,Y2,Y3,
     .                                  SIGMA)
      REAL X1, X2, X3, Y1, Y2, Y3, SIGMA
C
C***********************************************************
C
C                                                From TSPACK
...
C***********************************************************
C
      REAL COSHM1, COSHMS, DUMMY, DX1, DXS, S1,
     .                 SIG1, SIGS, T
C
      DX1 = X2 - X1
      DXS = X3 - X1
      IF (DX1*(DXS-DX1) .LE. 0.E0) GO TO 2
      SIG1 = ABS(SIGMA)
      IF (SIG1 .LT. 1.E-9) THEN
C
...
C
    1 T = ((Y3-Y1)*T-Y2+Y1)/(DXS*T-DX1)
      S1 = (Y2-Y1)/DX1
      IF (S1 .GE. 0.E0) THEN
        ENDSLP = MIN(MAX(0.E0,T), 3.E0*S1)
      ELSE
        ENDSLP = MAX(MIN(0.E0,T), 3.E0*S1)
      ENDIF
      RETURN
C
...
C
    2 ENDSLP = 0.E0
      RETURN
      END
      REAL FUNCTION HPPVAL (T,N,X,Y,YP,
     .                                  SIGMA, IER)
      INTEGER N, IER
...
C
      INTEGER I, IP1
      REAL B1, B2, CM, CM2, CMM, COSH2, D1, D2,
...
C
        HPPVAL = (D1 + D2 + 3.E0*(B2-B1)*(D2-D1))/DX
...
C
    2 HPPVAL = 0.E0
      ...
      RETURN
      END
      REAL FUNCTION HPVAL (T,N,X,Y,YP,
     .                                 SIGMA, IER)
      INTEGER N, IER
      REAL T, X(N), Y(N), YP(N), SIGMA(N)
C
...
C
      INTEGER I, IP1
      REAL B1, B2, CM, CM2, CMM, D1, D2, DUMMY,
...
C SIG = 0:  H is the Hermite cubic interpolant.
C
        HPVAL = S1 + B2*(D1 + D2 - 3.E0*B1*(D2-D1))
      ELSEIF (SIG .LE. .5D0) THEN
C
C 0 .LT. SIG .LE. .5:  use approximations designed to avoid
C   cancellation error in the hyperbolic functions.
...
C
     ...
     .                         ((E2+EMS*(E1-2.E0)-B1*TS)*D1+
     .                        (E1+EMS*(E2-2.E0)-B2*TS)*D2))/
     .                        (SIG*E)
        ENDIF
      ENDIF
      RETURN
...
C
    2 INTRVL = IL
      RETURN
      END
      REAL FUNCTION SIG0 (X1,X2,Y1,Y2,Y1P,Y2P,
     .                                IFL,HBND,TOL, IER)
      INTEGER IFL, IER
      REAL X1, X2, Y1, Y2, Y1P, Y2P, HBND, TOL
C
C***********************************************************
C
C                                                From TSPACK
...
C
C Intrinsic functions called by SIG0:  ABS, REAL, EXP, LOG,
C                                        MAX, MIN, SIGN,
C                                        SQRT
C
C***********************************************************
C
     ...
     .                 Y1L, Y2L
      REAL STORE
C
      DATA SBIG/85.E0/,  LUN/-1/
C
C Store local parameters and test for errors.
C
      RF = REAL(IFL)
      DX = X2 - X1
      IF (ABS(RF) .NE. 1.E0  .OR.  DX .LE. 0.E0) GO TO 8
      Y1L = Y1
      Y2L = Y2
      BND = HBND
C
...
C
      IF ((RF .LT. 0.E0  .AND.  MIN(Y1L,Y2L) .LT. BND)
     .    .OR.  (RF .GT. 0.E0  .AND.
...
C
      S1 = Y1P
      S2 = Y2P
      IF ((Y1L .EQ. BND  .AND.  RF*S1 .GT. 0.E0)  .OR.
     .    (Y2L .EQ. BND  .AND.  RF*S2 .LT. 0.E0)) GO TO 7
C
C Test for SIG = 0 sufficient.
C
      SIG = 0.E0
      IF (RF*S1 .LE. 0.E0  .AND.  RF*S2 .GE. 0.E0) GO TO 6
C
...
C
      IF (S1*S2 .GE. 0.E0  .AND.  (T0*(S1+S2) .GE. 0.E0
     .    .OR.  D0 .LT. 0.E0)) GO TO 6
      IF (A0 .EQ. 0.E0) THEN
C
...
C
        F0 = (BND - Y2L - DX*S2*S2/(4.E0*B0))*RF
      ELSE
C
...
C
        ...
        R = T/A0
        IF (RF*B0 .GT. 0.E0) R = S2/T
        F0 = (BND - Y2L - DX*(S2*B0+2.E0*D0*R)/(3.E0*A0))*RF
      ENDIF
C
C   F0 .GE. 0 iff SIG = 0 is sufficient to satisfy the
C     constraint.
C
      IF (F0 .GE. 0.E0) GO TO 6
C
...
C   of F is negative.
C
      FMAX = MAX(1.E-3,MIN(ABS(Y1L-BND),ABS(Y2L-BND)))
      T = MAX(ABS(Y1L-BND),ABS(Y2L-BND))
      SIG = DX*MAX(ABS(S1),ABS(S2))/T
      DMAX = SIG*(1.E0-T/FMAX)
      SNEG = SIG - DMAX
      IF (LUN .GE. 0  .AND.  RF .LT. 0.E0)
     .   WRITE (LUN,100) F0, FMAX, SNEG
      IF (LUN .GE. 0  .AND.  RF .GT. 0.E0)
...
C
      ...
      RTOL = 1.E0
    1 RTOL = RTOL/2.E0
        IF (STORE(RTOL+1.E0) .GT. 1.E0) GO TO 1
      RTOL = RTOL*200.E0
C
...
C
        TM = 1.E0 - EMS
        SSINH = TM*(1.E0+EMS)
        SSM = SSINH - 2.E0*SIG*EMS
        SCM = TM*TM
        C1 = SIG*SCM*D2 - SSM*D1PD2
        C2 = SIG*SSINH*D2 - SCM*D1PD2
        AA = 2.E0*(SIG*TM*D2 + (TM-SIG)*D1PD2)
        A = EMS*AA
        E = SIG*SSINH - SCM - SCM
      ENDIF
C
...
C
      ...
      D = B*B - A*C
      F = 0.E0
      IF (AA*C .EQ. 0.E0  .AND.  B .EQ. 0.E0) GO TO 3
      F = FMAX
      IF (D .LT. 0.E0) GO TO 3
      T1 = SQRT(D)
      T = -B - SIGN(T1,B)
      RSIG = 0.E0
      IF (RF*B .LT. 0.E0  .AND.  AA .NE. 0.) THEN
        IF (T/AA .GT. 0.E0) RSIG = SIG + LOG(T/AA)
      ENDIF
      IF ((RF*B .GT. 0.E0  .OR.  AA .EQ. 0.E0)  .AND.
     .    C/T .GT. 0.E0) RSIG = LOG(C/T)
      IF ((RSIG .LE. 0.E0  .OR.  RSIG .GE. SIG)  .AND.
     .    B .NE. 0.E0) GO TO 3
C
...
C
      IF (ABS(DSIG) .LT. STOL/2.E0)
     .  DSIG = -SIGN(STOL/2.E0,DMAX)
C
...
C
    9 IER = -2
      ...
      RETURN
      END
      REAL FUNCTION SIG1 (X1,X2,Y1,Y2,Y1P,Y2P,
     .                                IFL,HPBND,TOL, IER)
      INTEGER IFL, IER
      REAL X1, X2, Y1, Y2, Y1P, Y2P, HPBND, TOL
C
C***********************************************************
C
C                                                From TSPACK
...
C
C Intrinsic functions called by SIG1:  ABS, REAL, EXP, MAX,
C                                        MIN, SIGN, SQRT
C
C***********************************************************
C
      INTEGER LUN, NIT
      REAL A, A0, B0, BND, C0, C1, C2, COSHM,
...
C
      RF = REAL(IFL)
      DX = X2 - X1
      IF (ABS(RF) .NE. 1.E0  .OR.  DX .LE. 0.E0) GO TO 7
      S1 = Y1P
      S2 = Y2P
      S = (Y2-Y1)/DX
      BND = HPBND
...
C Test for a valid constraint.
C
      IF ((RF .LT. 0.E0  .AND.  MIN(S1,S2,S) .LT. BND)
     .    .OR.  (RF .GT. 0.E0  .AND.
...
C     sign of A0.
C
      IF (B0*C0 .LE. 0.E0  .OR.  A0*RF .GT. 0.E0) GO TO 5
C
C   A0*RF < 0 and HP0(R) = -D0/A0 at R = -B0/A0.
C
      D0 = T0*T0 - S1*S2
      F0 = (BND + D0/A0)*RF
      IF (F0 .GE. 0.E0) GO TO 5
C
...
C
      ...
      T1 = A*(C2+C1)
      IF (T1 .GE. 0.E0) THEN
        IF (C1*(SIG*COSHM*D1 - SINHM*D1PD2) .LT. 0.E0) THEN
C
...
C
    8 IER = -2
      ...
      RETURN
      END
      REAL FUNCTION SIG2 (X1,X2,Y1,Y2,Y1P,Y2P,
     .                                IFL,TOL, IER)
      INTEGER IFL, IER
      REAL X1, X2, Y1, Y2, Y1P, Y2P, TOL
C
C***********************************************************
C
C                                                From TSPACK
...
C
      INTEGER LUN, NIT
      REAL COSHM, D1, D2, DSIG, DUMMY, DX, EMS,
...
C
      DX = X2 - X1
      IF (ABS(IFL) .NE. 1.E0  .OR.  DX .LE. 0.E0) GO TO 5
C
...
C
      ...
      D1 = S - Y1P
      D2 = Y2P - S
      IF ((IFL .GT. 0.E0  .AND.  MIN(D1,D2) .LT. 0.E0)
     .    .OR.  (IFL .LT. 0.E0  .AND.
     .           MAX(D1,D2) .GT. 0.E0)) GO TO 6
C
C Test for infinite tension required.
C
      IF (D1*D2 .EQ. 0.E0  .AND.  D1 .NE. D2) GO TO 4
C
C Test for SIG = 0 sufficient.
C
      SIG = 0.E0
      IF (D1*D2 .EQ. 0.E0) GO TO 3
      T = MAX(D1/D2,D2/D1)
      IF (T .LE. 2.E0) GO TO 3
C
...
C
        ...
        T1 = COSHM/SINHM
        FP = T1 + SIG*(SIG/SINHM - T1*T1 + 1.E0)
      ELSE
C
C   Scale SINHM and COSHM by 2*exp(-SIG) in order to avoid
C     overflow.
C
        EMS = EXP(-SIG)
        SSM = 1.E0 - EMS*(EMS+SIG+SIG)
        T1 = (1.E0-EMS)*(1.E0-EMS)/SSM
        FP = T1 + SIG*(2.E0*SIG*EMS/SSM - T1*T1 + 1.E0)
      ENDIF
C
      F = SIG*T1 - TP1
...
C   Test for convergence.
C
      IF (FP .LE. 0.E0) GO TO 3
      DSIG = -F/FP
      IF (ABS(DSIG) .LE. RTOL*SIG  .OR.  (F .GE. 0.E0 .AND.
...
C
    6 IER = -2
     ...
     .                  DSMAX,IER)
      ...
      REAL X(N), Y(N), YP(N), TOL, B(5,N), BMAX,
     .                 SIGMA(N), DSMAX
C
C***********************************************************
C
...
C
      ...
      REAL BMX, BND, DSIG, DSM, S, SBIG, SIG,
     .                 SIGIN
      REAL SIG0, SIG1, SIG2
C
      DATA SBIG/85.E0/
...
C
     ...
     .                IFL,BND,TOL, IERR)
          ELSE
            IF (BND .EQ. 0.E0) GO TO 1
            IFL = -1
            IF (BND .GT. 0.E0) IFL = 1
...
C
        ...
        IF (SIG .GT. SIGIN) THEN
          ...
          ICNT = ICNT + 1
          DSIG = SIG-SIGIN
          IF (SIGIN .GT. 0.E0) DSIG = DSIG/SIGIN
          DSM = MAX(DSM,DSIG)
        ENDIF
    3   CONTINUE
C
...
C
    5 DSMAX = DSM
     ...
     .                  BMAX, SIGMA, DSMAX,IER)
      INTEGER N, IER
      REAL X(N), Y(N), XP(N), YP(N), TOL, BL(N),
...
C
      ...
      REAL A, A1, A2, AA, B, B0, BHI, BLO, BMX,
...
C
  ...
  100   FORMAT (//1X,'SIGBP -- INTERVAL',I4,', BL = ',D10.3,
     .          ', BU = ',D10.3,', SIGIN = ',D15.8)
        IF (BLO .GE. 0.E0  .OR.  BHI .LE. 0.E0) GO TO 9
...
C
          ...
          ELSE
            ...
            RM = T/(3.E0*S)
          ENDIF
          IF (V1 .LE. 0.E0  .AND.  V2 .GE. 0.E0) THEN
C
C   The maximum is DP = 0 at the endpoints.
C
            DP = 0.E0
          ELSE
            DP = RP*(1.E0-RP)*(RP*S - V2)
          ENDIF
          IF (V1 .GE. 0.E0  .AND.  V2 .LE. 0.E0) THEN
C
C   The minimum is DM = 0 at the endpoints.
C
            DM = 0.E0
          ELSE
            DM = RM*(1.E0-RM)*(RM*S - V2)
          ENDIF
        ENDIF
C
...
C
          TM = 1.E0 - EMS
          SINH = TM*(1.E0+EMS)
          SINHM = SINH - 2.E0*SIG*EMS
          COSHM = TM*TM
          A1 = SIG*COSHM*V2 - SINHM*V2M1
          A2 = SIG*SINH*V2 - COSHM*V2M1
          AA = 2.E0*(SIG*TM*V2 + (TM-SIG)*V2M1)
          A = EMS*AA
          E = SIG*SINH - COSHM - COSHM
        ENDIF
        IF (S .EQ. 0.E0) THEN
...
C
          RSP = 0.E0
          IF (B .LT. 0.E0  .AND.  AA .NE. 0.E0) THEN
            IF (T/AA .GT. 0.E0) RSP = SIG + LOG(T/AA)
          ENDIF
          IF ((B .GT. 0.E0  .OR.  AA .EQ. 0.E0)  .AND.
     .        C/T .GT. 0.E0) RSP = LOG(C/T)
          IF ((RSP .LE. 0.E0  .OR.  RSP .GE. SIG)  .AND.
     .        B .NE. 0.E0) THEN
C
C   The maximum is DP = 0 at the endpoints.
C
...
C
          RSM = 0.E0
          IF (B .GT. 0.E0  .AND.  AA .NE. 0.E0) THEN
            IF (T/AA .GT. 0.E0) RSM = SIG + LOG(T/AA)
          ENDIF
          IF ((B .LT. 0.E0  .OR.  AA .EQ. 0.E0)  .AND.
     .        C/T .GT. 0.E0) RSM = LOG(C/T)
          IF ((RSM .LE. 0.E0  .OR.  RSM .GE. SIG)  .AND.
     .        B .NE. 0.E0) THEN
C
C   The minimum is DM = 0 at the endpoints.
C
...
C
      INTEGER I, ICNT, IP1, LUN, NIT, NM1
      REAL A, C1, C2, COSHM, COSHMM, D0, D1,
...
C Compute an absolute tolerance FTOL = abs(TOL) and a
C   relative tolerance RTOL = 100*MACHEPS.
C  *BUG!! RTOL is too small, will end up in an infinite loop   **pf
C  * around line 2967, where DSIG < RTOL*SIG will never hold!! **pf
C
      FTOL = ABS(TOL)
      RTOL = 1.E0
    1 RTOL = RTOL/2.E0
...
C     ..To be sure, I've set RTOL=1E5*MACHEPS=1E-11 roughly **pf
      RTOL = RTOL*100.E0  
C
C Initialize change counter ICNT and maximum change DSM for
C   loop on intervals.
...
C
        SIG = SBIG
        IF ((D1D2 .EQ. 0.E0  .AND.  S1 .NE. S2)  .OR.
     .      (S .EQ. 0.E0  .AND.  S1*S2 .GT. 0.E0)) GO TO 7
C
C Test for SIGMA = 0 sufficient.  The data satisfies convex-
C   ity iff D1D2 .GE. 0, and D1D2 = 0 implies S1 = S = S2.
C
        SIG = 0.E0
        IF (D1D2 .LT. 0.E0) GO TO 3
        IF (D1D2 .EQ. 0.E0) GO TO 7
        T = MAX(D1/D2,D2/D1)
        IF (T .LE. 2.E0) GO TO 7
        TP1 = T + 1.E0
C
...
C   Test for convergence.
C
        IF (FP .LE. 0.E0) GO TO 7
        DSIG = -F/FP
        IF (ABS(DSIG) .LE. RTOL*SIG  .OR.  (F .GE. 0.E0
...
C
    3   IF (S1*S .LT. 0.E0  .OR.  S2*S .LT. 0.E0) GO TO 7
        T0 = 3.E0*S - S1 - S2
        D0 = T0*T0 - S1*S2
C
...
C   or D0 .LE. 0.
C
        IF (D0 .LE. 0.E0  .OR.  S*T0 .GE. 0.E0) GO TO 7
C
...
C
        SGN = SIGN(1.E0,S)
        SIG = SBIG
        FMAX = SGN*(SIG*S-S1-S2)/(SIG-2.E0)
        IF (FMAX .LE. 0.E0) GO TO 7
        STOL = RTOL*SIG
        F = FMAX
        F0 = SGN*D0/(3.E0*(D1-D2))
        FNEG = F0
        DSIG = SIG
        DMAX = SIG
        D1PD2 = D1 + D2
...
C
          F = FMAX
          IF (C1*(SIG*SCM*D1 - SSM*D1PD2) .GE. 0.E0) GO TO 5
          A = EMS2*(SIG*TM*D2 + (TM-SIG)*D1PD2)
          IF (A*(C2+C1) .LT. 0.E0) GO TO 5
          E = SIG*SSINH - SCM - SCM
        ENDIF
C
...
C
     ...
     .      F .LE. FTOL)  .OR.  ABS(F) .LE. RTOL ) GO TO 7
        DMAX = DMAX + DSIG
        IF ( F0*F .GT. 0.E0  .AND.  ABS(F) .GE. ABS(F0) )
     .     GO TO 6
        IF (F0*F .LE. 0.E0) THEN
C
...
C
      ...
      LOGICAL PER
      REAL C11, C12, C22, D, DMAX, DP, DX, G,
...
C
     ...
     .    SM .LE. 0.E0  .OR.  SMTOL .LE. 0.E0  .OR.
     .    SMTOL .GE. 1.E0) RETURN
      C11 = 0.E0
      C12 = 0.E0
      C22 = 0.E0
      R1 = 0.E0
...
C
     ...
     .                       Q2 .LE. Q2MAX) )      RETURN
      IF (DMAX .NE. 0.E0  .OR.  G .GT. 0.E0) GO TO 6
C
...
C***********************************************************
C
      REAL AX, EXPX, F, P, P1, P2, P3, P4, Q,
...
C
        EXPX = EXP(AX)
        SINHM = -(((1.E0/EXPX+AX)+AX)-EXPX)/2.E0
        IF (X .LT. 0.E0) SINHM = -SINHM
        COSHM = ((1.E0/EXPX-2.E0)+EXPX)/2.E0
        COSHMM = COSHM - XS/2.E0
      ENDIF
      RETURN
      END
...
C
      ...
      RETURN
      END
      REAL FUNCTION TSINTL (A,B,N,X,Y,YP,
     .                                  SIGMA, IER)
      INTEGER N, IER
...
C
      ...
      REAL B1, B2, CM, CM1, CM2, CMM, CMM1,
...
C
      DX = X(ILP1) - X(IL)
      IF (DX .LE. 0.E0) GO TO 8
      U = X(ILP1) - XL
      IF (U .EQ. 0.E0) GO TO 1
      B1 = U/DX
      Y2 = Y(ILP1)
      S = (Y2-Y(IL))/DX
      S2 = YP(ILP1)
...
C   SIG = 0.
C
        SUM = SUM + U*(Y2 - U*(6.E0*S2 - B1*(4.E0*D2 +
     .                 (3.E0*B1-4.E0)*(D1-D2)))/12.E0)
      ELSEIF (SIG .LE. .5D0) THEN
C
C   0 .LT. SIG .LE. .5.
C
        ...
        E = SIG*SM - CMM - CMM
        SUM = SUM + U*(Y2 - S2*U/2.E0) + ((CM*CMM1-SM*SM1)*
...
C
        ...
        ELSE
          ...
          T = SB1*SB1/2.E0 + 1.E0
          E = TM*(SIG*TP - TM - TM)
          SUM = SUM +U*(Y2 - S2*U/2.E0)+(SIG*TM*(TP*T-E1-E2-
     .               TM*SB1)*D2 - (TM*(TM*T-E1+E2-TP*SB1) +
     .               SIG*(E1*EMS-E2+2.E0*SB1*EMS))*(D1+D2))/
     .               ((SIG/DX)**2*E)
        ENDIF
      ENDIF
C
...
C   SIG = 0.
C
          SUM = SUM + DX*((Y(I)+Y(IP1))/2.E0 -
     .                    DX*(YP(IP1)-YP(I))/12.E0)
        ELSEIF (SIG .LE. .5D0) THEN
C
C   0 .LT. SIG .LE. .5.
C
...
C
          EMS = EXP(-SIG)
          SUM = SUM + DX*(Y(I)+Y(IP1) - DX*(SIG*(1.E0+EMS)/
     .                (1.E0-EMS)-2.E0)*(YP(IP1)-YP(I))/
     .                (SIG*SIG))/2.E0
        ENDIF
    3   CONTINUE
C
...
C   SIG = 0.
C
        SUM = SUM + U*(Y1 + U*(6.E0*S1 + B2*(4.E0*D1 +
     .                (4.E0-3.E0*B2)*(D1-D2)))/12.E0)
      ELSEIF (SIG .LE. .5D0) THEN
C
C   0 .LT. SIG .LE. .5.
C
        ...
        E = SIG*SM - CMM - CMM
        SUM = SUM + U*(Y1 + S1*U/2.E0) + ((CM*CMM2-SM*SM2)*
...
C
        ...
        ELSE
          ...
          T = SB2*SB2/2.E0 + 1.E0
          E = TM*(SIG*TP - TM - TM)
          SUM = SUM +U*(Y1 + S1*U/2.E0)+(SIG*TM*(TP*T-E1-E2-
     .               TM*SB2)*D1 - (TM*(TM*T-E2+E1-TP*SB2) +
     .               SIG*(E2*EMS-E1+2.E0*SB2*EMS))*(D1+D2))/
     .               ((SIG/DX)**2*E)
        ENDIF
      ENDIF
      GO TO 6
...
C
    8 IER = -2
     ...
     .                  YP, SIGMA,ICFLG,IER)
      ...
      LOGICAL PER
      REAL X(N), Y(N), B(5,N), BMAX, WK(LWK),
     .                 YP(N), SIGMA(N)
C
C***********************************************************
C
...
C
      ...
      LOGICAL LOOP2
      REAL DSMAX, DYP, DYPTOL, E, STOL, YP1, YPN
C
      DATA STOL/0.E0/,  MAXIT/49/,  DYPTOL/.01D0/
C
...
C
      ...
      LOGICAL LOOP2
      REAL DSMAX, DYP, DYPTOL, EX, EY, STOL,
     .                 XP1, XPN, YP1, YPN
C
      DATA STOL/0.E0/,  MAXIT/49/,  DYPTOL/.01D0/
C
...
C
    ...
    3     CONTINUE
     ...
     .               WK(N2M1), YP,IERR)
        ...
        DO 4 I = 2,NM1
          EX = ABS(XP(I)-WK(I))
          IF (WK(I) .NE. 0.E0) EX = EX/ABS(WK(I))
          EY = ABS(YP(I)-WK(NM1+I))
          IF (WK(NM1+I) .NE. 0.E0) EY = EY/ABS(WK(NM1+I))
          DYP = MAX(DYP,EX,EY)
    4     CONTINUE
        IF (ICNT .EQ. 0  .OR.  DYP .LE. DYPTOL) GO TO 7
        IF (.NOT. LOOP2) THEN
...
C
   16 IER = -6
     ...
     .                  YP,SIGMA, IER)
      INTEGER N, NCD, IENDC, LWK, IER
      LOGICAL PER, UNIFRM
      REAL X(N), Y(N), WK(LWK), YP(N), SIGMA(N)
C
C***********************************************************
C
C                                                From TSPACK
...
C
      INTEGER I, ICNT, IERR, ITER, MAXIT, NM1, NN
      REAL DSMAX, DYP, DYPTOL, E, SBIG, SIG,
     .                 STOL, YP1, YPN
C
      DATA SBIG/85.E0/
C
...
C              UNIFRM = FALSE
C
      DATA STOL/0.E0/,  MAXIT/99/,  DYPTOL/.01D0/
C
...
C
     ...
     .       (PER  .AND.  LWK .LT. 2*NM1)) ) GO TO 12
        SIG = SIGMA(1)
        IF (SIG .LT. 0.E0  .OR.  SIG .GT. SBIG) GO TO 13
      ELSE
        IF ( NCD .EQ. 2  .AND.  (LWK .LT. 2*NM1  .OR.
     .       (PER  .AND.  LWK .LT. 3*NM1)) ) GO TO 12
        SIG = 0.E0
...
C
      ...
      LOGICAL SCURV
      REAL DSMAX, DYP, DYPTOL, EX, EY, EZ, SBIG,
...
C
   16 IER = -6
     ...
     .                  SIGMA,YS,YP, NIT,IER)
      ...
      LOGICAL PER, UNIFRM
      REAL X(N), Y(N), W(N), SM, SMTOL, WK(LWK),
     .                 SIGMA(N), YS(N), YP(N)
C
C***********************************************************
C
...
C
      INTEGER I, ICNT, IERR, ITER, MAXIT, NM1, NN
      REAL DSMAX, DYS, DYSTOL, E, SBIG, SIG,
     .                 STOL
C
      DATA SBIG/85.E0/
C
...
C
      DATA STOL/0.E0/,  MAXIT/99/,  DYSTOL/.01D0/
C
...
C
   13 IER = -3
     ...
     .                   IER)
      ...
      REAL X(N), Y(N), YP(N), SIGMA(N), TE(NE),
     .                 V(NE)
C
C***********************************************************
C
...
C
    3 IER = -2
     ...
     .                   TE, VX,VY,VZ,IER)
      INTEGER N, IFLAG, NE, IER
      REAL T(N), X(N), Y(N), Z(N), XP(N), YP(N),
...
C
      INTEGER I, NM1
      REAL ASI, ASIM1, DX2, DXI, DXIM1, S2, SGN,
     .                 SI, SIM1, T
C
      NM1 = N - 1
      IF (NM1 .LT. 1) GO TO 2
...
C
      I = 2
      DX2 = X(3) - X(2)
      IF (DX2 .LE. 0.E0) GO TO 3
      S2 = (Y(3)-Y(2))/DX2
      T = SI + DXI*(SI-S2)/(DXI+DX2)
      IF (SI .GE. 0.E0) THEN
        YP(1) = MIN(MAX(0.E0,T), 3.E0*SI)
      ELSE
        YP(1) = MAX(MIN(0.E0,T), 3.E0*SI)
      ENDIF
C
...
C
      DO 1 I = 2,NM1
        ...
        ASI = ABS(SI)
        SGN = SIGN(1.E0,SI)
        IF (ASIM1 .GT. ASI) SGN = SIGN(1.E0,SIM1)
        IF (SGN .GT. 0.E0) THEN
          YP(I) = MIN(MAX(0.E0,T),3.E0*MIN(ASIM1,ASI))
        ELSE
          YP(I) = MAX(MIN(0.E0,T),-3.E0*MIN(ASIM1,ASI))
        ENDIF
    1   CONTINUE
C
...
C
      ...
      IF (SI .GE. 0.E0) THEN
        YP(N) = MIN(MAX(0.E0,T), 3.E0*SI)
      ELSE
        YP(N) = MAX(MIN(0.E0,T), 3.E0*SI)
      ENDIF
      IER = 0
      RETURN
C
...
C
      INTEGER I, NM1
      REAL ASI, ASIM1, DXI, DXIM1, SGN, SI,
     .                 SIM1, T
C
      NM1 = N - 1
      IF (NM1 .LT. 2) GO TO 2
...
C
      ...
      IF (ASIM1 .GT. ASI) SGN = SIGN(1.E0,SIM1)
      IF (SGN .GT. 0.E0) THEN
        YP(1) = MIN(MAX(0.E0,T), 3.E0*MIN(ASIM1,ASI))
      ELSE
        YP(1) = MAX(MIN(0.E0,T), -3.E0*MIN(ASIM1,ASI))
      ENDIF
      YP(N) = YP(1)
C
C No error encountered.
...
C
      INTEGER I, NM1, NN
      REAL D, D1, D2, DX, R1, R2, S, SD1, SD2,
     .                 SIG, YP1, YPN
      REAL ENDSLP
C
      NN = N
...
C
      ...
      REAL D, D1, D2, DIN, DNM1, DX, R1, R2,
...
C***********************************************************
C
      REAL COSHM, COSHMM, E, EMS, SCM, SIG,
     .                 SINHM, SSINH, SSM
C
      SIG = SIGMA
      IF (SIG .LT. 1.E-9) THEN
...
C
      ...
      SUBROUTINE TSPSPT (N,ND,X,Y,Z,NCD,IENDC,PER,UNIFRM,
     .                  LWK, WK, T,XP,YP,ZP,SIGMA,IER)
      INTEGER N, ND, NCD, IENDC, LWK, IER
      LOGICAL PER, UNIFRM
      REAL X(N), Y(N), Z(N), WK(LWK), T(N),
     .                 XP(N), YP(N), ZP(N), SIGMA(N)
C
C *wdh* Bill's version that takes T as input
C
C***********************************************************
C
C                                                From TSPACK
...
c*wdh
      ierr=0
c*wdh      IF (.NOT. SCURV) THEN
c*wdh        CALL ARCL2D (NN,X,Y, T,IERR)
c*wdh      ELSE
c*wdh        CALL ARCL3D (NN,X,Y,Z, T,IERR)
c*wdh      ENDIF
c*wdh      IF (IERR .GT. 0) GO TO 14
C
C Initialize iteration count ITER, and store uniform
C   tension factors, or initialize SIGMA to zeros.
C
