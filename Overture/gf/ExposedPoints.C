// This file automatically generated from ExposedPoints.bC with bpp.
...
#define MASK2D(k1,k2,k3) (MASK2(k1,k2,k3)>0 ? 1 : MASK2(k1,k2,k3)==0 ? 0 : -1) 
      	...
      	if( MASK2(k1  ,k2  ,k3)!=0 )
      	{
        	  if( MASK2(k1  ,k2  ,k3)>0 )
        	  {
          	    ...
          	    printf("    : i=%i : grid2=%i interp:i=(%i,%i,%i) interpolee:kv=(%i,%i,%i)\n",
               		   i,grid2,i1,i2,i3,k1,k2,k3,MASK2D(k1,k2,k3));
          	    if( debug & 2 )
          	    {
            	      fprintf(debugFile,"INFO: one point interpolation, pt=(%i,(from a discretization pt\n");
        ...
        // 	  FIND_EXPOSED_POINT(!=);
        	  ...
        	  // ****** Find exposed points on cg1 that are needed for discretization *****
        		  ...
        		  // avoid looking at neighbours that are outside the array dimensions:
                              		  ...
                              		  if( mask2(i1m1,i2m1,i3z) != 0 || mask2(i1z ,i2m1,i3z) != 0 || mask2(i1p1,i2m1,i3z) != 0 || 
                                  		      mask2(i1m1,i2z ,i3z) != 0 || mask2(i1z ,i2z ,i3z) != 0 || mask2(i1p1,i2z ,i3z) != 0 || 
                                  		      mask2(i1m1,i2p1,i3z) != 0 || mask2(i1z ,i2p1,i3z) != 0 || mask2(i1p1,i2p1,i3z) != 0 )
                              		  {
        		    ...
        		    // find a nearby interpolation point so we can guess a donor grid for interpolation
                        // 		    findInterp(i1m1,i1p1,i2m1,i2p1,i3z,i3z);
                          ...
                          // -- ipog does not currently use a guess for the donor ---
                                                  interpPoint=-1;
                                                  for( j3=i3z; j3<=i3z && interpPoint<0 ; j3++)
                                                  for( j2=i2m1; j2<=i2p1 && interpPoint<0 ; j2++)
                                                  for( j1=i1m1; j1<=i1p1; j1++)
                                                  {
                           ...
                           // const int maxExtraWidth=5;  *wdh* 050708
                                                      ...
                                                      { // increase original box by k lines and look for an interp. pt.
                                                          int k3a=max(n3a,i3z-k), k3b=min(n3b,i3z+k);
                                                          int k2a=max(n2a,i2m1-k), k2b=min(n2b,i2p1+k);
                                                          int k1a=max(n1a,i1m1-k), k1b=min(n1b,i1p1+k);
        		    ...
        		    // find a nearby interpolation point so we can guess a donor grid for interpolation
                        // 		    findInterp(i1m2,i1p2,i2m2,i2p2,i3z,i3z);
                          ...
                          // -- ipog does not currently use a guess for the donor ---
                                                  interpPoint=-1;
                                                  for( j3=i3z; j3<=i3z && interpPoint<0 ; j3++)
                                                  for( j2=i2m2; j2<=i2p2 && interpPoint<0 ; j2++)
                                                  for( j1=i1m2; j1<=i1p2; j1++)
                                                  {
                                                      if( mask1(j1,j2,j3)<0 )  // note: use mask1 here (only mask1 is marked with interp pt's)
                                                      {
                                                          interpPoint=-mask1(j1,j2,j3)-1;
                           ...
                           // const int maxExtraWidth=5;  *wdh* 050708
                                                      ...
                                                      { // increase original box by k lines and look for an interp. pt.
                                                          int k3a=max(n3a,i3z-k), k3b=min(n3b,i3z+k);
                                                          int k2a=max(n2a,i2m2-k), k2b=min(n2b,i2p2+k);
                                                          int k1a=max(n1a,i1m2-k), k1b=min(n1b,i1p2+k);
                                                          for( int k3=k3a; k3<=k3b && interpPoint<0 ; k3++)
                                                          for( int k2=k2a; k2<=k2b && interpPoint<0 ; k2++)
                                                          for( int k1=k1a; k1<=k1b; k1++)
                                                          {
                                 ...
                                 // make sure we have found an interp point that was marked.
                                                  ...
                                                  {
                                                      printf("Exposed:findInterp:ERROR: unable to find a neighbouring interp pt!\n"
                                                                    " grid=%i, i1,i2,i3=%i,%i,%i check box=[%i,%i][%i,%i][%i,%i]\n",grid,i1,i2,i3,i1m2,i1p2,i2m2,i2p2,i3z,i3z);
                                                      printf(" refinement level number=%i \n",cg1.refinementLevelNumber(grid));
                                                      printf(" grid bounds: [n1a,n1b][n2a,n2b][n3a,n3b]=[%i,%i][%i,%i][%i,%i]\n",n1a,n1b,n2a,n2b,n3a,n3b);
                                                      for( j3=i3z; j3<=i3z && interpPoint<0 ; j3++)
                                                      for( j2=i2m2; j2<=i2p2 && interpPoint<0 ; j2++)
                ...
                #undef MASK5
        		  ...
        		  // avoid looking at neighbours that are outside the array dimensions:
                              		  ...
                              		  i2m1=minmax2(i2-1); i2p1=minmax2(i2+1); 
                                                    i1z =minmax1(i1);   i2z =minmax2(i2);
                              		  if( mask2(i1m1,i2m1,i3m1) != 0 || mask2(i1z ,i2m1,i3m1) != 0 || mask2(i1p1,i2m1,i3m1) != 0 || 
                                  		      mask2(i1m1,i2z ,i3m1) != 0 || mask2(i1z ,i2z ,i3m1) != 0 || mask2(i1p1,i2z ,i3m1) != 0 || 
                                  		      mask2(i1m1,i2p1,i3m1) != 0 || mask2(i1z ,i2p1,i3m1) != 0 || mask2(i1p1,i2p1,i3m1) != 0 ||
                                  		      mask2(i1m1,i2m1,i3z ) != 0 || mask2(i1z ,i2m1,i3z ) != 0 || mask2(i1p1,i2m1,i3z ) != 0 || 
                                  		      mask2(i1m1,i2z ,i3z ) != 0 || mask2(i1z ,i2z ,i3z ) != 0 || mask2(i1p1,i2z ,i3z ) != 0 || 
                                  		      mask2(i1m1,i2p1,i3z ) != 0 || mask2(i1z ,i2p1,i3z ) != 0 || mask2(i1p1,i2p1,i3z ) != 0 ||
                                  		      mask2(i1m1,i2m1,i3p1) != 0 || mask2(i1z ,i2m1,i3p1) != 0 || mask2(i1p1,i2m1,i3p1) != 0 || 
                                  		      mask2(i1m1,i2z ,i3p1) != 0 || mask2(i1z ,i2z ,i3p1) != 0 || mask2(i1p1,i2z ,i3p1) != 0 || 
                                  		      mask2(i1m1,i2p1,i3p1) != 0 || mask2(i1z ,i2p1,i3p1) != 0 || mask2(i1p1,i2p1,i3p1) != 0 ) 
                              		  {
                                		    ia(numberOfExposedPoints,0)=i1;
                                		    ia(numberOfExposedPoints,1)=i2;
                                		    ia(numberOfExposedPoints,2)=i3;
        		    // find a nearby interpolation point so we can guess a donor grid for interpolation
                        // 		    findInterp(i1m1,i1p1,i2m1,i2p1,i3m1,i3p1);
                          ...
                          // -- ipog does not currently use a guess for the donor ---
                                                  interpPoint=-1;
                                                  for( j3=i3m1; j3<=i3p1 && interpPoint<0 ; j3++)
                                                  for( j2=i2m1; j2<=i2p1 && interpPoint<0 ; j2++)
                                                  for( j1=i1m1; j1<=i1p1; j1++)
                                                  {
                                                      if( mask1(j1,j2,j3)<0 )  // note: use mask1 here (only mask1 is marked with interp pt's)
                           ...
                           // const int maxExtraWidth=5;  *wdh* 050708
                                                      ...
                                                      for( int k=1; k<maxExtraWidth; k++ )
                                                      { // increase original box by k lines and look for an interp. pt.
                                                          int k3a=max(n3a,i3m1-k), k3b=min(n3b,i3p1+k);
                                                          int k2a=max(n2a,i2m1-k), k2b=min(n2b,i2p1+k);
                                                          int k1a=max(n1a,i1m1-k), k1b=min(n1b,i1p1+k);
                                                          for( int k3=k3a; k3<=k3b && interpPoint<0 ; k3++)
                                                          for( int k2=k2a; k2<=k2b && interpPoint<0 ; k2++)
                                 ...
                                 // make sure we have found an interp point that was marked.
                                                  ...
                                                  {
                                                      printf("Exposed:findInterp:ERROR: unable to find a neighbouring interp pt!\n"
                                                                    " grid=%i, i1,i2,i3=%i,%i,%i check box=[%i,%i][%i,%i][%i,%i]\n",grid,i1,i2,i3,i1m1,i1p1,i2m1,i2p1,i3m1,i3p1);
                                                      printf(" refinement level number=%i \n",cg1.refinementLevelNumber(grid));
                                                      printf(" grid bounds: [n1a,n1b][n2a,n2b][n3a,n3b]=[%i,%i][%i,%i][%i,%i]\n",n1a,n1b,n2a,n2b,n3a,n3b);
                                                      for( j3=i3m1; j3<=i3p1 && interpPoint<0 ; j3++)
                                                      for( j2=i2m1; j2<=i2p1 && interpPoint<0 ; j2++)
                ...
                #define MASKIN() (  mask1(i1m1,i2m1,i3m1)<0 || mask1(i1  ,i2m1,i3m1)<0 || mask1(i1p1,i2m1,i3m1)<0 || 		    mask1(i1m1,i2  ,i3m1)<0 || mask1(i1  ,i2  ,i3m1)<0 || mask1(i1p1,i2  ,i3m1)<0 || 		    mask1(i1m1,i2p1,i3m1)<0 || mask1(i1  ,i2p1,i3m1)<0 || mask1(i1p1,i2p1,i3m1)<0 || 		    mask1(i1m1,i2m1,i3  )<0 || mask1(i1  ,i2m1,i3  )<0 || mask1(i1p1,i2m1,i3  )<0 || 		    mask1(i1m1,i2  ,i3  )<0 ||                            mask1(i1p1,i2  ,i3  )<0 || 		    mask1(i1m1,i2p1,i3  )<0 || mask1(i1  ,i2p1,i3  )<0 || mask1(i1p1,i2p1,i3  )<0 || 		    mask1(i1m1,i2m1,i3p1)<0 || mask1(i1  ,i2m1,i3p1)<0 || mask1(i1p1,i2m1,i3p1)<0 || 		    mask1(i1m1,i2  ,i3p1)<0 || mask1(i1  ,i2  ,i3p1)<0 || mask1(i1p1,i2  ,i3p1)<0 || 		    mask1(i1m1,i2p1,i3p1)<0 || mask1(i1  ,i2p1,i3p1)<0 || mask1(i1p1,i2p1,i3p1)<0 )
        		    ...
        		    // find a nearby interpolation point so we can guess a donor grid for interpolation
                        // 		    findInterp(i1m2,i1p2,i2m2,i2p2,i3m2,i3p2);
                          ...
                          // -- ipog does not currently use a guess for the donor ---
                                                  interpPoint=-1;
                                                  for( j3=i3m2; j3<=i3p2 && interpPoint<0 ; j3++)
                                                  for( j2=i2m2; j2<=i2p2 && interpPoint<0 ; j2++)
                                                  for( j1=i1m2; j1<=i1p2; j1++)
                                                  {
                                                      if( mask1(j1,j2,j3)<0 )  // note: use mask1 here (only mask1 is marked with interp pt's)
                           ...
                           // const int maxExtraWidth=5;  *wdh* 050708
                                                      ...
                                                      for( int k=1; k<maxExtraWidth; k++ )
                                                      { // increase original box by k lines and look for an interp. pt.
                                                          int k3a=max(n3a,i3m2-k), k3b=min(n3b,i3p2+k);
                                                          int k2a=max(n2a,i2m2-k), k2b=min(n2b,i2p2+k);
                                                          int k1a=max(n1a,i1m2-k), k1b=min(n1b,i1p2+k);
                                                          for( int k3=k3a; k3<=k3b && interpPoint<0 ; k3++)
                                                          for( int k2=k2a; k2<=k2b && interpPoint<0 ; k2++)
                                 ...
                                 // make sure we have found an interp point that was marked.
                                                  ...
                                                  {
                                                      printf("Exposed:findInterp:ERROR: unable to find a neighbouring interp pt!\n"
                                                                    " grid=%i, i1,i2,i3=%i,%i,%i check box=[%i,%i][%i,%i][%i,%i]\n",grid,i1,i2,i3,i1m2,i1p2,i2m2,i2p2,i3m2,i3p2);
                                                      printf(" refinement level number=%i \n",cg1.refinementLevelNumber(grid));
                                                      printf(" grid bounds: [n1a,n1b][n2a,n2b][n3a,n3b]=[%i,%i][%i,%i][%i,%i]\n",n1a,n1b,n2a,n2b,n3a,n3b);
                                                      for( j3=i3m2; j3<=i3p2 && interpPoint<0 ; j3++)
                                                      for( j2=i2m2; j2<=i2p2 && interpPoint<0 ; j2++)
