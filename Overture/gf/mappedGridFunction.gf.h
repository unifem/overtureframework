...
//
//  Notes:
//   o a MappedGridFunction is derived from an A++ array
//   o a MappedGridFunction contains a pointer to a MappedGrid
//
...
//----------------------------------------------------------------------------
class MappedGridFunctionRCData : public ReferenceCounting 
{
  friend class MappedGridFunction;
  ...
  MappedGridFunctionRCData & operator=( const MappedGridFunctionRCData & rcData );  // deep copy

private:
  
  // These are used by list's of ReferenceCounting objects
  virtual void reference( const ReferenceCounting & mgf )
  { MappedGridFunctionRCData::reference( (MappedGridFunctionRCData&) mgf ); }
  virtual ReferenceCounting & operator=( const ReferenceCounting & mgf )
  { return MappedGridFunctionRCData::operator=( (MappedGridFunctionRCData&) mgf ); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ) const
  { return ::new MappedGridFunctionRCData(); }  
};
...
//
//==================================================================
class MappedGridFunction : public doubleDistributedArray, public GenericGridFunction
{
 public:
  
  enum
  {
    maximumNumberOfIndicies=MappedGridFunctionRCData::maximumNumberOfIndicies,  // we support this many Indicies
    numberOfIndicies=MappedGridFunctionRCData::numberOfIndicies,             // A++ supports this many Indicies
 ...
 public:
  ...
  MappedGridFunction();
  MappedGridFunction(MappedGrid & grid);

  MappedGridFunction(const MappedGridFunction & cgf, const CopyType copyType = DEEP );
  ...
  MappedGridFunction(const GridFunctionParameters::GridFunctionType & type, 
		     const Range & component0=nullRange,       // defaults to Range(0,0)
		     const Range & component1=nullRange,
		     const Range & component2=nullRange,
		     const Range & component3=nullRange,
  ...
  MappedGridFunction & operator=( const MappedGridFunction & cgf );
  MappedGridFunction & operator=( const double x );
  MappedGridFunction & operator=( const doubleDistributedArray & X );
  ...
  int dataCopy( const MappedGridFunction & mgf );  // copy the array data only
  ...
  void link(const MappedGridFunction & mgf, 
  ...
  updateReturnValue updateToMatchGrid(MappedGrid & grid, 
				      ...
				      const Range & component3=nullRange,
				      const Range & component4=nullRange );
  updateReturnValue updateToMatchGrid(MappedGridData & grid, 
				      const GridFunctionParameters::GridFunctionType & type, 
				      const Range & component0,
				      const Range & component1=nullRange, // defaults to Range(0,0)
				      const Range & component2=nullRange,
  ...
  updateReturnValue updateToMatchGridFunction(const MappedGridFunction & cgf);
  updateReturnValue updateToMatchGridFunction(const MappedGridFunction & cgf, 
				const Range & R0, 
				const Range & R1=nullRange,
				const Range & R2=nullRange,
  ...
  // reference 
  void reference( const MappedGridFunction & cgf );
  // break a reference
  void breakReference();
  ...
  //  argument "axis" has been specified, otherwise the current value for getFaceCentering():
...
//   int zeroUnusedPoints(MappedGridFunction & coeff, 
//                        double value = 0,
// 		       const Index & component0=nullIndex,
// 		       const Index & component1=nullIndex,
// 		       const Index & component2=nullIndex,
...
#define FUNCTION(type) \
  virtual MappedGridFunction type(const Index & I1 = nullIndex,      \
				  ...
				  const Index & I8 = nullIndex ) const; \
                                                                     \
  virtual MappedGridFunction type(const GridFunctionParameters & gfType,   \
...
#undef FUNCTION  
  ...
  //compute (u.grad)w (convective derivative of passive variable(s))
    virtual MappedGridFunction convectiveDerivative (
						     const MappedGridFunction &w,
						     const Index & I1 = nullIndex,
						     const Index & I2 = nullIndex,
						     const Index & I3 = nullIndex
						     ) const;
  virtual MappedGridFunction convectiveDerivative (const GridFunctionParameters & gfType,
						   const MappedGridFunction &w,
						   const Index & I1 = nullIndex,
						   const Index & I2 = nullIndex,
						   const Index & I3 = nullIndex
...
#define SCALAR_FUNCTION(type) \
  virtual MappedGridFunction type(\
					   const MappedGridFunction & s,\
					   const Index & I1 = nullIndex, \
					   const Index & I2 = nullIndex, \
					   const Index & I3 = nullIndex, \
					   const Index & I4 = nullIndex,\
...
#undef SCALAR_FUNCTION
  ...
  virtual MappedGridFunction derivativeScalarDerivative(
					   const MappedGridFunction & s,
  ...
  virtual MappedGridFunction derivativeScalarDerivative(const GridFunctionParameters & gfType,
					   const MappedGridFunction & s,
				           const int & direction1,
				           const int & direction2,
					   const Index & I1 = nullIndex, 
  ...
  virtual MappedGridFunction derivativeScalarDerivativeCoefficients(				
						       const MappedGridFunction & s,
				                       const int & direction1,
				                       const int & direction2,
						       const Index & I1 = nullIndex,
  ...
  virtual MappedGridFunction FCgrad (
				     const int c0 = 0,
				     const int c1 = 0,
				     const int c2 = 0,
				     const int c3 = 0,
  ...
  virtual MappedGridFunction FCgrad (const GridFunctionParameters & gfType,
				     const int c0 = 0,
				     const int c1 = 0,
				     const int c2 = 0,
				     const int c3 = 0,
...
// 			      const int & grid=0 );
 ...
 private:
  ...
  void updateRanges(const Range & R0,             // update the R[] array
		    ...
		    const Range & R6,
		    const Range & R7,
                    const GridFunctionParameters::GridFunctionType & type=GridFunctionParameters::defaultCentering);
  

  // These are used by list's of ReferenceCounting objects
  virtual void reference( const ReferenceCounting & mgf )
  { MappedGridFunction::reference( (MappedGridFunction&) mgf ); }
  virtual ReferenceCounting & operator=( const ReferenceCounting & mgf )
  { return MappedGridFunction::operator=( (MappedGridFunction&) mgf ); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
  { return ::new MappedGridFunction(*this,ct); }  
  ...
  aString className;
  MappedGridFunctionRCData *rcData;
...
MappedGridFunction &
multiply( const MappedGridFunction & a, const MappedGridFunction & coeff );

MappedGridFunction &
multiply( const doubleDistributedArray & a, const MappedGridFunction & coeff );
