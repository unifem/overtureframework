...
//      <type>MappedGridFunction.C  where <type> is one of float, double or int
//========================================================================================================
...
//  positionOfCoordinate.redim(maximumNumberOfIndicies);
  ...
  stencilType            =MappedGridFunction::generalStencil;
//  stencilOffset.redim(1);
  stencilWidth           =3;
  updateToMatchGridOption=MappedGridFunction::updateSize | MappedGridFunction::updateCoefficientMatrix;
  numberOfNames          =0;
  name                   =NULL;
}
MappedGridFunctionRCData::
...
MappedGridFunctionRCData & MappedGridFunctionRCData::
operator=( const MappedGridFunctionRCData & rcData )  // deep copy
{
  if( this == &rcData )
    return *this;
...
// ================ member functions for MappedGridFunction ========
...
//---------------------------------------------------------------------------------------------
MappedGridFunction::
MappedGridFunction(MappedGridData & grid0)
{
  initialize();
  grid=&grid0;
			   ...
			   const Range & R7=nullRange ) 
{
  ...
  for( int i=0; i<MappedGridFunction::maximumNumberOfIndicies; i++ )
    if( Ru[i].length() > 0 )
      count++;
  return count;
}
...
//-----------------------------------------------------------------------------------------
MappedGridFunction::
MappedGridFunction(MappedGridData & gridData, 
		   const Range & R0, 
		   const Range & R1,
		   const Range & R2,
		   const Range & R3,
...
//-----------------------------------------------------------------------------------------
{
  ...
  case GridFunctionParameters::faceCenteredAll:
    ...
    default:
      cout << "MappedGridFunction::constructor:ERROR too many components, only 4 allowed \n";
      Overture::abort("MappedGridFunction::constructor:ERROR too many components, only 4 allowed" );
    }
    break;
...
MappedGridFunction(const MappedGridFunction & cgf, 
   ...
   : doubleDistributedArray(cgf, copyType==DEEP ? DEEPCOPY : SHALLOWCOPY)
//-----------------------------------------------------------------------------------------
// /Description:
...
//-----------------------------------------------------------------------------------------
{
//   printf("Inside MappedGridFunction copy constructor. cgf.name=%s\n",(const char*)cgf.getName());
...
//-----------------------------------------------------------------------------------------
void MappedGridFunction::
initialize()  
{
  rcData = ::new MappedGridFunctionRCData;  //    new : don't call A++ new
...
//==================================================================================
{
  if_int boundaryConditionError(); 
  #ifdef OV_USE_DOUBLE
    if_float  boundaryConditionError(); 
    if_double operators->applyBoundaryConditions( *this, time );
  #else
    if_double  boundaryConditionError(); 
    if_float operators->applyBoundaryConditions( *this, time );
  #endif
...
//==================================================================================
{
  ...
  #ifdef OV_USE_DOUBLE
    if_float  boundaryConditionError(); 
    if_double operators->assignBoundaryConditionCoefficients( *this, time );
  #else
    if_double  boundaryConditionError(); 
    if_float operators->assignBoundaryConditionCoefficients( *this, time );
  #endif
...
//==================================================================================
{
  if( operators==NULL )
  {
    cout << "MappedGridFunction: ERROR: trying to applyBoundaryCondition without defining an operator\n";
    Overture::abort("MappedGridFunction: ERROR: trying to applyBoundaryCondition without defining an operator");
  }
  if_int boundaryConditionError(); 
  #ifdef OV_USE_DOUBLE
    if_float  boundaryConditionError(); 
    if_double operators->applyBoundaryCondition( *this, Components,bcType,bc,forcing,time,bcParameters,grid_ );
  #else
    if_double  boundaryConditionError(); 
    if_float operators->applyBoundaryCondition( *this, Components,bcType,bc,forcing,time,bcParameters,grid_ );
  #endif
...
//==================================================================================
{
  ...
  #ifdef OV_USE_DOUBLE
    if_float  boundaryConditionError(); 
    if_double operators->applyBoundaryCondition(*this, Components,bcType,bc,forcing,forcinga,time,bcParameters,grid_ );
  #else
    if_double  boundaryConditionError(); 
    if_float operators->applyBoundaryCondition(*this, Components,bcType,bc,forcing,forcinga,time,bcParameters,grid_ );
  #endif
}
     ...
     const Range & C0 /* =nullRange */)
{
  if( operators==NULL )
  {
    cout << "MappedGridFunction: ERROR: trying to finishBoundaryConditions without defining an operator\n";
    Overture::abort("MappedGridFunction: ERROR: trying to finishBoundaryConditions without defining an operator");
  }
  if_int boundaryConditionError(); 
  #ifdef OV_USE_DOUBLE
    if_float  boundaryConditionError(); 
    if_double operators->finishBoundaryConditions(*this,bcParameters,C0);
  #else
    if_double  boundaryConditionError(); 
    if_float operators->finishBoundaryConditions(*this,bcParameters,C0);
  #endif
}
		                   ...
		                   const int & grid_ /* =0  */  )
{
  if( operators==NULL )
  {
    cout << "MappedGridFunction: ERROR: trying to applyBoundaryConditionCoefficients without defining an operator\n";
    Overture::abort("MappedGridFunction: ERROR: trying to applyBoundaryConditionCoefficients without defining an operator");
  }
  if_int boundaryConditionError(); 
  #ifdef OV_USE_DOUBLE
    if_float  boundaryConditionError(); 
    if_double operators->applyBoundaryConditionCoefficients(*this, Equation,Components,bcType,bc,bcParameters,grid_ );
  #else
    if_double  boundaryConditionError(); 
    if_float operators->applyBoundaryConditionCoefficients(*this, Equation,Components,bcType,bc,bcParameters,grid_ );
  #endif
}
...
{
  cout << "MappedGridFunction: ERROR: do not know how to apply boundary conditions to this grid function\n";
}
...
//-----------------------------------------------------------------------------------------
{
  ...
  {
    MappedGridFunction cgf = *this;  // makes a deep copy
...
//==================================================================================
{
  if( this == &mgf )
    return 0;
  #ifndef USE_PPP
    doubleDistributedArray::operator= (mgf);
  #else
    doubleSerialArray u,v;
...
//static MappedGridFunction nullMappedGridFunction;
// extern MappedGridFunction nullMappedGridFunction;
...
{
  cout << "MappedGridFunction: ERROR: do not know how to differentiate this grid function\n"
...
//==================================================================================
{
  ...
  dir.find(subDir,name,"MappedGridFunction");
...
doubleSerialArray & MappedGridFunction::
getSerialArray()
//==================================================================================
// /Description:
...
const doubleSerialArray & MappedGridFunction::
...
//==================================================================================
{
  ...
  {
    cout << "MappedGridFunction::getComponentBase:ERROR invalid argument, component = " 
         << component << endl;
    Overture::abort("MappedGridFunction::getComponentBase:ERROR invalid argument");
  }
...
//==================================================================================
{
  if( component<0 || component>=maximumNumberOfComponents )
  {
    cout << "MappedGridFunction::getComponentBound:ERROR invalid argument, component = " 
         << component << endl;
    Overture::abort("MappedGridFunction::getComponentBound:ERROR invalid argument");
  }
...
//==================================================================================
{
  if( component<0 || component>=maximumNumberOfComponents )
  {
    cout << "MappedGridFunction::getComponentDimension:ERROR invalid argument, component = " 
         << component << endl;
    Overture::abort("MappedGridFunction::getComponentDimension:ERROR invalid argument");
  }
  return rcData->R[positionOfComponent(component)].getBound()
...
//==================================================================================
{
  ...
  {
    cout << "MappedGridFunction::getCoordinateBase:ERROR invalid argument, coordinate = " 
         << coordinate << endl;
    Overture::abort("MappedGridFunction::getCoordinateBase:ERROR invalid argument");
  }
...
//==================================================================================
{
  if( coordinate<0 || coordinate>=3 )
  {
    cout << "MappedGridFunction::getCoordinateBound:ERROR invalid argument, coordinate = " 
         << coordinate << endl;
    Overture::abort("MappedGridFunction::getCoordinateBound:ERROR invalid argument");
  }
...
//==================================================================================
{
  if( coordinate<0 || coordinate>=3 )
  {
    cout << "MappedGridFunction::getCoordinateDimension:ERROR invalid argument, coordinate = " 
         << coordinate << endl;
    Overture::abort("MappedGridFunction::getCoordinateDimension:ERROR invalid argument");
  }
  return rcData->R[positionOfCoordinate(coordinate)].getBound()
...
//==================================================================================
{
  if( operators!=NULL )
  {
    if_int  derivativeError(); 
    #ifdef OV_USE_DOUBLE
      if_float  derivativeError(); 
      if_double operators->getDerivatives(*this,I1,I2,I3,I4,I5);
    #else
      if_double derivativeError(); 
      if_float  operators->getDerivatives(*this,I1,I2,I3,I4,I5);
    #endif
  }
  else
  {
    cout << "MappedGridFunction: ERROR: trying to take a derivative without defining operators\n";
  }
}


//\begin{>>MappedGridFunctionInclude.tex}{\subsubsection{getFaceCentering}}
GridFunctionParameters::faceCenteringType MappedGridFunction::
...
GridFunctionParameters::GridFunctionType MappedGridFunction::
...
//-----------------------------------------------------------------------------------------
{
  ...
  else 
  {
	...
	cout << "MappedGridFunction::getGridFunctionType:ERROR: cannot handle more than 3 components\n";
...
GridFunctionParameters::GridFunctionTypeWithComponents MappedGridFunction::
...
//-----------------------------------------------------------------------------------------
{
  ...
  default:
    cerr << "MappedGridFunction::getGridFunctionTypeWithComponents:ERROR unknown grid function type!! \n";
  ...
  if( this )
  {
    cerr << "MappedGridFunction::getGridFunctionTypeWithComponents:ERROR: numberOfLocalComponents=="
...
// ====================================================================================================
{
  ...
  if( Rc[3].length()!=1 || Rc[4].length()!=1 )
    cout << "MappedGridFunction::getIsCellCentered:ERROR: cannot handle more than 3 components\n";
  Index Axes;
...
//==================================================================================
{
  ...
  if( Rc[3].length()!=1 || Rc[4].length()!=1 )
    cout << "MappedGridFunction::getIsFaceCentered:ERROR: cannot handle more than 3 components\n";
...
//==================================================================================
{
  ...
  {
    cout << "MappedGridFunction:getMappedGrid:ERROR: The pointer to the MappedGrid is NULL \n";
    Overture::abort("error");
  }
  return mappedGrid;
}
...
MappedGridOperators*  MappedGridFunction::
getOperators() const
//==================================================================================
// /Description:
...
MappedGridFunction::stencilTypes MappedGridFunction:: 
...
//======================================================================================================
{
  ...
  if( grid==NULL )
  {
    cerr << "MappedGridFunction:ERROR in link, trying to link to a MappedGridFunction \n"
...
// stencilOffset          =mgf.stencilOffset;
  ...
  if( link<0 )
  {
    cout << "MappedGridFunction::link:ERROR nothing to link to!\n";
  ...
  { // if linking to more than one component then Ranges before the link must be full size
    for( i=0; i<link; i++ )
    {
      ...
      {
	cout << "MappedGridFunction::link:ERROR trying to link to invalid components\n";
  ...
  { // we can link to sets of components of the form  (all,...,all,partial,1,...1)
	...
	{
          cout << "MappedGridFunction::link:ERROR trying to link to an invalid component\n";
          cout << "The name of the grid function is " << mgf.getName() << endl;
...
// Reference this grid function to the appropriate view of the linkee
  doubleDistributedArray::reference(mgf(Index(linkBase(0),Ra[0].length()),
...
//-----------------------------------------------------------------------------
void MappedGridFunction::
link(const MappedGridFunction & mgf, const int componentToLinkTo, const int numberOfComponents0 )
{
...
MappedGridFunction &
multiply( const MappedGridFunction & a_, const MappedGridFunction & coeff_ )
//==================================================================================
// /Description:
...
//==================================================================================
{
  MappedGridFunction & a = (MappedGridFunction&)a_;  // cast away const
  MappedGridFunction & coeff = (MappedGridFunction &) coeff_;
  
  
  Index I1,I2,I3;
  if( a.getLength(3)!=1 )
  {
    cout << "MappedGridFunction: multiply(a,coeff):ERROR: `a' should be a scalar grid function\n";
...
//\begin{>>MappedGridFunctionInclude.tex}{\subsubsection{multiply(a,coeff)}}
MappedGridFunction &
multiply( const doubleDistributedArray & a_, const MappedGridFunction & coeff_ )
//==================================================================================
// /Description:
...
//==================================================================================
{
  doubleDistributedArray & a = (doubleDistributedArray&) a_; // cast away const
  MappedGridFunction & coeff = (MappedGridFunction &) coeff_;  // cast away const

  Index I1,I2,I3;
  if( a.getLength(3)!=1 )
  {
    cout << "MappedGridFunction: multiply(a,coeff):ERROR: `a' should not be a scalar grid function\n";
    Overture::abort("multiply(a,coeff):ERROR");
  }
  a.reshape(1,Range(a.getBase(0),a.getBound(0)),
		  Range(a.getBase(1),a.getBound(1)),
...
//---------------------------------------------------------------------------------------------------
doubleDistributedArray  MappedGridFunction::  
...
{
  cout << "MappedGridFunction::ERROR: The operator (Index,Index,Index,Index,Index) is not implemented!\n";
  if( &I0 ) // if( TRUE ) to fool kcc compiler
    Overture::abort("MappedGridFunction::ERROR");
...
{
  cout << "MappedGridFunction::ERROR: The operator (Index,Index,Index,Index,Index,Index) is not implemented!\n";
  if( &I0 )
    Overture::abort("MappedGridFunction::ERROR");
  return doubleDistributedArray::operator()(I0);
}
...
{
  cout << "MappedGridFunction::ERROR: The operator (Index,Index,Index,Index,Index,Index,Index) is not implemented!\n";
  if( &I0 )
    Overture::abort("MappedGridFunction::ERROR");
  return doubleDistributedArray::operator()(I0);
}
...
{
  cout << "MappedGridFunction::ERROR: The operator (Index,Index,Index,Index,Index,Index,Index,Index) is not implemented!\n";
  if( &I0 )
    Overture::abort("MappedGridFunction::ERROR");
  return doubleDistributedArray::operator()(I0);
}
...
//-----------------------------------------------------------------------------------------
//\begin{>>MappedGridFunctionInclude.tex}{\subsubsection{operator = MappedGridFunction}}  
...
//==================================================================================
{
  ...
  if( grid!=0 && cgf.grid!=0 )
  {
    ...
    if( !isConformable(cgf) )
      printf("MappedGridFunction:operator= ERROR: Try to assign two grid functions that are not conformable\n");
    doubleDistributedArray::operator= (cgf);
  }
  else if( cgf.grid!=0 )
  {
    // DEEP COPY
    ...
    *rcData                =*cgf.rcData;  // deep copy
    ...
    if( grid==NULL )
    {
      ...
      {
	cerr << "MappedGridFunction:ERROR in operator=, grid is NULL and array is not null!" << endl;
	if( getName()!=" " ) cout << "GridFunctionName = " << getName() << endl;
	exit(1);
      }
      else
    ...
    else
    {
      ...
      // doubleDistributedArray::operator= ( doubleDistributedArray(cgf) );
      doubleDistributedArray::operator= ( (doubleDistributedArray&)cgf );
    }
  }
  else
...
//-----------------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------------
//\begin{>>MappedGridFunctionInclude.tex}{\subsubsection{operator = double}}
MappedGridFunction & MappedGridFunction::
...
//==================================================================================
{
  if( grid==NULL )
  {
    printf("MappedGridFunction::operator=:ERROR: trying to assign a mappedGridFunction=value but the grid is NULL!\n");
    Overture::abort("error");
  }
  #ifndef USE_PPP
    doubleDistributedArray::operator= (x);
  #else
    // do this so we assign ghost boundaries too.
    doubleSerialArray uLocal;  ::getLocalArrayWithGhostBoundaries(*this,uLocal);
    uLocal=x;
  #endif
  return *this;
}
...
//-----------------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------------
//\begin{>>MappedGridFunctionInclude.tex}{\subsubsection{operator = doubleDistributedArray}}
MappedGridFunction & MappedGridFunction::
...
//==================================================================================
{
  if( grid==NULL )
  {
    printf("MappedGridFunction::operator=:ERROR: trying to assign a mappedGridFunction=array but the grid is NULL!\n");
    Overture::abort("error");
  }
  if( isNull() )
  {
...
void periodicUpdateOpt(const int&nd, 
		  ...
		  double & u, const int&ca,const int&cb, const int&indexRange, 
...
//==================================================================================
{
  ...
  if( grid->gridType==MappedGrid::unstructuredGrid ) 
	...
	return; 
      ...
      if_int int *ptr = getDataPointer();
      if_float float *ptr = getDataPointer();
      if_double double *ptr = getDataPointer();
...
#ifndef USE_PPP
if_double  if( positionOfComponent(0)==3 && numberOfComponents()<=1 && !derivativePeriodic )
if_double  {
if_double    int ca,cb;
if_double    if( C0!=nullRange )
if_double    {
if_double      ca=C0.getBase(); cb=C0.getBound();
if_double    }
if_double    else
if_double    {
if_double      ca=getBase(3);  cb=getBound(3);
if_double    }
if_double    periodicUpdateOpt(numberOfDimensions(), 
if_double	        getBase(0),getBound(0),getBase(1),getBound(1),getBase(2),getBound(2),getBase(3),getBound(3),
if_double		      *(getDataPointer()),ca,cb, grid->indexRange(0,0), grid->gridIndexRange(0,0), 
if_double		      grid->dimension(0,0), grid->isPeriodic(0) );
if_double    GenericMappedGridOperators::timeForPeriodicUpdate+=getCPU()-time;
if_double    return;
if_double  }
#endif
...
//   updateComponentsChanged = 4 : component dimensions may have changed (but grid was not resized or reshaped)
//-----------------------------------------------------------------------------------------
MappedGridFunction::updateReturnValue MappedGridFunction::
privateUpdateToMatchGrid()
{
  int returnValue=0;
...
// **      Ra[positionOfCoordinate(axis)]=Range(grid->dimension(Start,axis),grid->dimension(End,axis));
  ...
  {
    cout << "MappedGridFunction:updateToMatchGrid:ERROR: grid is NULL but array is not! " << endl;
  }
...
//==================================================================================
{
  ...
  dir.create(subDir,name,"MappedGridFunction");                      // create a sub-directory 
...
//-----------------------------------------------------------------------------------------
// /Description:
//   Use this function to reference one MappedGridFunction to another.
...
//---------------------------------------------------------------------------------------------
{
  ...
  {
    cout << "MappedGridFunction::setFaceCentering:ERROR cannot face center this grid function\n"
         << " since getFaceCentering()==none \n";
    Overture::abort("MappedGridFunction::setFaceCentering:ERROR");
  }
  else if( getFaceCentering()==GridFunctionParameters::all )
  {
    ...
    if( faceCentre== -1 )
    {
      cout << "MappedGridFunction::setFaceCentering::ERROR: unable to find the component for the `face'\n";
...
//   used when the MappedGridFunction is really part of a CompositeGRidFunction. 
// 
//  /Author: WDH
//\end{MappedGridFunctionInclude.tex} 
//---------------------------------------------------------------------------------------------
...
//==================================================================================
{
  ...
  if( Rc[3].length()!=1 || Rc[4].length()!=1 )
    cout << "MappedGridFunction::setIsCellCentered:ERROR: cannot handle more than 3 components\n";
...
//======================================================================================================
{
  ...
  if( Rc[3].length()!=1 || Rc[4].length()!=1 )
    cout << "MappedGridFunction::setIsFaceCentered:ERROR: cannot handle more than 3 components\n";
    ...
    for( int i=0; i<maximumNumberOfComponents; i++ )
    {
      if( c[i]==defaultValue )
        c[i]=R[positionOfComponent(i)].getBase();
 	      ...
 	      c[i] > R[positionOfComponent(i)].getBound() )
      {
	printf("MappedGridFunction::setName:ERROR component%i=%i is invalid ! \n",i,c[i]);
             ...
             const GridFunctionParameters::GridFunctionType & type /* =defaultCentering */ )
{
  ...
  {
    cout << "MappedGridFunction:ERROR: there is no grid associated with this grid function!\n";
    Overture::abort("error");
  }
  Range *R = rcData->R;  // make a reference to the array in rcData
  Range *Ra= rcData->Ra; // make a reference to the array in rcData
  ...
  for( i=0, axis=0, component=0; i<maximumNumberOfIndicies; i++ )
  {
        ...
        if( R[i].getBase() > bigNegativeNumber )
	{
	  ...
	  else
	  {
	    ...
	    {
	      printf("MappedGridFunction::updateRanges::ERROR: unknown grid function type for an unstructured grid\n");
	      Overture::abort("error");
	    }
	  }
	}
  ...
  {
    cout << "MappedGridFunction::ERROR:When you declare a grid function there must be room for " <<
...
// assign default positions for unused coordinates
  ...
  else 
  {
    ...
    {
      cout << "MappedGridFunction::updateRanges:ERROR invalid component Ranges!\n";
      cout << "I am unable to merge these components\n";
      Overture::abort("MappedGridFunction::updateRanges:ERROR");
    }
  }
...
extern "C"
{
  void fixupOpt( const int&nd,const int&nd1a,const int&nd1b,const int&nd2a,const int&nd2b,const int&nd3a,const int&nd3b,const int&nd4a,const int&nd4b,
	       const int&n1a,const int&n1b,const int&n2a,const int&n2b,const int&n3a,const int&n3b, 
               double&u,const double&val, const int&mask, const int&bc, const int&nMin,const int&nMax, const int&nGhost );
...
//==================================================================================
{
  ...
  MappedGrid & mg = *mappedGrid;
    ...
    const doubleDistributedArray & ud = *this;
    doubleSerialArray uu; ::getLocalArrayWithGhostBoundaries(ud,uu);
    ...
    doubleDistributedArray & uu = *this;
...
if_double  if( useOpt )
if_double  {
if_double    // Here is the optimised version
if_double
if_double    fixupOpt( mg.numberOfDimensions(),
if_double              uu.getBase(0),uu.getBound(0),
if_double              uu.getBase(1),uu.getBound(1),
if_double              uu.getBase(2),uu.getBound(2),
if_double              uu.getBase(3),uu.getBound(3),
if_double              gid(0,0),gid(1,0),
if_double              gid(0,1),gid(1,1),
if_double              gid(0,2),gid(1,2),
if_double	      *(::getDataPointer(uu)),*val.getDataPointer(),*mask.getDataPointer(),*bc.getDataPointer(),nMin,nMax,numberOfGhostlines);
if_double  }
if_double  else
  {
    Index I1,I2,I3;
...
// int MappedGridFunction::
// zeroUnusedPoints(MappedGridFunction & coeff, 
//                  double value /* = 0 */,
// 		 const Index & component0 /* =nullIndex */,
// 		 const Index & component1 /* =nullIndex */,
// 		 const Index & component2 /* =nullIndex */,
...
//      or for double (if compiled in double precision)
//---------------------------------------------------------------------------------------
...
{                                                                                                            \
  if( operators!=NULL )                                                                                      \
  {                                                                                                          \
    if_int  derivativeError(); return *this;                                                                 \
    if_float  derivativeError(); return *this;                                                               \
    if_double return operators->x(*this,ARGS2);                                                              \
  }                                                                                                          \
  else                                                                                                       \
  {                                                                                                          \
    cout << "MappedGridFunction: ERROR: trying to take a derivative without defining a derivative routine\n";\
    if( this )  \
      Overture::abort("MappedGridFunction: ERROR");                                                                       \
    return *this;                                                                                            \
  }                                                                                                          \
...
{                                                                                                            \
  ...
  {                                                                                                          \
    if_int  derivativeError(); return *this;                                                                 \
    if_float  derivativeError(); return *this;                                                               \
    if_double return operators->x(*this,gfType,ARGS2);                                                              \
  }                                                                                                          \
  else                                                                                                       \
  {                                                                                                          \
    cout << "MappedGridFunction: ERROR: trying to take a derivative without defining a derivative routine\n";\
...
{                                                                                                            \
  ...
  {                                                                                                          \
    if_int  derivativeError(); return *this;                                                                 \
    if_double derivativeError(); return *this;                                                               \
    if_float  return operators->x(*this,ARGS2);                                                              \
  }                                                                                                          \
  else                                                                                                       \
  {                                                                                                          \
    cout << "MappedGridFunction: ERROR: trying to take a derivative without defining a derivative routine\n";\
...
{                                                                                                            \
  ...
  {                                                                                                          \
    if_int  derivativeError(); return *this;                                                                 \
    if_double derivativeError(); return *this;                                                               \
    if_float  return operators->x(*this,gfType,ARGS2);                                                              \
  }                                                                                                          \
  else                                                                                                       \
  {                                                                                                          \
    cout << "MappedGridFunction: ERROR: trying to take a derivative without defining a derivative routine\n";\
...
#undef  ARGS1
#undef  ARGS2
#define ARGS1 const MappedGridFunction &w, \
              const Index & I1, \
              const Index & I2, \
              const Index & I3
...
{                                                                                                            \
  ...
  {                                                                                                          \
    if_int  derivativeError(); return *this;                                                                 \
    if_float  derivativeError(); return *this;                                                               \
    if_double return operators->X(ARGS2);                                                                    \
  }                                                                                                          \
  else                                                                                                       \
  {                                                                                                          \
    cout << "MappedGridFunction: ERROR: trying to get coefficients without defining a derivative routine\n"; \
    return *this;                                                                                            \
  }                                                                                                          \
}        \
MappedGridFunction MappedGridFunction::                                                                      \
...
{                                                                                                            \
  ...
  {                                                                                                          \
    if_int  derivativeError(); return *this;                                                                 \
    if_float  derivativeError(); return *this;                                                               \
    if_double return operators->X(gfType, ARGS2);                                                            \
  }                                                                                                          \
  else                                                                                                       \
  {                                                                                                          \
    cout << "MappedGridFunction: ERROR: trying to get coefficients without defining a derivative routine\n"; \
...
{                                                                                                            \
  ...
  {                                                                                                          \
    if_int  derivativeError(); return *this;                                                                 \
    if_double derivativeError(); return *this;                                                               \
    if_float  return operators->X(ARGS2);                                                                    \
  }                                                                                                          \
  else                                                                                                       \
  {                                                                                                          \
    cout << "MappedGridFunction: ERROR: trying to get coefficients without defining a derivative routine\n"; \
...
{                                                                                                            \
  ...
  {                                                                                                          \
    if_int  derivativeError(); return *this;                                                                 \
    if_double derivativeError(); return *this;                                                               \
    if_float  return operators->X(gfType, ARGS2);                                                                    \
  }                                                                                                          \
  else                                                                                                       \
  {                                                                                                          \
    cout << "MappedGridFunction: ERROR: trying to get coefficients without defining a derivative routine\n"; \
...
#undef  ARGS1
#define ARGS1 const MappedGridFunction &s, \
              const Index & I1, \
              const Index & I2, \
              const Index & I3, \
              const Index & I4, \
...
intDistributedArray operator!=(const MappedGridFunction& x, const MappedGridFunction& y)
  { return (doubleDistributedArray&)x != (doubleDistributedArray&)y; }
intDistributedArray operator> (const MappedGridFunction& x, const MappedGridFunction& y)
  { return (doubleDistributedArray&)x >  (doubleDistributedArray&)y; }
intDistributedArray operator<=(const MappedGridFunction& x, const MappedGridFunction& y)
  { return (doubleDistributedArray&)x <= (doubleDistributedArray&)y; }
intDistributedArray operator>=(const MappedGridFunction& x, const MappedGridFunction& y)
  { return (doubleDistributedArray&)x >= (doubleDistributedArray&)y; }
intDistributedArray operator!=(const MappedGridFunction& x, const double& y)
  { return (doubleDistributedArray&)x != y ; }
intDistributedArray operator> (const MappedGridFunction& x, const double& y)
  { return (doubleDistributedArray&)x >  y ; }
intDistributedArray operator<=(const MappedGridFunction& x, const double& y)
  { return (doubleDistributedArray&)x <= y ; }
intDistributedArray operator>=(const MappedGridFunction& x, const double& y)
  { return (doubleDistributedArray&)x >= y ; }
intDistributedArray operator!=(const double& x, const MappedGridFunction& y)
  { return x != (doubleDistributedArray&)y ; }
intDistributedArray operator> (const double& x, const MappedGridFunction& y)
  { return x >  (doubleDistributedArray&)y ; }
intDistributedArray operator<=(const double& x, const MappedGridFunction& y)
  { return x <= (doubleDistributedArray&)y ; }
...
doubleDistributedArray min(const MappedGridFunction & u, const MappedGridFunction & v )
{ return min((doubleDistributedArray&)u, (doubleDistributedArray&)v ); }
doubleDistributedArray min(const MappedGridFunction & u, const double& x )
{ return min((doubleDistributedArray&)u, x ); }
doubleDistributedArray min(const double& x, const MappedGridFunction & v )
{ return min(x, (doubleDistributedArray&)v ); }

doubleDistributedArray max(const MappedGridFunction & u, const MappedGridFunction & v )
{ return max((doubleDistributedArray&)u, (doubleDistributedArray&)v ); }
doubleDistributedArray max(const MappedGridFunction & u, const double& x )
{ return max((doubleDistributedArray&)u, x ); }
doubleDistributedArray max(const double& x, const MappedGridFunction & v )
{ return max(x, (doubleDistributedArray&)v ); }
