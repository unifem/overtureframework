...
//  positionOfCoordinate.redim(maximumNumberOfIndicies);
  ...
  stencilType            =intMappedGridFunction::generalStencil;
//  stencilOffset.redim(1);
  stencilWidth           =3;
  updateToMatchGridOption=intMappedGridFunction::updateSize | intMappedGridFunction::updateCoefficientMatrix;
  numberOfNames          =0;
  name                   =NULL;
}
intMappedGridFunctionRCData::
...
intMappedGridFunctionRCData & intMappedGridFunctionRCData::
operator=( const intMappedGridFunctionRCData & rcData )  // deep copy
{
  if( this == &rcData )
    return *this;
...
// ================ member functions for intMappedGridFunction ========
...
//\begin{>>MappedGridFunctionInclude.tex}{\subsubsection{Constructors}} 
intMappedGridFunction::
intMappedGridFunction(MappedGrid & grid0)
//-----------------------------------------------------------------------------------------
// /Description:
...
//     Range all;
//     MappedGrid mg(...);
//     intMappedGridFunction u(mg,all,all,all);
//   \end{verbatim}  
...
//---------------------------------------------------------------------------------------------
intMappedGridFunction::
intMappedGridFunction(MappedGridData & grid0)
{
  initialize();
  grid=&grid0;
			   ...
			   const Range & R7=nullRange ) 
{
  ...
  for( int i=0; i<intMappedGridFunction::maximumNumberOfIndicies; i++ )
    if( Ru[i].length() > 0 )
      count++;
  return count;
}
...
//
//        intMappedGridFunction u(mg);                           //  --> u(R1,R2,R3);
//
//        intMappedGridFunction u(mg,all,all,all,1);             //  --> u(R1,R2,R3,0:1);
//        intMappedGridFunction u(mg,all,all,Range(1,1));        //  --> u(R1,R2,1:1,R3);
//
//        intMappedGridFunction u(mg,2,all);                     //  --> u(0:2,R1,R2,R3);
//        intMappedGridFunction u(mg,Range(0,2),all,all,all);    //  --> u(0:2,R1,R2,R3);
//        intMappedGridFunction u(mg,all,Range(3,3),all,all);    //  --> u(R1,3:3,R2,R3);
//    
//    \end{verbatim}
//    }   
//    
...
//-----------------------------------------------------------------------------------------
{
  ...
  case GridFunctionParameters::faceCenteredAll:
    ...
    default:
      cout << "intMappedGridFunction::constructor:ERROR too many components, only 4 allowed \n";
      Overture::abort("intMappedGridFunction::constructor:ERROR too many components, only 4 allowed" );
    }
    break;
...
//\begin{>>MappedGridFunctionInclude.tex}{} 
intMappedGridFunction::
intMappedGridFunction(const intMappedGridFunction & cgf, 
                   const CopyType copyType /* =DEEP */  ) 
   : intDistributedArray(cgf, copyType==DEEP ? DEEPCOPY : SHALLOWCOPY)
//-----------------------------------------------------------------------------------------
// /Description:
...
//-----------------------------------------------------------------------------------------
{
//   printf("Inside intMappedGridFunction copy constructor. cgf.name=%s\n",(const char*)cgf.getName());
...
//-----------------------------------------------------------------------------------------
void intMappedGridFunction::
initialize()  
{
  rcData = ::new intMappedGridFunctionRCData;  //    new : don't call A++ new
...
//==================================================================================
{
  if( operators==NULL )
  {
    cout << "intMappedGridFunction: ERROR: trying to applyBoundaryCondition without defining an operator\n";
    Overture::abort("intMappedGridFunction: ERROR: trying to applyBoundaryCondition without defining an operator");
  }
   boundaryConditionError(); 
  #ifdef OV_USE_DOUBLE
    //  boundaryConditionError(); 
     ...
     const Range & C0 /* =nullRange */)
{
  if( operators==NULL )
  {
    cout << "intMappedGridFunction: ERROR: trying to finishBoundaryConditions without defining an operator\n";
    Overture::abort("intMappedGridFunction: ERROR: trying to finishBoundaryConditions without defining an operator");
  }
   boundaryConditionError(); 
  #ifdef OV_USE_DOUBLE
    //  boundaryConditionError(); 
		                   ...
		                   const int & grid_ /* =0  */  )
{
  if( operators==NULL )
  {
    cout << "intMappedGridFunction: ERROR: trying to applyBoundaryConditionCoefficients without defining an operator\n";
    Overture::abort("intMappedGridFunction: ERROR: trying to applyBoundaryConditionCoefficients without defining an operator");
  }
   boundaryConditionError(); 
  #ifdef OV_USE_DOUBLE
    //  boundaryConditionError(); 
...
//-----------------------------------------------------------------------------------------
{
  ...
  {
    intMappedGridFunction cgf = *this;  // makes a deep copy
...
int intMappedGridFunction::
dataCopy( const intMappedGridFunction & mgf )
//==================================================================================
// /Description:
...
//==================================================================================
{
  if( this == &mgf )
    return 0;
  #ifndef USE_PPP
    intDistributedArray::operator= (mgf);
  #else
    intSerialArray u,v;
...
// extern intMappedGridFunction nullIntMappedGridFunction;
...
//==================================================================================
{
//  *this=nullIntMappedGridFunction;
...
//==================================================================================
{
  ...
  dir.find(subDir,name,"intMappedGridFunction");
...
intSerialArray & intMappedGridFunction::
getSerialArray()
//==================================================================================
// /Description:
...
const intSerialArray & intMappedGridFunction::
...
//==================================================================================
{
  ...
  {
    cout << "intMappedGridFunction::getComponentBase:ERROR invalid argument, component = " 
         << component << endl;
    Overture::abort("intMappedGridFunction::getComponentBase:ERROR invalid argument");
  }
...
//==================================================================================
{
  if( component<0 || component>=maximumNumberOfComponents )
  {
    cout << "intMappedGridFunction::getComponentBound:ERROR invalid argument, component = " 
         << component << endl;
    Overture::abort("intMappedGridFunction::getComponentBound:ERROR invalid argument");
  }
...
//==================================================================================
{
  if( component<0 || component>=maximumNumberOfComponents )
  {
    cout << "intMappedGridFunction::getComponentDimension:ERROR invalid argument, component = " 
         << component << endl;
    Overture::abort("intMappedGridFunction::getComponentDimension:ERROR invalid argument");
  }
  return rcData->R[positionOfComponent(component)].getBound()
...
//==================================================================================
{
  ...
  {
    cout << "intMappedGridFunction::getCoordinateBase:ERROR invalid argument, coordinate = " 
         << coordinate << endl;
    Overture::abort("intMappedGridFunction::getCoordinateBase:ERROR invalid argument");
  }
...
//==================================================================================
{
  if( coordinate<0 || coordinate>=3 )
  {
    cout << "intMappedGridFunction::getCoordinateBound:ERROR invalid argument, coordinate = " 
         << coordinate << endl;
    Overture::abort("intMappedGridFunction::getCoordinateBound:ERROR invalid argument");
  }
...
//==================================================================================
{
  if( coordinate<0 || coordinate>=3 )
  {
    cout << "intMappedGridFunction::getCoordinateDimension:ERROR invalid argument, coordinate = " 
         << coordinate << endl;
    Overture::abort("intMappedGridFunction::getCoordinateDimension:ERROR invalid argument");
  }
  return rcData->R[positionOfCoordinate(coordinate)].getBound()
...
//==================================================================================
{
  ...
  {
    cout << "intMappedGridFunction: ERROR: trying to take a derivative without defining operators\n";
  }
}


//\begin{>>MappedGridFunctionInclude.tex}{\subsubsection{getFaceCentering}}
GridFunctionParameters::faceCenteringType intMappedGridFunction::
...
GridFunctionParameters::GridFunctionType intMappedGridFunction::
...
//-----------------------------------------------------------------------------------------
{
  ...
  else 
  {
	...
	cout << "intMappedGridFunction::getGridFunctionType:ERROR: cannot handle more than 3 components\n";
...
GridFunctionParameters::GridFunctionTypeWithComponents intMappedGridFunction::
...
//-----------------------------------------------------------------------------------------
{
  ...
  default:
    cerr << "intMappedGridFunction::getGridFunctionTypeWithComponents:ERROR unknown grid function type!! \n";
  ...
  if( this )
  {
    cerr << "intMappedGridFunction::getGridFunctionTypeWithComponents:ERROR: numberOfLocalComponents=="
...
//     A {\ff intMappedGridFunction} can be used for finite
//   difference and finite volume codes. Finite volume
//   codes often require that the grid function be cell-centered.
//   By default a {\ff intMappedGridFunction} will be cell-centered
...
// ====================================================================================================
{
  ...
  if( Rc[3].length()!=1 || Rc[4].length()!=1 )
    cout << "intMappedGridFunction::getIsCellCentered:ERROR: cannot handle more than 3 components\n";
  Index Axes;
...
//==================================================================================
{
  ...
  if( Rc[3].length()!=1 || Rc[4].length()!=1 )
    cout << "intMappedGridFunction::getIsFaceCentered:ERROR: cannot handle more than 3 components\n";
...
MappedGrid* intMappedGridFunction::
...
//==================================================================================
{
  ...
  {
    cout << "intMappedGridFunction:getMappedGrid:ERROR: The pointer to the MappedGrid is NULL \n";
    Overture::abort("error");
  }
  return mappedGrid;
}
...
MappedGridOperators*  intMappedGridFunction::
getOperators() const
//==================================================================================
// /Description:
...
intMappedGridFunction::stencilTypes intMappedGridFunction:: 
...
void intMappedGridFunction::
link(const intMappedGridFunction & mgf, 
...
//======================================================================================================
{
  ...
  if( grid==NULL )
  {
    cerr << "intMappedGridFunction:ERROR in link, trying to link to a intMappedGridFunction \n"
...
// stencilOffset          =mgf.stencilOffset;
  ...
  if( link<0 )
  {
    cout << "intMappedGridFunction::link:ERROR nothing to link to!\n";
  ...
  { // if linking to more than one component then Ranges before the link must be full size
    for( i=0; i<link; i++ )
    {
      ...
      {
	cout << "intMappedGridFunction::link:ERROR trying to link to invalid components\n";
  ...
  { // we can link to sets of components of the form  (all,...,all,partial,1,...1)
	...
	{
          cout << "intMappedGridFunction::link:ERROR trying to link to an invalid component\n";
          cout << "The name of the grid function is " << mgf.getName() << endl;
...
// Reference this grid function to the appropriate view of the linkee
  intDistributedArray::reference(mgf(Index(linkBase(0),Ra[0].length()),
...
//-----------------------------------------------------------------------------
void intMappedGridFunction::
link(const intMappedGridFunction & mgf, const int componentToLinkTo, const int numberOfComponents0 )
{
...
intMappedGridFunction &
multiply( const intMappedGridFunction & a_, const intMappedGridFunction & coeff_ )
//==================================================================================
// /Description:
...
//==================================================================================
{
  intMappedGridFunction & a = (intMappedGridFunction&)a_;  // cast away const
  intMappedGridFunction & coeff = (intMappedGridFunction &) coeff_;
  
  
  Index I1,I2,I3;
  if( a.getLength(3)!=1 )
  {
    cout << "intMappedGridFunction: multiply(a,coeff):ERROR: `a' should be a scalar grid function\n";
...
//\begin{>>MappedGridFunctionInclude.tex}{\subsubsection{multiply(a,coeff)}}
intMappedGridFunction &
multiply( const intDistributedArray & a_, const intMappedGridFunction & coeff_ )
//==================================================================================
// /Description:
...
//==================================================================================
{
  intDistributedArray & a = (intDistributedArray&) a_; // cast away const
  intMappedGridFunction & coeff = (intMappedGridFunction &) coeff_;  // cast away const

  Index I1,I2,I3;
  if( a.getLength(3)!=1 )
  {
    cout << "intMappedGridFunction: multiply(a,coeff):ERROR: `a' should not be a scalar grid function\n";
    Overture::abort("multiply(a,coeff):ERROR");
  }
  a.reshape(1,Range(a.getBase(0),a.getBound(0)),
		  Range(a.getBase(1),a.getBound(1)),
...
//---------------------------------------------------------------------------------------------------
intDistributedArray  intMappedGridFunction::  
...
{
  cout << "intMappedGridFunction::ERROR: The operator (Index,Index,Index,Index,Index) is not implemented!\n";
  if( &I0 ) // if( TRUE ) to fool kcc compiler
    Overture::abort("intMappedGridFunction::ERROR");
...
{
  cout << "intMappedGridFunction::ERROR: The operator (Index,Index,Index,Index,Index,Index) is not implemented!\n";
  if( &I0 )
    Overture::abort("intMappedGridFunction::ERROR");
  return intDistributedArray::operator()(I0);
}
...
{
  cout << "intMappedGridFunction::ERROR: The operator (Index,Index,Index,Index,Index,Index,Index) is not implemented!\n";
  if( &I0 )
    Overture::abort("intMappedGridFunction::ERROR");
  return intDistributedArray::operator()(I0);
}
...
{
  cout << "intMappedGridFunction::ERROR: The operator (Index,Index,Index,Index,Index,Index,Index,Index) is not implemented!\n";
  if( &I0 )
    Overture::abort("intMappedGridFunction::ERROR");
  return intDistributedArray::operator()(I0);
}
...
//-----------------------------------------------------------------------------------------
//\begin{>>MappedGridFunctionInclude.tex}{\subsubsection{operator = intMappedGridFunction}}  
intMappedGridFunction & intMappedGridFunction::
operator= ( const intMappedGridFunction & cgf )
//==================================================================================
// /Description:
...
//==================================================================================
{
  ...
  if( grid!=0 && cgf.grid!=0 )
  {
    ...
    if( !isConformable(cgf) )
      printf("intMappedGridFunction:operator= ERROR: Try to assign two grid functions that are not conformable\n");
    intDistributedArray::operator= (cgf);
  }
  else if( cgf.grid!=0 )
  {
    // DEEP COPY
    ...
    *rcData                =*cgf.rcData;  // deep copy
    ...
    if( grid==NULL )
    {
      ...
      {
	cerr << "intMappedGridFunction:ERROR in operator=, grid is NULL and array is not null!" << endl;
	if( getName()!=" " ) cout << "GridFunctionName = " << getName() << endl;
	exit(1);
      }
      else
    ...
    else
    {
      ...
      // intDistributedArray::operator= ( intDistributedArray(cgf) );
      intDistributedArray::operator= ( (intDistributedArray&)cgf );
    }
  }
  else
...
//-----------------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------------
//\begin{>>MappedGridFunctionInclude.tex}{\subsubsection{operator = int}}
intMappedGridFunction & intMappedGridFunction::
...
//==================================================================================
{
  if( grid==NULL )
  {
    printf("intMappedGridFunction::operator=:ERROR: trying to assign a mappedGridFunction=value but the grid is NULL!\n");
    Overture::abort("error");
  }
  #ifndef USE_PPP
    intDistributedArray::operator= (x);
  #else
    // do this so we assign ghost boundaries too.
    intSerialArray uLocal;  ::getLocalArrayWithGhostBoundaries(*this,uLocal);
    uLocal=x;
  #endif
  return *this;
}
...
//-----------------------------------------------------------------------------------------
//-----------------------------------------------------------------------------------------
//\begin{>>MappedGridFunctionInclude.tex}{\subsubsection{operator = intDistributedArray}}
intMappedGridFunction & intMappedGridFunction::
operator= ( const intDistributedArray & X )
//==================================================================================
// /Description:
...
//==================================================================================
{
  if( grid==NULL )
  {
    printf("intMappedGridFunction::operator=:ERROR: trying to assign a mappedGridFunction=array but the grid is NULL!\n");
    Overture::abort("error");
  }
  if( isNull() )
  {
...
void periodicUpdateOpt(const int&nd, 
		  ...
		  int & u, const int&ca,const int&cb, const int&indexRange, 
...
//   updateComponentsChanged = 4 : component dimensions may have changed (but grid was not resized or reshaped)
//-----------------------------------------------------------------------------------------
intMappedGridFunction::updateReturnValue intMappedGridFunction::
privateUpdateToMatchGrid()
{
  int returnValue=0;
...
// **      Ra[positionOfCoordinate(axis)]=Range(grid->dimension(Start,axis),grid->dimension(End,axis));
  ...
  {
    cout << "intMappedGridFunction:updateToMatchGrid:ERROR: grid is NULL but array is not! " << endl;
  }
...
//==================================================================================
{
  ...
  dir.create(subDir,name,"intMappedGridFunction");                      // create a sub-directory 
...
void intMappedGridFunction::
reference(const intMappedGridFunction & cgf)
//-----------------------------------------------------------------------------------------
// /Description:
//   Use this function to reference one intMappedGridFunction to another.
...
//-----------------------------------------------------------------------------------------
{
  ...
  intDistributedArray::reference( cgf );
}
...
//---------------------------------------------------------------------------------------------
{
  ...
  {
    cout << "intMappedGridFunction::setFaceCentering:ERROR cannot face center this grid function\n"
         << " since getFaceCentering()==none \n";
    Overture::abort("intMappedGridFunction::setFaceCentering:ERROR");
  }
  else if( getFaceCentering()==GridFunctionParameters::all )
  {
    ...
    if( faceCentre== -1 )
    {
      cout << "intMappedGridFunction::setFaceCentering::ERROR: unable to find the component for the `face'\n";
...
//   used when the intMappedGridFunction is really part of a CompositeGRidFunction. 
// 
//  /Author: WDH
//\end{MappedGridFunctionInclude.tex} 
//---------------------------------------------------------------------------------------------
...
//==================================================================================
{
  ...
  if( Rc[3].length()!=1 || Rc[4].length()!=1 )
    cout << "intMappedGridFunction::setIsCellCentered:ERROR: cannot handle more than 3 components\n";
...
//======================================================================================================
{
  ...
  if( Rc[3].length()!=1 || Rc[4].length()!=1 )
    cout << "intMappedGridFunction::setIsFaceCentered:ERROR: cannot handle more than 3 components\n";
    ...
    for( int i=0; i<maximumNumberOfComponents; i++ )
    {
      if( c[i]==defaultValue )
        c[i]=R[positionOfComponent(i)].getBase();
 	      ...
 	      c[i] > R[positionOfComponent(i)].getBound() )
      {
	printf("intMappedGridFunction::setName:ERROR component%i=%i is invalid ! \n",i,c[i]);
             ...
             const GridFunctionParameters::GridFunctionType & type /* =defaultCentering */ )
{
  ...
  {
    cout << "intMappedGridFunction:ERROR: there is no grid associated with this grid function!\n";
    Overture::abort("error");
  }
  Range *R = rcData->R;  // make a reference to the array in rcData
  Range *Ra= rcData->Ra; // make a reference to the array in rcData
  ...
  for( i=0, axis=0, component=0; i<maximumNumberOfIndicies; i++ )
  {
        ...
        if( R[i].getBase() > bigNegativeNumber )
	{
	  ...
	  else
	  {
	    ...
	    {
	      printf("intMappedGridFunction::updateRanges::ERROR: unknown grid function type for an unstructured grid\n");
	      Overture::abort("error");
	    }
	  }
	}
  ...
  {
    cout << "intMappedGridFunction::ERROR:When you declare a grid function there must be room for " <<
...
// assign default positions for unused coordinates
  ...
  else 
  {
    ...
    {
      cout << "intMappedGridFunction::updateRanges:ERROR invalid component Ranges!\n";
      cout << "I am unable to merge these components\n";
      Overture::abort("intMappedGridFunction::updateRanges:ERROR");
    }
  }
...
intMappedGridFunction::updateReturnValue intMappedGridFunction::
updateToMatchGridFunction(const intMappedGridFunction & cgf)
//==================================================================================
//\end{MappedGridFunctionInclude.tex} 
//==================================================================================
{
...
extern "C"
{
  void fixupOpt( const int&nd,const int&nd1a,const int&nd1b,const int&nd2a,const int&nd2b,const int&nd3a,const int&nd3b,const int&nd4a,const int&nd4b,
	       const int&n1a,const int&n1b,const int&n2a,const int&n2b,const int&n3a,const int&n3b, 
               int&u,const int&val, const int&mask, const int&bc, const int&nMin,const int&nMax, const int&nGhost );
...
//==================================================================================
{
  ...
  MappedGrid & mg = *mappedGrid;
    ...
    const intDistributedArray & ud = *this;
    intSerialArray uu; ::getLocalArrayWithGhostBoundaries(ud,uu);
...
// int intMappedGridFunction::
// zeroUnusedPoints(intMappedGridFunction & coeff, 
//                  int value /* = 0 */,
// 		 const Index & component0 /* =nullIndex */,
// 		 const Index & component1 /* =nullIndex */,
// 		 const Index & component2 /* =nullIndex */,
...
//      or for int (if compiled in int precision)
//---------------------------------------------------------------------------------------
...
{                                                                                                            \
  ...
  {                                                                                                          \
    cout << "intMappedGridFunction: ERROR: trying to take a derivative without defining a derivative routine\n";\
    if( this )  \
      Overture::abort("intMappedGridFunction: ERROR");                                                                       \
    return *this;                                                                                            \
  }                                                                                                          \
...
#undef  ARGS1
#undef  ARGS2
#define ARGS1 const intMappedGridFunction &w, \
              const Index & I1, \
              const Index & I2, \
              const Index & I3
...
{                                                                                                            \
  ...
  {                                                                                                          \
    cout << "intMappedGridFunction: ERROR: trying to get coefficients without defining a derivative routine\n"; \
    return *this;                                                                                            \
  }                                                                                                          \
}        \
intMappedGridFunction intMappedGridFunction::                                                                      \
...
#undef  ARGS1
#define ARGS1 const intMappedGridFunction &s, \
              const Index & I1, \
              const Index & I2, \
              const Index & I3, \
              const Index & I4, \
...
intDistributedArray operator!=(const intMappedGridFunction& x, const intMappedGridFunction& y)
  { return (intDistributedArray&)x != (intDistributedArray&)y; }
intDistributedArray operator> (const intMappedGridFunction& x, const intMappedGridFunction& y)
  { return (intDistributedArray&)x >  (intDistributedArray&)y; }
intDistributedArray operator<=(const intMappedGridFunction& x, const intMappedGridFunction& y)
  { return (intDistributedArray&)x <= (intDistributedArray&)y; }
...
intDistributedArray operator!=(const intMappedGridFunction& x, const int& y)
  { return (intDistributedArray&)x != y ; }
intDistributedArray operator> (const intMappedGridFunction& x, const int& y)
  { return (intDistributedArray&)x >  y ; }
intDistributedArray operator<=(const intMappedGridFunction& x, const int& y)
  { return (intDistributedArray&)x <= y ; }
...
intDistributedArray operator!=(const int& x, const intMappedGridFunction& y)
  { return x != (intDistributedArray&)y ; }
intDistributedArray operator> (const int& x, const intMappedGridFunction& y)
  { return x >  (intDistributedArray&)y ; }
intDistributedArray operator<=(const int& x, const intMappedGridFunction& y)
  { return x <= (intDistributedArray&)y ; }
