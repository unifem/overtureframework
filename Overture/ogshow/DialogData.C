...
//-----------------------------------------------------
{
  ...
  for( int i=0; i<n_text; i++ )
    printf("[%s]\n",(const char*)textBoxes[i].textLabel);
...
// use the first choice if initCommand is not valid
  {
    ...
    printf("addOptionMenu:WARNING: option menu=[%s]: initCommand=%i out of bounds, using the first "
	   "item as initial choice\n", (const char*)opMainLabel,initCommand);
  }
...
// -----------------------------------------------------------------------------
{
  ...
  printf("ERROR:getPulldownMenu: label [%s] not found!\n"
         "   Valid labels are\n",(const char*)label);
  for( int i=0; i<n_pullDownMenu; i++ )
    printf("[%s]\n",(const char*)pdMenuData[i].menuTitle);
  return pdMenuData[0];
}
...
// -----------------------------------------------------------------------------
{
  ...
  printf("ERROR:getOptionMenu: opMainLabel [%s] not found!\n"
         "   Valid labels are\n",(const char*)opMainLabel);
  for( int i=0; i<n_optionMenu; i++ )
    printf("[%s]\n",(const char*)opMenuData[i].optionLabel);
  return opMenuData[0];
}
...
// -----------------------------------------------------------------------------
{
  ...
  for( int i=0; i<n_radioBoxes; i++ )
    printf("[%s]\n",(const char*)radioBoxData[i].radioLabel);
...
{
  int len=0;
  if( len=answer.matches(label) )
  {
    int value;
    sScanF(answer(len,answer.length()-1),"%i",&value); target=value;
...
// -----------------------------------------------------------------------------
{
  ...
  {
    aString line;
    sScanF(answer(len,answer.length()-1),"%e",&target); 
    setTextLabel(label,sPrintF(line,format,target));
    printf(" getTextValue: answer found: [%s %s]\n",(const char*)label,(const char*)sPrintF(line,format,target));
    return true;
  }
  return false;
}
...
// -----------------------------------------------------------------------------
{
  ...
  {
    aString line;
    sScanF(answer(len,answer.length()-1),"%i",&target); 
    setTextLabel(label,sPrintF(line,format,target));
    printf(" getTextValue: answer found: [%s %s]\n",(const char*)label,(const char*)sPrintF(line,format,target));
    return true;
  }
...
// -----------------------------------------------------------------------------
{
  ...
  if( len=answer.matches(label) )
  {
    target=answer(len,answer.length()-1);
