...
main(int argc, char *argv[])
{
  ...
  printf(" ------------------------------------------------------------------- \n");
  printf(" Solve a convection-diffusion equation on an annulus (unstructured)  \n");
  ...
  umap.setPreferTriangles(true);                     // make triangles when converting 2D structured Mappings
//   umap.buildFromAMapping(annulus);
  BoxMapping box;
  umap.buildFromAMapping(box);

  MappedGrid mg(umap);                               // MappedGrid for a square
