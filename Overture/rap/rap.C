...
// the convention is that the .cmd in the commandFileName doesn't have to be there
	if (dot==NULL) 
	{
	...
	printf("commandFileName = `%s', modelFileName = `%s'\n", SC commandFileName, SC modelFileName);
    ...
    cout << "Usage: `rap [noplot][nopause][abortOnEnd][file.cmd]' \n"
...
// create a graphics interface
  ...
  GenericGraphicsInterface * psPointer = Overture::getGraphicsInterface("Rap", false, argc, argv); 
  GenericGraphicsInterface & gi = *psPointer;

  MappingInformation mapInfo;
  mapInfo.graphXInterface= psPointer;
...
// by default, start logging all output to the echo file "rap.log"
  ...
  model.update(mapInfo,modelFileName);
...
//	db.setMode(GenericDataBase::noStreamMode);  TOO SLOW FOR AP
    ...
    {
      rapSimpleGeometry(mapInfo, model, curveList, points);
    }
    else if (answer == "liner...")
    {
      ...
      linerGeometry(model, gi, points);
    }
    else if (model.numberOfSubSurfaces()>0 && answer == "heal model")
    {
      rapEditModel(mapInfo, model, deletedSurfaces, curveList, points);
    }
