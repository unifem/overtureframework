...
! --- local variables
     ...
     & bcEqn2d0,bcEqn2d1
      ...
      integer dir
      real coeff(0:2), cGhost,eqMinusGhost,uGhost,uGhost2,omegaGhost,
     & eqg,uPP,omegat,omegac
...
! ---------begin statement functions-------------
     ...
     & +(coeff0*d22(1)+coeff1*d22(2))*( u(i1-1,i2-1,i3,m)+u(i1+1,i2-1,
     & i3,m)+ u(i1-1,i2+1,i3,m)+u(i1+1,i2+1,i3,m))
     &  )/6. +
...
! getBoundaryIndexBoundsMacro(side,axis)
           ...
           else if( bc(side,axis).eq.pointsFixed .and. 
     & ghostPointOption.ne.0 )then
             ! --------------------------------------------------
             ...
             do ghost=1,numGhostToSmooth
               write(*,'(" elliptic: smooth ghost=",i2," side,axis=",
     & 2i2, "omegaGhost=",f5.2)') ghost,side,axis,omegaGhost
               is1=1-2*side
               is(0)=0
               is(1)=0
               is(axis)=is1
...
! getGhostIndexMacro(side,axis, ghostm1, ghostm1, m1a,m1b,m2a,m2b,m3a,m3b)
                do s=0,1
                  do a=0,2
                    if( a.eq.axis )then
                      ghostIndex(s,axis)=indexRange(side,axis)+ghostm1*
     & (2*side-1)
                    else if( a.lt.md )then
                      ghostIndex(s,a)=indexRange(s,a)+ghostm1*(2*s-1)
                    else
               ...
               do i1=m1a,m1b
                 ...
                 do dir=0,nd-1
                   ...
                   if( .true. )then
                     cGhost = coeff(axis)*( 1./drv(axis)**2 - is(axis)*
     & source(i1,i2,i3,axis)/(2.*drv(axis)) )
                     eqMinusGhost = eqn2d(i1,i2,i3,dir) - cGhost*u(j1,
     & j2,j3,dir)
                   else
                     ! galerkin approximation -- couples to corner points more
                     cGhost= coeff(axis)*( 1./(3.*drv(axis)**2)  - is(
     & axis)*source(i1,i2,i3,axis)/(2.*drv(axis)) )
                     eqMinusGhost = eqn2dGalerkin(i1,i2,i3,dir) - 
     & cGhost*u(j1,j2,j3,dir)
                   end if
                   ...
                   uExtrap = 2.*u(i1,i2,i3,dir) - u(i1+is1,i2+is2,i3,
     & dir)
                   ! add some smoothing in the tangential direction
                   omegat=.5;
                   uSmooth = (1.-omegat)*u(i1,i2,i3,dir) + omegat*.5*( 
     & u(j1+js1,j2+js2,j3,dir) + u(j1-js1,j2-js2,j3,dir) )
                   ...
                   u(j1,j2,j3,dir)= omegaPDE*uPDE + omegaSmooth*
     & uSmooth + omegaExtrap*uExtrap
...
! getCornerPoint2d(side,axis,side2,i1,i2,is1,is2)
                   ...
                   do dir=0,nd-1
                      ...
                      !
                      uPP = u(j1+is1,j2,j3,dir) +  u(j1,j2+is2,j3,dir) 
     & - u(j1+is1,j2+is2,j3,dir)
                      ! Use rotated Laplace equation: 
                      !   u(i1+1,i2+1) + u(i1+1,i2-1) - 4*u(i1,i2) + u(i1-1,i2+1) + u(i1-1,i2-1) = 0 
                      uGhost = 4.*u(i1,i2,i3,dir) - u(i1-1,i2-1,i3,dir)
     &  - u(i1+1,i2-1,i3,dir) - u(i1-1,i2+1,i3,dir) - u(i1+1,i2+1,i3,
     & dir) + u(j1,j2,j3,dir)
                      ! 9-pt average: 
                      uGhost2 = 8.*u(i1,i2,i3,dir) - u(i1-1,i2-1,i3,
     & dir) - u(i1+1,i2-1,i3,dir) - u(i1-1,i2+1,i3,dir) - u(i1+1,i2+1,
     & i3,dir) - u(i1-1,i2,i3,dir) - u(i1+1,i2,i3,dir) - u(i1,i2-1,i3,
     & dir)  - u(i1,i2+1 ,i3,dir) + u(j1,j2,j3,dir)
                     ...
                     write(*,'(" corner: (i1,i2)=(",i3,",",i3,") u(
     & ghost)=",f6.3," uGhost=",f6.3," uPP=",f6.3)') i1,i2,u(j1,j2,j3,
     & dir),uGhost,UPP
                     uExtrap = 2.*u(i1,i2,i3,dir) - u(i1+is1,i2+is2,i3,
     & dir)
                     omegac=.25
                     ...
                     ! uGhost= uPP 
                     u(j1,j2,j3,dir) = (1.-omegaGhost)*u(j1,j2,j3,dir) 
     & + omegaGhost*uGhost
...
! assignBoundaryConditions3d()
         ...
         do axis=0,1
           ...
           if( bc(side,axis).eq.pointsFixed .and. 
     & ghostPointOption.ne.0 )then
             ! --------------------------------------------------
             ...
             ! Apply the elliptic PDE on the boundary to determine the ghost points
             omegaGhost=.5
             write(*,'(" elliptic: smooth ghost side,axis=",2i2, 
     & "omegaGhost=",f5.2)') side,axis,omegaGhost
             is(0)=0
             is(1)=0
             is(2)=0
             is(axis)=1-2*side
...
! getEllipticCoeff3d(i1,i2,i3)
     ...
     & source(i1,i2,i3,axis)/(2.*drv(axis)) )
               do dir=0,nd-1
                 eqMinusGhost = eqn3d(i1,i2,i3,dir) - cGhost*u(j1,j2,
     & j3,dir)
                 ! should we under-relax? What if cGhost is small??
                 ! u(j1,j2,j3,dir)= -eqMinusGhost/cGhost 
                 uPDE = -eqMinusGhost/cGhost
                 ! Location based on extrapolation (corresponds to a reflection BC)
                 uExtrap = 2.*u(i1,i2,i3,dir) - u(i1+is1,i2+is2,i3+is3,
     & dir)
                 ! add some smoothing in the tangential direction
                 omegat=.5;
                 if( axis.eq.0 )then
                   uSmooth = (1.-omegat)*u(i1,i2,i3,dir) + omegat*.25*(
     &  u(j1,j2+1,j3,dir) + u(j1,j2-1,j3,dir)  + u(j1,j2,j3+1,dir)+ u(
     & j1,j2,j3-1,dir) )
                 else if( axis.eq.1 )then
                   uSmooth = (1.-omegat)*u(i1,i2,i3,dir) + omegat*.25*(
     &  u(j1+1,j2,j3,dir) + u(j1-1,j2,j3,dir)  + u(j1,j2,j3+1,dir)+ u(
     & j1,j2,j3-1,dir) )
                 else
                   uSmooth = (1.-omegat)*u(i1,i2,i3,dir) + omegat*.25*(
     &  u(j1,j2+1,j3,dir) + u(j1,j2-1,j3,dir)  + u(j1+1,j2,j3,dir)+ u(
     & j1-1,j2,j3,dir) )
                 end if
                 write(*,'(" (i1,i2,i3)=(",i3,",",i3,",",i3,") u(ghost)
     & =",f6.3," uPDE=",f6.3," uExtrap=",f6.3," uSmooth=",f6.3)') i1,
     & i2,i3,u(j1,j2,j3,dir),uPDE,uExtrap,uSmooth
                 ! Here are the weights for each of the different approximations for the ghost point
                 omegaPDE=.25
                 omegaSmooth=.25
                 omegaExtrap=1.-omegaPDE-omegaSmooth
                 u(j1,j2,j3,dir)= omegaPDE*uPDE + omegaSmooth*uSmooth +
     &  omegaExtrap*uExtrap
...
! getEdgeIndexBoundsMacro(side,axis, side2,axis2, m1a,m1b, m2a,m2b, m3a,m3b )
                    ...
                    do s=0,1
                      edgeIndex(s,axis)=indexRange(side,axis)
                   ...
                   do i1=m1a,m1b
                     ...
                     do dir=0,nd-1
                        ...
                        ! corner ghost CG = X3 + ( X1 - X2) 
                        !
                        uGhost = u(i1-is1,i2,i3,dir) +  u(i1,i2-is2,i3,
     & dir) +  u(i1,i2,i3-is3,dir) - 2*u(i1,i2,i3,dir)
              write(*,'(" (j1,=j2,j3)=(",i3,",",i3,",",i3,") EDGE (
     & side,axis)=(",i2,",",i2,"(side2,axis2)=(",i2,",",i2,") u=",
     & f6.3)') j1,j2,j3,side,axis,side2,axis2,uGhost
             ...
             do side2=0,1  ! adjacent sides
               ...
               if( bc(side2,axisp1).gt.periodic .and. bc(side3,axisp2)
     & .gt.periodic )then
! getCornerPoint3d(side,axis,side2,axisp1,side3,axisp2,i1,i2,i3,is1,is2,is3)
                   iv(axis)=indexRange(side,axis)
                   iv(axisp1)=indexRange(side2,axisp1)
                   iv(axisp2)=indexRange(side3,axisp2)
                   i1=iv(0)
                   i2=iv(1)
                   i3=iv(2)
                   is(axis)=1-2*side
                 ...
                 do dir=0,nd-1
                    ...
                    uGhost = u(i1-is1,i2,i3,dir) +  u(i1,i2-is2,i3,dir)
     &  +  u(i1,i2,i3-is3,dir) - 2.*u(i1,i2,i3,dir)
                    u(j1,j2,j3,dir) = uGhost
                 end do
              write(*,'(" (j1,=j2,j3)=(",i3,",",i3,",",i3,") CORNER 
     & u=",f7.3,f7.3,f7.3)') j1,j2,j3,u(j1,j2,j3,0),u(j1,j2,j3,1),u(
     & j1,j2,j3,2)
               end if
             end do ! end side2
             end do ! end side3
