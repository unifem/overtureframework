...
#undef PARALLEL_DISPLAY
  ...
  floatArray xd;  xd.partition(partition); 
...
// DEFAULT_FORMAT = dp.iFormat or dp.fFormat or dp.dformat
    const char *format = (format_!=NULL && strlen(format_)>0) ? format_ : (const char *)DEFAULT_FORMAT;
