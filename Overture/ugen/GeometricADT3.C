...
//=======================================================
{
  ...
  dimension = 2*rangeDimension;
//  delete [] boundingBox;
//  boundingBox = new real [dimension2];
...
int
GeometricADT<dataT>::
insert(GeometricADT<dataT>::iterator &insParent, int leaf, GeomADTTuple<dataT> &data)
//=======================================================
...
int
GeometricADT<dataT>::
insert(GeometricADT<dataT>::iterator &insParent, int leaf)
//=======================================================
  ...
  if (numberOfItems == 0)  // this is the first element to be added 
    {
      ...
      adtRoot = new ADTType();
      adtRoot->getData().setData(dimension, boundingBox, coords, data);
      ...
      GeometricADT<dataT>::iterator iter(dimension,*this,coords);    // ** can we avoid building an iterator?
	...
	//cout << iter.getDepth()<<endl;
      ...
      //  real *newBox = new real [dimension2];
      real newBox[12];
      ...
      if (coords[splitAxis]<=splitLoc) // go left 
	{
	  ...
	  err = insert(iter, ADT_LEFT);
          iter.current->getLeaf(ADT_LEFT).getData().setData(dimension, newBox, coords, data);

	} else { // go right
	  newBox[2*splitAxis] = splitLoc;

	  err = insert(iter, ADT_RIGHT);
          iter.current->getLeaf(ADT_RIGHT).getData().setData(dimension, newBox, coords, data);
...
int 
GeometricADT<dataT>::
delElement(GeometricADT<dataT>::iterator &delItem)
//=======================================================
...
void
GeometricADT<dataT>::
verifyNode(GeometricADT<dataT>::ADTType &node, int depth)
{
...
void
GeometricADT<dataT>::
shiftTreeUp(GeometricADT<dataT>::ADTType *node, int depth)
{
...
template<class dataT>
__GeometricADTiterator<dataT>::
__GeometricADTiterator(__GeometricADTiterator<dataT> &x) : thisADT(x.thisADT)
// ====================================================================================================
...
__GeometricADTtraversor(int dimension_, const GeometricADT<dataT> &gADT, const real *target_) 
// ====================================================================================================
// /Purpose : initialize the iterator with a given {\tt GeometricADT} and target bounding box location
// /gADT (input) : the {\tt GeometricADT} through which to iterate
...
// ====================================================================================================
{
  dimension=dimension_;
//   target=new real [dimension];
//   a=new real [dimension];
//   b=new real [dimension];
...
template<class dataT>
__GeometricADTtraversor<dataT>::
__GeometricADTtraversor(int dimension_, const GeometricADT<dataT> &gADT )
{
  dimension=dimension_;
//   target=new real [dimension];
//   a=new real [dimension];
