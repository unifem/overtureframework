...
/*  090312 : __i386__ is defined on a Mac but there is fpu_control.h on the Mac. In any case we now use
    the gcc option -ffloat-store which seems to fix things so fpu_control.h is no longer used */
/* #ifdef __i386__  */
#if 0
  /* kkc Added for Intel machines following the GTS code */
  /* from the gts library rounding.h file */
#  include <fpu_control.h>
   /* kkc,wdh:  using the gcc flag -ffloat-store seems to fix things without the fudges below */
   static fpu_control_t fpu_round_double = (_FPU_DEFAULT & ~ _FPU_EXTENDED)|_FPU_DOUBLE; 
   /* *wdh* static fpu_control_t fpu_round_double = 4722; this was from triangle.c */

   static fpu_control_t fpu_init;
#  define GTS_FPU_ROUND_DOUBLE  { _FPU_GETCW(fpu_init);  \
                             _FPU_SETCW(fpu_round_double); }
#  define GTS_FPU_RESTORE    {_FPU_SETCW(fpu_init);}

#else
#  define GTS_FPU_ROUND_DOUBLE
#  define GTS_FPU_RESTORE
#endif
