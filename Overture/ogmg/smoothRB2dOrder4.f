...
! SMOOTH_SUBROUTINE(smoothRB2dOrder4,2,4)
        subroutine smoothRB2dOrder4( nd, nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,
...
c      end if
     ...
     & j2.le.n2b )then
                     v(j1,j2,i3)=update2dSparse4(j1,j2,i3) ! update2dSparse4 points R2 or B2
...
c      end if
                    ...
                    if( mask(j1,j2,i3).gt.0 .and. j1.le.n1d .and. 
     & j2.le.n2b )then
                      v(j1,j2,i3)=update2dSparseCC4(j1,j2,i3) ! update2dSparseCC4 points R2 or B2
c        write(*,*) 'RB: j1',j1,j2,v(j1,j2,i3)
                    end if
                  end do
                end do
...
! computeOmega2dFourthOrder(i1,i2,i3)
                         c1=abs(c(m32,i1,i2,i3)+c(m34,i1,i2,i3))
                         c2=abs(c(m23,i1,i2,i3)+c(m43,i1,i2,i3))
                         cmin=min(c1,c2)/(c1+c2)
                         omega=(1.23-.16*cmin)*variableOmegaFactor  ! w(.5)=1.15 w(.25)=1.19 -- this is for a W[2,1] ***
                         ! write(*,'(''i1,i2='',2i3,'' cmin,omega='',2(f7.4,1x))') i1,i2,cmin,omega
...
! computeOmega2dFourthOrder(j1,j2,j3)
                         c1=abs(c(m32,j1,j2,j3)+c(m34,j1,j2,j3))
                         c2=abs(c(m23,j1,j2,j3)+c(m43,j1,j2,j3))
                         cmin=min(c1,c2)/(c1+c2)
                         omega=(1.23-.16*cmin)*variableOmegaFactor  ! w(.5)=1.15 w(.25)=1.19 -- this is for a W[2,1] ***
                         ! write(*,'(''j1,j2='',2i3,'' cmin,omega='',2(f7.4,1x))') j1,j2,cmin,omega
                        v(j1,j2,i3)=update2d4(j1,j2,i3) ! update2d4 points R2 or B2
...
c      end if
                    ...
                    if( mask(j1,j2,i3).gt.0 .and. j1.le.n1d .and. 
     & j2.le.n2b )then
                      v(j1,j2,i3)=update2dCC4(j1,j2,i3) ! update2dCC4 points R2 or B2
c        write(*,*) 'RB: j1',j1,j2,v(j1,j2,i3)
                    end if
                  end do
                end do
