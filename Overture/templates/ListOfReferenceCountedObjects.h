#ifndef LIST_OF_REFERENCE_COUNTED_OBJECTS_H
#define LIST_OF_REFERENCE_COUNTED_OBJECTS_H "ListOfReferenceCountedObjects.h"
// *wdh* for pgi: 110529
#ifdef __PGI
...
class ListOfReferenceCountedObjects : public ReferenceCounting
{
  ...
  ListOfReferenceCountedObjects(const ListOfReferenceCountedObjects&, 
    const CopyType=DEEP  ); 

  ~ListOfReferenceCountedObjects();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfReferenceCountedObjects& operator=(const ListOfReferenceCountedObjects&);
  ...
  void addElement( const T & t, const int index ); // Add an object and reference to object t

  void addElement( const T & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  T& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const T & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfReferenceCountedObjects<T>;
     RCData(){ }     
     ~RCData(){ }
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfReferenceCountedObjects<T> &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfReferenceCountedObjects<T> &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfReferenceCountedObjects<T>(*this, ct); }
};
