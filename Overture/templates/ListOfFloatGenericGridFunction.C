...
ListOfFloatGenericGridFunction::
ListOfFloatGenericGridFunction( const ListOfFloatGenericGridFunction & X, const CopyType copyType )
{
  switch (copyType)
  {
  case DEEP:
...
void ListOfFloatGenericGridFunction::
initialize()
{
  rcData = ::new RCData;
  rcData->incrementReferenceCount();
...
ListOfFloatGenericGridFunction & ListOfFloatGenericGridFunction::
operator=(const ListOfFloatGenericGridFunction &X)
{
  ...
  {
    cout << "ListOfFloatGenericGridFunction:ERROR in operator=" << endl;
...
void ListOfFloatGenericGridFunction::
reference( const ListOfFloatGenericGridFunction & list )
{
  if( rcData->decrementReferenceCount() == 0 )
    deleteStuff();  
  rcData=list.rcData;
...
breakReference()
{
  ...
  {
    ListOfFloatGenericGridFunction list = *this;  // makes a deep copy
...
//   Whoever calls this function is responsible for "newing" a floatGenericGridFunction and adding it to the list
// =================================================================================================
{
  ...
  {  
    rcData->listLength++;
//    rcData->aList[rcData->listLength++]= ::new floatGenericGridFunction;
//    assert(rcData->aList[rcData->listLength-1]!=NULL);
  ...
  if(listLength() >= rcData->memAlloc)
  {
    ...
    floatGenericGridFunction **aListTmp;

    aListTmp = ::new floatGenericGridFunction* [rcData->memAlloc]; // create an array of pointers
...
//  rcData->aList[index] = ::new floatGenericGridFunction;			// Put it at the desired location.
...
addElement( const int index )
{
  ...
  rcData->aList[index]= ::new floatGenericGridFunction;
  assert(rcData->aList[index]!=NULL);
...
void ListOfFloatGenericGridFunction::
addElement( const floatGenericGridFunction & t, const int index )
{
//  addElement( index );
...
// *wdh   rcData->aList[index]= ::new floatGenericGridFunction(t,SHALLOW);  // trouble with A++ arrays
// *****
  rcData->aList[index]= ::new floatGenericGridFunction;
...
void ListOfFloatGenericGridFunction::
addElement( const floatGenericGridFunction & t )
{
...
void ListOfFloatGenericGridFunction::
deleteElement( const floatGenericGridFunction &X)
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
    {
      ...
      return;
					...
					// Not there!
  cerr << "ListOfFloatGenericGridFunction: Object not found in list for deletion!" << endl;
...
floatGenericGridFunction& ListOfFloatGenericGridFunction::
...
int ListOfFloatGenericGridFunction::
getIndex(const floatGenericGridFunction & X) const
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
...
checkRange( const int index) const 
{
  ...
  {
    cerr << "ListOfFloatGenericGridFunction: List Index Out of Range!" << endl;
    ...
    Overture::abort("ListOfFloatGenericGridFunction: List Index Out of Range!");
  }  
}
