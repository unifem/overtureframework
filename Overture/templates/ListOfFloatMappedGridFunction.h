#ifndef LISTOFFLOATMAPPEDGRIDFUNCTION_H
#define LISTOFFLOATMAPPEDGRIDFUNCTION_H "ListOfFloatMappedGridFunction.h"
// *wdh* for pgi: 110529
#ifdef __PGI
...
//==========================================================================
//	ListOfFloatMappedGridFunction Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class floatMappedGridFunction to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          floatMappedGridFunction::reference( const floatMappedGridFunction & t ) 
//          floatMappedGridFunction::breakReference()
//  that behave in the same way as A++ arrays.
//  In addition the assignment operator "=" must be deep copy.
//
//  This class keeps an array of pointers to objects of class floatMappedGridFunction. 
...
//
//     floatArray a(10),b(5);
//     ListOfFloatMappedGridFunction<floatArray> list,list2;
...
class ListOfFloatMappedGridFunction : public ReferenceCounting
{
  ...
  ListOfFloatMappedGridFunction(const ListOfFloatMappedGridFunction&, 
    const CopyType=DEEP  ); 

  ~ListOfFloatMappedGridFunction();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfFloatMappedGridFunction& operator=(const ListOfFloatMappedGridFunction&);
  ...
  void addElement( const floatMappedGridFunction & t, const int index ); // Add an object and reference to object t

  void addElement( const floatMappedGridFunction & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  floatMappedGridFunction& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const floatMappedGridFunction & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfFloatMappedGridFunction;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     floatMappedGridFunction **aList;		// Pointer to a list of pointers to object floatMappedGridFunction.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfFloatMappedGridFunction &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfFloatMappedGridFunction &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfFloatMappedGridFunction(*this, ct); }
};
