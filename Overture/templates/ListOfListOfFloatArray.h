#ifndef LISTOFLISTOFFLOATARRAY_H
#define LISTOFLISTOFFLOATARRAY_H "ListOfListOfFloatArray.h"
// *wdh* for pgi: 110529
#ifdef __PGI
...
//==========================================================================
//	ListOfListOfFloatArray Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class ListOfFloatArray to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          ListOfFloatArray::reference( const ListOfFloatArray & t ) 
//          ListOfFloatArray::breakReference()
//  that behave in the same way as A++ arrays.
//  In addition the assignment operator "=" must be deep copy.
//
//  This class keeps an array of pointers to objects of class ListOfFloatArray. 
...
//
//     floatArray a(10),b(5);
//     ListOfListOfFloatArray<floatArray> list,list2;
...
class ListOfListOfFloatArray : public ReferenceCounting
{
  ...
  ListOfListOfFloatArray(const ListOfListOfFloatArray&, 
    const CopyType=DEEP  ); 

  ~ListOfListOfFloatArray();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfListOfFloatArray& operator=(const ListOfListOfFloatArray&);
  ...
  void addElement( const ListOfFloatArray & t, const int index ); // Add an object and reference to object t

  void addElement( const ListOfFloatArray & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  ListOfFloatArray& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const ListOfFloatArray & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfListOfFloatArray;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     ListOfFloatArray **aList;		// Pointer to a list of pointers to object ListOfFloatArray.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfListOfFloatArray &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfListOfFloatArray &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfListOfFloatArray(*this, ct); }
};
