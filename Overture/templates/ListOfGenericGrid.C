...
ListOfGenericGrid::
ListOfGenericGrid( const ListOfGenericGrid & X, const CopyType copyType )
{
  switch (copyType)
  {
  case DEEP:
...
operator=(const ListOfGenericGrid &X)
{
  ...
  {
    cout << "ListOfGenericGrid:ERROR in operator=" << endl;
...
void ListOfGenericGrid::
reference( const ListOfGenericGrid & list )
{
  if( rcData->decrementReferenceCount() == 0 )
    deleteStuff();  
  rcData=list.rcData;
...
breakReference()
{
  ...
  {
    ListOfGenericGrid list = *this;  // makes a deep copy
...
//   Whoever calls this function is responsible for "newing" a GenericGrid and adding it to the list
// =================================================================================================
{
  ...
  {  
    rcData->listLength++;
//    rcData->aList[rcData->listLength++]= ::new GenericGrid;
//    assert(rcData->aList[rcData->listLength-1]!=NULL);
  ...
  if(listLength() >= rcData->memAlloc)
  {
    ...
    GenericGrid **aListTmp;

    aListTmp = ::new GenericGrid* [rcData->memAlloc]; // create an array of pointers
...
//  rcData->aList[index] = ::new GenericGrid;			// Put it at the desired location.
...
addElement( const int index )
{
  ...
  rcData->aList[index]= ::new GenericGrid;
  assert(rcData->aList[index]!=NULL);
...
void ListOfGenericGrid::
addElement( const GenericGrid & t, const int index )
{
//  addElement( index );
...
// *wdh   rcData->aList[index]= ::new GenericGrid(t,SHALLOW);  // trouble with A++ arrays
// *****
  rcData->aList[index]= ::new GenericGrid;
...
void ListOfGenericGrid::
addElement( const GenericGrid & t )
{
...
void ListOfGenericGrid::
deleteElement( const GenericGrid &X)
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
    {
      ...
      return;
					...
					// Not there!
  cerr << "ListOfGenericGrid: Object not found in list for deletion!" << endl;
...
int ListOfGenericGrid::
getIndex(const GenericGrid & X) const
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
...
checkRange( const int index) const 
{
  ...
  {
    cerr << "ListOfGenericGrid: List Index Out of Range!" << endl;
    ...
    Overture::abort("ListOfGenericGrid: List Index Out of Range!");
  }  
}
