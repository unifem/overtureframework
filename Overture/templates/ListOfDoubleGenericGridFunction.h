#ifndef LISTOFDOUBLEGENERICGRIDFUNCTION_H
#define LISTOFDOUBLEGENERICGRIDFUNCTION_H "ListOfDoubleGenericGridFunction.h"
// *wdh* for pgi: 110529
#ifdef __PGI
 #include "GenericDataBase.h"
#endif
class doubleGenericGridFunction;
#include "OvertureTypes.h"
#include "ReferenceCounting.h"

//==========================================================================
//	ListOfDoubleGenericGridFunction Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class doubleGenericGridFunction to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          doubleGenericGridFunction::reference( const doubleGenericGridFunction & t ) 
//          doubleGenericGridFunction::breakReference()
//  that behave in the same way as A++ arrays.
//  In addition the assignment operator "=" must be deep copy.
//
//  This class keeps an array of pointers to objects of class doubleGenericGridFunction. 
...
//
//     floatArray a(10),b(5);
//     ListOfDoubleGenericGridFunction<floatArray> list,list2;
...
class ListOfDoubleGenericGridFunction : public ReferenceCounting
{
  ...
  ListOfDoubleGenericGridFunction(const ListOfDoubleGenericGridFunction&, 
    const CopyType=DEEP  ); 

  ~ListOfDoubleGenericGridFunction();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfDoubleGenericGridFunction& operator=(const ListOfDoubleGenericGridFunction&);
  ...
  void addElement( const doubleGenericGridFunction & t, const int index ); // Add an object and reference to object t

  void addElement( const doubleGenericGridFunction & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  doubleGenericGridFunction& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const doubleGenericGridFunction & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfDoubleGenericGridFunction;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     doubleGenericGridFunction **aList;		// Pointer to a list of pointers to object doubleGenericGridFunction.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfDoubleGenericGridFunction &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfDoubleGenericGridFunction &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfDoubleGenericGridFunction(*this, ct); }
};
