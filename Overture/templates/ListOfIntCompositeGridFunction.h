#ifndef LISTOFINTCOMPOSITEGRIDFUNCTION_H
#define LISTOFINTCOMPOSITEGRIDFUNCTION_H "ListOfIntCompositeGridFunction.h"
// *wdh* for pgi: 110529
#ifdef __PGI
...
//==========================================================================
//	ListOfIntCompositeGridFunction Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class intCompositeGridFunction to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          intCompositeGridFunction::reference( const intCompositeGridFunction & t ) 
//          intCompositeGridFunction::breakReference()
//  that behave in the same way as A++ arrays.
//  In addition the assignment operator "=" must be deep copy.
//
//  This class keeps an array of pointers to objects of class intCompositeGridFunction. 
...
//
//     floatArray a(10),b(5);
//     ListOfIntCompositeGridFunction<floatArray> list,list2;
...
class ListOfIntCompositeGridFunction : public ReferenceCounting
{
  ...
  ListOfIntCompositeGridFunction(const ListOfIntCompositeGridFunction&, 
    const CopyType=DEEP  ); 

  ~ListOfIntCompositeGridFunction();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfIntCompositeGridFunction& operator=(const ListOfIntCompositeGridFunction&);
  ...
  void addElement( const intCompositeGridFunction & t, const int index ); // Add an object and reference to object t

  void addElement( const intCompositeGridFunction & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  intCompositeGridFunction& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const intCompositeGridFunction & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfIntCompositeGridFunction;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     intCompositeGridFunction **aList;		// Pointer to a list of pointers to object intCompositeGridFunction.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfIntCompositeGridFunction &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfIntCompositeGridFunction &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfIntCompositeGridFunction(*this, ct); }
};
