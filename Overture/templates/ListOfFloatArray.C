...
ListOfFloatArray::
ListOfFloatArray( const ListOfFloatArray & X, const CopyType copyType )
{
  switch (copyType)
  {
  case DEEP:
...
operator=(const ListOfFloatArray &X)
{
  ...
  {
    cout << "ListOfFloatArray:ERROR in operator=" << endl;
...
void ListOfFloatArray::
reference( const ListOfFloatArray & list )
{
  if( rcData->decrementReferenceCount() == 0 )
    deleteStuff();  
  rcData=list.rcData;
...
breakReference()
{
  ...
  {
    ListOfFloatArray list = *this;  // makes a deep copy
  ...
  {	
    if(&X == rcData->aList[i])
    {
      ...
      return;
					...
					// Not there!
  cerr << "ListOfFloatArray: Object not found in list for deletion!" << endl;
...
checkRange( const int index) const 
{
  ...
  {
    cerr << "ListOfFloatArray: List Index Out of Range!" << endl;
    ...
    Overture::abort("ListOfFloatArray: List Index Out of Range!");
  }  
}
