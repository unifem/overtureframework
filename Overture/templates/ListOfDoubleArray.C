...
ListOfDoubleArray::
ListOfDoubleArray( const ListOfDoubleArray & X, const CopyType copyType )
{
  switch (copyType)
  {
  case DEEP:
...
operator=(const ListOfDoubleArray &X)
{
  ...
  {
    cout << "ListOfDoubleArray:ERROR in operator=" << endl;
...
void ListOfDoubleArray::
reference( const ListOfDoubleArray & list )
{
  if( rcData->decrementReferenceCount() == 0 )
    deleteStuff();  
  rcData=list.rcData;
...
breakReference()
{
  ...
  {
    ListOfDoubleArray list = *this;  // makes a deep copy
  ...
  {	
    if(&X == rcData->aList[i])
    {
      ...
      return;
					...
					// Not there!
  cerr << "ListOfDoubleArray: Object not found in list for deletion!" << endl;
...
checkRange( const int index) const 
{
  ...
  {
    cerr << "ListOfDoubleArray: List Index Out of Range!" << endl;
    ...
    Overture::abort("ListOfDoubleArray: List Index Out of Range!");
  }  
}
