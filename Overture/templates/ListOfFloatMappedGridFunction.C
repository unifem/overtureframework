...
ListOfFloatMappedGridFunction::
ListOfFloatMappedGridFunction( const ListOfFloatMappedGridFunction & X, const CopyType copyType )
{
  switch (copyType)
  {
  case DEEP:
...
void ListOfFloatMappedGridFunction::
initialize()
{
  rcData = ::new RCData;
  rcData->incrementReferenceCount();
...
operator=(const ListOfFloatMappedGridFunction &X)
{
  ...
  {
    cout << "ListOfFloatMappedGridFunction:ERROR in operator=" << endl;
...
void ListOfFloatMappedGridFunction::
reference( const ListOfFloatMappedGridFunction & list )
{
  if( rcData->decrementReferenceCount() == 0 )
    deleteStuff();  
  rcData=list.rcData;
...
breakReference()
{
  ...
  {
    ListOfFloatMappedGridFunction list = *this;  // makes a deep copy
...
//   Whoever calls this function is responsible for "newing" a floatMappedGridFunction and adding it to the list
// =================================================================================================
{
  ...
  {  
    rcData->listLength++;
//    rcData->aList[rcData->listLength++]= ::new floatMappedGridFunction;
//    assert(rcData->aList[rcData->listLength-1]!=NULL);
  ...
  if(listLength() >= rcData->memAlloc)
  {
    ...
    floatMappedGridFunction **aListTmp;

    aListTmp = ::new floatMappedGridFunction* [rcData->memAlloc]; // create an array of pointers
...
//  rcData->aList[index] = ::new floatMappedGridFunction;			// Put it at the desired location.
...
addElement( const int index )
{
  ...
  rcData->aList[index]= ::new floatMappedGridFunction;
  assert(rcData->aList[index]!=NULL);
...
void ListOfFloatMappedGridFunction::
addElement( const floatMappedGridFunction & t, const int index )
{
//  addElement( index );
...
// *wdh   rcData->aList[index]= ::new floatMappedGridFunction(t,SHALLOW);  // trouble with A++ arrays
// *****
  rcData->aList[index]= ::new floatMappedGridFunction;
...
void ListOfFloatMappedGridFunction::
addElement( const floatMappedGridFunction & t )
{
...
void ListOfFloatMappedGridFunction::
deleteElement( const floatMappedGridFunction &X)
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
    {
      ...
      return;
					...
					// Not there!
  cerr << "ListOfFloatMappedGridFunction: Object not found in list for deletion!" << endl;
...
floatMappedGridFunction& ListOfFloatMappedGridFunction::
...
int ListOfFloatMappedGridFunction::
getIndex(const floatMappedGridFunction & X) const
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
...
checkRange( const int index) const 
{
  ...
  {
    cerr << "ListOfFloatMappedGridFunction: List Index Out of Range!" << endl;
    ...
    Overture::abort("ListOfFloatMappedGridFunction: List Index Out of Range!");
  }  
}
