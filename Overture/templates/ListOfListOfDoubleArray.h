#ifndef LISTOFLISTOFDOUBLEARRAY_H
#define LISTOFLISTOFDOUBLEARRAY_H "ListOfListOfDoubleArray.h"
// *wdh* for pgi: 110529
#ifdef __PGI
...
//==========================================================================
//	ListOfListOfDoubleArray Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class ListOfDoubleArray to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          ListOfDoubleArray::reference( const ListOfDoubleArray & t ) 
//          ListOfDoubleArray::breakReference()
//  that behave in the same way as A++ arrays.
//  In addition the assignment operator "=" must be deep copy.
//
//  This class keeps an array of pointers to objects of class ListOfDoubleArray. 
...
//
//     floatArray a(10),b(5);
//     ListOfListOfDoubleArray<floatArray> list,list2;
...
class ListOfListOfDoubleArray : public ReferenceCounting
{
  ...
  ListOfListOfDoubleArray(const ListOfListOfDoubleArray&, 
    const CopyType=DEEP  ); 

  ~ListOfListOfDoubleArray();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfListOfDoubleArray& operator=(const ListOfListOfDoubleArray&);
  ...
  void addElement( const ListOfDoubleArray & t, const int index ); // Add an object and reference to object t

  void addElement( const ListOfDoubleArray & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  ListOfDoubleArray& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const ListOfDoubleArray & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfListOfDoubleArray;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     ListOfDoubleArray **aList;		// Pointer to a list of pointers to object ListOfDoubleArray.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfListOfDoubleArray &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfListOfDoubleArray &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfListOfDoubleArray(*this, ct); }
};
