#ifndef LISTOFINTGENERICGRIDFUNCTION_H
#define LISTOFINTGENERICGRIDFUNCTION_H "ListOfIntGenericGridFunction.h"
// *wdh* for pgi: 110529
#ifdef __PGI
 #include "GenericDataBase.h"
#endif
class intGenericGridFunction;
#include "OvertureTypes.h"
#include "ReferenceCounting.h"

//==========================================================================
//	ListOfIntGenericGridFunction Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class intGenericGridFunction to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          intGenericGridFunction::reference( const intGenericGridFunction & t ) 
//          intGenericGridFunction::breakReference()
//  that behave in the same way as A++ arrays.
//  In addition the assignment operator "=" must be deep copy.
//
//  This class keeps an array of pointers to objects of class intGenericGridFunction. 
...
//
//     floatArray a(10),b(5);
//     ListOfIntGenericGridFunction<floatArray> list,list2;
...
class ListOfIntGenericGridFunction : public ReferenceCounting
{
  ...
  ListOfIntGenericGridFunction(const ListOfIntGenericGridFunction&, 
    const CopyType=DEEP  ); 

  ~ListOfIntGenericGridFunction();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfIntGenericGridFunction& operator=(const ListOfIntGenericGridFunction&);
  ...
  void addElement( const intGenericGridFunction & t, const int index ); // Add an object and reference to object t

  void addElement( const intGenericGridFunction & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  intGenericGridFunction& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const intGenericGridFunction & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfIntGenericGridFunction;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     intGenericGridFunction **aList;		// Pointer to a list of pointers to object intGenericGridFunction.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfIntGenericGridFunction &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfIntGenericGridFunction &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfIntGenericGridFunction(*this, ct); }
};
