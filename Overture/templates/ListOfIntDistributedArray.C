...
ListOfIntDistributedArray::
ListOfIntDistributedArray( const ListOfIntDistributedArray & X, const CopyType copyType )
{
  switch (copyType)
  {
  case DEEP:
...
void ListOfIntDistributedArray::
initialize()
{
  rcData = ::new RCData;
  rcData->incrementReferenceCount();
...
operator=(const ListOfIntDistributedArray &X)
{
  ...
  {
    cout << "ListOfIntDistributedArray:ERROR in operator=" << endl;
...
void ListOfIntDistributedArray::
reference( const ListOfIntDistributedArray & list )
{
  if( rcData->decrementReferenceCount() == 0 )
    deleteStuff();  
  rcData=list.rcData;
...
breakReference()
{
  ...
  {
    ListOfIntDistributedArray list = *this;  // makes a deep copy
...
//   Whoever calls this function is responsible for "newing" a intArray and adding it to the list
// =================================================================================================
{
  ...
  {  
    rcData->listLength++;
//    rcData->aList[rcData->listLength++]= ::new intArray;
//    assert(rcData->aList[rcData->listLength-1]!=NULL);
  ...
  if(listLength() >= rcData->memAlloc)
  {
    ...
    aListTmp = ::new intArray* [rcData->memAlloc]; // create an array of pointers
...
//  rcData->aList[index] = ::new intArray;			// Put it at the desired location.
...
addElement( const int index )
{
  ...
  rcData->aList[index]= ::new intArray;
  assert(rcData->aList[index]!=NULL);
...
void ListOfIntDistributedArray::
addElement( const intArray & t, const int index )
{
//  addElement( index );
...
// *wdh   rcData->aList[index]= ::new intArray(t,SHALLOW);  // trouble with A++ arrays
// *****
  rcData->aList[index]= ::new intArray;
...
void ListOfIntDistributedArray::
deleteElement( const intArray &X)
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
    {
      ...
      return;
					...
					// Not there!
  cerr << "ListOfIntDistributedArray: Object not found in list for deletion!" << endl;
...
intArray& ListOfIntDistributedArray::
...
int ListOfIntDistributedArray::
getIndex(const intArray & X) const
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
...
checkRange( const int index) const 
{
  ...
  {
    cerr << "ListOfIntDistributedArray: List Index Out of Range!" << endl;
    ...
    Overture::abort("ListOfIntDistributedArray: List Index Out of Range!");
  }  
}
