...
ListOfIntCompositeGridFunction::
ListOfIntCompositeGridFunction( const ListOfIntCompositeGridFunction & X, const CopyType copyType )
{
  switch (copyType)
  {
  case DEEP:
...
void ListOfIntCompositeGridFunction::
initialize()
{
  rcData = ::new RCData;
  rcData->incrementReferenceCount();
...
operator=(const ListOfIntCompositeGridFunction &X)
{
  ...
  {
    cout << "ListOfIntCompositeGridFunction:ERROR in operator=" << endl;
...
void ListOfIntCompositeGridFunction::
reference( const ListOfIntCompositeGridFunction & list )
{
  if( rcData->decrementReferenceCount() == 0 )
    deleteStuff();  
  rcData=list.rcData;
...
breakReference()
{
  ...
  {
    ListOfIntCompositeGridFunction list = *this;  // makes a deep copy
...
//   Whoever calls this function is responsible for "newing" a intCompositeGridFunction and adding it to the list
// =================================================================================================
{
  ...
  {  
    rcData->listLength++;
//    rcData->aList[rcData->listLength++]= ::new intCompositeGridFunction;
//    assert(rcData->aList[rcData->listLength-1]!=NULL);
  ...
  if(listLength() >= rcData->memAlloc)
  {
    ...
    intCompositeGridFunction **aListTmp;

    aListTmp = ::new intCompositeGridFunction* [rcData->memAlloc]; // create an array of pointers
...
//  rcData->aList[index] = ::new intCompositeGridFunction;			// Put it at the desired location.
...
addElement( const int index )
{
  ...
  rcData->aList[index]= ::new intCompositeGridFunction;
  assert(rcData->aList[index]!=NULL);
...
void ListOfIntCompositeGridFunction::
addElement( const intCompositeGridFunction & t, const int index )
{
//  addElement( index );
...
// *wdh   rcData->aList[index]= ::new intCompositeGridFunction(t,SHALLOW);  // trouble with A++ arrays
// *****
  rcData->aList[index]= ::new intCompositeGridFunction;
...
void ListOfIntCompositeGridFunction::
addElement( const intCompositeGridFunction & t )
{
...
void ListOfIntCompositeGridFunction::
deleteElement( const intCompositeGridFunction &X)
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
    {
      ...
      return;
					...
					// Not there!
  cerr << "ListOfIntCompositeGridFunction: Object not found in list for deletion!" << endl;
...
intCompositeGridFunction& ListOfIntCompositeGridFunction::
...
int ListOfIntCompositeGridFunction::
getIndex(const intCompositeGridFunction & X) const
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
...
checkRange( const int index) const 
{
  ...
  {
    cerr << "ListOfIntCompositeGridFunction: List Index Out of Range!" << endl;
    ...
    Overture::abort("ListOfIntCompositeGridFunction: List Index Out of Range!");
  }  
}
