#ifndef LISTOFINTARRAY_H
#define LISTOFINTARRAY_H "ListOfIntArray.h"
// *wdh* for pgi: 110529
#ifdef __PGI
...
//
//     floatArray a(10),b(5);
//     ListOfIntArray<floatArray> list,list2;
...
class ListOfIntArray : public ReferenceCounting
{
  ...
  ListOfIntArray(const ListOfIntArray&, 
    const CopyType=DEEP  ); 

  ~ListOfIntArray();			//   Destructor.
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfIntArray &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfIntArray &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfIntArray(*this, ct); }
};
