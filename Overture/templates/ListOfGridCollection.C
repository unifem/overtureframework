...
ListOfGridCollection::
ListOfGridCollection( const ListOfGridCollection & X, const CopyType copyType )
{
  switch (copyType)
  {
  case DEEP:
...
operator=(const ListOfGridCollection &X)
{
  ...
  {
    cout << "ListOfGridCollection:ERROR in operator=" << endl;
...
void ListOfGridCollection::
reference( const ListOfGridCollection & list )
{
  if( rcData->decrementReferenceCount() == 0 )
    deleteStuff();  
  rcData=list.rcData;
...
breakReference()
{
  ...
  {
    ListOfGridCollection list = *this;  // makes a deep copy
...
//   Whoever calls this function is responsible for "newing" a GridCollection and adding it to the list
// =================================================================================================
{
  ...
  {  
    rcData->listLength++;
//    rcData->aList[rcData->listLength++]= ::new GridCollection;
//    assert(rcData->aList[rcData->listLength-1]!=NULL);
  ...
  if(listLength() >= rcData->memAlloc)
  {
    ...
    GridCollection **aListTmp;

    aListTmp = ::new GridCollection* [rcData->memAlloc]; // create an array of pointers
...
//  rcData->aList[index] = ::new GridCollection;			// Put it at the desired location.
...
addElement( const int index )
{
  ...
  rcData->aList[index]= ::new GridCollection;
  assert(rcData->aList[index]!=NULL);
...
void ListOfGridCollection::
addElement( const GridCollection & t, const int index )
{
//  addElement( index );
...
// *wdh   rcData->aList[index]= ::new GridCollection(t,SHALLOW);  // trouble with A++ arrays
// *****
  rcData->aList[index]= ::new GridCollection;
...
void ListOfGridCollection::
addElement( const GridCollection & t )
{
...
void ListOfGridCollection::
deleteElement( const GridCollection &X)
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
    {
      ...
      return;
					...
					// Not there!
  cerr << "ListOfGridCollection: Object not found in list for deletion!" << endl;
...
GridCollection& ListOfGridCollection::
...
int ListOfGridCollection::
getIndex(const GridCollection & X) const
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
...
checkRange( const int index) const 
{
  ...
  {
    cerr << "ListOfGridCollection: List Index Out of Range!" << endl;
    ...
    Overture::abort("ListOfGridCollection: List Index Out of Range!");
  }  
}
