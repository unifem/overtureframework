#ifndef LISTOFINTGRIDCOLLECTIONFUNCTION_H
#define LISTOFINTGRIDCOLLECTIONFUNCTION_H "ListOfIntGridCollectionFunction.h"
// *wdh* for pgi: 110529
#ifdef __PGI
...
//==========================================================================
//	ListOfIntGridCollectionFunction Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class intGridCollectionFunction to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          intGridCollectionFunction::reference( const intGridCollectionFunction & t ) 
//          intGridCollectionFunction::breakReference()
//  that behave in the same way as A++ arrays.
//  In addition the assignment operator "=" must be deep copy.
//
//  This class keeps an array of pointers to objects of class intGridCollectionFunction. 
...
//
//     floatArray a(10),b(5);
//     ListOfIntGridCollectionFunction<floatArray> list,list2;
...
class ListOfIntGridCollectionFunction : public ReferenceCounting
{
  ...
  ListOfIntGridCollectionFunction(const ListOfIntGridCollectionFunction&, 
    const CopyType=DEEP  ); 

  ~ListOfIntGridCollectionFunction();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfIntGridCollectionFunction& operator=(const ListOfIntGridCollectionFunction&);
  ...
  void addElement( const intGridCollectionFunction & t, const int index ); // Add an object and reference to object t

  void addElement( const intGridCollectionFunction & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  intGridCollectionFunction& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const intGridCollectionFunction & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfIntGridCollectionFunction;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     intGridCollectionFunction **aList;		// Pointer to a list of pointers to object intGridCollectionFunction.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfIntGridCollectionFunction &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfIntGridCollectionFunction &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfIntGridCollectionFunction(*this, ct); }
};
