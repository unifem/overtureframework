#ifndef LISTOFGRIDCOLLECTION_H
#define LISTOFGRIDCOLLECTION_H "ListOfGridCollection.h"
// *wdh* for pgi: 110529
#ifdef __PGI
...
//
//  This class keeps an array of pointers to objects of class GridCollection. 
...
//
//     floatArray a(10),b(5);
//     ListOfGridCollection<floatArray> list,list2;
...
class ListOfGridCollection : public ReferenceCounting
{
  ...
  ListOfGridCollection(const ListOfGridCollection&, 
    const CopyType=DEEP  ); 

  ~ListOfGridCollection();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfGridCollection& operator=(const ListOfGridCollection&);
  ...
  void addElement( const GridCollection & t, const int index ); // Add an object and reference to object t

  void addElement( const GridCollection & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  GridCollection& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const GridCollection & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfGridCollection;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     GridCollection **aList;		// Pointer to a list of pointers to object GridCollection.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfGridCollection &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfGridCollection &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfGridCollection(*this, ct); }
};
