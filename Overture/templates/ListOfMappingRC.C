...
ListOfMappingRC::
ListOfMappingRC( const ListOfMappingRC & X, const CopyType copyType )
{
  switch (copyType)
  {
  case DEEP:
...
operator=(const ListOfMappingRC &X)
{
  ...
  {
    cout << "ListOfMappingRC:ERROR in operator=" << endl;
...
void ListOfMappingRC::
reference( const ListOfMappingRC & list )
{
  if( rcData->decrementReferenceCount() == 0 )
    deleteStuff();  
  rcData=list.rcData;
...
breakReference()
{
  ...
  {
    ListOfMappingRC list = *this;  // makes a deep copy
...
//   Whoever calls this function is responsible for "newing" a MappingRC and adding it to the list
// =================================================================================================
{
  ...
  {  
    rcData->listLength++;
//    rcData->aList[rcData->listLength++]= ::new MappingRC;
//    assert(rcData->aList[rcData->listLength-1]!=NULL);
  ...
  if(listLength() >= rcData->memAlloc)
  {
    ...
    aListTmp = ::new MappingRC* [rcData->memAlloc]; // create an array of pointers
...
//  rcData->aList[index] = ::new MappingRC;			// Put it at the desired location.
...
addElement( const int index )
{
  ...
  rcData->aList[index]= ::new MappingRC;
  assert(rcData->aList[index]!=NULL);
...
void ListOfMappingRC::
addElement( const MappingRC & t, const int index )
{
//  addElement( index );
...
// *wdh   rcData->aList[index]= ::new MappingRC(t,SHALLOW);  // trouble with A++ arrays
// *****
  rcData->aList[index]= ::new MappingRC;
...
void ListOfMappingRC::
deleteElement( const MappingRC &X)
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
    {
      ...
      return;
					...
					// Not there!
  cerr << "ListOfMappingRC: Object not found in list for deletion!" << endl;
...
int ListOfMappingRC::
getIndex(const MappingRC & X) const
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
...
checkRange( const int index) const 
{
  ...
  {
    cerr << "ListOfMappingRC: List Index Out of Range!" << endl;
    ...
    Overture::abort("ListOfMappingRC: List Index Out of Range!");
  }  
}
