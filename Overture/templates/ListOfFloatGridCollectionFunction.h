#ifndef LISTOFFLOATGRIDCOLLECTIONFUNCTION_H
#define LISTOFFLOATGRIDCOLLECTIONFUNCTION_H "ListOfFloatGridCollectionFunction.h"
// *wdh* for pgi: 110529
#ifdef __PGI
...
//==========================================================================
//	ListOfFloatGridCollectionFunction Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class floatGridCollectionFunction to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          floatGridCollectionFunction::reference( const floatGridCollectionFunction & t ) 
//          floatGridCollectionFunction::breakReference()
//  that behave in the same way as A++ arrays.
//  In addition the assignment operator "=" must be deep copy.
//
//  This class keeps an array of pointers to objects of class floatGridCollectionFunction. 
...
//
//     floatArray a(10),b(5);
//     ListOfFloatGridCollectionFunction<floatArray> list,list2;
...
class ListOfFloatGridCollectionFunction : public ReferenceCounting
{
  ...
  ListOfFloatGridCollectionFunction(const ListOfFloatGridCollectionFunction&, 
    const CopyType=DEEP  ); 

  ~ListOfFloatGridCollectionFunction();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfFloatGridCollectionFunction& operator=(const ListOfFloatGridCollectionFunction&);
  ...
  void addElement( const floatGridCollectionFunction & t, const int index ); // Add an object and reference to object t

  void addElement( const floatGridCollectionFunction & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  floatGridCollectionFunction& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const floatGridCollectionFunction & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfFloatGridCollectionFunction;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     floatGridCollectionFunction **aList;		// Pointer to a list of pointers to object floatGridCollectionFunction.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfFloatGridCollectionFunction &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfFloatGridCollectionFunction &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfFloatGridCollectionFunction(*this, ct); }
};
