...
ListOfDoubleGridCollectionFunction::
ListOfDoubleGridCollectionFunction( const ListOfDoubleGridCollectionFunction & X, const CopyType copyType )
{
  switch (copyType)
  {
  case DEEP:
...
void ListOfDoubleGridCollectionFunction::
initialize()
{
  rcData = ::new RCData;
  rcData->incrementReferenceCount();
...
operator=(const ListOfDoubleGridCollectionFunction &X)
{
  ...
  {
    cout << "ListOfDoubleGridCollectionFunction:ERROR in operator=" << endl;
...
void ListOfDoubleGridCollectionFunction::
reference( const ListOfDoubleGridCollectionFunction & list )
{
  if( rcData->decrementReferenceCount() == 0 )
    deleteStuff();  
  rcData=list.rcData;
...
breakReference()
{
  ...
  {
    ListOfDoubleGridCollectionFunction list = *this;  // makes a deep copy
...
//   Whoever calls this function is responsible for "newing" a doubleGridCollectionFunction and adding it to the list
// =================================================================================================
{
  ...
  {  
    rcData->listLength++;
//    rcData->aList[rcData->listLength++]= ::new doubleGridCollectionFunction;
//    assert(rcData->aList[rcData->listLength-1]!=NULL);
  ...
  if(listLength() >= rcData->memAlloc)
  {
    ...
    doubleGridCollectionFunction **aListTmp;

    aListTmp = ::new doubleGridCollectionFunction* [rcData->memAlloc]; // create an array of pointers
...
//  rcData->aList[index] = ::new doubleGridCollectionFunction;			// Put it at the desired location.
...
addElement( const int index )
{
  ...
  rcData->aList[index]= ::new doubleGridCollectionFunction;
  assert(rcData->aList[index]!=NULL);
...
void ListOfDoubleGridCollectionFunction::
addElement( const doubleGridCollectionFunction & t, const int index )
{
//  addElement( index );
...
// *wdh   rcData->aList[index]= ::new doubleGridCollectionFunction(t,SHALLOW);  // trouble with A++ arrays
// *****
  rcData->aList[index]= ::new doubleGridCollectionFunction;
...
void ListOfDoubleGridCollectionFunction::
addElement( const doubleGridCollectionFunction & t )
{
...
void ListOfDoubleGridCollectionFunction::
deleteElement( const doubleGridCollectionFunction &X)
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
    {
      ...
      return;
					...
					// Not there!
  cerr << "ListOfDoubleGridCollectionFunction: Object not found in list for deletion!" << endl;
...
swapElements( const int i, const int j)
{
  ...
  doubleGridCollectionFunction *tmp;
  tmp = rcData->aList[i];
...
doubleGridCollectionFunction& ListOfDoubleGridCollectionFunction::
...
int ListOfDoubleGridCollectionFunction::
getIndex(const doubleGridCollectionFunction & X) const
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
...
checkRange( const int index) const 
{
  ...
  {
    cerr << "ListOfDoubleGridCollectionFunction: List Index Out of Range!" << endl;
    ...
    Overture::abort("ListOfDoubleGridCollectionFunction: List Index Out of Range!");
  }  
}
