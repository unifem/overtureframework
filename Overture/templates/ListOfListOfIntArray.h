#ifndef LISTOFLISTOFINTARRAY_H
#define LISTOFLISTOFINTARRAY_H "ListOfListOfIntArray.h"
// *wdh* for pgi: 110529
#ifdef __PGI
 #include "GenericDataBase.h"
#endif
class ListOfIntArray;
#include "OvertureTypes.h"
#include "ReferenceCounting.h"

//==========================================================================
//	ListOfListOfIntArray Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class ListOfIntArray to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          ListOfIntArray::reference( const ListOfIntArray & t ) 
//          ListOfIntArray::breakReference()
//  that behave in the same way as A++ arrays.
//  In addition the assignment operator "=" must be deep copy.
//
//  This class keeps an array of pointers to objects of class ListOfIntArray. 
...
//
//     floatArray a(10),b(5);
//     ListOfListOfIntArray<floatArray> list,list2;
...
class ListOfListOfIntArray : public ReferenceCounting
{
  ...
  ListOfListOfIntArray(const ListOfListOfIntArray&, 
    const CopyType=DEEP  ); 

  ~ListOfListOfIntArray();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfListOfIntArray& operator=(const ListOfListOfIntArray&);
  ...
  void addElement( const ListOfIntArray & t, const int index ); // Add an object and reference to object t

  void addElement( const ListOfIntArray & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  ListOfIntArray& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const ListOfIntArray & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfListOfIntArray;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     ListOfIntArray **aList;		// Pointer to a list of pointers to object ListOfIntArray.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfListOfIntArray &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfListOfIntArray &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfListOfIntArray(*this, ct); }
};
