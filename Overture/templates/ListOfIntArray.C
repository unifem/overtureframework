...
ListOfIntArray::
ListOfIntArray( const ListOfIntArray & X, const CopyType copyType )
{
  switch (copyType)
  {
  case DEEP:
...
operator=(const ListOfIntArray &X)
{
  ...
  {
    cout << "ListOfIntArray:ERROR in operator=" << endl;
...
void ListOfIntArray::
reference( const ListOfIntArray & list )
{
  if( rcData->decrementReferenceCount() == 0 )
    deleteStuff();  
  rcData=list.rcData;
...
breakReference()
{
  ...
  {
    ListOfIntArray list = *this;  // makes a deep copy
  ...
  {	
    if(&X == rcData->aList[i])
    {
      ...
      return;
					...
					// Not there!
  cerr << "ListOfIntArray: Object not found in list for deletion!" << endl;
...
checkRange( const int index) const 
{
  ...
  {
    cerr << "ListOfIntArray: List Index Out of Range!" << endl;
    ...
    Overture::abort("ListOfIntArray: List Index Out of Range!");
  }  
}
