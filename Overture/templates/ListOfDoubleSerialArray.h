#ifndef LISTOFDOUBLESERIALARRAY_H
#define LISTOFDOUBLESERIALARRAY_H "ListOfDoubleSerialArray.h"
// *wdh* for pgi: 110529
#ifdef __PGI
...
//==========================================================================
//	ListOfDoubleSerialArray Class Template
//
//  This is a template list class for holding items that are reference
//  counted in the style of A++ arrays. In particular, any class doubleSerialArray to
//  be used with this list must have the following characteristics:
//
//  There must be member functions:
//          doubleSerialArray::reference( const doubleSerialArray & t ) 
...
//
//  This class keeps an array of pointers to objects of class doubleSerialArray. 
...
//
//     floatArray a(10),b(5);
//     ListOfDoubleSerialArray<floatArray> list,list2;
...
class ListOfDoubleSerialArray : public ReferenceCounting
{
  ...
  ListOfDoubleSerialArray(const ListOfDoubleSerialArray&, 
    const CopyType=DEEP  ); 

  ~ListOfDoubleSerialArray();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfDoubleSerialArray& operator=(const ListOfDoubleSerialArray&);
  ...
  void addElement( const doubleSerialArray & t, const int index ); // Add an object and reference to object t

  void addElement( const doubleSerialArray & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  doubleSerialArray& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const doubleSerialArray & X );		     // Find an element on the list and delete it.
...
// private:
 public:
  int listLength() const
   ...
   {
     friend class ListOfDoubleSerialArray;
     RCData(){ }     
     ~RCData(){ }
     int listLength;	// Number of elements in the list.
     int memAlloc;	// Current amount of memory allocated for the list.
     doubleSerialArray **aList;		// Pointer to a list of pointers to object doubleSerialArray.
   };
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfDoubleSerialArray &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfDoubleSerialArray &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfDoubleSerialArray(*this, ct); }
};
