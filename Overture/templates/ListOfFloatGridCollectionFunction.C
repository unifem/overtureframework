...
ListOfFloatGridCollectionFunction::
ListOfFloatGridCollectionFunction( const ListOfFloatGridCollectionFunction & X, const CopyType copyType )
{
  switch (copyType)
  {
  case DEEP:
...
void ListOfFloatGridCollectionFunction::
initialize()
{
  rcData = ::new RCData;
  rcData->incrementReferenceCount();
...
operator=(const ListOfFloatGridCollectionFunction &X)
{
  ...
  {
    cout << "ListOfFloatGridCollectionFunction:ERROR in operator=" << endl;
...
void ListOfFloatGridCollectionFunction::
reference( const ListOfFloatGridCollectionFunction & list )
{
  if( rcData->decrementReferenceCount() == 0 )
    deleteStuff();  
  rcData=list.rcData;
...
breakReference()
{
  ...
  {
    ListOfFloatGridCollectionFunction list = *this;  // makes a deep copy
...
//   Whoever calls this function is responsible for "newing" a floatGridCollectionFunction and adding it to the list
// =================================================================================================
{
  ...
  {  
    rcData->listLength++;
//    rcData->aList[rcData->listLength++]= ::new floatGridCollectionFunction;
//    assert(rcData->aList[rcData->listLength-1]!=NULL);
  ...
  if(listLength() >= rcData->memAlloc)
  {
    ...
    floatGridCollectionFunction **aListTmp;

    aListTmp = ::new floatGridCollectionFunction* [rcData->memAlloc]; // create an array of pointers
...
//  rcData->aList[index] = ::new floatGridCollectionFunction;			// Put it at the desired location.
...
addElement( const int index )
{
  ...
  rcData->aList[index]= ::new floatGridCollectionFunction;
  assert(rcData->aList[index]!=NULL);
...
void ListOfFloatGridCollectionFunction::
addElement( const floatGridCollectionFunction & t, const int index )
{
//  addElement( index );
...
// *wdh   rcData->aList[index]= ::new floatGridCollectionFunction(t,SHALLOW);  // trouble with A++ arrays
// *****
  rcData->aList[index]= ::new floatGridCollectionFunction;
...
void ListOfFloatGridCollectionFunction::
addElement( const floatGridCollectionFunction & t )
{
...
void ListOfFloatGridCollectionFunction::
deleteElement( const floatGridCollectionFunction &X)
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
    {
      ...
      return;
					...
					// Not there!
  cerr << "ListOfFloatGridCollectionFunction: Object not found in list for deletion!" << endl;
...
swapElements( const int i, const int j)
{
  ...
  floatGridCollectionFunction *tmp;
  tmp = rcData->aList[i];
...
floatGridCollectionFunction& ListOfFloatGridCollectionFunction::
...
int ListOfFloatGridCollectionFunction::
getIndex(const floatGridCollectionFunction & X) const
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
...
checkRange( const int index) const 
{
  ...
  {
    cerr << "ListOfFloatGridCollectionFunction: List Index Out of Range!" << endl;
    ...
    Overture::abort("ListOfFloatGridCollectionFunction: List Index Out of Range!");
  }  
}
