#ifndef LISTOFMAPPEDGRID_H
#define LISTOFMAPPEDGRID_H "ListOfMappedGrid.h"
// *wdh* for pgi: 110529
#ifdef __PGI
...
//
//     floatArray a(10),b(5);
//     ListOfMappedGrid<floatArray> list,list2;
...
class ListOfMappedGrid : public ReferenceCounting
{
  ...
  ListOfMappedGrid(const ListOfMappedGrid&, 
    const CopyType=DEEP  ); 

  ~ListOfMappedGrid();			//   Destructor.
    
  // Assignment operator (deep copy)
  ListOfMappedGrid& operator=(const ListOfMappedGrid&);
  ...
  void addElement( const MappedGrid & t, const int index ); // Add an object and reference to object t

  void addElement( const MappedGrid & t );                  // Add an object to the end, and reference to object t

  int getLength() const                      // Get length of list.
           {return rcData->listLength;};    

  MappedGrid& operator[](const int index) const;	     // Reference the object at a given location.

  void deleteElement( const MappedGrid & X );		     // Find an element on the list and delete it.
...
//
  virtual ReferenceCounting& operator=(const ReferenceCounting& x)
    { return operator=( (ListOfMappedGrid &)x); }
  virtual void reference( const ReferenceCounting& x)
    { reference( (ListOfMappedGrid &) x); }
  virtual ReferenceCounting* virtualConstructor( const CopyType ct = DEEP ) const
    { return ::new ListOfMappedGrid(*this, ct); }
};
