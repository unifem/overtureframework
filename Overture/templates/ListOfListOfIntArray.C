...
ListOfListOfIntArray::
ListOfListOfIntArray( const ListOfListOfIntArray & X, const CopyType copyType )
{
  switch (copyType)
  {
  case DEEP:
...
operator=(const ListOfListOfIntArray &X)
{
  ...
  {
    cout << "ListOfListOfIntArray:ERROR in operator=" << endl;
...
void ListOfListOfIntArray::
reference( const ListOfListOfIntArray & list )
{
  if( rcData->decrementReferenceCount() == 0 )
    deleteStuff();  
  rcData=list.rcData;
...
breakReference()
{
  ...
  {
    ListOfListOfIntArray list = *this;  // makes a deep copy
...
//   Whoever calls this function is responsible for "newing" a ListOfIntArray and adding it to the list
// =================================================================================================
{
  ...
  {  
    rcData->listLength++;
//    rcData->aList[rcData->listLength++]= ::new ListOfIntArray;
//    assert(rcData->aList[rcData->listLength-1]!=NULL);
  ...
  if(listLength() >= rcData->memAlloc)
  {
    ...
    ListOfIntArray **aListTmp;

    aListTmp = ::new ListOfIntArray* [rcData->memAlloc]; // create an array of pointers
...
//  rcData->aList[index] = ::new ListOfIntArray;			// Put it at the desired location.
...
addElement( const int index )
{
  ...
  rcData->aList[index]= ::new ListOfIntArray;
  assert(rcData->aList[index]!=NULL);
...
void ListOfListOfIntArray::
addElement( const ListOfIntArray & t, const int index )
{
//  addElement( index );
...
// *wdh   rcData->aList[index]= ::new ListOfIntArray(t,SHALLOW);  // trouble with A++ arrays
// *****
  rcData->aList[index]= ::new ListOfIntArray;
...
void ListOfListOfIntArray::
addElement( const ListOfIntArray & t )
{
...
void ListOfListOfIntArray::
deleteElement( const ListOfIntArray &X)
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
    {
      ...
      return;
					...
					// Not there!
  cerr << "ListOfListOfIntArray: Object not found in list for deletion!" << endl;
...
ListOfIntArray& ListOfListOfIntArray::
...
int ListOfListOfIntArray::
getIndex(const ListOfIntArray & X) const
{
  for(int i=0; i<listLength(); i++)
  {	
    if(&X == rcData->aList[i])
...
checkRange( const int index) const 
{
  ...
  {
    cerr << "ListOfListOfIntArray: List Index Out of Range!" << endl;
    ...
    Overture::abort("ListOfListOfIntArray: List Index Out of Range!");
  }  
}
