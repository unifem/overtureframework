...
#           -vIn=<> -rtolc=<> -atolc=<> -option=[none,piston3d,closed]
#
...
# -option : piston3d || piston3dAnnulus
# 
# ============================================================================
$grid=".hdf";  $tFinal=1.; $tPlot=.05; $bodyDensity=10.;  
$fluidDensity=1.; $go="halt"; $show=" "; $vIn=.1;
$nu=.05; $gravity=1.; $cfl=.9; $dtMax=.1; $restart=""; $debug=1; $ogesDebug=0; 
$option="none"; 
#
$amp=.25; $freq=1.; $depth=1.;  # for piston3d solution
#
...
$solver="best"; $psolver="best";  
$relaxRigidBody=0; # set to 1 for "light bodies" which are otherwise unstable (for TP -QT?)
...
$omega=.5; $inertia=1.; 
$rtolc=1.e-3; $atolc=1.e-7; 
$mbpbc=0; $mbpbcc=1.; # fix for light bodies: mbpbc=1 (for TP -QT?)
# ----------------------------- get command line arguments ---------------------------------------
GetOptions( "g=s"=>\$grid,"tf=f"=>\$tFinal,"degreex=i"=>\$degreex, "degreet=i"=>\$degreet, "model=s"=>\$model,\
 "tp=f"=>\$tPlot, "solver=s"=>\$solver,"psolver=s"=>\$psolver, "tz=s"=>\$tz, "show=s"=>\$show,"order=i"=>\$order,"debug=i"=>\$debug, "ogesDebug=i"=>\$ogesDebug, \
...
#-----------------------------
  ...
  specify grids to move
    rigid body
      ...
      if( $option eq "piston3d" || $option eq "piston3dAnnulus" ){ $cmd="initial velocity\n $vBody 0. 0. "; }else{ $cmd="#"; }
      $cmd
      ...
      if( $option eq "piston3d" || $option eq "piston3dAnnulus"){ $useKnownSolution=1; }else{ $useKnownSolution=0; }
...
if( $option eq "piston3d" || $option eq "piston3dAnnulus"){ $cmd="OBTZ:user defined known solution\n rigid body piston\n $amp $freq $depth\n done"; }else{ $cmd="#"; }
$cmd
# 
...
#
  boundary conditions
   all=slipWall
   plug(0,0)=noSlipWall
   if( $option eq "piston3dAnnulus" ) { $cmd ="innerAnnulus(0,1)=noSlipWall"; }else{ $cmd="#"; }
   $cmd
   if( $option eq "piston3d" ||  $option eq "piston3dAnnulus") { $cmd ="userDefinedBoundaryData\n known solution\n done"; }else{ $cmd=""; }
   box(1,0)=outflow, pressure(1.*p+0.*p.n=0.) $cmd
   # For testing a singular p-equation, make a closed container
   if( $option eq "closed" ){ $cmd="box(1,0)=noSlipWall"; }else{ $cmd="#"; }
   $cmd
  done
#
  boundary conditions...
   # for light moving bodies: (1=turn on for moving bodies, 2=turn on for all walls (for testing)
   # what is going on here? -QT
...
# 
  ...
  if( ($option eq "piston3d" || $option eq "piston3dAnnulus") && $restart eq "" ) { $iccmds="OBIC:known solution"; }
#
  initial conditions
    $iccmds
  exit
