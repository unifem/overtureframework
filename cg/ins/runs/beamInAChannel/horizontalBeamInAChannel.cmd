...
#       
#  cgins [-noplot] horizontalBeamInAChannel -g=<name> -tf=<tFinal> -tp=<tPlot> -solver=<yale/best> -ts=<implicit> -debug=<num> ..,
...
# 
# Examples:
#    cgins horizontalBeamInAChannel -g=horizontalBeamInAChannelGride2.order2.hdf -tf=1. -tp=.01
# 
...
# 
  ...
  boundary conditions
    all=noSlipWall 
    $halfH=0.2;
    if( $option eq "horizontalFlow" ){ $cmd = "bcNumber1=inflowWithVelocityGiven, parabolic(d=$halfH, p=1.,u=$uIn,T=$Tin)\n  bcNumber2=outflow, pressure(.1*p+$cpn*p.n=0.)\n bcNumber3=slipWall \n bcNumber4=slipWall"; }
#
    if( $option eq "upwardFlow" ){ $cmd="bcNumber3=inflowWithVelocityGiven, parabolic(d=$halfH, p=1.,v=$uIn,T=$Tin)\n bcNumber4=outflow, pressure(.1*p+$cpn*p.n=0.) \n bcNumber2=slipWall"; }
    $cmd
    #
