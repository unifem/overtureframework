# =============================================================================================
# cgins: 2d heart valve example (rbins example, modified from 2d stir)
# 
# Usage:
#   
#  cgins [-noplot] heartValveMoving2d -bcOption=<inflowOutflow|inflowGivenPressure|inflowCardiacCycle> -go=[run/halt/og]
#        -bodyForce=<restrictAngle|restricAngleDamp> -restart=*.show -restartframe=
#
#  inflowCardiacCycle: mimic a full cardiac cycle by changing the potential gradient over time
# 
#  -go : run, halt, og=open graphics
# 
# Examples: 
# 
#  cgins heartValveMoving2d -g=*.hdf -nu=.05 -tf=1. -tp=.025 -go=halt 
#
#
# =============================================================================================
# 
...
$nu=.05; $cfl=.9;
$ts="implicit";
$fullImplicit=0; #used to control the full implicit system for implicit velocit solver
#
#the following modified from cylDrop.cmd
$move=1; 
$density=10; 
...
#
$bodyForce="";
#to control the repulsive torque and restric the range of angle
#angle and deltaAngle is in raidan
...
GetOptions( "g=s"=>\$grid,"tf=f"=>\$tFinal,"move=i"=>\$move,\
 ...
 "rtol=f"=>\$rtol,"atol=f"=>\$atol,"rtolp=f"=>\$rtolp,"atolp=f"=>\$atolp,"restart=s"=>\$restart,"restartf=i"=>\$restartframe,\
 ...
 "bodyForce=s"=>\$bodyForce,"debug=i"=>\$debug,"fullImplicit=i"=>\$fullImplicit,\
...
#  bottome leaflet
   rigid body
    ...
    debug: $debug
     ...
     if( $inertia eq "-1" ){ $cmd="#"; }else{ $cmd="moments of inertia\n $inertia"; }
     $cmd
#   the current setup is symmetric
