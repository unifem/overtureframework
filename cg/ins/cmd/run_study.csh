...
#set gf = $Overture/sampleGrids/cylinder.cmd; set Dts=(0.1 0.025 0.00625 1.5625e-3); set is=(1 2 3 4); set gfac=(1 2 4 8); set tf=1; 
...
set gf = $Overture/sampleGrids/mycicArg.cmd; set Ns = (21 41 81 161 ); set Dts=(0.1 0.05 0.025 0.0125); set is=(1 2 3 4); set gfac=(2 4 8 16); set tf=.5; set gridtomove="Annulus"; set order=2; set ad2=1; set ad4=0; #set psolver=yale
...
cgins $CG/ins/cmd/tz   -move=$move nopause -gridToMove=$gridtomove -gridToMove2=$gridtomove2  -rate=.125 -ad2=$ad2 -ad4=$ad4  -rgd=fixed  -interp=e --grid_nge=1 --grid_factor=$gfa --boxorder=$order --grid_order=$order -cicorder=$order -tzorder=$order -g=$g -gf=$gf --N=$N -degreex=$degx -degreet=$degt  -ts=$ts -tz=$tz -newts=1 -debug=$debug --boxbc=$bbc --squarebc=$sbc -bcn=$bcn -fx=$fx -fy=$fy -fz=$fz  -ft=$ft -advectionCoefficient=$adc -nu=$nu -tf=$tf -go=$go -tp=$tp $noplot  -cfl=$cfl -dtMax=$tp -do=$do -psolver=$psolver -model=$model -kThermal=$kThermal -thermalExpansivity=$thermalExpansivity  -aftol=1e-10 -afit=40 -uplot="$uplot" |grep -E "^   0\.100|^   0\.500|^   1\.0|total number of grid" | tee -a convergence.out
