...
c============================================================================================
         ...
         if( useLocalTimeStepping.eq.0 )then
          ! -- Use a single global dt --
           if( gridIsImplicit.eq.0 )then
            ...
            !  getTimeSteppingEigenvaluesByOrder(INSVP,GLOBAL,EXPLICIT,NONE)
            ! else if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.0 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSVP,GLOBAL,EXPLICIT,AD2)
            ! else if( use2ndOrderAD.eq.0 .and. use4thOrderAD.eq.1 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSVP,GLOBAL,EXPLICIT,AD4)
            ! else if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.1 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSVP,GLOBAL,EXPLICIT,AD24)
            ! else
            !   stop 123
            ! end if
     ...
     & nuTy)/dx(1) )
                             ! "
                               nuMax= nu+nuT
                              ! **** finish this ****
                              rePart= crr*nuMax*( 1./dx(0)**2 + 1./dx(
     & 1)**2 )
                          if( use2ndOrderAD.eq.1 )then
                                rePart=rePart + 8.*( ad21 + cd22*( abs(
     ...
     & i3))
                             ! "
                               nuMax= nu+nuT
                              ! **** finish this ****
                              a1=a1 -nuMax*( rxx42(i1,i2,i3) + ryy42(
     & i1,i2,i3) )
                              a2=a2 -nuMax*( sxx42(i1,i2,i3) + syy42(
     & i1,i2,i3) )
                              rePart = nuMax*( ( rx(i1,i2,i3)**2       
     &    + ry(i1,i2,i3)**2          )*(crr/(dr(0)*dr(0))) +abs( rx(
     & i1,i2,i3)*sx(i1,i2,i3)+ry(i1,i2,i3)*sy(i1,i2,i3) )*( cr/(dr(0)*
     & dr(1))) +( sx(i1,i2,i3)**2          + sy(i1,i2,i3)**2          
     & )*(crr/(dr(1)*dr(1))) )
                            imPart=cr*(abs(a1)/dr(0)+abs(a2)/dr(1))
                          if( use2ndOrderAD.eq.1 )then
                                rePart=rePart + 8.*( ad21 + cd22*( abs(
     ...
     & i3)+nuTz*tz(i1,i2,i3))
                             ! "
                               nuMax= nu+nuT
                              ! **** finish this ****
                              a1=a1 -nuMax*( rxx43(i1,i2,i3)+ryy43(i1,
     & i2,i3)+rzz43(i1,i2,i3) )
                              a2=a2 -nuMax*( sxx43(i1,i2,i3)+syy43(i1,
     & i2,i3)+szz43(i1,i2,i3) )
                              a3=a3 -nuMax*( txx43(i1,i2,i3)+tyy43(i1,
     ...
     & i2,i3)
            ...
            ! Don't split by ADTYPE since this makes the file too long for no big benefit.
            ! if( use2ndOrderAD.eq.0 .and. use4thOrderAD.eq.0 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSVP,GLOBAL,IMPLICIT,NONE)
            ! else if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.0 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSVP,GLOBAL,IMPLICIT,AD2)
            ! else if( use2ndOrderAD.eq.0 .and. use4thOrderAD.eq.1 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSVP,GLOBAL,IMPLICIT,AD4)
            ! else if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.1 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSVP,GLOBAL,IMPLICIT,AD24)
            ! else
            !   stop 123
            ! end if
              if( orderOfAccuracy.eq.2 )then
     ...
     & abs(a3)/dr(2) )
          ...
          ! -- Use a local dt --
           if( gridIsImplicit.eq.0 )then
            ...
            ! Don't split by ADTYPE since this makes the file too long for no big benefit.
            ! if( use2ndOrderAD.eq.0 .and. use4thOrderAD.eq.0 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSVP,LOCAL,EXPLICIT,NONE)
            ! else if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.0 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSVP,LOCAL,EXPLICIT,AD2)
            ! else if( use2ndOrderAD.eq.0 .and. use4thOrderAD.eq.1 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSVP,LOCAL,EXPLICIT,AD4)
            ! else if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.1 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSVP,LOCAL,EXPLICIT,AD24)
            ! else
            !   stop 123
            ! end if
              if( orderOfAccuracy.eq.2 )then
     ...
     & rePart**2 )      ! explicit local time stepping
            ...
            ! Don't split by ADTYPE since this makes the file too long for no big benefit.
            ! if( use2ndOrderAD.eq.0 .and. use4thOrderAD.eq.0 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSVP,LOCAL,IMPLICIT,NONE)
            ! else if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.0 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSVP,LOCAL,IMPLICIT,AD2)
            ! else if( use2ndOrderAD.eq.0 .and. use4thOrderAD.eq.1 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSVP,LOCAL,IMPLICIT,AD4)
            ! else if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.1 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSVP,LOCAL,IMPLICIT,AD24)
            ! else
            !   stop 123
            ! end if
              if( orderOfAccuracy.eq.2 )then
