...
#define getIntFromDataBaseCgins EXTERN_C_NAME(getintfromdatabasecgins)
int getIntFromDataBaseCgins( DataBase *pdb, char *name_, int & value, int & nameLength)
// =======================================================================
/// \brief Use this routine from fortran to look-up a int or enum variables in the Cgins data base
///
/// \return values:
///                1=found, 
///                0=not found,
...
// =======================================================================
{
  ...
  // -- look for Cgins enums --
  if( name=="pdeModel" && dbase.has_key("pdeModel") )
  {
    value = (int) dbase.get<InsParameters::PDEModel >("pdeModel");
  }
  else if( name=="implicitVariation" && dbase.has_key("implicitVariation") )
  {
    value = (int)dbase.get<InsParameters::ImplicitVariation>("implicitVariation");
    return 1;
  }
  else 
  {
