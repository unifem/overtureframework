...
#define evalOscillatingBubbleVelocity EXTERN_C_NAME(evaloscillatingbubblevelocity)
#define evalOscillatingBubblePressure EXTERN_C_NAME(evaloscillatingbubblepressure)
...
#include "rotatingDiskPressure.h"
  ...
  else if( userKnownSolution=="oscillatingBubble" )
  {
	...
	// if (isnan(vrr)) {
	//   printF("vr: (x,y,r)=(%.3e,%.3e,%.3e), pr=%.3e\n",x,y,r,pr);
	// }
	// if (isnan(vtr)) {
	//   printF("vt: (x,y,r)=(%.3e,%.3e,%.3e), pr=%.3e\n",x,y,r,pr);
	// }
	...
	// if (count < 10) {
	//   printF("(x,y,r) = (%.16e,%.16e,%.16e)\n",x,y,r);
	//   printF("       vr=%.16e, vt=%.16e, p=%.16e\n",vr,vt,p);
	//   ++count;
	// }
	// convert to v1 and v2
