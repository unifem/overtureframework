! This file automatically generated from ins_merged_factors.bf with bpp.
! -*- mode: fortran -*- !
...
!!! subroutine ins_rfactor
      subroutine ins_mfactor(nd,nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     & index_range(1,2)
      ...
      interp_bc = .false. .and. ((bc(0,dir).eq.0 .and. idir.le.(
     & index_range(0,dir)+1)).or.(bc(1,dir).eq.0 .and. idir.ge.(
     & index_range(1,dir)-1)))
     ...
     & .lt.0
            ...
            near_interp = mask(i1m,i2m,i3m).lt.0 .or. mask(i1mm,i2mm,
     & i3mm).lt.0 .or. mask(i1p,i2p,i3p).lt.0 .or. mask(i1pp,i2pp,
     & i3pp).lt.0
         endif

         if ( near_interp ) then
            call get_op_coeffs(half_width,1,min(order,4),
     & finite_difference,px_coeff,dx_coeff)
            call get_op_coeffs(half_width,2,min(order,4),
     & finite_difference,pxx_coeff,dxx_coeff)
            off = 0
         elseif ( interp_bc .or. is_periodic .or. (idir.gt.(
     & index_range(0,dir)+1) .and. idir.lt.(index_range(1,dir)-1))) 
     & then
            call get_mop_coeffs(half_width,1,order,disc_approx,
     & px_coeff,dx_coeff)
            call get_mop_coeffs(half_width,2,order,disc_approx,
     & pxx_coeff,dxx_coeff)
            off =  0
         elseif ( .false. .and. idir.eq.index_range(0,dir) ) then
            call get_os_mop_coeffs(half_width,1,0,order,disc_approx,
     & px_coeff,dx_coeff)
            call get_os_mop_coeffs(half_width,2,0,order,disc_approx,
     & pxx_coeff,dxx_coeff)
            off = -2
         elseif ( .false. .and. idir.eq.index_range(1,dir) ) then
            call get_os_mop_coeffs(half_width,1,1,order,disc_approx,
     & px_coeff,dx_coeff)
            call get_os_mop_coeffs(half_width,2,1,order,disc_approx,
     & pxx_coeff,dxx_coeff)
            off =  2
         elseif ( idir.le.(index_range(0,dir)+1) ) then
            call get_bos_mop_coeffs(half_width,1,0,order,disc_approx,
     & px_coeff,dx_coeff)
            call get_bos_mop_coeffs(half_width,2,0,order,disc_approx,
     & pxx_coeff,dxx_coeff)
            off = -1
         else if ( idir.ge.(index_range(1,dir)-1) ) then
            call get_bos_mop_coeffs(half_width,1,1,order,disc_approx,
     & px_coeff,dx_coeff)
            call get_bos_mop_coeffs(half_width,2,1,order,disc_approx,
     & pxx_coeff,dxx_coeff)
            off =  1
         endif
...
C          endif
     ...
     & id)*advection_coefficient - nu_eq*rxx(io,dir,id) + nu_eq*4d0*
     & rxr(io,dir,id)*rx(io,dir,id)
               enddo
     ...
     & dxc(io)*a_ik(io) - dto2*drid*drid*dxxc(io)*b_ik(io))*ul(i1o,
     & i2o,i3o,cc)
            enddo
            ...
            do io=-half_width,half_width !!!x this loop may need to be changed if the grid does not have ghost points with sufficient width
               !!!                                 P U^{*}
                     ...
                     i2o = i2l + (io-off)*is2
                     i3o = i3l + (io-off)*is3
               rhs(i1,i2,i3) = rhs(i1,i2,i3) + pxc(io)*ul(i1o,i2o,i3o,
     & cc)
            enddo
               !!!                                 (P + (dt/2) D_r a_ik + (dt/2) D_rr b_ik) U^{**}_i
            io = 0+off
            d(i1,i2,i3)      = d(i1,i2,i3)   + dto2*drid*dxc(io)*a_ik(
     & io) + dto2*drid*drid*dxxc(io)*b_ik(io)
            if ( off.eq.0 .or. off.eq.-1 .or. off.eq.1 ) then
               io = -1+off
               dl1(i1,i2,i3)    = dl1(i1,i2,i3) + dto2*drid*dxc(io)*
     & a_ik(io) + dto2*drid*drid*dxxc(io)*b_ik(io)
               io = 1+off
...
! d u1 u2 l1 l2    penta
     ...
     & a_ik(io) + dto2*drid*drid*dxxc(io)*b_ik(io)
               if ( is_penta ) then
                  io = off+2
                  du2(i2,i3,i3) = du2(i1,i2,i3) + dto2*drid*dxc(io)*
     & a_ik(io) + dto2*drid*drid*dxxc(io)*b_ik(io)
                  io = off+3
                  dl1(i1,i2,i3) = dl1(i1,i2,i3) + dto2*drid*dxc(io)*
     & a_ik(io) + dto2*drid*drid*dxxc(io)*b_ik(io)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!! subroutine ins_rfactor
      subroutine ins_fscoeff(nd,nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,
     & nd4b,mask, rsxy, u, ul, gv, bc, boundaryCondition, ndbcd1a,
     & ndbcd1b,ndbcd2a,ndbcd2b,ndbcd3a,ndbcd3b,ndbcd4a,ndbcd4b,bcData,
     &  ipar, rpar, pdb, mode, dir, cc,dl2,dl1,d,du1,du2, rhs, ierr )
!     Incompressible Navier Stokes factor for an approximate factorization scheme
...
!!      do i1l=index_range(0,0)+ilo1,index_range(1,0)-ilo1
      ...
      if ( mask(i1,i2,i3).ne.0 ) then
         ...
         if ( mask(i1,i2,i3).gt.0 ) then
            if ( mask(i1m,i2m,i3m).lt.0 .or. mask(i1mm,i2mm,i3mm).lt.0 
     & .or. mask(i1p,i2p,i3p).lt.0 .or. mask(i1pp,i2pp,i3pp).lt.0 ) 
     & then
               call get_op_coeffs(half_width,1,min(order,4),
     & finite_difference,px_coeff,dx_coeff)
               call get_op_coeffs(half_width,2,min(order,4),
...
!!            else if ( is_periodic .or. (idir.gt.(index_range(0,dir)+2) .and. idir.lt.(index_range(1,dir)-2))) then
            else if ( is_periodic .or. (idir.gt.(gir(0,dir)+2) .and. 
     & idir.lt.(gir(1,dir)-2))) then
               call get_mop_coeffs(half_width,1,order,disc_approx,
     & px_coeff,dx_coeff)
               call get_mop_coeffs(half_width,2,order,disc_approx,
     & pxx_coeff,dxx_coeff)
               ...
               call get_op_coeffs(half_width,1,order,finite_difference,
     & px_coeff,dx_coeff)
               call get_op_coeffs(half_width,2,order,finite_difference,
     & pxx_coeff,dxx_coeff)
               off =  0
            else if ( (idir.eq.(gir(0,dir)+2) .or. idir.eq.(gir(1,dir)-
     & 2))) then
               call get_op_coeffs(half_width,1,order,disc_approx,
     & px_coeff,dx_coeff)
               call get_op_coeffs(half_width,2,order,disc_approx,
     & pxx_coeff,dxx_coeff)
               off =  0
            else if (idir.eq.(gir(0,dir)+1)) then
               call get_bos_mop_coeffs(half_width,1,0,order,
     & disc_approx,px_coeff,dx_coeff)
               call get_bos_mop_coeffs(half_width,2,0,order,
     & disc_approx,pxx_coeff,dxx_coeff)
               off = -1
            else if ( idir.eq.(gir(1,dir)-1) ) then
               call get_bos_mop_coeffs(half_width,1,1,order,
     & disc_approx,px_coeff,dx_coeff)
               call get_bos_mop_coeffs(half_width,2,1,order,
     & disc_approx,pxx_coeff,dxx_coeff)
               off =  1
     ...
     & io,1,ic)*rxs(id,ic,1) + rx(io,2,ic)*rxs(id,ic,2)
         ...
         do io=-half_width,half_width !!!x this loop may need to be changed if the grid does not have ghost points with sufficient width
            ...
            do id=0,nd-1
               b_ik(io) = b_ik(io) + rx(io,dir,id)*rx(io,dir,id)
               a_ik(io) = a_ik(io) + u(i1o,i2o,i3o,uc+id)*rx(io,dir,id)
     & *advection_coefficient - nu_eq*rxx(io,dir,id)
            enddo
            b_ik(io) = -nu_eq*b_ik(io)

            if (is_moving) then
               do id=0,nd-1
                  a_ik(io) = a_ik(io) - gv(i1o,i2o,i3o,id)*rx(io,dir,
     & id)
               enddo
            endif
         enddo
...
c         !!!! The - sign before the second term is due to the way we have implemented the curvilinear grid correction
            rhs(i1,i2,i3) = rhs(i1,i2,i3) + drid*dxc(io)*a_ik(io) - 
     & drid*drid*dxxc(io)*b_ik(io)
         enddo
      else
      ...
      if ( .not. is_periodic ) then
         ...
         bc_range(1,1,dir) = index_range(1,dir)
               do side=0,1
               if ( bc(side,dir).ge.0 .or. bc(side,dir).eq.parallelBC 
     & .or. (bc(side,dir).lt.0 .and. bc(mod(side+1,2),dir)
     & .eq.parallelBC)) then
               do i3l=bc_range(side,0,2),bc_range(side,1,2)
               do i2l=bc_range(side,0,1),bc_range(side,1,1)
               do i1l=bc_range(side,0,0),bc_range(side,1,0)
...
!!! subroutine ins_evalux
      subroutine ins_evalux(nd,nd1a,nd1b,nd2a,nd2b,nd3a,nd3b,nd4a,nd4b,
     & mask, rsxy, u, ul, gv, bc, boundaryCondition, ndbcd1a,ndbcd1b,
     & ndbcd2a,ndbcd2b,ndbcd3a,ndbcd3b,ndbcd4a,ndbcd4b,bcData, ipar, 
     & rpar, pdb, mode, dir, cc,dl2,dl1,d,du1,du2, rhs, ierr )
!     Incompressible Navier Stokes factor for an approximate factorization scheme
!     
!     INPUT::
!     nd : number of dimensions
...
!!!      do i1l=gir(0,0),gir(1,0)
      ...
      if ( mask(i1,i2,i3).ne.0 ) then
         ...
         if ( mask(i1,i2,i3).gt.0 ) then
            if ( is_periodic .or. (idir.ge.(gir(0,dir)+2) .and. 
     & idir.le.(gir(1,dir)-2))) then
               call get_mop_coeffs(half_width,1,order,disc_approx,
     & px_coeff,dx_coeff)
               call get_mop_coeffs(half_width,2,order,disc_approx,
     & pxx_coeff,dxx_coeff)
...
!!! NOT NEEDED FOR PARAMETRIC DERIVATIVE 100929         EVALUATE_JACOBIAN_DERIVATIVES(half_width)
         ...
         do io=-half_width,half_width
                  ...
                  i2o = i2l + (io-off)*is2
                  i3o = i3l + (io-off)*is3
            rhs(i1,i2,i3) = rhs(i1,i2,i3) + drid*dxc(io)*u(i1o,i2o,i3o,
     & cc)
         enddo
