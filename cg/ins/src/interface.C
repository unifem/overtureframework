	  ...
	  // find a previous time value we can use
	    ...
	    //   -- for real : just assume constant ? 
            ...
            // f(t) = (t-tp)/(t0-tp) *f(t0) + (t0-t)/(t0-tp) *f(tp)
          	    ...
          	    if( debug() & 2 )
            	      printP("Cgins::interfaceRHS: Extrapolate traction in time: t0=%9.3e, tp=%9.3e, cex1=%9.3e cex2=%9.3e\n",
                 		     t0,tp,cex1,cex2);
	      ...
	      //   l2 = (t-t0)*(t-t1)/( (t2-t0)*(t2-t1) );
            	      ...
            	      if( debug() & 2 )
            		printP("Cgins::interfaceRHS: get d(traction)/dt (2nd order): t=%9.3e, (t0=%9.3e) "
