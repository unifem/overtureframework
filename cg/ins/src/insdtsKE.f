! This file automatically generated from insdts.bf with bpp.
        subroutine insdtsKE(nd, n1a,n1b,n2a,n2b,n3a,n3b, nd1a,nd1b,
...
c============================================================================================
     ...
     & adzzmCSA(i1  ,i2  ,i3+1)*(u(i1,i2,i3+1,c)-u(i1,i2,i3,c))
       ...
       ! epsVP             =rpar(30)
        kc=nc
        ec=kc+1
          write(*,'("*** insdts INSKE: Entering.. gridIsImplicit=",i2,
         ...
         if( useLocalTimeStepping.eq.0 )then
          ! -- Use a single global dt --
           if( gridIsImplicit.eq.0 )then
            ...
            !  getTimeSteppingEigenvaluesByOrder(INSKE,GLOBAL,EXPLICIT,NONE)
            ! else if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.0 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSKE,GLOBAL,EXPLICIT,AD2)
            ! else if( use2ndOrderAD.eq.0 .and. use4thOrderAD.eq.1 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSKE,GLOBAL,EXPLICIT,AD4)
            ! else if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.1 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSKE,GLOBAL,EXPLICIT,AD24)
            ! else
            !   stop 123
            ! end if
     ...
     & nuTy)/dx(1) )
                           ! "
                             nuMax= max( nu+nuT, nu+sigmaKI*nuT, nu+
     & sigmaEpsI*nuT )
                            ! **** finish this ****
     ...
     & i2,i3)
            ...
            ! Don't split by ADTYPE since this makes the file too long for no big benefit.
            ! if( use2ndOrderAD.eq.0 .and. use4thOrderAD.eq.0 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSKE,GLOBAL,IMPLICIT,NONE)
            ! else if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.0 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSKE,GLOBAL,IMPLICIT,AD2)
            ! else if( use2ndOrderAD.eq.0 .and. use4thOrderAD.eq.1 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSKE,GLOBAL,IMPLICIT,AD4)
            ! else if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.1 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSKE,GLOBAL,IMPLICIT,AD24)
            ! else
            !   stop 123
            ! end if
              if( orderOfAccuracy.eq.2 )then
     ...
     & abs(a3)/dr(2) )
          ...
          ! -- Use a local dt --
           if( gridIsImplicit.eq.0 )then
            ...
            ! Don't split by ADTYPE since this makes the file too long for no big benefit.
            ! if( use2ndOrderAD.eq.0 .and. use4thOrderAD.eq.0 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSKE,LOCAL,EXPLICIT,NONE)
            ! else if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.0 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSKE,LOCAL,EXPLICIT,AD2)
            ! else if( use2ndOrderAD.eq.0 .and. use4thOrderAD.eq.1 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSKE,LOCAL,EXPLICIT,AD4)
            ! else if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.1 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSKE,LOCAL,EXPLICIT,AD24)
            ! else
            !   stop 123
            ! end if
              if( orderOfAccuracy.eq.2 )then
     ...
     & 2 )      ! explicit local time stepping
            ...
            ! Don't split by ADTYPE since this makes the file too long for no big benefit.
            ! if( use2ndOrderAD.eq.0 .and. use4thOrderAD.eq.0 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSKE,LOCAL,IMPLICIT,NONE)
            ! else if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.0 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSKE,LOCAL,IMPLICIT,AD2)
            ! else if( use2ndOrderAD.eq.0 .and. use4thOrderAD.eq.1 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSKE,LOCAL,IMPLICIT,AD4)
            ! else if( use2ndOrderAD.eq.1 .and. use4thOrderAD.eq.1 )then 
            !  getTimeSteppingEigenvaluesByOrder(INSKE,LOCAL,IMPLICIT,AD24)
            ! else
            !   stop 123
            ! end if
              if( orderOfAccuracy.eq.2 )then
