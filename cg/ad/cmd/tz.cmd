...
# 
  boundary conditions
    if( $bc eq "d" ){ $bcCommand="all=dirichletBoundaryCondition"; }elsif( $bc eq "m"){ $bcCommand="all=mixedBoundaryCondition, mixedDerivative(1.*t+1.*t.n=0.)"; }elsif( $bc eq "dm"){ $bcCommand="all=dirichletBoundaryCondition\n square(1,0)=mixedBoundaryCondition"; }else{ $bcCommand="all=neumannBoundaryCondition"; }
 # ****
