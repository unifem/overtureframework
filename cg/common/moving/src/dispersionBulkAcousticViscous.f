...
! Exact solution for a viscous incompressible flow and an Acoustic bulk solid
...
! ---------------------------------------------------------------------------------- 
      subroutine dispersionViscousAcoustic(wr,wi, detr,deti,
     ...
     #* Ca)
      ...
      a23 = -II * w * Sas
      a24 = 0
      a31 = -2 * II * u1Varies * mu * k * (a * Ca * Sk - k * Ck * Sa) * 
     #Sk
      a32 = -u1Varies * mu * (II / k * a ** 2 * (a * Ca * Sk - k * Ck * 
...
! u1Varies=0 : u1==0,  u1Varies=1 : u1 is allowed to vary.
! ---------------------------------------------------------------------------------- 
      subroutine dispersionViscousAcousticDeriv(wr,wi, dwr,dwi,
     &             rho,mu,H,k,rhos,cp,cs,Hs,u1Varies )
      implicit none 
     ...
     #Sa + a * Ca * Sk) * dSadw
      a23 = -II * w * Sas
      da23 = -II * Sas - II * w * dSasdw
      a24 = 0
      da24 = 0
      a31 = -2 * II * u1Varies * mu * k * (a * Ca * Sk - k * Ck * Sa) * 
     #Sk
      da31 = -2 * II * u1Varies * mu * k * Ca * Sk ** 2 * dadw - 2 * II 
     #* u1Varies * mu * k * a * Sk ** 2 * dCadw + 2 * II * u1Varies * mu
     # * k ** 2 * Ck * Sk * dSadw
      a32 = -u1Varies * mu * (II / k * a ** 2 * (a * Ca * Sk - k * Ck * 
     #Sa) * Sa + II * k * (a * Ca * Sk - k * Ck * Sa) * Sa)
      da32 = -u1Varies * mu * (2 * II / k * a * (a * Ca * Sk - k * Ck * 
     #Sa) * Sa + II / k * a ** 2 * Ca * Sk * Sa + II * k * Ca * Sk * Sa)
     # * dadw - u1Varies * mu * (II / k * a ** 3 * Sk * Sa + II * k * a 
     #* Sk * Sa) * dCadw - u1Varies * mu * (-II * a ** 2 * Ck * Sa + II 
     #/ k * a ** 2 * (a * Ca * Sk - k * Ck * Sa) - II * k ** 2 * Ck * Sa
     # + II * k * (a * Ca * Sk - k * Ck * Sa)) * dSadw
      a33 = 0
      da33 = 0
      a34 = 1 - u1Varies + u1Varies * rcs2 * bs * Cbs
      da34 = u1Varies * rcs2 * Cbs * dbsdw + u1Varies * rcs2 * bs * dCbs
     #dw
      a41 = II * rho * w / k * (k * Sa + (a * Ca * Sk - k * Ck * Sa) * C
     #k) - 2 * mu * (k ** 2 * Sa - k * a * Sk + (a * Ca * Sk - k * Ck * 
     #Sa) * k * Ck)
     ...
     # k * Ck * a * Ca)) * dSadw
      a43 = rcp2 * as * Cas
      da43 = rcp2 * Cas * dasdw + rcp2 * as * dCasdw
      a44 = 0
      da44 = 0
      dw = da21 * a32 * a13 * a44 + a31 * a12 * a23 * da44 + da21 * a12 
     #* a34 * a43 - da21 * a12 * a33 * a44 - da21 * a32 * a14 * a43 - da
     #11 * a32 * a23 * a44 - da11 * a22 * a34 * a43 + da11 * a22 * a33 *
     # a44 - da11 * a42 * a24 * a33 + da11 * a42 * a23 * a34 + da21 * a4
     #2 * a14 * a33 + da31 * a12 * a23 * a44 - da31 * a12 * a24 * a43 - 
     #da31 * a22 * a13 * a44 + da31 * a22 * a14 * a43 + da31 * a42 * a13
     # * a24 - da31 * a42 * a14 * a23 - da41 * a12 * a23 * a34 + da41 * 
     #a12 * a24 * a33 + da41 * a22 * a13 * a34 - da41 * a22 * a14 * a33 
     #- da41 * a32 * a13 * a24 + da41 * a32 * a14 * a23 + a21 * a42 * da
     #14 * a33 + a11 * da22 * a33 * a44 - a11 * da22 * a34 * a43 - a11 *
     # da32 * a23 * a44 + a11 * da32 * a24 * a43 + a11 * da42 * a23 * a3
     #4 - a11 * da42 * a24 * a33 - a21 * da12 * a33 * a44 + a21 * da12 *
     # a34 * a43 + a21 * da32 * a13 * a44 - a21 * da32 * a14 * a43 - a21
     # * da42 * a13 * a34 + a21 * da42 * a14 * a33 + a31 * da12 * a23 * 
     #a44 - a31 * da12 * a24 * a43 - a31 * da22 * a13 * a44 + a31 * da22
     # * a14 * a43 + a31 * da42 * a13 * a24 - a31 * da42 * a14 * a23 - a
     #41 * da12 * a23 * a34 + a41 * da12 * a24 * a33 + a41 * da22 * a13 
     #* a34 - a41 * da22 * a14 * a33 - a41 * da32 * a13 * a24 + a41 * da
     #32 * a14 * a23 - a21 * a42 * a13 * da34 + a11 * a22 * da33 * a44 -
     # a11 * a22 * a34 * da43 - a11 * a32 * da23 * a44 + a11 * a32 * a24
     # * da43 + a11 * a42 * da23 * a34 - a11 * a42 * a24 * da33 - a21 * 
     #a12 * da33 * a44 + a21 * a12 * a34 * da43 + a21 * a32 * da13 * a44
     # - a21 * a32 * a14 * da43 - a21 * a42 * da13 * a34 + a21 * a42 * a
     #14 * da33 + a31 * a12 * da23 * a44 - a31 * a12 * a24 * da43 - a31 
     #* a22 * da13 * a44 + a31 * a22 * a14 * da43 + a31 * a42 * da13 * a
     #24 - a31 * a42 * a14 * da23 - a41 * a12 * da23 * a34 + a41 * a12 *
     # a24 * da33 + a41 * a22 * da13 * a34 - a41 * a22 * a14 * da33 - a4
     #1 * a32 * da13 * a24 + a41 * a32 * a14 * da23 + a11 * a22 * a33 * 
     #da44 - a11 * a22 * da34 * a43 - a11 * a32 * a23 * da44 + a11 * a32
     # * da24 * a43 + a11 * a42 * a23 * da34 - a11 * a42 * da24 * a33 - 
     #a21 * a12 * a33 * da44 + a21 * a12 * da34 * a43 + a21 * a32 * a13 
     #* da44 - a21 * a32 * da14 * a43 - a31 * a12 * da24 * a43 - a31 * a
     #22 * a13 * da44 + a31 * a22 * da14 * a43 + a31 * a42 * a13 * da24 
     #- a31 * a42 * da14 * a23 - a41 * a12 * a23 * da34 + a41 * a12 * da
     #24 * a33 + a41 * a22 * a13 * da34 - a41 * a22 * da14 * a33 - a41 *
     # a32 * a13 * da24 + a41 * a32 * da14 * a23 + da11 * a32 * a24 * a4
     #3 - da21 * a42 * a13 * a34
      dwr=real(dw)
      dwi=imag(dw)
      return
