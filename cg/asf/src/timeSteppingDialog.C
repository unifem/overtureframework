...
// ====================================================================================
{
  ...
  int nt=0,initialChoice=0;
  cmd[nt]="implicitAllSpeed";   nt++; 
  cmd[nt]="implicitAllSpeed2";  nt++; 
  cmd[nt]="all speed implicit";   nt++; 
  cmd[nt]="linearized all speed implicit";   initialChoice=nt; nt++; 
  ...
  if( nt>0 )
    dialog.addOptionMenu("method", cmd, cmd, initialChoice);

  // build all the generic options now
  DomainSolver::buildTimeSteppingDialog(dialog);

  // Over-ride the defaults
  dialog.setWindowTitle("Cgasf Time Stepping Parameters");
...
//====================================================================
{
  ...
  if( answer=="forwardEuler" || answer=="forward Euler" )
  {
    ...
    parameters.dbase.get<int >("explicitMethod")=true;
  }
  else if( answer=="linearized all speed implicit" )
  {
    ...
    dialog.getOptionMenu("method").setCurrentChoice("all speed implicit");
  }
  else if( answer=="all speed implicit" || answer=="implicitAllSpeed" )
  {
    parameters.dbase.get<Parameters::TimeSteppingMethod >("timeSteppingMethod")=Parameters::implicitAllSpeed;
    parameters.dbase.get<int >("linearizeImplicitMethod")=false;
    dialog.getOptionMenu("method").setCurrentChoice("all speed implicit");
  }
  else if( answer=="implicitAllSpeed2" )
  {
    parameters.dbase.get<Parameters::TimeSteppingMethod >("timeSteppingMethod")=Parameters::implicitAllSpeed2;
    dialog.getOptionMenu("method").setCurrentChoice("implicitAllSpeed");
  }
