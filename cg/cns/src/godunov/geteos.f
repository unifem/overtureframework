      subroutine geteos (r,e,y,vs,vg,p,dp,iform,ier)
c
c Evaluate the equation of state p=p(rho,rho*e,rho*lambda) for the
c mixture JWL EOS used for Ignition and Growth detonation modeling.
c
c Variables: r=density, rho
c            e=internal energy per unit volume, E-rho*|velocity|^2/2
c            y=volume fraction of gas products, rho*lambda
c            vs=specific volume of solid explosive
c            vg=specific volume of gas products
c            p=pressure
c            dp=derivative of pressure w/r.t. (r,e,y)
c            iform=integer flag (see below)
c            ier=error return flag
c
c
c Note: if y/r>1-tiny or if vs<tiny, then all of the solid is considered
c       to be gone.  In this case, vs=0 is taken (vs is no longer needed)
c       and vg=1/r.  Newton's method is no longer needed to evaluate the
c       EOS model.
c
c
...
c                   pause
c                   end if
                    call fail1 (r1,e1,y1,vs,vg,vshold,vghold,
     *                          dtemp,err,vmin)
                    if (err.gt.tol) then
                      ifail=1
                      ferr=err
...
c       more than one iteration was needed...
            ...
            if (dabs(t3).lt.eps) then
              write(6,*)'Error (geteos) : bad error, t3=0'
              stop
            end if
            p=t2/t3
          end if
...
c                 pause
c                 end if
                  call fail2 (r1,p1,y1,vs,vg,vshold,vghold,
     *                        dtemp,err,vmin)
                  if (err.gt.tol) then
                    ifail=1
                    ferr=err
...
c+++++++++++++++++++
c
      subroutine fail1 (r,e,y,vs,vg,vshold,vghold,dtemp,err,vmin)
c
      implicit real*8 (a-h,o-z)
      include 'eosdat.h'
c      common / eosdat / omeg(2),ajwl(2,2),rjwl(2,2),vs0,ts0,
...
c
      err=0. ! *wdh* 2012/03/25 err is checked on return so we need to always give it a value
      x=r-y
      alam=y/r
c
...
c+++++++++++++++++++
c
      subroutine fail2 (r,p,y,vs,vg,vshold,vghold,dtemp,err,vmin)
c
      implicit real*8 (a-h,o-z)
      include 'eosdat.h'
c      common / eosdat / omeg(2),ajwl(2,2),rjwl(2,2),vs0,ts0,
