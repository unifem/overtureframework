...
getInterfaceDataOptions( GridFaceDescriptor & info, int & interfaceDataOptions ) const
{
    ...
    {
        printP("Cgcns::getInterfaceDataOptions:ERROR: invalid values: (grid,side,axis)=(%i,%i,%i)\n",
         	   grid,side,axis);
        OV_ABORT("Cgcns::getInterfaceDataOptions:ERROR");
    }
    ...
    else if( interfaceType(side,axis,grid)==Parameters::tractionInterface ) 
    {
        ...
        if( debug() & 2 )
            printP("*** Cgcns:getInterfaceDataOptions: projectInterface = %i ***\n",projectInterface);
    ...
    {
        printP("Cgcns::getInterfaceDataOptions:ERROR: interfaceType(grid=%i,side=%i,axis=%i)=%i\n",
                                                ...
                                                bool saveTimeHistory /* = false */ )
{
  ...
  // CompositeGrid & cg = gf[0].cg;
    ...
    {
        printP("Cgcns::interfaceRightHandSide:ERROR: invalid values: (grid,side,axis)=(%i,%i,%i)\n",
         	   grid,side,axis);
        OV_ABORT("Cgcns::interfaceRightHandSide:ERROR");
    }
    ...
    // ****************************************
        ...
        if( debug() & 4 )
        {
            printP("Cgcns::interfaceRHS:heatFlux %s RHS for (side,axis,grid)=(%i,%i,%i) a=[%5.2f,%5.2f]"
    ...
    // We could optimize this for rectangular grids 
      ...
      // **** get the RHS ****
        ...
        {
            printF("Cgcns::interfaceRightHandSide:ERROR: unknown option=%i\n",option);
    ...
    // *******************************************
        ...
        if( debug() & 2 )
            printP("Cgcns::interfaceRHS: TRACTION: %s RHS for (grid,side,axis)=(%i,%i,%i) "
      ...
      //   (TZ is done below) <- todo 
        ...
        // --- interface position is given ---
      	...
      	if( debug() & 2 )
        	  printP("Cgcns::interfaceRHS: Set interface positions at t=%9.3e in bd components V=[%i,%i] from C=[%i,%i]\n",
        ...
        // --- interface velocity is given ---
      	C=Range(numSaved,numSaved+numberOfDimensions-1);
                if( debug() & 4 )
        	  printP("Cgcns:interfaceRightHandSide: interface velocity provided t=%9.3e in C=[%i,%i]\n",t,C.getBase(),C.getBound());
	...
	// bd(I1,I2,I3,Dc)=f(I1,I2,I3,C);     // where should we put this?          
        ...
        // --- interface acceleration is given ---
      	C=Range(numSaved,numSaved+numberOfDimensions-1);
                if( debug() & 4 )
        	  printP("Cgcns:interfaceRightHandSide: interface acceleration provided t=%9.3e in C=[%i,%i]\n",t,C.getBase(),C.getBound());
	...
	// bd(I1,I2,I3,Dc)=f(I1,I2,I3,C);    // where should we put this?
        ...
        // --- interface traction is given ---
                if( debug() & 4 )
        	  printP("Cgcns:interfaceRightHandSide: interface traction provided t=%9.3e\n",t);
	...
	// save the time derivative of the traction:
                if( debug() & 4 )
        	  printP("Cgcns:interfaceRightHandSide: the tractionRate is provided at t=%9.3e\n",t);
	...
	// -- return the position of the boundary --
      	if( debug() & 4 )
        	  printP("Cgcns:interfaceRightHandSide: Save the interface position.\n");
	...
	// -- return the interface velocity --
      	if( debug() & 4 )
        	  printP("Cgcns:interfaceRightHandSide: Save the interface velocity.\n");
	...
	// -- return the acceleration of the boundary --
      	if( debug() & 4 )
        	  printP("Cgcns:interfaceRightHandSide: Save the interface acceleration.\n");

      	Range Rx(0,numberOfDimensions-1);                // vertex components 
      	C=Range(numSaved,numSaved+numberOfDimensions-1); // save displacement in these components of f

      	printP("Cgcns:interfaceRightHandSide: save acceleration - FINISH ME!\n");
      ...
      // getBoundaryIndex(mg.gridIndexRange(),side,axis,Ib1,Ib2,Ib3);
            ...
            if( debug() & 2 )
                printP("Cgcns::interfaceRHS: Get normal force (traction) at t=%9.3e\n",gf[gfIndex].t);

            if( interfaceDataOptions & Parameters::tractionInterfaceData )
            {
      	if( debug() & 2 )
        	  printP("Cgcns:interfaceRightHandSide: Save the interface traction at t=%8.2e in components Ct=[%i,%i].\n",t,Ct.getBase(),Ct.getBound());
	...
	// -- save the interface traction rate --
	  ...
	  // find a previous time value we can use
            ...
            // if tp is the same as t0, look for an earlier time: 
          	    ...
          	    {
            	      printP("Cgcns::interfaceRHS:WARNING: there are no previous history values to compute tractionRate.\n"
	    ...
	    //   -- for real : just assume constant ? 
            ...
            // f(t) = (t-tp)/(t0-tp) *f(t0) + (t0-t)/(t0-tp) *f(tp)
          	    ...
          	    if( debug() & 2 )
            	      printP("Cgcns::interfaceRHS: Extrapolate traction in time: t0=%9.3e, tp=%9.3e, cex1=%9.3e cex2=%9.3e\n",
                 		     t0,tp,cex1,cex2);
	      ...
	      // this does not work exactly anyway since the grid is not at the new time!
              ...
              // do this for now: set to exact 
                            ...
                            printP("Cgcns::interfaceRHS: **** Setting traction at t=%9.3e to exact for TZ ****\n",t);
	      ...
	      // The sign is correct here: normalForce = p*normal 
            ...
            // for 2nd-order accuracy we need two previous levels;
          	    ...
          	    if( orderOfAccuracyTractionRate==1 || im2==-1 )
          	    {
            	      ...
            	      if( debug() & 2 )
            		printP("Cgcns::interfaceRHS: get d(traction)/dt (1st order): t=%9.3e, "
              ...
              // Compute the traction-rate to second order accuracy 
            	      ...
            	      if( dt0<=0. || dt1<=0. )
            	      {
            		printP("Cgcns::interfaceRHS:ERROR: computing traction rate: t0=%9.3e, prev=%i, tp=%9.3e, "
                                              "im2=%i, t2=%9.3e\n",t0,prev,tp,im2,t2);
            		OV_ABORT("error");
            	      }
            	      assert( dt0>0. && dt1>0. );
	      ...
	      //   l2 = (t-t0)*(t-t1)/( (t2-t0)*(t2-t1) );
            	      ...
            	      if( debug() & 2 )
            		printP("Cgcns::interfaceRHS: get d(traction)/dt (2nd order): t=%9.3e, (t0=%9.3e) "
          	    ...
          	    if( debug() & 8 )
          	    {
            	      ::display(f1(I1,I2,I3,Ct),sPrintF("Cgcns::interfaceRHS: old traction at tp=%9.3e",tp),"%8.2e ");
            	      ::display(f(I1,I2,I3,Ct),sPrintF("Cgcns::interfaceRHS: new traction at t=%9.3e ",t),"%8.2e ");
            	      ::display(f(I1,I2,I3,Ctr),sPrintF("Cgcns::interfaceRHS: time derivative of the traction, t=%9.3e",t),"%8.2e ");
          	    }
        	  }
      	...
      	{
        	  printP("Cgcns::interfaceRightHandSide:traction: t=%9.3e There is NO time history. "
...
// #endif
            ...
            if( debug() & 8 )
            {
      	f(I1,I2,I3,Ct).display("Cgcns::interfaceRightHandSide: Here is the RHS (traction=normalForce)");
            }
	...
	//   subtract off TZ flow:
        ...
        // ::display(f(I1,I2,I3,Ct),"Cgcns::interface fluid traction before TZ fix");
        ...
        // ::display(f(I1,I2,I3,Ct),"Cgcns::interface fluid traction after TZ fix");
    ...
    {
        printF("Cgcns::interfaceRightHandSide:unexpected interfaceType=%i\n",interfaceType(side,axis,grid));
        OV_ABORT("error");
    }
