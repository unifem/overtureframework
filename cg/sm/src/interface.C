                  			...
                  			int gfIndex, real t )
{
  ...
  // *wdh* 081212 CompositeGrid & cg = gf[0].cg;
	...
	// Some solid-mechanics solvers compute the displacements, others the full deformation.	
          ...
          // *********************************************************************************** FIX ME ****************
	  // ** do this for now: here is the position of the interface over time: 
          // ***********************************************************************************************************
                    printP("interface:INFO: set TZ interface position for (grid,side,axis)=(%i,%i,%i) to be x0 + (a0,a1,a2)*t, t=%9.3e\n",grid,side,axis,t);
          // const real av[3]={1.,0.,0.}; //
                    const real av[3]={0.,0.,0.}; //
        	  for( int axis=0; axis<numberOfDimensions; axis++ )
        	  {
          	    f(I1,I2,I3,Cd.getBase()+axis) += xLocal(I1,I2,I3,axis) + av[axis]*t;
	    // f(I1,I2,I3,Cd.getBase()+axis) = xLocal(I1,I2,I3,axis) + av[axis]*t;
        	  }
