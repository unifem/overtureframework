! This file automatically generated from advSmCons3dOrder2r.bf with bpp.
...
c       
      subroutine advSmCons3dOrder2r(nd,n1a,n1b,n2a,n2b,n3a,n3b,nd1a,
     & nd1b,
...
!     --- 3D --- Bills macros
     ...
     & kd)+uzz23r(i1,i2,i3,kd)
      ...
      sm23ru(i1,i2,i3)=c2*(ulaplacian23r(i1,i2,i3,uc))+c1*(uxx23r(i1,
     & i2,i3,uc) + uxy23r(i1,i2,i3,vc)+ uxz23r(i1,i2,i3,wc))
      sm23rv(i1,i2,i3)=c2*(ulaplacian23r(i1,i2,i3,vc))+c1*(uxy23r(i1,
     & i2,i3,uc) + uyy23r(i1,i2,i3,vc)+ uyz23r(i1,i2,i3,wc))
      sm23rw(i1,i2,i3)=c2*(ulaplacian23r(i1,i2,i3,wc))+c1*(uxz23r(i1,
     & i2,i3,uc) + uyz23r(i1,i2,i3,vc)+ uzz23r(i1,i2,i3,wc))
...
c     -(fourth difference)
     ...
     & i1,i2,i3,c) )
      ...
      if( materialFormat.ne.constantMaterialProperties )then
        write(*,'(" ***advSmCons3dOrder2r:ERROR: Finish me for 
     & variable material")')
        stop 11122
      end if
      ...
      if( t.lt.dt )then
        write(*,'(" ***advSmCons3dOrder2r:INFO: constmaterial=",i4," 
     & version=",i2," (4=var)")') constmaterial,version
      end if
     ...
     & diff=",e9.2)') dt,dtOld,dt-dtOld
      ...
      if(version.eq.1) then           ! Bills version constant coeff
       if(addForcing.ne.0) then
        ...
        do j=n2a,n2b
        do i=n1a,n1b
        un(i,j,k,uc)=cu*u(i,j,k,uc)+cum*um(i,j,k,uc)+dtsq*(sm23ru(i,j,
     & k)+f(i,j,k,uc))
        un(i,j,k,vc)=cu*u(i,j,k,vc)+cum*um(i,j,k,vc)+dtsq*(sm23rv(i,j,
     & k)+f(i,j,k,vc))
        un(i,j,k,wc)=cu*u(i,j,k,wc)+cum*um(i,j,k,wc)+dtsq*(sm23rw(i,j,
     & k)+f(i,j,k,wc))
        end do
        end do
        end do
...
!     inner loop
      if(version.eq.2) then           ! constant coeff
       ...
       if(addForcing.ne.0) then
        ...
        un(i,j,k,uc)=cu*u(i,j,k,uc)+cum*um(i,j,k,uc)+dtsq*(f(i,j,k,uc) 
     & + u0 + fxxl2m*(u(ip,j,k,uc)+u(im,j,k,uc)+u(ip,j,k,uc)+u(im,j,k,
     & uc)) + fyym*(u(i,jp,k,uc)+u(i,jm,k,uc)+u(i,jp,k,uc)+u(i,jm,k,
     & uc)) + fzzm*(u(i,j,kp,uc)+u(i,j,km,uc)+u(i,j,kp,uc)+u(i,j,km,
     & uc)) + fxyml*(u(ip,jp,k,vc)-u(ip,jm,k,vc)-u(im,jp,k,vc)+u(im,
     & jm,k,vc)) + fxzml*(u(ip,j,kp,wc)-u(ip,j,km,wc)-u(im,j,kp,wc)+u(
     & im,j,km,wc)))
        un(i,j,k,vc)=cu*u(i,j,k,vc)+cum*um(i,j,k,vc)+dtsq*(f(i,j,k,vc) 
     & + v0 + fxxm*(u(ip,j,k,vc)+u(im,j,k,vc)+u(ip,j,k,vc)+u(im,j,k,
     & vc)) + fyyl2m*(u(i,jp,k,vc)+u(i,jm,k,vc)+u(i,jp,k,vc)+u(i,jm,k,
     & vc)) + fzzm*(u(i,j,kp,vc)+u(i,j,km,vc)+u(i,j,kp,vc)+u(i,j,km,
     & vc)) + fyzml*(u(i,jp,kp,wc)-u(i,jp,km,wc)-u(i,jm,kp,wc)+u(i,jm,
     & km,wc)) + fxyml*(u(ip,jp,k,uc)-u(ip,jm,k,uc)-u(im,jp,k,uc)+u(
     & im,jm,k,uc)))
        un(i,j,k,wc)=cu*u(i,j,k,wc)+cum*um(i,j,k,wc)+dtsq*(f(i,j,k,wc) 
     & + w0 + fxxm*(u(ip,j,k,wc)+u(im,j,k,wc)+u(ip,j,k,wc)+u(im,j,k,
     & wc)) + fyym*(u(i,jp,k,wc)+u(i,jm,k,wc)+u(i,jp,k,wc)+u(i,jm,k,
     & wc)) + fxxl2m*(u(i,j,kp,wc)+u(i,j,km,wc)+u(i,j,kp,wc)+u(i,j,km,
     & wc)) + fyzml*(u(i,jp,kp,vc)-u(i,jp,km,vc)-u(i,jm,kp,vc)+u(i,jm,
     & km,vc)) + fxzml*(u(ip,j,kp,uc)-u(ip,j,km,uc)-u(im,j,kp,uc)+u(
     & im,j,km,uc)))
        end do
        end do
        end do
        ...
        v0=-4.d0*u(i,j,k,vc)*(fxxm+fyyl2m+fzzm)
        w0=-4.d0*u(i,j,k,wc)*(fxxm+fyym+fzzl2m)
        un(i,j,k,uc)=cu*u(i,j,k,uc)+cum*um(i,j,k,uc)+dtsq*(u0 + fxxl2m*
     & (u(ip,j,k,uc)+u(im,j,k,uc)+u(ip,j,k,uc)+u(im,j,k,uc)) + fyym*(
     & u(i,jp,k,uc)+u(i,jm,k,uc)+u(i,jp,k,uc)+u(i,jm,k,uc)) + fzzm*(u(
     & i,j,kp,uc)+u(i,j,km,uc)+u(i,j,kp,uc)+u(i,j,km,uc)) + fxyml*(u(
     & ip,jp,k,vc)-u(ip,jm,k,vc)-u(im,jp,k,vc)+u(im,jm,k,vc)) + fxzml*
     & (u(ip,j,kp,wc)-u(ip,j,km,wc)-u(im,j,kp,wc)+u(im,j,km,wc)))
        un(i,j,k,vc)=cu*u(i,j,k,vc)+cum*um(i,j,k,vc)+dtsq*(v0 + fxxm*(
     & u(ip,j,k,vc)+u(im,j,k,vc)+u(ip,j,k,vc)+u(im,j,k,vc)) + fyyl2m*(
     & u(i,jp,k,vc)+u(i,jm,k,vc)+u(i,jp,k,vc)+u(i,jm,k,vc)) + fzzm*(u(
     & i,j,kp,vc)+u(i,j,km,vc)+u(i,j,kp,vc)+u(i,j,km,vc)) + fyzml*(u(
     & i,jp,kp,wc)-u(i,jp,km,wc)-u(i,jm,kp,wc)+u(i,jm,km,wc)) + fxyml*
     & (u(ip,jp,k,uc)-u(ip,jm,k,uc)-u(im,jp,k,uc)+u(im,jm,k,uc)))
        un(i,j,k,wc)=cu*u(i,j,k,wc)+cum*um(i,j,k,wc)+dtsq*(w0 + fxxm*(
     & u(ip,j,k,wc)+u(im,j,k,wc)+u(ip,j,k,wc)+u(im,j,k,wc)) + fyym*(u(
     & i,jp,k,wc)+u(i,jm,k,wc)+u(i,jp,k,wc)+u(i,jm,k,wc)) + fxxl2m*(u(
     & i,j,kp,wc)+u(i,j,km,wc)+u(i,j,kp,wc)+u(i,j,km,wc)) + fyzml*(u(
     & i,jp,kp,vc)-u(i,jp,km,vc)-u(i,jm,kp,vc)+u(i,jm,km,vc)) + fxzml*
     & (u(ip,j,kp,uc)-u(ip,j,km,uc)-u(im,j,kp,uc)+u(im,j,km,uc)))
        end do
        end do
        end do
       endif
...
!     inner loop
     ...
     & uc)))
        un(i,j,k,vc)=cu*u(i,j,k,vc)+cum*um(i,j,k,vc)+dtsq*(f(i,j,k,vc) 
     & + fxyml*(u(ip,jp,k,uc)-u(ip,jm,k,uc)-u(im,jp,k,uc)+u(im,jm,k,
     & uc)))
        un(i,j,k,wc)=cu*u(i,j,k,wc)+cum*um(i,j,k,wc)+dtsq*(f(i,j,k,wc) 
     & + fxzml*(u(ip,j,kp,uc)-u(ip,j,km,uc)-u(im,j,kp,uc)+u(im,j,km,
     & uc)))

        un(i,j,k,uc) = un(i,j,k,uc) + dtsq*fxyml*(u(ip,jp,k,vc)-u(ip,
     & jm,k,vc)-u(im,jp,k,vc)+u(im,jm,k,vc))
        un(i,j,k,vc) = un(i,j,k,vc) + dtsq*(v0 + fxxm*(u(ip,j,k,vc)+u(
     & im,j,k,vc)+u(ip,j,k,vc)+u(im,j,k,vc)) + fyyl2m*(u(i,jp,k,vc)+u(
     & i,jm,k,vc)+u(i,jp,k,vc)+u(i,jm,k,vc)) + fzzm*(u(i,j,kp,vc)+u(i,
     & j,km,vc)+u(i,j,kp,vc)+u(i,j,km,vc)))
        un(i,j,k,wc) = un(i,j,k,wc) + dtsq*fyzml*(u(i,jp,kp,vc)-u(i,jp,
     & km,vc)-u(i,jm,kp,vc)+u(i,jm,km,vc))

        un(i,j,k,uc) = un(i,j,k,uc) + dtsq*fxzml*(u(ip,j,kp,wc)-u(ip,j,
     & km,wc)-u(im,j,kp,wc)+u(im,j,km,wc))
        un(i,j,k,vc) = un(i,j,k,vc) + dtsq*fyzml*(u(i,jp,kp,wc)-u(i,jp,
     & km,wc)-u(i,jm,kp,wc)+u(i,jm,km,wc))
        un(i,j,k,wc) = un(i,j,k,wc) + dtsq*(w0 + fxxm*(u(ip,j,k,wc)+u(
     & im,j,k,wc)+u(ip,j,k,wc)+u(im,j,k,wc)) + fyym*(u(i,jp,k,wc)+u(i,
     & jm,k,wc)+u(i,jp,k,wc)+u(i,jm,k,wc)) + fxxl2m*(u(i,j,kp,wc)+u(i,
     & j,km,wc)+u(i,j,kp,wc)+u(i,j,km,wc)))
     ...
     & (u(ip,j,k,uc)+u(im,j,k,uc)+u(ip,j,k,uc)+u(im,j,k,uc)) + fyym*(
     & u(i,jp,k,uc)+u(i,jm,k,uc)+u(i,jp,k,uc)+u(i,jm,k,uc)) + fzzm*(u(
     & i,j,kp,uc)+u(i,j,km,uc)+u(i,j,kp,uc)+u(i,j,km,uc)))
        un(i,j,k,vc)=cu*u(i,j,k,vc)+cum*um(i,j,k,vc)+dtsq*(fxyml*(u(ip,
     & jp,k,uc)-u(ip,jm,k,uc)-u(im,jp,k,uc)+u(im,jm,k,uc)))
        un(i,j,k,wc)=cu*u(i,j,k,wc)+cum*um(i,j,k,wc)+dtsq*(fxzml*(u(ip,
     & j,kp,uc)-u(ip,j,km,uc)-u(im,j,kp,uc)+u(im,j,km,uc)))
...
!     variable coeff
      if(version.eq.4) then
       ...
       ! u terms in u eq.
       rhtmpu = rhtmpu +  0.5*dri(0)*(l2mip*duip-l2mim*duim+l2m0*(duip-
     & duim))
       ...
       rhtmpv = rhtmpv +  0.5*dri(1)*(l2mjp*dujp-l2mjm*dujm+l2m0*(dujp-
     & dujm))
       ...
       rhtmpw = rhtmpw +  0.5*dri(2)*(l2mkp*dukp-l2mkm*dukm+l2m0*(dukp-
     & dukm))
       !// w terms in u eq.
     ...
     & dukm))
       ...
       rhtmpv = rhtmpv + fr*fs*(ljp*(ujpkp-ujpkm)-ljm*(ujmkp-ujmkm))
       rhtmpv = rhtmpv + fr*fs*(mkp*(ujpkp-ujmkp)-mkm*(ujpkm-ujmkm))
       un(i,j,k,uc)=cu*u(i,j,k,uc)+cum*um(i,j,k,uc)+dtsq*(rhtmpu+f(i,j,
     & k,uc))
       un(i,j,k,vc)=cu*u(i,j,k,vc)+cum*um(i,j,k,vc)+dtsq*(rhtmpv+f(i,j,
     & k,vc))
       un(i,j,k,wc)=cu*u(i,j,k,wc)+cum*um(i,j,k,wc)+dtsq*(rhtmpw+f(i,j,
     & k,wc))
       end do
       end do
       end do
...
!     Finally Corners
      if((bc(0,0).eq.stressFree).and.(bc(0,1).eq.stressFree).and.(bc(0,
     & 2).eq.stressFree)) then
       do k=n3a,n3a
       do j=n2a,n2a
       do i=n1a,n1a
       rhtmpu=0.
     ...
     & k,wc))
      ...
      if((bc(0,0).eq.stressFree).and.(bc(0,1).eq.stressFree).and.(bc(1,
     & 2).eq.stressFree)) then
       do k=n3b,n3b
       do j=n2a,n2a
       do i=n1a,n1a
     ...
     & k,wc))
      ...
      if((bc(0,0).eq.stressFree).and.(bc(1,1).eq.stressFree).and.(bc(0,
     & 2).eq.stressFree)) then
       do k=n3a,n3a
       do j=n2b,n2b
       do i=n1a,n1a
     ...
     & k,wc))
      ...
      if((bc(0,0).eq.stressFree).and.(bc(1,1).eq.stressFree).and.(bc(1,
     & 2).eq.stressFree)) then
       do k=n3b,n3b
       do j=n2b,n2b
       do i=n1a,n1a
     ...
     & k,wc))
      ...
      if((bc(1,0).eq.stressFree).and.(bc(0,1).eq.stressFree).and.(bc(0,
     & 2).eq.stressFree)) then
       do k=n3a,n3a
       do j=n2a,n2a
       do i=n1b,n1b
     ...
     & k,wc))
      ...
      if((bc(1,0).eq.stressFree).and.(bc(0,1).eq.stressFree).and.(bc(1,
     & 2).eq.stressFree)) then
       do k=n3b,n3b
       do j=n2a,n2a
       do i=n1b,n1b
     ...
     & k,wc))
      ...
      if((bc(1,0).eq.stressFree).and.(bc(1,1).eq.stressFree).and.(bc(0,
     & 2).eq.stressFree)) then
       do k=n3a,n3a
       do j=n2b,n2b
       do i=n1b,n1b
     ...
     & k,wc))
      ...
      if((bc(1,0).eq.stressFree).and.(bc(1,1).eq.stressFree).and.(bc(1,
     & 2).eq.stressFree)) then
       do k=n3b,n3b
       do j=n2b,n2b
       do i=n1b,n1b
