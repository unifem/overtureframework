...
//   }
  ...
  for( int inter=0; inter < interfaceList.size(); inter++ )
  {
	...
	underRelaxGuess=true;
      ...
      if( interfaceType==Parameters::tractionInterface )
      {
        // we currently don't under-relax for traction interfaces
	underRelaxGuess=false;
      }      
	...
	if( idi.interfaceDataList.size()>=1 )
	{
	  ...
	  if( omega != 1. )
	  {
	    ...
	    {
	      printF("interfaceRHS:WARNING: relaxation not applied to a traction interface\n");
	    }
                       ...
                       InterfaceValueEnum saveInterfaceValues /* =doNotSaveInterfaceValues */ )
{
  ...
  for( int inter=0; inter < interfaceList.size(); inter++ )
  {
	...
	{ 
	  ...
	  {
	    if( saveInterfaceValues==saveInterfaceTimeHistoryValues )
	      printP("Cgmp::getInterfaceResiduals: saving time history of traction: interface=%i, t=%9.3e\n",inter,t);
            else
	      printP("Cgmp::getInterfaceResiduals: saving interface iterate of traction: interface=%i, t=%9.3e\n",inter,t);
	  }
