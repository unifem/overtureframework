...
use SquareMapping;
$fluid = SquareMapping->new(); $$fluid{MAPPINGNAME} = "fluid";
$solid = SquareMapping->new(); $$solid{MAPPINGNAME} = "solid";
...
$$fluid{Y_MAX} = $H;
if( $flatPlateIsPeriodic ){ $$solid{BC} = "-1 -1 $bot_isoT_bc $interface_bc"; }else{ $$solid{BC} = "4 5 $bot_isoT_bc $interface_bc"; }
if( $flatPlateIsPeriodic ){ $$fluid{BC} = "-1 -1 $interface_bc $top_isoT_bc"; }else{ $$fluid{BC} = "4 5 $interface_bc $top_isoT_bc"; }
if( $flatPlateIsPeriodic ){ $$fluid{PERIODICITY} = $$solid{PERIODICITY} = "periodicity: axis 0 derivative periodic"; }
$$solid{LINES} = $$fluid{LINES} ="$Nx $Ny";
*# CREATE THE MAPPINGS
create mappings
