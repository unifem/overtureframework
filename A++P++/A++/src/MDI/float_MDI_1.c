...
float* MDI_float_Allocate ( array_domain* Descriptor )
{
  ...
  */
...
#if MDI_DEBUG
  if (APP_DEBUG > 0) printf ("Inside of MDI_float_Allocate \n");
#endif

  if (Size > 0)
  {
    Array_Data_Pointer = (float*) MDI_MALLOC ( Size * sizeof(float) );

    if (Array_Data_Pointer == NULL)
    {
      printf ("HEAP ERROR: Array_Data_Pointer == NULL in float* MDI_float_Allocate (Size = %ld)! \n", Size);
      exit (1);
    }
  }
  else
  {
    ...
    if (Size < 0)
    {
      printf ("ERROR: Size < 0 Size = %d in MDI_float_Allocate! \n",Size);
      exit (1);
    }
#endif
...
void MDI_float_Deallocate ( float* Array_Data_Pointer , array_domain* Descriptor )
{
#if MDI_DEBUG
  if (APP_DEBUG > 0)
    printf ("Inside of MDI_float_Deallocate Memory pointer = %p \n",
      Array_Data_Pointer);
   
  if (Array_Data_Pointer == NULL)
  {
    printf ("ERROR: Array_Data_Pointer == NULL in float* MDI_float_Deallocate! \n");
    exit (1);
  }
#endif
...
void MDI_float_Print_Array ( float* Lhs , array_domain *Lhs_Descriptor , 
			  int Display_Format )
{
