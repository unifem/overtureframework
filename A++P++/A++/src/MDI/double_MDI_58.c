...
void MDI_d_Sum_Array_Along_Axis 
   ...
   {
  /* Initial values in result must be already setup before this function is called */
     /*--------------------------------------------------------*/
...
#if MDI_DEBUG
     if (APP_DEBUG > 0)
          printf ("Inside of MDI_d_Sum_Array_Along_Axis \n");
#endif
...
#  pragma _CRI novector
            ...
            /* ... arrays are only 2d or less ... */
               ...
               printf ("Sorry, Indirect addressing not implemented in MDI_d_Sum_Array_Along_Axis! \n");
               exit(1);
/* iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii*/
             }
            else
             {
            /* Macro_Loop_Structure_Bases_NOT_Equal_Strides_NOT_Equal_Result_Lhs(Result [i0] += Lhs [i1]) */
               /*------------------------------------------------------------------*/
  ...
  /* ... possible fix compressibility and stride for operations such as sum along an axis ... */
     /* start an m4 macro with a comment so that the ifndef will start in the left most column! */

#ifndef NODIMCOMPRESS
  /* ... make sure higher dimensions aren't collapsed into the axis being summed along ... */
     compressible_Lhs[Axis]    = 0;
     compressible_Result[Axis] = 0;
     locstride_Result[Axis]    = 0;
     if (Axis>0)
        {
       /* ... make sure sum axis isn't compressed into a lower dimension ... */
          compressible_Lhs[Axis-1] = 0;
          compressible_Result[Axis-1] = 0;
        }
#else
     locstride_Result[Axis] = 0;
#endif
...
#if 0
            /* Modify definition of Conformable_Result_Lhs using the 
	       assumtion that the Result is always unit stride! */
               /*--------------------------------------------------------*/
     ...
     if (APP_DEBUG)
        {
          for (nd=0;nd<MAXDIMS;nd++)
               ...
               if (Binary_Conformable)
                  ...
                  {
                 /* Commented out until conformable part is implemented! */
                 /* Macro_Loop_Structure_Bases_NOT_Equal_Strides_NOT_Equal_Result_Lhs(Result [i0] = +(Lhs [i1])) */
             }
#endif
             }
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         if (Mask_Array_Pointer[Address_Mask]) Result[Address_Result] = +(Lhs[Address_Lhs]);
...
#  pragma _CRI novector
               ...
               printf ("Sorry, Indirect addressing (with where mask) not implemented in MDI_d_Sum_Array_Along_Axis! \n");
               exit(1);
/* iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii*/
             }
            else
             {
            /* Macro_Where_Loop_Structure_Bases_NOT_Equal_Strides_NOT_Equal_Result_Lhs(Result [i0] = +(Lhs [i1])) */
               printf ("Sorry, Direct addressing (with where mask) not implemented in MDI_d_Sum_Array_Along_Axis! \n");
               exit(1);
             }
        }
   }
