...
Diagnostic_Manager::report()
  ...
  // This turns off the smartReleaseOfMemory mechanism within A++/P++
  // We have to turn this off within this function because the number of 
  // arrays might already have been counted down to zero.
     bool smartReleaseOfMemorySet = FALSE;
     if (getSmartReleaseOfInternalMemory() == TRUE)
        {
          setSmartReleaseOfInternalMemory(OFF);
          smartReleaseOfMemorySet = TRUE;
	}
  ...
  // Now that we are through the report functions (and any A++/P++ array 
  // objects that it had to build we can prepare to release internal memory)
     if (smartReleaseOfMemorySet == TRUE)
        {
          setSmartReleaseOfInternalMemory(ON);
	}
   }
...
Optimization_Manager::Initialize_Virtual_Machine ( 
  ...
  // NOTE: ==== Only print P++ header info if flag -pppHeaders is provided *wdh* 2013/11/05 
     bool printHeaderInfo=false;
     for( int i=1; i<argc; i++ )
     {
       if( strcmp(argv[i],"-pppHeaders")==0 )
       {
     ...
     if (strcmp(Application_Program_Name,"") == 0)   // meaning equal to ""
        {
          if( printHeaderInfo ) printf ("No application program name specified (searching internally for correct name ...) \n");
#if defined(PVM)
...
#elif defined(PXX_ENABLE_MP_INTERFACE_MPI)
          pathName[0] = 0;
          Application_Program_Name = strcat(pathName,argv[0]);
#endif
          if( printHeaderInfo ) printf ("Application_Program_Name = %s \n",Application_Program_Name);
        }
       else
        {
          if( printHeaderInfo ) printf ("Application_Program_Name set to something (Application_Program_Name = %s) \n",Application_Program_Name);
        }
#endif
#endif
...
#if defined(SERIAL_APP)
     // *wdh* 100924 -- add (char*) : we should change Communication_Manager to take a const char *
     if (Communication_Manager_Pointer == NULL)
       Communication_Manager_Pointer = new Communication_Manager ((char*)Application_Program_Name,Number_Of_Processors,argc,argv);
  ...
  //           Application_Program_Name,Number_Of_Processors);
     if( printHeaderInfo ) printf ("P++ Virtual Machine Initialized: \n");
     if( printHeaderInfo ) printf ("     Process Number                 = %d \n", Communication_Manager::My_Process_Number);

     if( printHeaderInfo ) printf ("     UNIX process ID                = %d \n",getpid());
  ...
  // Call MPI function to get host name
     ...
     if( printHeaderInfo ) printf ("     Process located on machine     = %s \n",namehost);
     if( printHeaderInfo ) printf ("     Number_Of_Processors           = %d \n", Communication_Manager::Number_Of_Processors);
#if defined(AUTO_INITIALIZE_APPLICATION_PATH_NAME)
     // ... memory leak, store this so it can be deleted ...
     char* temp_path = getcwd(NULL, 500);
     if( printHeaderInfo ) printf ("     Local directory (for file I/O) = %s  \n", temp_path);
  ...
  // While we redebug the overlap update we want to make the VSG update the default.
     if( printHeaderInfo ) printf ("     Default communication model    = %s \n",
  ...
  // Use these to avoid compiler warnings
     ...
     if( printHeaderInfo ) printf ("A++ Virtual Machine Initialized (Application_Program_Name = %s  Number_Of_Processors = %d) \n",
               Application_Program_Name,Number_Of_Processors);
#endif
...
DiagnosticInfo::getTypeString() const
  ...
  // This function outputs a string based upon the value of the typeCode
     ...
     switch (typeCode)
        {
	case APP_INT_ELEMENT_TYPE:       returnString = (char*)"int";       break;
          case APP_FLOAT_ELEMENT_TYPE:     returnString = (char*)"float";     break;
          case APP_DOUBLE_ELEMENT_TYPE:    returnString = (char*)"double";    break;
          case APP_UNDEFINED_ELEMENT_TYPE: returnString = (char*)"undefined"; break;
          default:
