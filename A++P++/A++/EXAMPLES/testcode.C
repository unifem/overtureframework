...
// define SUN_CXX_COMPILER "CC"
// if (APP_CXX_Compiler == SUN_CXX_COMPILER)
// if (APP_CXX_Compiler)

// This sort of cpp string compare does not work (is not legal -- I think)
#if (APP_ARCH == "solaris" && APP_CXX_Compiler == "CC")
...
#error "ERROR: USE_PPP IS DEFINED (tested preprocessor macro done only on Solaris with CC)!"
#else
#error "CORRECT: USE_PPP NOT DEFINED (tested preprocessor macro done only on Solaris with CC)!"
#endif
#endif
...
// DQ: bugfix 6/23/2000 (we have to decrement the reference count on the data since the user deletes it)
// The use must know that in this case BB should not be referenced any more (since the memory for the data
// in the array object has been returned).
// BB.decrementRawDataReferenceCount();
   delete bptr;
...
//APP_ABORT();
...
#if 1
  //===========================================================
  //================= Kyle Chand's bug of 7oct99 ==============
  // " Problems with indirect addressing when one dimension has a scalar index."
  //   consider two A++ arrays
  //     array(2,n)     slice(m)
  //   array(0,slice) gives me array(0, slice(0)), array(1,slice(1)), etc.
  //   In other words, with a scalar index A++ is not computing
  //   the first index correctly when creating the new array object.

  intArray templ(2,4);

  templ(0, 0) = 0;  // templ =  0   1   2  -1
  templ(0, 1) = 1;  //          2   3   0  -1
  templ(0, 2) = 2;
  templ(0, 3) = -1;
  ...
  emslice(0) = 0;  // emslice = 0   1
  emslice(1) = 1;

  emslice.display("emslice");

  // Set res to a slice of templ.  Did we get the right one?
  res = templ(0,emslice); // res should = templ(0,0) templ(0,1) = 0   1
  //                    but bug had res = templ(0,0) templ(1,1) = 0   3
  if ( res(0)==templ(0,0) && res(1)==templ(0,1) ) {
    cout << "Chand's scalar indexing test is ok" << endl;
  }
  else {
    cout << "ERROR: FAILED Chand's scalar indexing test" << endl;
    res.display("res");
...
// If we have choosen to exit from the Global release function then it should 
// be an error to return from main with ZERO (since we want it to force a error)
