...
/*
   DQ (1/2/2002): modified configuration to avoid running configure in PARTI directory
   and so we no longer build this PARTI specitif config.h file.  The only relavant macro
   is PARTI_ENABLE_MP_INTERFACE_MPI so we will just define it explicitly (it is always set
   the same way since we only use MPI presently).
