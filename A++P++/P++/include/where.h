...
// IBM xlC compiler needs these declared in the same scope as where they are used
#if defined(PPP) || defined(SERIAL_APP)
int where_function ( const intSerialArray & Mask );
int elsewhere_function ( const intSerialArray & Mask );
#endif
int where_function ( const intArray & Mask );
