...
#if 0
  printf ("Commented out code below %d \n",__LINE__);
#else
  //============================================================
...
// Bugfix (11/20/2000) the partitioning object is a reference counted object 
// and so must have its reference count checked before it can be deleted.
// delete partition;
...
// and so must have its reference count checked before it can be deleted.
// delete P;
// printf ("P->getReferenceCount() = %d \n",P->getReferenceCount());
   APP_ASSERT (P->getReferenceCount() >= Internal_Partitioning_Type::getReferenceCountBase());
   P->decrementReferenceCount();
   if (P->getReferenceCount() < Internal_Partitioning_Type::getReferenceCountBase())
      {
     // printf ("calling delete P \n");
        delete P;
      }
   P = NULL;
