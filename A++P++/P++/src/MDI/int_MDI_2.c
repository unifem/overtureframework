...
void MDI_Build_Index_Map_Array
          ( int *Result , int *Lhs ,
   ...
   {
   /* 
     This function initializes the elements of the result array with 
     the index positions of the nonzero entries of the Lhs array.  It is 
     used to build an index map into an array.  Typically it is used on 
     the intArray result from a relational operator!
     For example: intArray Index_Map = (A == 5).Build_Index_Map();  
     Index_Map is an array (of length cooresponding to the number of 
     elements of A with value 5) with values equal to the index position 
     in A where the values was 5.  Thus function only makes sense in 1D, 
     so we only handle that case!

     This function requires that the Result array have a base of zero.
     This is set by the calling function! 
   */
...
#if MDI_DEBUG
     if (APP_DEBUG > 0)
          printf ("Inside of MDI_Build_Index_Map_Array \n");
#endif
     ...
     Indirect_Addressing_Used = Lhs_Descriptor ->Uses_Indirect_Addressing;
          ...
          if (Indirect_Addressing_Used)
            ...
            /* Strides NOT Equal and Bases NOT Equal */
               ...
               printf ("Indirect address may be incorrect in MDI_Build_Index_Map_Array! \n");
               printf ("Exiting .... \n");
               exit (1);

            /* Macro_Indirect_Addressing_Loop_Structure_Bases_NOT_Equal_Strides_NOT_Equal_Lhs(if(Lhs[Address_Lhs]) Result[Counter++] = index_data_ptr_Lhs[0][i1]) */
             }
            else
             {
               /*--------------------------------------------------------*/
          ...
          for (nd=0;nd<MAXDIMS;nd++)
            ...
            /*
               if (Binary_Conformable)
                  {
                    Macro_Loop_Structure_Binary_Conformable(if(Lhs [i1]) Result [Counter++] = i1)
                  }
                 else
             */
                  {
                 /* Macro_Loop_Structure_Bases_Equal_Strides_Equal_Lhs(if(Lhs [i1]) Result[Counter++] = i1) */
                 /* Macro_Loop_Structure_Bases_Equal_Strides_Equal_Lhs( */
                    /*------------------------------------------------------------------*/
               ...
               for (i1  = Lo_Lhs_I; i1 <= Hi_Lhs_I; i1 += Stride_Lhs_I)
                  {
                    ...
                    if(Lhs [i1]) 
	               {
                      /* (6/14/2000): Adjust the index to get only the longest axis contribution */
                      /* int icntr = i1 - Sclstride_Lhs[1] * j1 - Lo_Lhs_J; */
                         int icntr = i1 - Sclstride_Lhs[1] * j1;
                         for (nd=2; nd < locndim; nd++)
                              icntr -= ICounter[nd] * Sclstride_Lhs[nd];
                         Result[Counter] = icntr + Lhs_Descriptor->Data_Base[0];
                         if (locndim > 1)
                              Result[Counter + Sclstride_Result[1]] = j1 + Lhs_Descriptor->Data_Base[1];
                         for (nd=2; nd < locndim; nd++)
                              Result[Counter + Sclstride_Result[1]*nd] = ICounter[nd] + Lhs_Descriptor->Data_Base[nd];
                         Counter++;
	               }
                    ;
                    i0 += Stride_Result_I; 
...
/********************************************************************/
     ...
     Indirect_Addressing_Used = Mask_Descriptor ->Uses_Indirect_Addressing 
          ...
          if (Indirect_Addressing_Used)
            ...
            /* Macro_Indirect_Addressing_Where_Loop_Structure_Bases_NOT_Equal_Strides_NOT_Equal_Lhs(if(Lhs[Address_Lhs]) Result[Counter++] = Lhs_Index_Data_I[i1]) */
               /*------------------------------------------------------------------*/
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         if (Mask_Array_Pointer[Address_Mask]) if(Lhs[Address_Lhs]) Result[Counter++] = index_data_ptr_Lhs[0][i1];
                      /* for (nd=1;nd<MAXDIMS;nd++)*/
                         for (nd=0;nd<MAXDIMS;nd++)
                            {
#if MDI_DEBUG
...
#  pragma _CRI novector
            ...
            /* ... arrays are only 2d or less ... */
             ...
             {
            /* Macro_Where_Loop_Structure_Bases_NOT_Equal_Strides_NOT_Equal_Lhs(if(Lhs [i1]) Result[Counter++] = i1) */
               /*------------------------------------------------------------------*/
               ...
               for (i1  = Lo_Lhs_I; i1 <= Hi_Lhs_I; i1 += Stride_Lhs_I)
                  {
                    ...
                    if (Mask_Array_Pointer[i3]) if(Lhs [i1])
	               {
                         int icntr = i1 - Sclstride_Lhs[1] * j1;
                         for (nd=2;nd<locndim;nd++)
                              icntr -= ICounter[nd] * Sclstride_Lhs[nd];
...
/********************************************************************/
  ...
  /* iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii*/
  /* Counter-1 acounts for the ++ increment after the last access */
     if (Counter-1 > Result_Descriptor->Bound[0])
        {
          printf ("Error in MDI_Build_Index_Map_Array: Counter > Result_Descriptor->Bound[0] \n");
          exit (1);
        }
   }

/*
Macro_Setup_Function_For_Index_Pointers(Result)
Macro_Setup_Function_For_Index_Pointers(Lhs)
Macro_Setup_Function_For_Index_Pointers(Rhs)
Macro_Setup_Function_For_Index_Pointers(Mask)
*/
#endif
