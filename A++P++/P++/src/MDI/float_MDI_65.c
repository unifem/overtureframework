...
void MDI_f_Array_convertTo_doubleArray_Array_Accumulate_To_Operand 
          ( double *Lhs , float *Rhs , int *Mask_Array_Pointer ,
...
#if MDI_DEBUG
     if (APP_DEBUG > 0)
        printf ("Inside of MDI_f_Array_convertTo_doubleArray_Array_Accumulate_To_Operand \n");
#endif
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         Lhs[Address_Lhs] = (double) Rhs[Address_Rhs];
                      /* for (nd=1;nd<MAXDIMS;nd++)*/
                         for (nd=0;nd<MAXDIMS;nd++)
                            {
#if MDI_DEBUG
     ...
     if (APP_DEBUG)
        {
          ...
          for (nd=0;nd<MAXDIMS;nd++)
		     ...
		     IT IS 20% SLOWER TO USE MEMCPY! */
                  /* memcpy ( Lhs , Rhs , Block_Size_Lhs_L*sizeof(float) ); */
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         if (Mask_Array_Pointer[Address_Mask]) Lhs[Address_Lhs] = (double) Rhs[Address_Rhs];
               ...
               for (i1  = Lo_Lhs_I; i1 <= Hi_Lhs_I; i1 += Stride_Lhs_I)
                  {
                    ...
                    if (Mask_Array_Pointer [i3]) Lhs[i1] = (double) Rhs[i2];
...
void MDI_f_Array_convertTo_doubleArray_Scalar_Accumulate_To_Operand
          ( double *Lhs , float x , int *Mask_Array_Pointer ,
            array_domain *Lhs_Descriptor , 
            array_domain *Mask_Descriptor )
   {
     /*--------------------------------------------------------*/
...
#if MDI_DEBUG
     if (APP_DEBUG > 0)
          printf ("Inside of MDI_f_Array_convertTo_doubleArray_Scalar_Accumulate_To_Operand \n");
#endif
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         if (Mask_Array_Pointer[Address_Mask]) Lhs[Address_Lhs] = (double) x;
                      /* for (nd=1;nd<MAXDIMS;nd++)*/
                         for (nd=0;nd<MAXDIMS;nd++)
                            {
#if MDI_DEBUG
               ...
               for (i1  = Lo_Lhs_I; i1 <= Hi_Lhs_I; i1 += Stride_Lhs_I)
                  {
                    ...
                    if (Mask_Array_Pointer[i3]) Lhs[i1] = (double) x;
                    i3 += Stride_Mask_I; 
                  }
               Offset_Lhs += Sclstride_Lhs[second];
               Offset_Mask += Sclstride_Mask[second];
