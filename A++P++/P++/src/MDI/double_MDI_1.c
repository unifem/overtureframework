...
double* MDI_double_Allocate ( array_domain* Descriptor )
{
  ...
  */
...
#if MDI_DEBUG
  if (APP_DEBUG > 0) printf ("Inside of MDI_double_Allocate \n");
#endif

  if (Size > 0)
  {
    Array_Data_Pointer = (double*) MDI_MALLOC ( Size * sizeof(double) );

    if (Array_Data_Pointer == NULL)
    {
      printf ("HEAP ERROR: Array_Data_Pointer == NULL in double* MDI_double_Allocate (Size = %ld)! \n", Size);
      exit (1);
    }
  }
  else
  {
    ...
    if (Size < 0)
    {
      printf ("ERROR: Size < 0 Size = %d in MDI_double_Allocate! \n",Size);
      exit (1);
    }
#endif
...
void MDI_double_Deallocate ( double* Array_Data_Pointer , array_domain* Descriptor )
{
#if MDI_DEBUG
  if (APP_DEBUG > 0)
    printf ("Inside of MDI_double_Deallocate Memory pointer = %p \n",
      Array_Data_Pointer);
   
  if (Array_Data_Pointer == NULL)
  {
    printf ("ERROR: Array_Data_Pointer == NULL in double* MDI_double_Deallocate! \n");
    exit (1);
  }
#endif
...
void MDI_double_Print_Array ( double* Lhs , array_domain *Lhs_Descriptor , 
			  int Display_Format )
{
