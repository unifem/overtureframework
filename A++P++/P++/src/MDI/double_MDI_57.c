...
#define MINIMUM_VALUE -DBL_MAX
#elif defined(FLOATARRAY)
#define MINIMUM_VALUE -FLT_MAX
...
double MDI_d_Max_Array_Returning_Scalar
  ...
  /* Initialize to default value of ZERO */
     double Return_Value = MINIMUM_VALUE;
...
#if MDI_DEBUG
     if (APP_DEBUG > 0)
          printf ("Inside of MDI_d_Max_Array_Returning_Int \n");
#endif
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         Return_Value = (Return_Value < Lhs[Address_Lhs]) ? Lhs[Address_Lhs] : Return_Value;
                         for (nd=0;nd<MAXDIMS;nd++)
                            {
#if MDI_DEBUG
                         if (APP_DEBUG > 10)
     ...
     for (i1=0; i1 < Array_Size; i1++)
        {
          ...
          Return_Value = (Return_Value < Lhs [i1]) ? Lhs [i1] : Return_Value;
        }
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         if (Mask_Array_Pointer[Address_Mask]) Return_Value = (Return_Value < Lhs[Address_Lhs]) ? Lhs[Address_Lhs] : Return_Value;
                      /* for (nd=1;nd<MAXDIMS;nd++)*/
                         for (nd=0;nd<MAXDIMS;nd++)
                            {
#if MDI_DEBUG
               ...
               for (i1  = Lo_Lhs_I; i1 <= Hi_Lhs_I; i1 += Stride_Lhs_I)
                  {
                    ...
                    if (Mask_Array_Pointer[i3]) Return_Value = (Return_Value < Lhs [i1]) ? Lhs [i1] : Return_Value;
                    i3 += Stride_Mask_I; 
                  }
               Offset_Lhs += Sclstride_Lhs[second];
