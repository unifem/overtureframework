...
#define  DOUBLEARRAY
/* define(MDI_sign,(d < double) ? -d : d) */
/* Bug fix (5/19/94) sign function was not working properly! */ 
/* define(MDI_sign,(d < 0) ? -double : double) */
/* 
   Bugfix (31/10/97) contributed from Kubo at skubo@totoro.rsn.hp.com
   define CNX_SIGN(L,R) ((L)<0 && (R)>=0) || ((L)>=0 && (R)<0) ? -(R) : (R)
   Result[i1] = CNX_SIGN(Lhs[i1],Rhs[i1]);
   define(MDI_sign,((d)<0 && (double)>=0) || ((d)>=0 && (double)<0) ? -(double) : (double) )
 */
...
#if MDI_DEBUG
     if (APP_DEBUG > 0)
        printf ("Inside of MDI_d_Sign_Array_Of_Array \n");
#endif
...
/* iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii*/
  ...
  /* ... Some examples of parameter definitions are: 
       ...
       $ 4 <==> ' ' ...  */
          ...
          MDI_Indirect_Setup_Lhs_Result_Rhs (Lhs_Descriptor,indr_loclo_Lhs,indr_locbound_Lhs,
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         Result[Address_Result] = ( Lhs[Address_Lhs] < 0 && Rhs[Address_Rhs] >= 0 ) || ( Lhs[Address_Lhs] >= 0 && Rhs[Address_Rhs] < 0 ) ? -Rhs[Address_Rhs] : Rhs[Address_Rhs] ;
...
/* iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii*/
     ...
     for (i1=0; i1 < Array_Size; i1++)
        {
          ...
          Result[i1] = ( Lhs[i1] < 0 && Rhs[i1] >= 0 ) || ( Lhs[i1] >= 0 && Rhs[i1] < 0 ) ? -Rhs[i1] : Rhs[i1] ;
        }
  ...
  /* ... turn off compressibility if both arrays aren't compressible
      in the same dimension ... */

     /*------------------------------------------------------------------*/

  /* ... arrays Lhs,Rhs and Result will be conformable after collapsing
     ...
     for(nd=locndim-2;nd>=0;nd--)
        {
          if (!compressible_Lhs[nd] || !compressible_Rhs[nd] || !compressible_Result[nd])
             {
  ...
  /* ... collapse local dimensions for longest loop and second
     ...
     numcompress = MDI_Compress_Lhs_Result_Rhs (loclo_Lhs,locbound_Lhs,locsize_Lhs,locdim_Lhs,compressible_Lhs,
  ...
  /* offsetrange = locbound_Lhs[longest]; */
     ...
     do {
...
#if MDI_DEBUG
                    if (APP_DEBUG) printf ("i1 = %d i0 = %d i2 = %d \n",i1,i0,i2);
#endif

                    Result[i0] = ( Lhs[i1] < 0 && Rhs[i2] >= 0 ) || ( Lhs[i1] >= 0 && Rhs[i2] < 0 ) ? -Rhs[i2] : Rhs[i2] ;
                    i2 += Stride_Rhs_I; 
                    i0 += Stride_Result_I; 
                  }
       ...
       /* ... if arrays are more than 1d fix offsets, otherwise
          ...
          if (locndim>1)
             {
            /* ... one extra Sclstride was added at end of j loop so remove that first
               then reset by the whole amount incremented by going through j loop ... */
               Offset_Lhs -= Sclstride_Lhs[second];
               Offset_Rhs -= Sclstride_Rhs[second];
               Offset_Result -= Sclstride_Result[second];
...
/* iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii*/
  ...
  /* ... Some examples of parameter definitions are: 
       ...
       $ 4 <==> ' ' ...  */
          ...
          MDI_Indirect_Setup_Lhs_Result_Rhs_Mask (Lhs_Descriptor,indr_loclo_Lhs,indr_locbound_Lhs,
                                                  indr_locstride_Lhs, indr_locsize_Lhs,indr_locdim_Lhs,
                                                  indr_compressible_Lhs,indr_Offset_Lhs,indr_Sclstride_Lhs,
                                                  indr_Offsetreset_Lhs,index_data_ptr_Lhs,Lhs_Local_Map,Base_Offset_Lhs,
                                                  Result_Descriptor,indr_loclo_Result,indr_locbound_Result,
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         if (Mask_Array_Pointer[Address_Mask]) Result[Address_Result] = ( Lhs[Address_Lhs] < 0 && Rhs[Address_Rhs] >= 0 ) || ( Lhs[Address_Lhs] >= 0 && Rhs[Address_Rhs] < 0 ) ? -Rhs[Address_Rhs] : Rhs[Address_Rhs] ;
...
/* iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii*/
  ...
  /* ... arrays Lhs,Rhs,Result and Mask will be conformable after collapsing
       dimensions only if they are all compressible in the 
       same dimension, if not turn off the compressible flag
       so the arrays won't have that dimension collapsed (last dim 
       is never compressible so don't check) ... */
     ...
     for(nd=locndim-2;nd>=0;nd--)
        {
          if (!compressible_Lhs[nd] || !compressible_Rhs[nd] || !compressible_Result[nd] || !compressible_Mask[nd] )
             {
               compressible_Lhs[nd] = 0;
               compressible_Rhs[nd] = 0;
               compressible_Result[nd] = 0;
  ...
  /* ... collapse local dimensions for longest loop and second
     ...
     numcompress = MDI_Compress_Lhs_Result_Rhs_Mask (loclo_Lhs,locbound_Lhs,locsize_Lhs,locdim_Lhs,compressible_Lhs,
                                                     loclo_Result,locbound_Result,locsize_Result,locdim_Result,compressible_Result,
...
#if MDI_DEBUG
                    if (APP_DEBUG) printf ("i1 = %d i0 = %d i2 = %d i3 = %d \n",i1,i0,i2,i3);
#endif

                    if (Mask_Array_Pointer[i3]) Result[i0] = ( Lhs[i1] < 0 && Rhs[i2] >= 0 ) || ( Lhs[i1] >= 0 && Rhs[i2] < 0 ) ? -Rhs[i2] : Rhs[i2] ;
...
#if MDI_DEBUG
     if (APP_DEBUG > 0)
          printf ("Inside of MDI_d_Sign_Array_Of_Scalar \n");
#endif
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         Result[Address_Result] = ( Lhs[Address_Lhs] < 0 && x >= 0 ) || ( Lhs[Address_Lhs] >= 0 && x < 0 ) ? -x : x ;
                      /* for (nd=1;nd<MAXDIMS;nd++)*/
                         for (nd=0;nd<MAXDIMS;nd++)
                            {
#if MDI_DEBUG
     ...
     for (i1=0; i1 < Array_Size; i1++)
        {
          ...
          Result[i1] = ( Lhs[i1] < 0 && x >= 0 ) || ( Lhs[i1] >= 0 && x < 0 ) ? -x : x ;
        }
               ...
               for (i1  = Lo_Lhs_I; i1 <= Hi_Lhs_I; i1 += Stride_Lhs_I)
                  {
                    ...
                    Result[i0] = ( Lhs[i1] < 0 && x >= 0 ) || ( Lhs[i1] >= 0 && x < 0 ) ? -x : x ;
                    i0 += Stride_Result_I; 
                  }
               Offset_Lhs += Sclstride_Lhs[second];
               Offset_Result += Sclstride_Result[second];
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         if (Mask_Array_Pointer[Address_Mask]) Result[Address_Result] = ( Lhs[Address_Lhs] < 0 && x >= 0 ) || ( Lhs[Address_Lhs] >= 0 && x < 0 ) ? -x : x ;
               ...
               for (i1  = Lo_Lhs_I; i1 <= Hi_Lhs_I; i1 += Stride_Lhs_I)
                  {
                    ...
                    if (Mask_Array_Pointer [i3]) Result[i0] = ( Lhs[i1] < 0 && x >= 0 ) || ( Lhs[i1] >= 0 && x < 0 ) ? -x : x ;
...
#if MDI_DEBUG
     if (APP_DEBUG > 0)
        printf ("Inside of MDI_d_Sign_Scalar_Of_Array \n");
#endif
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         Result[Address_Result] = ( x < 0 && Lhs[Address_Lhs] >= 0 ) || ( x >= 0 && Lhs[Address_Lhs] < 0 ) ? -Lhs[Address_Lhs] : Lhs[Address_Lhs] ;
                      /* for (nd=1;nd<MAXDIMS;nd++)*/
                         for (nd=0;nd<MAXDIMS;nd++)
                            {
#if MDI_DEBUG
     ...
     for (i1=0; i1 < Array_Size; i1++)
        {
          ...
          Result[i1] = ( x < 0 && Lhs[i1] >= 0 ) || ( x >= 0 && Lhs[i1] < 0 ) ? -Lhs[i1] : Lhs[i1] ;
        }
               ...
               for (i1  = Lo_Lhs_I; i1 <= Hi_Lhs_I; i1 += Stride_Lhs_I)
                  {
                    ...
                    Result[i0] = ( x < 0 && Lhs[i1] >= 0 ) || ( x >= 0 && Lhs[i1] < 0 ) ? -Lhs[i1] : Lhs[i1] ;
                    i0 += Stride_Result_I; 
                  }
               Offset_Lhs += Sclstride_Lhs[second];
               Offset_Result += Sclstride_Result[second];
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         if (Mask_Array_Pointer[Address_Mask]) Result[Address_Result] = ( x < 0 && Lhs[Address_Lhs] >= 0 ) || ( x >= 0 && Lhs[Address_Lhs] < 0 ) ? -Lhs[Address_Lhs] : Lhs[Address_Lhs] ;
               ...
               for (i1  = Lo_Lhs_I; i1 <= Hi_Lhs_I; i1 += Stride_Lhs_I)
                  {
                    ...
                    if (Mask_Array_Pointer [i3]) Result[i0] = ( x < 0 && Lhs[i1] >= 0 ) || ( x >= 0 && Lhs[i1] < 0 ) ? -Lhs[i1] : Lhs[i1] ;
...
#if MDI_DEBUG
     if (APP_DEBUG > 0)
          printf ("Inside of MDI_d_Sign_Scalar_Of_Array_Accumulate_To_Operand \n");
#endif
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         Lhs[Address_Lhs] = ( x < 0 && Lhs[Address_Lhs] >= 0 ) || ( x >= 0 && Lhs[Address_Lhs] < 0 ) ? -Lhs[Address_Lhs] : Lhs[Address_Lhs] ;
                         for (nd=0;nd<MAXDIMS;nd++)
                            {
#if MDI_DEBUG
                         if (APP_DEBUG > 10)
...
/* iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii*/
             ...
             {
            /* Checking for Binary Conformable operands in this case makes little sense (since they are the same) */
               /*--------------------------------------------------------*/
     ...
     for (i1=0; i1 < Array_Size; i1++)
        {
          ...
          Lhs[i1] = ( x < 0 && Lhs[i1] >= 0 ) || ( x >= 0 && Lhs[i1] < 0 ) ? -Lhs[i1] : Lhs[i1] ;
        }
...
#  pragma _CRI novector
     ...
     Indirect_Addressing_Used = Mask_Descriptor ->Uses_Indirect_Addressing 
          ...
          if (Indirect_Addressing_Used)
             {
/* 790iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii*/
            /* Strides NOT Equal and Bases NOT Equal */
               /*------------------------------------------------------------------*/
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         if (Mask_Array_Pointer[Address_Mask]) Lhs[Address_Lhs] = ( x < 0 && Lhs[Address_Lhs] >= 0 ) || ( x >= 0 && Lhs[Address_Lhs] < 0 ) ? -Lhs[Address_Lhs] : Lhs[Address_Lhs] ;
                      /* for (nd=1;nd<MAXDIMS;nd++)*/
                         for (nd=0;nd<MAXDIMS;nd++)
                            {
#if MDI_DEBUG
               ...
               for (i1  = Lo_Lhs_I; i1 <= Hi_Lhs_I; i1 += Stride_Lhs_I)
                  {
                    ...
                    if (Mask_Array_Pointer[i3]) Lhs[i1] = ( x < 0 && Lhs[i1] >= 0 ) || ( x >= 0 && Lhs[i1] < 0 ) ? -Lhs[i1] : Lhs[i1] ;
                    i3 += Stride_Mask_I; 
                  }
               Offset_Lhs += Sclstride_Lhs[second];
               Offset_Mask += Sclstride_Mask[second];
...
void MDI_d_Sign_Array_Of_Array_Accumulate_To_Operand 
          ( double *Lhs , double *Rhs ,
            ...
            array_domain *Mask_Descriptor )
   {
   /* NOTE THAT THE Lhs IS USED FOR ACCUMULATION! */
...
#if MDI_DEBUG
     if (APP_DEBUG > 0)
          printf ("Inside of MDI_d_Sign_Array_Of_Array_Accumulate_To_Operand \n");
#endif
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         Lhs[Address_Lhs] = ( Lhs[Address_Lhs] < 0 && Rhs[Address_Rhs] >= 0 ) || ( Lhs[Address_Lhs] >= 0 && Rhs[Address_Rhs] < 0 ) ? -Rhs[Address_Rhs] : Rhs[Address_Rhs] ;
                      /* for (nd=1;nd<MAXDIMS;nd++)*/
                         for (nd=0;nd<MAXDIMS;nd++)
                            {
#if MDI_DEBUG
     ...
     for (i1=0; i1 < Array_Size; i1++)
        {
          ...
          Lhs[i1] = ( Lhs[i1] < 0 && Rhs[i1] >= 0 ) || ( Lhs[i1] >= 0 && Rhs[i1] < 0 ) ? -Rhs[i1] : Rhs[i1] ;
        }
               ...
               for (i1  = Lo_Lhs_I; i1 <= Hi_Lhs_I; i1 += Stride_Lhs_I)
                  {
                    ...
                    Lhs[i1] = ( Lhs[i1] < 0 && Rhs[i2] >= 0 ) || ( Lhs[i1] >= 0 && Rhs[i2] < 0 ) ? -Rhs[i2] : Rhs[i2] ;
                    i2 += Stride_Rhs_I; 
                  }
               Offset_Lhs += Sclstride_Lhs[second];
               Offset_Rhs += Sclstride_Rhs[second];
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         if (Mask_Array_Pointer[Address_Mask]) Lhs[Address_Lhs] = ( Lhs[Address_Lhs] < 0 && Rhs[Address_Rhs] >= 0 ) || ( Lhs[Address_Lhs] >= 0 && Rhs[Address_Rhs] < 0 ) ? -Rhs[Address_Rhs] : Rhs[Address_Rhs] ;
               ...
               for (i1  = Lo_Lhs_I; i1 <= Hi_Lhs_I; i1 += Stride_Lhs_I)
                  {
                    ...
                    if (Mask_Array_Pointer [i3]) Lhs[i1] = ( Lhs[i1] < 0 && Rhs[i2] >= 0 ) || ( Lhs[i1] >= 0 && Rhs[i2] < 0 ) ? -Rhs[i2] : Rhs[i2] ;
...
/* Variables used for initialization and debugging mode */
...
#if MDI_DEBUG
     if (APP_DEBUG > 0)
          printf ("Inside of MDI_d_Sign_Array_Of_Scalar_Accumulate_To_Operand \n");
#endif
  ...
  */
     ...
     Indirect_Addressing_Used = Lhs_Descriptor ->Uses_Indirect_Addressing;
          ...
          if (Indirect_Addressing_Used)
             {
/* 900iiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiiii*/
            /* Strides NOT Equal and Bases NOT Equal */
               /*------------------------------------------------------------------*/
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         Lhs[Address_Lhs] = ( Lhs[Address_Lhs] < 0 && x >= 0 ) || ( Lhs[Address_Lhs] >= 0 && x < 0 ) ? -x : x ;
                         for (nd=0;nd<MAXDIMS;nd++)
                            {
#if MDI_DEBUG
                         if (APP_DEBUG > 10)
     ...
     for (i1=0; i1 < Array_Size; i1++)
        {
          ...
          Lhs[i1] = ( Lhs[i1] < 0 && x >= 0 ) || ( Lhs[i1] >= 0 && x < 0 ) ? -x : x ;
        }
                    ...
                    for (nd=1;nd<MAXDIMS;nd++)
                       {
                         ...
                         if (Mask_Array_Pointer[Address_Mask]) Lhs[Address_Lhs] = ( Lhs[Address_Lhs] < 0 && x >= 0 ) || ( Lhs[Address_Lhs] >= 0 && x < 0 ) ? -x : x ;
                      /* for (nd=1;nd<MAXDIMS;nd++)*/
                         for (nd=0;nd<MAXDIMS;nd++)
                            {
#if MDI_DEBUG
               ...
               for (i1  = Lo_Lhs_I; i1 <= Hi_Lhs_I; i1 += Stride_Lhs_I)
                  {
                    ...
                    if (Mask_Array_Pointer[i3]) Lhs[i1] = ( Lhs[i1] < 0 && x >= 0 ) || ( Lhs[i1] >= 0 && x < 0 ) ? -x : x ;
                    i3 += Stride_Mask_I; 
                  }
               Offset_Lhs += Sclstride_Lhs[second];
               Offset_Mask += Sclstride_Mask[second];
