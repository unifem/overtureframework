...
#ifdef GNU
#pragma implementation "p_domain.h"
#endif
...
bool Array_Domain_Type::globalMemoryReleaseAlreadyCalled        = FALSE;

// Support for deferred calling of GlobalMemoryRelease() when using P++
bool Array_Domain_Type::PppIsAvailableToCallGlobalMemoryRelease = FALSE;
bool Array_Domain_Type::letPppCallGlobalMemoryRelease           = FALSE;
#endif
...
GlobalMemoryRelease()
  ...
  // Mark this function as already called (so that this can be tested elsewhere for error checking)
     Array_Domain_Type::globalMemoryReleaseAlreadyCalled = TRUE;
...
Array_Domain_Type::checkForArrayIDOnStack ( int x )
  ...
  // printf ("Array_Domain_Type::checkForArrayIDOnStack (%d) \n",x);
...
Array_Domain_Type::Push_Array_ID ( int x )
  ...
  // For P++ we only track the serial array objects not the serial array objects
     if (Diagnostic_Manager::getTrackArrayData() == TRUE)
       ...
       // have to move this test to another location (the Array_Domain_Type destructor).
...
#if defined(APP) || ( defined(SERIAL_APP) && !defined(PPP) )
void
Array_Domain_Type::setupDiagnostics() 
   {
  ...
  // For P++ we only track the serial array objects not the serial array objects
     if (Diagnostic_Manager::getTrackArrayData() == TRUE)
       ...
       // Diagnostic_Manager::diagnosticInfoArray[arrayIndex] = new DiagnosticInfo( &((Array_Domain_Type&) X) );
...
void
Array_Domain_Type::initializeReferenceCount() 
   {
          ...
          if (getNumberOfArraysInUse() <= numberOfOutstandingArrayObjects)
             {
               if ( (Array_Reference_Count_Array [Array_ID()] < getReferenceCountBase()) ||
                  ...
                  {
                    if (Array_Domain_Type::PppIsAvailableToCallGlobalMemoryRelease == TRUE)
                       {
#if COMPILE_DEBUG_STATEMENTS
                         if ( APP_DEBUG > 1 )
                            {
                              printf ("In ~Array_Domain_Type(): A++ Defering call to GlobalMemoryRelease() until P++ can call it ... \n");
                            }
#endif
                         Array_Domain_Type::letPppCallGlobalMemoryRelease = TRUE;
                       }
                      else
                       {
#if COMPILE_DEBUG_STATEMENTS
     ...
     if (SerialArray_Domain_Type::letPppCallGlobalMemoryRelease == TRUE)
        {
          ...
          if ( APP_DEBUG > 1 )
             {
               printf ("In ~Array_Domain_Type(): P++ calling to GlobalMemoryRelease() now that SerialArray is deleted ... \n");
             }
#endif
          GlobalMemoryRelease();
        }
...
// if defined(USE_EXPRESSION_TEMPLATES)
     DOMAIN_SIZE_Type ExpectedOffset = Base[0];
     for (temp=1; temp < MAX_ARRAY_DIMENSION; temp++)
        {
...
#else
     internalArrayID           = Array_Domain_Type::Pop_Array_ID();
   ...
   const Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type Internal_Index_List )
{
   ...
   {
      printf ("Inside of Array_Domain_Type::Initialize_Domain(const Array_Domain_Type & X,Internal_Index_List) \n");
   }
#endif
     ...
     const Indirect_Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type Indirect_Index_List )
{
   ...
   if (APP_DEBUG > 0)
      {
        printf ("Inside of Array_Domain_Type::Initialize_Domain(const Array_Domain_Type& X,Indirect_Index_List)\n");
      }
#endif
...
// endif
   ...
   if (APP_DEBUG > 3)
     ...
     // display ("Called from Array_Domain_Type::Initialize_Domain (Array_Domain_Type,Indirect_Index_List)");
        printf ("Leaving Array_Domain_Type::Initialize_Domain (const Array_Domain_Type& X,Indirect_Index_List)\n");
      }
#endif
}
...
Array_Domain_Type::Array_Domain_Type ( const Internal_Index & I )
   {
  // printf ("At TOP of Array_Domain_Type constructor \n");
     Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type Internal_Index_List;
  ...
  // printf ("At BASE of Array_Domain_Type constructor \n");
   }
...
#else
     APP_ASSERT (Array_Domain_Type::checkForArrayIDOnStack (internalArrayID) == 0);
#endif
...
#else
     int* StackEntry     = Array_Domain_Type::getTopOfArrayIDStack();
     int localStackDepth = Array_Domain_Type::getDepthOfArrayIDStack();
#endif
...
#else
               APP_ASSERT (Array_Domain_Type::checkForArrayIDOnStack (*StackEntry) == 1);
#endif
               StackEntry--;
             }
        }
