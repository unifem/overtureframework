...
// Avoid the accidental case of leaving the n squared test in a final distribution
...
// int SerialArray_Domain_Type::Memory_Block_Index                = 0;
// const int SerialArray_Domain_Type::Max_Number_Of_Memory_Blocks = MAX_NUMBER_OF_MEMORY_BLOCKS;
// unsigned char *SerialArray_Domain_Type::Memory_Block_List [MAX_NUMBER_OF_MEMORY_BLOCKS];

#if !defined(PPP)
// See note in header file as to how these are used and why they are important!
int  SerialArray_Domain_Type::Array_Reference_Count_Array_Length      = INITIAL_REFERENCE_COUNT_ARRAY_SIZE;
int* SerialArray_Domain_Type::Array_Reference_Count_Array             = NULL;
bool SerialArray_Domain_Type::smartReleaseOfInternalMemory            = FALSE;
bool SerialArray_Domain_Type::exitFromGlobalMemoryRelease             = FALSE;
bool SerialArray_Domain_Type::globalMemoryReleaseAlreadyCalled        = FALSE;

// Support for deferred calling of GlobalMemoryRelease() when using P++
bool SerialArray_Domain_Type::PppIsAvailableToCallGlobalMemoryRelease = FALSE;
bool SerialArray_Domain_Type::letPppCallGlobalMemoryRelease           = FALSE;
#endif

#if !defined(PPP)
// Support for stack operations in the SerialArray_Domain_Type.
// Push and Pop member functions of SerialArray_Domain_Type use these
...
// different compilers.
int  SerialArray_Domain_Type::Max_Stack_Size  = 0;
int  SerialArray_Domain_Type::Stack_Depth     = 0;
int *SerialArray_Domain_Type::Top_Of_Stack    = NULL;
int *SerialArray_Domain_Type::Bottom_Of_Stack = NULL;
#endif

// we only want to declare these once
int  SerialArray_Domain_Type::IndirectAddressingMap_Index = -1;
int* SerialArray_Domain_Type::IndirectAddressingMap_Data [APP_STATIC_NUMBER_OF_MAPS];
int  SerialArray_Domain_Type::IndirectAddressingMap_Minimum_Base [APP_STATIC_NUMBER_OF_MAPS];
int  SerialArray_Domain_Type::IndirectAddressingMap_Maximum_Bound [APP_STATIC_NUMBER_OF_MAPS];
int  SerialArray_Domain_Type::IndirectAddressingMap_Length [APP_STATIC_NUMBER_OF_MAPS];
...
inline
int*
SerialArray_Domain_Type::getDefaultIndexMapData ( int Local_Min_Base, int Local_Max_Bound ) const
   {
...
#if COMPILE_DEBUG_STATEMENTS
          if (APP_DEBUG > 1)
               printf ("SETUP of Map pointers in SerialArray_Domain_Type::getDefaultIndexMapData \n");
#endif
...
void
SerialArray_Domain_Type::setupIndirectAddressingSupport() const
   {
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 0)
          printf ("Inside of SerialArray_Domain_Type::setupIndirectAddressingSupport() \n");
#endif
     ...
     for (int i=0; i < MAX_ARRAY_DIMENSION; i++)
        {
          if ( (usesIndirectAddressing() == TRUE) && (Index_Array[i] != NULL) )
            ...
            // An intArray was used for indexing this axis
               ...
               ((SerialArray_Domain_Type*)(this))->IndexBase        [i] = Index_Array[i]->Array_Descriptor.Array_Domain.Base   [i];
               ((SerialArray_Domain_Type*)(this))->IndexStride      [i] = Index_Array[i]->Array_Descriptor.Array_Domain.Stride [i];
               ((SerialArray_Domain_Type*)(this))->IndexDataPointer [i] = Index_Array[i]->getDataPointer();
#if COMPILE_DEBUG_STATEMENTS
               if (APP_DEBUG > 1)
                  {
                    ...
                    intSerialArray* IndexArrayPointer = NULL;
...
#endif
                    ((SerialArray_Domain_Type*)(this))->IndexBase        [i] = Data_Base[i] + Base[i];
                    ((SerialArray_Domain_Type*)(this))->IndexStride      [i] = 0;
                  }
                 else
                  {
#if COMPILE_DEBUG_STATEMENTS
...
#endif
                    ((SerialArray_Domain_Type*)(this))->IndexBase        [i] = getBase(i);
                    ((SerialArray_Domain_Type*)(this))->IndexStride      [i] = getStride(i);
                  }
            ...
            // get space for IntegerList
               ...
               ((SerialArray_Domain_Type*)(this))->IndexDataPointer [i] = getDefaultIndexMapData (localBase,localBound);
...
void
SerialArray_Domain_Type::New_Function_Loop ()
   {
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 0)
        printf ("Inside of SerialArray_Domain_Type::freeMemoryInUse() \n");
#endif 
...
GlobalMemoryRelease()
  ...
  // Mark this function as already called (so that this can be tested elsewhere for error checking)
     SerialArray_Domain_Type::globalMemoryReleaseAlreadyCalled = TRUE;
...
#if 0
  // These variables do not exist (DQ 4/18/2001)
  // Cleanup the memory used for the stack of Array_IDs in use
...
// This function is only defined for A++ and the P++ serial array objects.

int
SerialArray_Domain_Type::checkForArrayIDOnStack ( int x )
   {
  // Search the stack for the number of times that the input appears in the stack.
  // If the array id is that of an existing array object:
  //      then it should not appear in the stack (returning 0)
  // If the array id is taken from the stack:
  //      then is should be unique (returning 1)

  // Using this function in Test_Consistency() redundent entries are searched for 
  // in the stack (this is very expensive and it's use is controled by the cpp macro
  // USE_ORDER_N_SQUARED_TESTS)

     int numberOfReferences = 0;

#if USE_ORDER_N_SQUARED_TESTS
  // printf ("SerialArray_Domain_Type::checkForArrayIDOnStack (%d) \n",x);
     ...
     if (StackEntry != NULL)
       ...
       // Note Pointer arithmatic
          while (StackEntry > Top_Of_Stack - Stack_Depth)
            ...
            // printf ("In while loop: *StackEntry = %d == x = %d \n",*StackEntry,x);
               if (*StackEntry == x)
                  {
                    numberOfReferences++;
            ...
            // It is ok for x to match an entry in the stack since x might be an entry from the stack!
            // This function just returns the number of entries of value x in the stack.
...
#if !defined(PPP)
void
SerialArray_Domain_Type::Push_Array_ID ( int x )
   {
#if COMPILE_DEBUG_STATEMENTS
  ...
  // Temp code to force failure so we can test call stack in dbx for line info
  // APP_ABORT();
#endif

#if USE_ORDER_N_SQUARED_TESTS
  // This is part of an expensive O(n^2) operation so use it sparingly as a test
     ...
     if (Stack_Depth >= Max_Stack_Size)
       ...
       // Old 4.1.3 systems must malloc and can not allow realloc to take a NULL pointer!
          ...
          APP_ASSERT (Bottom_Of_Stack != NULL);
       ...
       // printf ("Stack depth sufficient for another value! \n");

       // As a special case of when a P++ array is trying to give back it's array id after 
       // all A++/P++memory has been cleaned up don't increment this pointer if it is NULL
       // NOTE: Later we will fix this to not permit a P++ serial array to call the 
       // cleanup of memory but for now we test A++/P++ with this short cut.
          if (Top_Of_Stack != NULL)
               Top_Of_Stack++;
        }

  // printf ("Bottom_Of_Stack = %p Top_Of_Stack = %p \n",Bottom_Of_Stack,Top_Of_Stack);
...
#if 0
  // The adopt function sets the reference count one higher since the user controls 
  // the actual data.  If the data comes from the use of the adopt function then the
  // variable builtUsingExistingData is set to TRUE.
     int referenceCountBase = (builtUsingExistingData == TRUE) ? getReferenceCountBase()+1 : getReferenceCountBase();

  // Error checking
     if (Array_Reference_Count_Array[x] >= referenceCountBase)
        {
          printf ("Error: Array_Reference_Count_Array [%d] = %d \n",x,Array_Reference_Count_Array[x]);
        }
     APP_ASSERT (Array_Reference_Count_Array[x] < referenceCountBase);
#endif
  ...
  // For P++ we only track the serial array objects not the serial array objects
     if (Diagnostic_Manager::getTrackArrayData() == TRUE)
        {
       // Make sure that we have a valid allocated array of pointers to diagnostic information
          APP_ASSERT (Diagnostic_Manager::diagnosticInfoArray != NULL);

       // Since we use a single stack mechanism for all array ids and only build 
       // diagnostic objects for the SerialArray objects - we can't expect all pointers 
       // associated with the Diagnostic_Manager::diagnosticInfoArray array to be valid.
       // APP_ASSERT (Diagnostic_Manager::diagnosticInfoArray[x] != NULL);

       // check the pointer and delete only if non NULL
          if (Diagnostic_Manager::diagnosticInfoArray[x] != NULL)
...
#if 0
       // This test is not consistant with the adopt function and because we can't
       // test the value of the builtUsingExistingData from this static function we 
       // have to move this test to another location (the SerialArray_Domain_Type destructor).
          if (Array_Reference_Count_Array [x] >= APP_REFERENCE_COUNT_BASE)
               APP_ASSERT (Diagnostic_Manager::diagnosticInfoArray[x] != NULL);
#endif
        }
#endif

  // APP_ASSERT (Top_Of_Stack != NULL);

  // See note of special case for P++ (above)
     if (Top_Of_Stack != NULL)
          *Top_Of_Stack = x;
...
#if !defined(PPP)
int
SerialArray_Domain_Type::Pop_Array_ID ()
   {
  // This function provides the stack mechanism for the Array_IDs
     ...
     if (Stack_Depth <= 0)
       ...
       // Get value on stack!
          if (Top_Of_Stack == NULL)
             {
               printf ("This may indicate the construction of array objects after the cleanup of internal memory! \n");
             }
          APP_ASSERT (Top_Of_Stack != NULL);
          Result = *Top_Of_Stack;
          Stack_Depth--;
  ...
  // We can't initialize the reference counts from this function since we don't know if 
  // it is called to get an array Id for a serial or a parallel array and they must be
...
#if 0
#if defined(SERIAL_APP)
  // Case of local serial array in P++
...
#else
  // Case of parallel distributed array in P++
...
#if 0
// This functionality is not placed into a separate function which is called
// explicitly (in each location where Pop_Array_ID was previously called.  This
// allows the Pop_Array_ID function to be static (which is a better design)).
#if defined(APP) || ( defined(SERIAL_APP) && !defined(PPP) )
  // For P++ we only track the serial array objects not the serial array objects
     if (Diagnostic_Manager::getTrackArrayData() == TRUE)
        {
  ...
  // P++ array objects have their own reference counts (in the objects) and their data 
  // IS an A++ array which has its own rference count (in the serial array).  The 
  // SerialArray_Domain_Type::Array_Reference_Count_Array is only used to store the 
  // reference counts for the SERIAL array data which can NOT has a reference count 
  // stored in it's data because it s an array of primative values (no pleace for 
...
#if defined(APP) || ( defined(SERIAL_APP) && !defined(PPP) )
void
SerialArray_Domain_Type::setupDiagnostics() 
   {
  // NOTE: that the internalArrayID must be set before this function is called (because we call Array_ID())
  // For P++ we only track the serial array objects not the serial array objects
     if (Diagnostic_Manager::getTrackArrayData() == TRUE)
        {
          int arrayIndex = Array_ID();
          APP_ASSERT (Diagnostic_Manager::diagnosticInfoArray != NULL);
          APP_ASSERT (Diagnostic_Manager::diagnosticInfoArray[arrayIndex] == NULL);
       // Note: const is cast away
       // Diagnostic_Manager::diagnosticInfoArray[arrayIndex] = new DiagnosticInfo( &((SerialArray_Domain_Type&) X) );
          Diagnostic_Manager::diagnosticInfoArray[arrayIndex] = new DiagnosticInfo(*this);
          APP_ASSERT (Diagnostic_Manager::diagnosticInfoArray[arrayIndex] != NULL);
        }
   }
#endif
...
void
SerialArray_Domain_Type::initializeReferenceCount() 
   {
  // NOTE: that the internalArrayID must be set before this 
  // function is called (because we call Array_ID())
     int Result = Array_ID();
  ...
  // Adjust the length of the array are required to fix the 
  // maximum array ID into the array of reference counts
     if (Result >= Array_Domain_Type::Array_Reference_Count_Array_Length)
          Array_Domain_Type::Allocate_Larger_Reference_Count_Array ( Result );
...
#if defined(APP) || ( defined(SERIAL_APP) && !defined(PPP) )
int
SerialArray_Domain_Type::getNumberOfArraysInUse()
   {
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 0)
        printf ("Inside of SerialArray_Domain_Type::getNumberOfArraysInUse() \n");
#endif 
                    ...
                    if (Array_Reference_Count_Array [i] >= APP_REFERENCE_COUNT_BASE)
                       {
                         if (Diagnostic_Manager::diagnosticInfoArray[i] == NULL)
                            {
                              printf ("Error: Diagnostic_Manager::diagnosticInfoArray[%d] == NULL \n",i);
                            }
...
void
SerialArray_Domain_Type::display ( const char *Label ) const
   {
     printf ("SerialArray_Domain_Type::display() -- %s \n",Label);
  ...
  // parallelDescriptor.display(Label);
     if (parallelPADRE_DescriptorPointer != NULL)
       ...
       // printf ("Call to parallelPADRE_DescriptorPointer->display(Label) commented out! \n");
        ...
        {
          printf ("In SerialArray_Domain_Type::display(%s) ##### parallelPADRE_DescriptorPointer == NULL ##### \n",Label);
        }
#else
...
SerialArray_Domain_Type::~SerialArray_Domain_Type ()
   {
     ...
     if ( (APP_DEBUG > 1) || (Diagnostic_Manager::getReferenceCountingReport() > 0) )
        {
          printf ("Call SerialArray_Domain_Type::destructor! %p Array_ID = %d referenceCount = %d typeCode = %d \n",
       ...
       // Delete the intArrays used to support indirect addressing (for A++ and P++)
          if (Index_Array[i] != NULL)
            ...
            // operator delete no longer decriments the referenceCount.
               ...
               if (APP_DEBUG > 2)
                  {
                    printf ("In SerialArray_Domain_Type::destructor Index_Array[%d] Array_ID = %d referenceCount = %d \n",
                         i,Index_Array[i]->Array_ID(),Index_Array[i]->getReferenceCount());
                    Index_Array[i]->displayReferenceCounts("In SerialArray_Domain_Type::destructor");
                  }
#endif
       ...
       // Delete the intSerialArrays used to support parallel (P++) indirect addressing
          if (Local_Index_Array[i] != NULL)
            ...
            // operator delete no longer decriments the referenceCount.
               ...
               if (APP_DEBUG > 2)
                  {
                    printf ("In SerialArray_Domain_Type::destructor Local_Index_Array[%d] Array_ID = %d referenceCount = %d \n",
                         i,Local_Index_Array[i]->Array_ID(),Local_Index_Array[i]->getReferenceCount());
                    Local_Index_Array[i]->displayReferenceCounts("In SerialArray_Domain_Type::destructor");
                  }
#endif
  ...
  // Pointer to a P++ partitioning object
  // APP_ASSERT(Partitioning_Object_Pointer != NULL);
  // printf ("In ~SerialArray_Domain_Type -- Partitioning_Object_Pointer = %p \n",Partitioning_Object_Pointer);
     if (Partitioning_Object_Pointer != NULL)
        {
#if COMPILE_DEBUG_STATEMENTS
          if (APP_DEBUG > 1)
  ...
  // APP_ASSERT(BlockPartiArrayDecomposition != NULL);
     if (BlockPartiArrayDecomposition != NULL)
       ...
       // printf ("In SerialArray_Domain_Type::~Array_Domain_Type (SerialArray_Domain_Type) --- Calling delete_DECOMP \n");
#if COMPILE_DEBUG_STATEMENTS
          if (APP_DEBUG > 1)
             {
  ...
  // APP_ASSERT(BlockPartiArrayDomain != NULL);
     if (BlockPartiArrayDomain != NULL)
       ...
       // printf ("In SerialArray_Domain_Type::~Array_Domain_Type (SerialArray_Domain_Type) --- Calling delete_DARRAY \n");
...
#if defined(PPP)
  // P++ specific stuff: returning the array id of the parallel array object

  // If we are in the destructor then this is our last chance to give back the array id
  // It should be possible to assert that getReferenceCount() == getReferenceCountBase()
  // bool resetReferenceCountAndReturnArrayID = (getReferenceCount() <= getReferenceCountBase());
  // bool resetReferenceCountAndReturnArrayID = (getRawDataReferenceCount() < getReferenceCountBase());
     int localReferenceCount = SerialArray_Domain_Type::Array_Reference_Count_Array [Array_ID()];

  // The adopt function sets the reference count one higher since the user controls 
  // the actual data.  If the data comes from the use of the adopt function then the
  // variable builtUsingExistingData is set to TRUE.
     int tempBase = SerialArray_Domain_Type::getReferenceCountBase();
     int referenceCountBase = (builtUsingExistingData == TRUE) ? tempBase+1 : tempBase;

     bool resetReferenceCountAndReturnArrayID = (localReferenceCount < referenceCountBase);

  // Error checking
     if (localReferenceCount >= referenceCountBase)
        {
          printf ("Error: localReferenceCount (array id = %d) = %d \n",Array_ID(),localReferenceCount);
        }
     APP_ASSERT (localReferenceCount < referenceCountBase);

#if 0
     if (isView() == TRUE)
        {
       // printf ("Found a view: don't return the array id of a view (let the original owner to that) \n");
          printf ("FOUND A VIEW (id=%d): getReferenceCount() = %d \n",Array_ID(),localReferenceCount);
       // resetReferenceCountAndReturnArrayID = FALSE;
        }
       else
        {
          printf ("NOT A VIEW (id=%d): getReferenceCount() = %d \n",Array_ID(),localReferenceCount);
        }

     printf ("In ~Array_Domain_Type(): resetReferenceCountAndReturnArrayID = %s \n",
               (resetReferenceCountAndReturnArrayID == TRUE) ? "TRUE" : "FALSE");
#endif

  // This is not always true (TESTS/test2000_22.C causes it to be FALSE).
  // This should always be true unless we have a view of the parallel array
  // which is not possible since we only support views of the serial arrays 
  // used within a parallel array.
     APP_ASSERT (resetReferenceCountAndReturnArrayID == TRUE);

#if 1
     if (resetReferenceCountAndReturnArrayID == TRUE)
        {
       // Put used Array ID back on stack for later reuse by other array objects!
          SerialArray_Domain_Type::Push_Array_ID (internalArrayID);
	}
#endif
...
//    (since the destructor is called automatically).
  ...
  // Call this only if in the case of P++ we cannot call GlobalMemoryRelease() from the P++ array object instead
  // Background: A P++ array object might be the last array object and we only want to have the
  // GlobalMemoryRelease() function be called AFTER all A++/P++ operations are completed.
  // if ( (smartReleaseOfInternalMemory == TRUE) && (PppIsAvailableToCallGlobalMemoryRelease == TRUE) )
          ...
          if (getNumberOfArraysInUse() <= numberOfOutstandingArrayObjects)
             {
               if ( (Array_Reference_Count_Array [Array_ID()] < getReferenceCountBase()) ||
                  ...
                  {
                    if (SerialArray_Domain_Type::PppIsAvailableToCallGlobalMemoryRelease == TRUE)
                       {
#if COMPILE_DEBUG_STATEMENTS
                         if ( APP_DEBUG > 1 )
                            {
                              printf ("In ~SerialArray_Domain_Type(): A++ Defering call to GlobalMemoryRelease() until P++ can call it ... \n");
                            }
#endif
                         SerialArray_Domain_Type::letPppCallGlobalMemoryRelease = TRUE;
                       }
                      else
                       {
#if COMPILE_DEBUG_STATEMENTS
                         if ( APP_DEBUG > 1 )
                            {
                              printf ("CALLING GlobalMemoryRelease() from A++ (or Serial A++) \n");
                            }
#endif
                         GlobalMemoryRelease();
                       }
...
#else
  // Often a P++ array is the last to be deleted and if so it is a mistake to clean up 
  // internal memory in the SerialArray code while the P++ array is deleting it's serialArray 
  // object (and before the P++ array object is finished it's descructor). So in this case we 
  // have the call to the serialArray destructor set a flag that it would have called the 
  // function GlobalMemoryRelease() (but not make the call) so that the P++ destructor can do it.
     if (SerialArray_Domain_Type::letPppCallGlobalMemoryRelease == TRUE)
        {
#if COMPILE_DEBUG_STATEMENTS
          if ( APP_DEBUG > 1 )
             {
               printf ("In ~SerialArray_Domain_Type(): P++ calling to GlobalMemoryRelease() now that SerialArray is deleted ... \n");
             }
#endif
          GlobalMemoryRelease();
        }
  ...
  // printf ("In ~SerialArray_Domain_Type Array_ID on the stack is %d \n",SerialArray_Domain_Type::queryNextArrayID());

#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 1)
          printf ("Leaving ~SerialArray_Domain_Type() -- referenceCount = %d \n",referenceCount);
#endif
   }
...
#if defined(PPP)
void
SerialArray_Domain_Type::Initialize_Parallel_Parts_Of_Domain ( const SerialArray_Domain_Type & X )
   {
  ...
  // to some default values for now.
     ...
     if (APP_DEBUG > 1)
        {
          printf ("Inside of SerialArray_Domain_Type::Initialize_Parallel_Parts_Of_Domain() \n");
        }
#endif
  ...
  // serial array object (which we don't know yet so we have to set that later).
     parallelPADRE_DescriptorPointer =
          new PADRE_Descriptor<BaseArray,Array_Domain_Type,SerialArray_Domain_Type>
               ( X.parallelPADRE_DescriptorPointer ,
     ...
     if (BlockPartiArrayDecomposition != NULL)
        {
          APP_ASSERT (BlockPartiArrayDecomposition->referenceCount >= 0);
          BlockPartiArrayDecomposition->referenceCount++;
       // printf ("In SerialArray_Domain_Type::Initialize_Parallel_Parts_Of_Domain -- BlockPartiArrayDecomposition->referenceCount = %d \n",
       //      BlockPartiArrayDecomposition->referenceCount);
        }
     ...
     if (BlockPartiArrayDomain != NULL)
        {
          APP_ASSERT (BlockPartiArrayDomain->referenceCount >= 0);
          BlockPartiArrayDomain->referenceCount++;
       // printf ("In SerialArray_Domain_Type::Initialize_Parallel_Parts_Of_Domain -- BlockPartiArrayDomain->referenceCount = %d \n",
       //      BlockPartiArrayDomain->referenceCount);
        }
  // End of if not USE_PADRE
#endif
...
SerialArray_Domain_Type::Initialize_Domain(
  ...
  // So this becomes a much shorter function
     ...
     if (APP_DEBUG > 3)
        {
          printf ("Inside of SerialArray_Domain_Type::Initialize_Domain(Number_Of_Valid_Dimensions=%d,int*) ",
...
void
SerialArray_Domain_Type::Initialize_Domain ( const Integer_Array_MAX_ARRAY_DIMENSION_Type Integer_List )
   {
     Initialize_Domain (MAX_ARRAY_DIMENSION,Integer_List);

#if COMPILE_DEBUG_STATEMENTS
     Test_Preliminary_Consistency ("Called from constructor SerialArray_Domain_Type(int,int,int,int)");
#endif
   }

int
SerialArray_Domain_Type::computeDimension ( const Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type internalIndexList )
   {
...
int
SerialArray_Domain_Type::computeDimension ( const Integer_Array_MAX_ARRAY_DIMENSION_Type integerList )
   {
...
SerialArray_Domain_Type::Initialize_Domain (
  ...
  // initialization (all others call this function)
     ...
     if (APP_DEBUG > 3)
        {
          printf ("Inside of SerialArray_Domain_Type::Initialize_Domain(Number_Of_Valid_Dimensions=%d,int*) (Array ID=%d) ",
 ...
 // printf ("sizeof(SerialArray_Domain_Type) = %d \n",sizeof(SerialArray_Domain_Type));
    APP_ASSERT ( sizeof(array_domain) == sizeof(SerialArray_Domain_Type) );
#endif
  ...
  // We have to look at ALL the dimensions to figure out if we have a strided view
     ...
     for (temp=0; temp < Number_Of_Valid_Dimensions; temp++)
       ...
       // printf ("Setup i SerialArray_Domain_Type::Initialize_Domain (): User_Base [%d] = %d \n",temp,User_Base[temp]);
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 0)
          printf ("In SerialArray_Domain_Type constructor -- Domain_Dimension = %d \n",Domain_Dimension);
#endif
...
#if defined(PPP)
     internalArrayID           = SerialArray_Domain_Type::Pop_Array_ID();
  // printf ("P++: Called Pop_Array_ID to get an array id (id=%d) \n",internalArrayID);
#else
     internalArrayID           = SerialArray_Domain_Type::Pop_Array_ID();
  // printf ("A++: Called Pop_Array_ID to get an array id (id=%d) \n",internalArrayID);
#endif
...
#if !defined(PPP)
  // This function only records a reference to the array domain
  // if the diagnostic manager is activated by the user
     setupDiagnostics();
#endif
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 5)
          display("Called from bottom of SerialArray_Domain_Type constructor");
  // APP_ABORT();
#endif
   }
...
SerialArray_Domain_Type::Initialize_Domain (
  ...
  // This is all new code as of 11/17/97 (check for bugs)
     ...
     for (temp=0; temp < Number_Of_Valid_Dimensions; temp++)
        {
       // Internal_Index_List[temp]->display("Index in SerialArray_Domain_Type::Initialize_Domain");
...
#if COMPILE_DEBUG_STATEMENTS
     Test_Preliminary_Consistency ("Called from SerialArray_Domain_Type::Initialize_Domain(int,int,int,int,int)");
#endif
   }
 // ... end of PPP ...
#endif
...
#if defined(PPP)
void
SerialArray_Domain_Type::Initialize_Domain( ARGUMENT_LIST_MACRO_INTEGER_AND_CONST_REF_RANGE )
   {
  ...
  // compiler will not warn about unused variables
     ...
     printf ("ERROR: void SerialArray_Domain_Type::Initialize_Domain( ARGUMENT_LIST_MACRO_INTEGER_AND_CONST_REF_RANGE ) not implemented \n");
     APP_ABORT();
   ...
   const Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type Internal_Index_List )
{
   ...
   {
      printf ("Inside of SerialArray_Domain_Type::Initialize_Domain(const SerialArray_Domain_Type & X,Internal_Index_List) \n");
   }
#endif
...
// endif
   ...
   // Is_Contiguous_Data is automatically FALSE if view is non unit stride ( BUG 35 -- from Jeff Saltzman )
  ...
  // This is how the dependence is expressed between the view and the actual array variable.
#if defined(PPP)
  // Get a new array id for the case of P++ arrays so that we can return the array id 
  // without special handling.  This is part of the fix for Stefan's bug where in P++ 
  // array ids from P++ objects were not being handled properly.
  // printf ("P++: Setting the view's (internalArrayID = SerialArray_Domain_Type::Pop_Array_ID()) \n");
     internalArrayID = SerialArray_Domain_Type::Pop_Array_ID();
#else
  // printf ("A++: Setting the view's (internalArrayID = X.internalArrayID) \n");
...
// ******************************************************************************
void
SerialArray_Domain_Type::resetDomainForNullArray ()
   {
  ...
  // the case of a serialArray being a NullArray (because of indexing of the global array).
     ...
     if (APP_DEBUG > 0)
        {
          printf ("Inside of SerialArray_Domain_Type::resetDomainForNullArray ");
        }
#endif
...
// ******************************************************************************
void
SerialArray_Domain_Type::Initialize_Domain ( const SerialArray_Domain_Type & X , 
     const Indirect_Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type Indirect_Index_List )
{
#if COMPILE_DEBUG_STATEMENTS
   if (APP_DEBUG > 0)
      {
        printf ("Inside of SerialArray_Domain_Type::Initialize_Domain(const SerialArray_Domain_Type& X,Indirect_Index_List)\n");
      }
#endif
...
// endif
   ...
   if (APP_DEBUG > 3)
     ...
     // display ("Called from SerialArray_Domain_Type::Initialize_Domain (SerialArray_Domain_Type,Indirect_Index_List)");
        printf ("Leaving SerialArray_Domain_Type::Initialize_Domain (const SerialArray_Domain_Type& X,Indirect_Index_List)\n");
      }
#endif
}

void
SerialArray_Domain_Type::reshape( int Number_Of_Valid_Dimensions , const int* View_Sizes, const int* View_Bases)
   {
...
SerialArray_Domain_Type::SerialArray_Domain_Type ( 
     ...
     const Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type Index_List )
   {
     ...
     if (APP_DEBUG > 0)
        {
          printf ("Inside of SerialArray_Domain_Type::SerialArray_Domain_Type ");
...
#if COMPILE_DEBUG_STATEMENTS
     Test_Consistency ("Called from SerialArray_Domain_Type (SerialArray_Domain_Type,Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type*) \n");
#endif
   }
...
SerialArray_Domain_Type::displayReferenceCounts (const char* label) const
   {
     printf ("     Inside of SerialArray_Domain_Type::displayReferenceCounts(%s) \n",label);
     printf ("          SerialArray_Domain_Type Reference Count (Array id = %d) = %d \n",Array_ID(),getReferenceCount());
  ...
  // Cast away const here
     return *((SerialArray_Domain_Type*) this);
   }

// ************************************************************************
// SerialArray_Domain_Type constructors for indirect addressing
// ************************************************************************
SerialArray_Domain_Type::
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > FILE_LEVEL_DEBUG)
          printf ("Inside of SerialArray_Domain_Type::SerialArray_Domain_Type ( const SerialArray_Domain_Type & X , Internal_Indirect_Addressing_Index* Indirect_Index_List ) (this = %p)\n",this);
#endif
...
//       redundent with the one in the Initialize_Domain function!
     printf ("WARNING: In SerialArray_Domain_Type(SerialArray_Domain_Type,Indirect_Index_List): \n");
     ...
     for (i=0; i < MAX_ARRAY_DIMENSION; i++)
        {
          if (Indirect_Index_List[i] != NULL)
             {
               APP_ASSERT ( (Indirect_Index_List[i]->intSerialArrayInStorage == FALSE) || 
                            (Indirect_Index_List[i]->IndexInStorage == FALSE) );
               if (Indirect_Index_List[i]->intSerialArrayInStorage == TRUE)
                  {
                 // printf ("In indirect addressing constructor: Axis = %d Indirect_Index_List[%d]->intSerialArrayInStorage == TRUE \n",i,i);
                    Initialize_Indirect_Addressing_View_Of_Target ( i , *(Indirect_Index_List[i]->IndirectionSerialArrayPointer) , X );
                  }
                 else
                  {
...
// endif
       ...
       // display ("Axis by axis -- intermediate Domain in constructor SerialArray_Domain_Type (SerialArray_Domain_Type,Indirect_Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type) \n");
        }
...
#if COMPILE_DEBUG_STATEMENTS
     Test_Consistency ("Called from SerialArray_Domain_Type (SerialArray_Domain_Type,Internal_Indirect_Addressing_Index*)");
#endif

#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > FILE_LEVEL_DEBUG)
          printf ("Leaving SerialArray_Domain_Type::SerialArray_Domain_Type ( const SerialArray_Domain_Type & X , Internal_Indirect_Addressing_Index* Indirect_Index_List ) (this = %p)\n",this);
#endif
...
SerialArray_Domain_Type::SerialArray_Domain_Type() 
  ...
  // This builds the NULL Array_Domain (used in NULL arrays)!
     ...
     if (APP_DEBUG > 0)
        {
          printf ("Inside of SerialArray_Domain_Type::SerialArray_Domain_Type() this = %p \n",this);
...
void
SerialArray_Domain_Type::parseString ( const char* dataString, int & pos , int dimentionArray[] )
   {
...
// SerialArray_Domain_Type::SerialArray_Domain_Type ( const char* dataString )
SerialArray_Domain_Type::SerialArray_Domain_Type ( const AppString & dataString )
   {
  // This builds an array from a string
     ...
     if (APP_DEBUG > 0)
        {
          printf ("Inside of SerialArray_Domain_Type::SerialArray_Domain_Type(const char*) this = %p \n",this);
        }
#endif
...
SerialArray_Domain_Type::SerialArray_Domain_Type ( const Internal_Index & I )
   {
  // printf ("At TOP of SerialArray_Domain_Type constructor \n");
     Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type Internal_Index_List;
  ...
  // printf ("At BASE of SerialArray_Domain_Type constructor \n");
   }
#if (MAX_ARRAY_DIMENSION >= 2)
SerialArray_Domain_Type::SerialArray_Domain_Type ( const Internal_Index & I, const Internal_Index & J )
   {
     Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type Internal_Index_List;
     Internal_Index_List[0] = &((Internal_Index&) I);
...
SerialArray_Domain_Type::SerialArray_Domain_Type( const Integer_Array_MAX_ARRAY_DIMENSION_Type Integer_List )
   {
     Initialize_Domain (MAX_ARRAY_DIMENSION,Integer_List);
...
SerialArray_Domain_Type::SerialArray_Domain_Type( ARGUMENT_LIST_MACRO_INTEGER )
   {
     INTEGER_ARGUMENTS_TO_INTEGER_LIST_MACRO
     Initialize_Domain (MAX_ARRAY_DIMENSION,Integer_List);
...
#if defined(PPP)
SerialArray_Domain_Type::SerialArray_Domain_Type( ARGUMENT_LIST_MACRO_INTEGER_AND_CONST_REF_RANGE )
   {
...
SerialArray_Domain_Type::SerialArray_Domain_Type ( 
     ...
     const Internal_Partitioning_Type & Internal_Partition )
   {
  // I.display("Internal_Index in SerialArray_Domain_Type::SerialArray_Domain_Type(I,Internal_Partition)");
     Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type Internal_Index_List;
     Internal_Index_List[0] = &((Internal_Index&) I);
...
int
SerialArray_Domain_Type::getGhostBoundaryWidth ( int axis ) const
   {
...
void 
SerialArray_Domain_Type::setInternalGhostCellWidth ( 
     Integer_Array_MAX_ARRAY_DIMENSION_Type Integer_List )
   {
#if defined(PPP) 
  // This function modifies both the parallel and the serial Array_Domain
     ...
     if (APP_DEBUG > 0)
        {
          printf ("Inside of SerialArray_Domain_Type::setInternalGhostCellWidth -- ");
          for (temp=0; temp < MAX_ARRAY_DIMENSION; temp++)
...
#if defined(PPP) 
void
SerialArray_Domain_Type::partition ( const Internal_Partitioning_Type & Internal_Partition )
   {
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 0)
          printf ("Inside of SerialArray_Domain_Type::partition ( const Internal_Partitioning_Type & Internal_Partition ) \n");
#endif
  ...
  // Now delete the Parti specific data structures related to the old partitioning
     if (BlockPartiArrayDecomposition != NULL)
        {
       // printf ("In SerialArray_Domain_Type::partition (Internal_Partitioning_Type) --- Calling delete_DECOMP \n");
          delete_DECOMP (BlockPartiArrayDecomposition);
          BlockPartiArrayDecomposition = NULL;
        }
     APP_ASSERT (BlockPartiArrayDecomposition == NULL);
     ...
     if (BlockPartiArrayDomain != NULL)
        {
       // printf ("In SerialArray_Domain_Type::partition (Internal_Partitioning_Type) --- Calling delete_DARRAY \n");
          delete_DARRAY (BlockPartiArrayDomain);
          BlockPartiArrayDomain = NULL;
        }
     APP_ASSERT (BlockPartiArrayDomain == NULL);
  ...
  // Test_Consistency ("Called from SerialArray_Domain_Type::partition(Internal_Partitioning_Type)");
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 0)
          printf ("Leaving SerialArray_Domain_Type::partition \n");
#endif
   }
  // ... end of PPP ...
#endif
...
// ... only PPP  in i n c l u d e  ...
  ...
  // What PADRE function do we call here?
#else
DARRAY* SerialArray_Domain_Type::
Build_BlockPartiArrayDomain ()
   {
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 0)
          printf ("Inside of SerialArray_Domain_Type::Build_BlockPartiArrayDomain \n");
#endif
  ...
  // APP_ASSERT (Partitioning_Object_Pointer != NULL);
     ...
     if (BlockPartiArrayDecomposition == NULL)
        {
          printf ("In SerialArray_Domain_Type::Build_BlockPartiArrayDomain -- no BlockPartiArrayDecomposition available! \n");
          APP_ABORT();
        }
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 0)
          printf ("Inside of SerialArray_Domain_Type::SerialArray_Domain_Type (const SerialArray_Domain_Type & X, bool,int) \n");
#endif
          ...
          case DEEPCOPY:
            ...
            // Array_Conformability_Info = X.Array_Conformability_Info;
               ...
               for (i=0; i < MAX_ARRAY_DIMENSION; i++)
                 ...
                 // printf ("Setup in SerialArray_Domain_Type COPY CONSTRUCTOR (DEEPCOPY): User_Base [%d] = %d \n",i,User_Base[i]);

                    Scalar_Offset [i] = X.Scalar_Offset[i];

                 // printf ("Setup in SerialArray_Domain_Type COPY CONSTRUCTOR (DEEPCOPY): Scalar_Offset[%d] = %d \n",i,Scalar_Offset[i]);
            ...
            // raw data.  This is required here because the reference counting mechanism reused the
            // value of zero for one existing reference and no references (this will be fixed soon).
               SerialArray_Domain_Type::resetRawDataReferenceCount();
#endif
               ...
               for (temp = 0; temp < MAX_ARRAY_DIMENSION; temp++)
                  {
                    if (Index_Array [temp]) 
                      ...
                      // operator delete no longer decriments the referenceCount.
                         ...
                         if (Index_Array [temp]->getReferenceCount() < intSerialArray::getReferenceCountBase())
...
#if !defined(PPP)
     Test_Consistency ("Called from SerialArray_Domain_Type COPY CONSTRUCTOR #1");
#endif
#endif
   }
...
void
SerialArray_Domain_Type::equalityTest ( bool & returnValue, bool test, const char* description ) const
   {
...
bool
SerialArray_Domain_Type::operator== ( const SerialArray_Domain_Type & X ) const
   {
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 0)
          printf ("Inside of SerialArray_Domain_Type::operator== (const SerialArray_Domain_Type & X) \n");
#endif
...
// It is here to support the use of PADRE.  This function is a copy of the code 
// implemented in the COPY CONSTRUCTOR
SerialArray_Domain_Type::SerialArray_Domain_Type
...
// endif
  ...
  // OR: should the copy constructor always set Array_Conformability_Info = X.Array_Conformability_Info?
#if !defined(PPP)
     Test_Consistency ("Called from SerialArray_Domain_Type COPY CONSTRUCTOR #2");
#endif
#endif
   }

void SerialArray_Domain_Type::normalize()
   {
...
SerialArray_Domain_Type::operator=( const SerialArray_Domain_Type & X )
  ...
  // printf ("In SerialArray_Domain_Type::operator=(): Is_A_Null_Array = %s \n",(Is_A_Null_Array) ? "TRUE" : "FALSE");
  ...
  // set to non null uninitialized garbage so test will be incorrect ...
     if (!Is_A_Null_Array)
	  ...
	  if (BlockPartiArrayDecomposition != NULL) 
             {
            // printf ("In SerialArray_Domain_Type::operator= (SerialArray_Domain_Type) --- Calling delete_DECOMP \n");
	       delete_DECOMP(BlockPartiArrayDecomposition); 
               BlockPartiArrayDecomposition = NULL;
             }
            else
	  ...
	  if (BlockPartiArrayDomain != NULL)
             {
            // printf ("In SerialArray_Domain_Type::operator= (SerialArray_Domain_Type) --- Calling delete_DARRAY \n");
	       delete_DARRAY(BlockPartiArrayDomain); 
               BlockPartiArrayDomain = NULL;
             }
            else
  ...
  // Use_Fortran_Indexing = TRUE;
     ...
     for (i=0; i < MAX_ARRAY_DIMENSION; i++)
       ...
       // printf ("Setup in SerialArray_Domain_Type::operator=: User_Base [%d] = %d \n",i,User_Base[i]);

          Scalar_Offset [i] = X.Scalar_Offset [i];

       // printf ("Setup in SerialArray_Domain_Type::operator=: Scalar_Offset[%d] = %d \n",i,Scalar_Offset[i]);
...
// endif
  ...
  // printf ("In SerialArray_Domain_Type::operator=: Scalar_Offset not reinitialized! \n");
...
#if !defined(PPP)
  // printf ("Inside of SerialArray_Domain_Type::operator=() internalArrayID = %d \n",internalArrayID);
     if (internalArrayID >= Array_Reference_Count_Array_Length)
          Allocate_Larger_Reference_Count_Array ( internalArrayID );
#endif

#if COMPILE_DEBUG_STATEMENTS
     Test_Consistency ("Called from SerialArray_Domain_Type::operator=");
#endif
...
int
SerialArray_Domain_Type::Array_Size () const
   {
...
#if defined(PPP)
bool
SerialArray_Domain_Type::isLeftPartition( int Axis ) const
   {
...
bool
SerialArray_Domain_Type::isMiddlePartition( int Axis ) const
   {
  // Left_Number_Of_Points and Right_Number_Of_Points start counting at the left and right most
  // ghost boundaries which on small arrays (with the adjacent processor having a number of
  // elements in the partition equal to the ghost boundary width) can mean that a partition might
          ...
          if ( Axis < Domain_Dimension)
             {
               ...
               if (Result && Is_A_Null_Array)
                  {
                    printf ("Inside of SerialArray_Domain_Type::isMiddlePartition -- Middle_Processor is a NULL_ARRAY \n");
                  }
             }
            else
             {
...
bool
SerialArray_Domain_Type::isRightPartition( int Axis ) const
   {
  // Left_Number_Of_Points and Right_Number_Of_Points start counting at the left and right most
  // ghost boundaries which on small arrays (with the adjacent processor having a number of
  // elements in the partition equal to the ghost boundary width) can mean that a partition might
...
bool
SerialArray_Domain_Type::isNonPartition( int Axis ) const
   {
...
//---------------------------------------------------------------------
int
SerialArray_Domain_Type::findProcNum ( int* indexVals ) const
   {
...
int*
SerialArray_Domain_Type::setupProcessorList(
...
//---------------------------------------------------------------------
int*
SerialArray_Domain_Type::setupProcessorListOnPosition(
...
//---------------------------------------------------------------------
int
SerialArray_Domain_Type::getRawDataSize( int Axis ) const
   {
...
void
SerialArray_Domain_Type::getRawDataSize( int* Data_Size ) const
   {
...
bool
SerialArray_Domain_Type::isSameBase ( const SerialArray_Domain_Type & X ) const
   {
...
bool
SerialArray_Domain_Type::isSameBound ( const SerialArray_Domain_Type & X ) const
   {
  // This function is a dimension independent test of equality
...
bool
SerialArray_Domain_Type::isSameStride ( const SerialArray_Domain_Type & X ) const
   {
  // This function is a dimension independent test of equality
...
bool
SerialArray_Domain_Type::isSameLength ( const SerialArray_Domain_Type & X ) const
   {
  // This function is a dimension independent test of equality
...
#if defined(APP) || defined(PPP)
bool
SerialArray_Domain_Type::isSameGhostBoundaryWidth ( const SerialArray_Domain_Type & X ) const
   {
  // This function is a dimension independent test of equality
...
bool
SerialArray_Domain_Type::isSameDistribution ( const SerialArray_Domain_Type & X ) const
   {
     bool Return_Value = TRUE;
...
bool
SerialArray_Domain_Type::isSimilar ( const SerialArray_Domain_Type & X ) const
   {
  // This function is a dimension independent test of equality
  // between the bases of each axis of two array descriptors.
...
int
SerialArray_Domain_Type::computeArrayDimension ( int* Array_Sizes )
   {
#if 1
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 6)
        {
          printf ("Inside of SerialArray_Domain_Type::computeArrayDimension --- \n");
          for (int i=0; i < MAX_ARRAY_DIMENSION; i++)
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 6)
          printf ("Inside of SerialArray_Domain_Type::computeArrayDimension (int*) -- Array_Dimension = %d \n",
               Array_Dimension);
#endif
...
int
SerialArray_Domain_Type::computeArrayDimension ( const SerialArray_Domain_Type & X )
   {
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 6)
          printf ("Inside of SerialArray_Domain_Type::computeArrayDimension () \n");
  ...
  // Remember that Sizes in the SerialArray_descriptor_Type are stored already multiplied by 
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 6)
          printf ("Calling SerialArray_Domain_Type::computeArrayDimension () \n");
#endif
...
#if !defined(PPP)
void
SerialArray_Domain_Type::Allocate_Larger_Reference_Count_Array ( int New_Size )
   {
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 1)
          printf ("Inside of SerialArray_Domain_Type::Allocate_Larger_Reference_Count_Array (%d) current size = %d \n",
...
void
SerialArray_Domain_Type::Compute_Local_Index_Arrays ( const SerialArray_Domain_Type & originalDomain )
   {
  ...
  // support of P++ indirect addressing.
     ...
     if (APP_DEBUG > 0)
        {
          printf ("Inside of SerialArray_Domain_Type::Compute_Local_Index_Arrays()");
...
// This function is specific for use with P++ indirect addressing!
void
SerialArray_Domain_Type::Compute_Local_Index_Arrays ( const SerialArray_Domain_Type & Original_Domain )
   {
     int i; // Index value used below

#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 0)
          printf ("Inside of SerialArray_Domain_Type::Compute_Local_Index_Arrays() (internalArrayID = %d)\n",internalArrayID);
#endif
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 0)
          printf ("Inside of SerialArray_Domain_Type::Compute_Local_Index_Arrays() -- Caching of local intArray usage not implemented yet! \n");
#endif
  ...
  // Valid_Local_Entries_Mask.display("Inside of SerialArray_Domain_Type::Compute_Local_Index_Arrays -- Valid_Entries_Mask");
...
#if defined(PPP)
void
SerialArray_Domain_Type::Update_Parallel_Information_Using_Old_Domain ( const SerialArray_Domain_Type & Old_Array_Domain )
   {
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 5)
          printf ("Inside of SerialArray_Domain_Type::Update_Parallel_Information_Using_Old_Domain! \n");
#endif
  // Copy relevant data from old descriptor (this should be a function)

  // printf ("In SerialArray_Domain_Type::Update_Parallel_Information_Using_Old_Domain - INCREMENT REFERENCE COUNTS OF THESE OBJECTS!!! \n");
...
// ***************************************************************************
void
SerialArray_Domain_Type::Test_Preliminary_Consistency( const char *Label ) const
   {
#if (EXTRA_ERROR_CHECKING == FALSE)
     printf ("A++ version (EXTRA_ERROR_CHECKING) was incorrectly built since SerialArray_Domain_Type::Test_Preliminary_Consistency (%s) was called! \n",Label);
     APP_ABORT();
#endif

#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 5)
          printf ("Inside of SerialArray_Domain_Type::Test_Preliminary_Consistency! (Label = %s) (Domain_Dimension = %d) \n",
  ...
  // APP_ASSERT(internalArrayID < Array_Reference_Count_Array_Length);
     if (internalArrayID >= Array_Reference_Count_Array_Length)
        {
          printf ("ERROR: internalArrayID >= Array_Reference_Count_Array_Length in SerialArray_Domain_Type::Test_Preliminary_Consistency \n");
          APP_ABORT();
        }
#endif
  ...
  // so we might as well test it since it has would have caught a P++ bug I just fixed
     for (temp = 0; temp < MAX_ARRAY_DIMENSION; temp++)
       ...
       // APP_ASSERT(Uses_Indirect_Addressing || (Base[temp] >= 0));
          if ( Base[temp] < 0 )
             {
               display("ERROR in SerialArray_Domain_Type::Test_Preliminary_Consistency");
             }
	  ...
	  //  is used but it doesn't seem to matter ...
          if (!Uses_Indirect_Addressing)
	     {
               for (temp = 0; temp < MAX_ARRAY_DIMENSION; temp++)
                  {
#if 0
                    printf ("In SerialArray_Domain_Type::Test_Preliminary_Consistency: temp = %d Global_Index[%d].getLength() = %d  Local_Mask_Index[%d].getLength() = %d \n",
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 5)
          printf ("Leaving SerialArray_Domain_Type::Test_Preliminary_Consistency! (Label = %s) \n",Label);
#endif
   }

void
SerialArray_Domain_Type::Test_Distribution_Consistency ( const char *Label ) const
   {
#if (EXTRA_ERROR_CHECKING == FALSE)
     printf ("A++ version (EXTRA_ERROR_CHECKING) was incorrectly built since SerialArray_Domain_Type::Test_Distribution_Consistency (%s) was called! \n",Label);
     APP_ABORT();
#endif
     ...
     if (APP_DEBUG > 5)
        {
          printf ("Inside of SerialArray_Domain_Type::Test_Distribution_Consistency! (Label = %s) \n",Label);
        }
#endif
       ...
       // but there are many special cases where it fails!
          ...
          if (PartitionObjectPADRE_DistributionPointer != 
              ArrayDomainObjectPADRE_DistributionPointer)
             {
               ...
               printf ("SerialArray_Domain_Type::Test_Distribution_Consistency: Failed test PartitionObjectPADRE_DistributionPointer == ArrayDomainObjectPADRE_DistributionPointer! \n");
  ...
  // implement one that is similar.
     ...
     if (BlockPartiArrayDomain != NULL)
        ...
        {
       // printf ("In SerialArray_Domain_Type::Test_Distribution_Consistency(): BlockPartiArrayDomain == NULL \n");
        }
#endif
#endif
   }
...
void
SerialArray_Domain_Type::Test_Consistency( const char *Label ) const
   {
#if (EXTRA_ERROR_CHECKING == FALSE)
     printf ("A++ version (EXTRA_ERROR_CHECKING) was incorrectly built since SerialArray_Domain_Type::Test_Consistency (%s) was called! \n",Label);
     APP_ABORT();
#endif
...
#   error "PPP is NOT defined"
     ...
     if (APP_DEBUG > 5)
        {
       // printf ("Inside of SerialArray_Domain_Type::Test_Consistency! (Label = %s) (Domain_Dimension = %d) \n", Label,Domain_Dimension);
          printf ("Inside of SerialArray_Domain_Type::Test_Consistency! (Label = %s) \n",Label);
          printf ("Domain_Dimension = %d \n",Domain_Dimension);
        }
#endif
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 5)
          printf ("Inside of SerialArray_Domain_Type::Test_Consistency!  PASSED Description Check \n");
#endif
  ...
  // The current id is in use so it should not be on the stack!
#if defined(PPP)
     APP_ASSERT (SerialArray_Domain_Type::checkForArrayIDOnStack (internalArrayID) == 0);
#else
     APP_ASSERT (SerialArray_Domain_Type::checkForArrayIDOnStack (internalArrayID) == 0);
#endif

  // Now check all the entries in the stack (a more sophisticated test)

  // DQ (4/18/2001) Bugfix
  // Get the top of the stack: we use only ONE stack mechanism (implemented in the A++ 
  // Array_Domain_Type or the P++ SerialArray_Domain_Type depending upon if we are 
  // using A++ or P++)
  // int* StackEntry = Top_Of_Stack;
#if defined(PPP)
     int* StackEntry     = SerialArray_Domain_Type::getTopOfArrayIDStack();
     int localStackDepth = SerialArray_Domain_Type::getDepthOfArrayIDStack();
#else
     int* StackEntry     = SerialArray_Domain_Type::getTopOfArrayIDStack();
     int localStackDepth = SerialArray_Domain_Type::getDepthOfArrayIDStack();
#endif
     const int* localTopOfStack = StackEntry;

     if (StackEntry != NULL)
        {
       // NOTE: Pointer arithmetic
          while (StackEntry > localTopOfStack - localStackDepth)
             {
            // printf ("In while loop: *StackEntry = %d == internalArrayID = %d \n",*StackEntry,internalArrayID);
            // printf ("*StackEntry     = %d \n",*StackEntry);

            // Perform our test for each entry in the stack (each entry should appear only once!)
#if defined(PPP)
               APP_ASSERT (SerialArray_Domain_Type::checkForArrayIDOnStack (*StackEntry) == 1);
#else
               APP_ASSERT (SerialArray_Domain_Type::checkForArrayIDOnStack (*StackEntry) == 1);
#endif
               StackEntry--;
  ...
  // Reference this variable so that purify can check that it is initialized!
     ...
     if (Is_A_Null_Array == TRUE)
        ...
        // printf ("In SerialArray_Domain_Type::Test_Consistency(): Constant_Unit_Stride == %s \n",(Constant_Unit_Stride == TRUE) ? "TRUE" : "FALSE");
...
#if defined(PPP)
  // printf ("In SerialArray_Domain_Type::Test_Consistency(): Checking SerialArray_Domain_Type::Array_Reference_Count_Array[%d] = %d \n",
  //      internalArrayID,SerialArray_Domain_Type::Array_Reference_Count_Array[internalArrayID]);

  // Bugfix (11/4/2000)
  // P++ arrays should not use the SerialArray_Domain_Type::Array_Reference_Count_Array
  // for tracking reference counts (we need to detect this since it is an error).
     if (SerialArray_Domain_Type::Array_Reference_Count_Array[internalArrayID] != getReferenceCountBase()-1)
        {
          printf ("ERROR: SerialArray_Domain_Type::Array_Reference_Count_Array[%d] = %d (should be %d) \n",
               internalArrayID,
               SerialArray_Domain_Type::Array_Reference_Count_Array[internalArrayID],
               getReferenceCountBase()-1);
          printf ("SerialArray_Domain_Type::Array_Reference_Count_Array length = %d \n",
               SerialArray_Domain_Type::Array_Reference_Count_Array_Length);
        }
           ...
           for (i=0; i < MAX_ARRAY_DIMENSION; i++)
              {
                if (Local_Mask_Index [i].getMode() != Null_Index)
                     display("Called from SerialArray_Domain_Type::Test_Consistency");
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 5)
          printf ("Leaving SerialArray_Domain_Type::Test_Consistency! (Label = %s) \n",Label);
#endif
   }
...
void
SerialArray_Domain_Type::Error_Checking_For_Index_Operators (
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 0)
          printf ("Inside of SerialArray_Domain_Type::Error_Checking_For_Index_Operators \n");
#endif
  ...
  // objects can be either an intArray or a scalar (integer) so we have to process the list
     ...
     if ( Index_Object_In_Use && Indirect_Addressing_In_Use )
        {
          printf ("ERROR: Can't mix Index objects with intSerialArray object (indirect addressing) indexing! \n");
          APP_ABORT();
        }
               ...
               if ( (Min[i] < Local_Base[i]) || (Max[i] > Local_Bound[i]) )
                  {
                    printf ("\n");
                    printf ("**************************************************************** \n");
                    printf ("ERROR: (SerialArray:Error_Checking_For_Index_Operators) INDEXING OUT OF BOUNDS                      \n");
                    printf ("**************************************************************** \n");
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 3)
          printf ("Leaving SerialArray_Domain_Type::Error_Checking_For_Index_Operators! \n");
#endif
   }
...
SerialArray_Domain_Type::Error_Checking_For_Index_Operators (
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 0)
          printf ("Inside of SerialArray_Domain_Type::Error_Checking_For_Index_Operators(Indirect_Index_List) \n");
#endif
  ...
  // Not sure if this is the correct defaults
     ...
     intSerialArray* intArrayList[MAX_ARRAY_DIMENSION];
     for (i=0; i<MAX_ARRAY_DIMENSION; i++)
        {
          Index_List  [i] = NULL;
          intArrayList[i] = NULL;
  ...
  // objects can be either an intArray or a scalar (integer) so we have to process the list
     for (i = 0; i < MAX_ARRAY_DIMENSION; i++)
        {
          if (Indirect_Index_List[i] != NULL) 
             { 
               if (Indirect_Index_List[i]->intSerialArrayInStorage)
                    intArrayList[i] = Indirect_Index_List[i]->IndirectionSerialArrayPointer;
                 else
                  {
     ...
     for (i = 0; i < MAX_ARRAY_DIMENSION; i++)
       ...
       // We allow the mixing of scalars and intSerialArray indexing (indirect addressing)
 ...
 // Optionally turn off the bounds checking since it adds to the output when debugging!
  ...
  // The fix is to use *Parm0_int instead of *Parm0_int+Data_Base[0] along all axes.
     ...
     for (i=0; i < MAX_ARRAY_DIMENSION; i++)
          if (intArrayList[i] != NULL)
               intArrayList[i]->Test_Consistency("SerialArray_Domain_Type::Error_Checking_For_Index_Operators (intArrayList[i])");
#endif
...
// **************************************************************************
// Old prototype
// void SerialArray::Range_Checking ( int *i_ptr , int *j_ptr , int *k_ptr , int *l_ptr ) const
void
SerialArray_Domain_Type::Error_Checking_For_Scalar_Index_Operators ( 
...
void
SerialArray_Domain_Type::Initialize_View_Of_Target (
   int Axis , const Internal_Index & X , const SerialArray_Domain_Type & Target_Domain )
   {
  ...
  */
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > FILE_LEVEL_DEBUG)
          printf ("Inside of SerialArray_Domain_Type::Initialize_View_Of_Target ( int, Internal_Index, SerialArray_Domain_Type ) \n");

  // We don't want to print this out all the time
     if (APP_DEBUG > FILE_LEVEL_DEBUG+5)
        {
          X.display("Inside of SerialArray_Domain_Type::Initialize_View_Of_Target: X Internal_Index object");
          Target_Domain.display("Inside of SerialArray_Domain_Type::Initialize_View_Of_Target: Target_Domain");
        }
#endif
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > FILE_LEVEL_DEBUG+5)
          Global_Index [Axis].display ("Global_Index [Axis] in SerialArray_Domain_Type::Initialize_View_Of_Target");
#endif
  ...
  // the orginal geometry can be computed (i.e. it is preserved in the view)
     ...
     if (X.Index_Mode == Index_Triplet)
       ...
       // printf ("Setup in SerialArray_Domain_Type::Initialize_View_Of_Target ( int, Internal_Index, SerialArray_Domain_Type ): User_Base[%d] = %d \n",Axis,User_Base[Axis]);
            ...
            else
             {
               if (X.Index_Mode == Null_Index)
                 ...
                 // Geometry_Base [Axis] = Target_Domain.Geometry_Base[Axis];
                  ...
                  {
                    printf ("ERROR: Inside of SerialArray_Domain_Type::Initialize_View_Of_Target ");
...
// ************************************************************************************
void
SerialArray_Domain_Type::Initialize_Non_Indexed_Dimension ( int Axis , const SerialArray_Domain_Type & Target_Domain )
   {
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > FILE_LEVEL_DEBUG)
        {
          printf ("Inside of SerialArray_Domain_Type::Initialize_Non_Indexed_Dimension ");
          printf ("( SerialArray_Domain_Type & Target_Domain , ");
          printf ("int (Axis = %d) ) \n",Axis);
       }
#endif
...
#if 0
     printf ("In SerialArray_Domain_Type::Initialize_Non_Indexed_Dimension(): Scalar_Offset[%d] = %d \n",Axis,Scalar_Offset[Axis]);
     if ( Axis > 0 )
...
// ************************************************************************************
void 
SerialArray_Domain_Type::Initialize_Indirect_Addressing_View_Of_Target (
   int Axis , int x , const SerialArray_Domain_Type & Target_Domain )
   {
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > FILE_LEVEL_DEBUG)
          printf ("Inside of SerialArray_Domain_Type::Initialize_Indirect_Addressing_View_Of_Target ( int, int=%d, SerialArray_Domain_Type ) \n",x);
#endif
...
int Axis , const Internal_Index & X , const SerialArray_Domain_Type & Target_Domain )
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > FILE_LEVEL_DEBUG)
          printf ("Inside of SerialArray_Domain_Type::Initialize_Indirect_Addressing_View_Of_Target ( int, Internal_Index, SerialArray_Domain_Type ) \n");
#endif
  ...
  // I don't know if this is a good test to have here!
     ...
     //printf ("Since indirect addressing is not debugged the following may be incorrect (Initialize_Indirect_Addressing_View_Of_Target(int,Internal_Index,SerialArray_Domain_Type))! \n");
     //APP_ABORT();
...
void
SerialArray_Domain_Type::Initialize_Indirect_Addressing_View_Of_Target (
   int Axis , const intSerialArray & X , const SerialArray_Domain_Type & Target_Domain )
   {
  /* This function Initialized the Size, Base, Bound, and Stride of a view of a 
  // target array object.   It sets the Base to ZERO always, and then Bound to the size of
  // the intSerialArray used to index the SerialArray -1, so that the size of subsequent
  ...
  */
     ...
     if (APP_DEBUG > FILE_LEVEL_DEBUG+5)
        {
          printf ("Inside of SerialArray_Domain_Type::Initialize_Indirect_Addressing_View_Of_Target ( int, intSerialArray, SerialArray_Domain_Type ) \n");
          X.display("X intSerialArray Index object");
        }
#endif
  // Used in Initialize_Non_Indexed_Dimension but set in Initialize_Domain!
     Uses_Indirect_Addressing = TRUE;
  ...
  // Base      [Axis] = APP_Global_Array_Base;
     ...
     if (Target_Domain.Uses_Indirect_Addressing)
        {
       // If indexed with a intSerialArray we assume Geometry_Base [Axis] = 0
       // We have to assume something and we can not afford to search for the
       // minimum of the intSerialArray
...
// ************************************************************************************
void
SerialArray_Domain_Type::Initialize_Indirect_Addressing_View_Of_Target_Non_Indexed_Dimension(
   int Axis , const SerialArray_Domain_Type & X )
   {
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 1)
          printf ("Inside of SerialArray_Domain_Type::Initialize_Indirect_Addressing_View_Of_Target_Non_Indexed_Dimension ( SerialArray_Domain_Type & X , int (Axis = %d) ) \n",Axis);
#endif
...
// ************************************************************************************
void
SerialArray_Domain_Type::Preinitialize_Domain ( const SerialArray_Domain_Type & X )
   {
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > FILE_LEVEL_DEBUG)    
          printf ("Inside of SerialArray_Domain_Type::Preinitialize_Domain ( const SerialArray_Domain_Type & X )\n");
#endif
...
// ************************************************************************************
void
SerialArray_Domain_Type::Initialize_IndirectAddressing_Domain_Using_IndirectAddressing(
   const SerialArray_Domain_Type & X , 
   const Indirect_Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type Indirect_Index_List )
   {
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 1)
          printf ("Inside of SerialArray_Domain_Type::Initialize_IndirectAddressing_Domain_Using_IndirectAddressing ( const SerialArray_Domain_Type & X , ALL PARMETERS )\n");
#endif

     printf ("Inside of SerialArray_Domain_Type::Initialize_IndirectAddressing_Domain_Using_IndirectAddressing \n");
...
#endif
            // Index_Array [i] = new intSerialArray;
               if ( Domain_Dimension == 1 )
                 ...
                 // to INDEX the X.Index_Array in each dimension.
                    if (intArrayList[0] != NULL)
                      ...
                      // actually we only want the size of the Parm0_intSerialArray!
                      // the copying is redundent and inefficient
                         Index_Array[i] = new intSerialArray (*intArrayList[0]);
...
// ************************************************************************************
void
SerialArray_Domain_Type::Initialize_NonindirectAddressing_Domain_Using_IndirectAddressing (
   const SerialArray_Domain_Type & X ,
   const Indirect_Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type Indirect_Index_List )
   {
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 1)
          printf ("Inside of SerialArray_Domain_Type::Initialize_NonindirectAddressing_Domain_Using_IndirectAddressing ( const SerialArray_Domain_Type & X , Indirect_Index_List )\n");
#endif
...
#if COMPILE_DEBUG_STATEMENTS
                    if (APP_DEBUG > 1)
                         printf ("Indirect_Index_List[i]->intSerialArrayInStorage == TRUE \n");
                    APP_ASSERT (Indirect_Index_List[i]->IndirectionSerialArrayPointer != NULL);
                 // Test the integer array object being used as an indirection vector!
                    Indirect_Index_List[i]->IndirectionSerialArrayPointer->Test_Consistency
                         ("Inside of SerialArray_Domain_Type::Initialize_NonindirectAddressing_Domain_Using_IndirectAddressing(SerialArray_Domain_Type,Indirect_Index_Pointer_Array_MAX_ARRAY_DIMENSION_Type)");
#endif
...
#if COMPILE_DEBUG_STATEMENTS
                    if (APP_DEBUG > 1)
                         printf ("Indirect_Index_List[i]->intSerialArrayInStorage == FALSE \n");
#endif
                    if (Indirect_Index_List[i]->IndexInStorage)
                       {
#if COMPILE_DEBUG_STATEMENTS
          ...
          if (intArrayList[i] != NULL)
             {
#if COMPILE_DEBUG_STATEMENTS
               if (APP_DEBUG > FILE_LEVEL_DEBUG)    
                    printf ("Build intArrayList[%d] using intSerialArray \n",i);
#endif
...
#endif
                    APP_ASSERT (Index_Array [i] == NULL);
                    Index_Array [i] = new intSerialArray (*(intArrayList[i]));
                    APP_ASSERT (Index_Array [i] != NULL);
                  }
                 else
                  {
               ...
               if (Diagnostic_Manager::getReferenceCountingReport() > 0)
                 ...
                 // This mechanism outputs reports which allow us to trace the reference counts
                    Index_Array [i]->displayReferenceCounts("In SerialArray_Domain_Type::Initialize_NonindirectAddressing_Domain_Using_IndirectAddressing(): indirection vector (intArray)");
                  }
            ...
            // if (intArrayList[i]->Array_Descriptor.Array_Domain.Is_A_Temporary)
               if (intArrayList[i]->isTemporary())
                 ...
                 // operator delete no longer decriments the referenceCount.
                    ...
                    if (intArrayList [i]->getReferenceCount() < intSerialArray::getReferenceCountBase())
               ...
               if (Diagnostic_Manager::getReferenceCountingReport() > 0)
                  {
                 // This mechanism outputs reports which allow us to trace the reference counts
                    Index_Array [i]->displayReferenceCounts("Index_Array [i] in SerialArray_Domain_Type::Initialize_NonindirectAddressing_Domain_Using_IndirectAddressing");
                  }
#endif
             }
        }
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 1)
          printf ("Leaving SerialArray_Domain_Type::Initialize_NonindirectAddressing_Domain_Using_IndirectAddressing ( const SerialArray_Domain_Type & X , Indirect_Index_List )\n");
#endif
   }


SerialArray_Domain_Type*
SerialArray_Domain_Type::Vectorizing_Domain ( const SerialArray_Domain_Type & X )
   {
  // Build a descriptor to fill in!
     SerialArray_Domain_Type* Return_Domain = new SerialArray_Domain_Type;

#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > FILE_LEVEL_DEBUG)
          printf ("Inside of SerialArray_Domain_Type::Vectorizing_Domain ( const SerialArray_Domain_Type & X ) (this = %p)\n",Return_Domain);
#endif
  ...
  // X.parallelPADRE_DescriptorPointer->incrementReferenceCount();
     ...
     if (X.Is_Contiguous_Data)
       ...
       // intSerialArray Mask ( ((Range*)Internal_Index_List_1) );
          intSerialArray Mask ( Internal_Index_List_1 );

       // Build Map into interior
          intSerialArray *Map = new intSerialArray (Total_Size);
...
#if !defined(USE_EXPRESSION_TEMPLATES)
       // Build intSerialArray to access view where Mask is non-zero
          *Map = Mask.indexMap();
#else
          printf ("Inside of SerialArray_Domain_Type::Vectorizing_Domain call to \n");
...
#if COMPILE_DEBUG_STATEMENTS
     Return_Domain->Test_Consistency ("Called from SerialArray_Domain_Type::Vectorizing_Domain (SerialArray_Domain_Type)");
#endif
...
void
SerialArray_Domain_Type::setBase ( int New_Base_For_All_Axes )
   {
#if COMPILE_DEBUG_STATEMENTS
...
void
SerialArray_Domain_Type::setBase( int New_Base , int Axis )
   {
#if COMPILE_DEBUG_STATEMENTS
  // Enforce bound on Alt_Base depending on INT_MAX and INT_MIN
     static int Max_Base = INT_MAX;
...
#if defined(USE_EXPRESSION_TEMPLATES)
int
SerialArray_Domain_Type::conformsTo ( const Array_Domain_Type & X ) const
   {
...
#if defined(SERIAL_APP)
void
SerialArray_Domain_Type::adjustSerialDomainForStridedAllocation ( int axis,  const Array_Domain_Type & globalDomain )
   {
  // This function is called by the SerialArray_Descriptor_Type::Allocate_Parallel_Array() function
  ...
  // if this is a strided array then this must be false
     Constant_Unit_Stride = FALSE;
  // globalDomain.display("globalDomain in SerialArray_Domain_Type::adjustSerialDomainForStridedAllocation");
...
#if 0
int
SerialArray_Domain_Type::getUnitStrideBound ( int j )
   {
...
#if defined(PPP)
bool
SerialArray_Domain_Type::isLeftNullArray( SerialArray_Domain_Type & serialArrayDomain , int Axis ) const
   {
...
bool
SerialArray_Domain_Type::isRightNullArray( SerialArray_Domain_Type & serialArrayDomain , int Axis ) const
   {
  // A Null array can be part of the left edge or the right edge of a distributed axis
  // This function is helpful in determining which it is.
...
void
SerialArray_Domain_Type::postAllocationSupport ( SerialArray_Domain_Type & serialArrayDomain )
   {
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 0)
          printf ("Inside of SerialArray_Domain_Type::postAllocationSupport ( SerialArray_Domain_Type & X ) (this = %p)\n",this);
#endif
...
int SerialArray_Domain_Type::CLASS_ALLOCATION_POOL_SIZE = DEFAULT_CLASS_ALLOCATION_POOL_SIZE;
/* Static variable */

SerialArray_Domain_Type* SerialArray_Domain_Type::Current_Link                      = NULL;
...
void *SerialArray_Domain_Type::operator new ( size_t Size )
   {
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 1)
          printf ("Call SerialArray_Domain_Type::operator new! Memory_Block_Index = %d \n",Memory_Block_Index);
#endif
  ...
  // static int Memory_Block_Index = 0;
     ...
     if (Size != sizeof(SerialArray_Domain_Type))
        {
       ...
       // If this is an object derived from SerialArray_Domain_Type
       ...
       // printf ("In SerialArray_Domain_Type::operator new: Calling malloc because Size(%d) != sizeof(SerialArray_Domain_Type)(%d) \n",Size,sizeof(SerialArray_Domain_Type));

          return malloc(Size);
        }
       else
        {
       // printf ("In SerialArray_Domain_Type::operator new: Using the pool mechanism Size(%d) == sizeof(SerialArray_Domain_Type)(%d) \n",Size,sizeof(SerialArray_Domain_Type));
...
#if 1
               Current_Link = (SerialArray_Domain_Type*) APP_MALLOC ( CLASS_ALLOCATION_POOL_SIZE * sizeof(SerialArray_Domain_Type) );
#else
               Current_Link = (SerialArray_Domain_Type*) new char [ CLASS_ALLOCATION_POOL_SIZE * sizeof(SerialArray_Domain_Type) ];
#endif
  ...
  // first link as the new object!
     ...
     SerialArray_Domain_Type* Forward_Link = Current_Link;
     Current_Link = Current_Link->freepointer;

#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 0)
          printf ("Returning from SerialArray_Domain_Type::operator new! (with address of %p) \n",Forward_Link);
#endif
...
void SerialArray_Domain_Type::operator delete ( void *Pointer, size_t sizeOfObject )
   {
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 1)
        {
          printf ("In SerialArray_Domain_Type::operator delete: Size(%d)  sizeof(SerialArray_Domain_Type)(%d) \n",sizeOfObject,sizeof(SerialArray_Domain_Type));
        }
#endif
     ...
     free (Pointer);
#else
     if (sizeOfObject != sizeof(SerialArray_Domain_Type))
        {
       // Overture's Grid Function class derives from A++/P++ array objects
       // and so must be able to return a valid pointer to memory when using 
       // even the A++ or P++ new operator.
       ...
       // operator new!  So we just return the following!
          ...
          if (APP_DEBUG > 1)
             {
               printf ("In SerialArray_Domain_Type::operator delete: Calling global delete (free) because Size(%d) != sizeof(SerialArray_Domain_Type)(%d) \n",sizeOfObject,sizeof(SerialArray_Domain_Type));
             }
#endif
        ...
        {
          SerialArray_Domain_Type *New_Link = (SerialArray_Domain_Type*) Pointer;

#if COMPILE_DEBUG_STATEMENTS
          if (APP_DEBUG > 1)
             {
               printf ("In SerialArray_Domain_Type::operator delete (%p): Using the pool mechanism Size(%d) == sizeof(SerialArray_Domain_Type)(%d) \n",Pointer,sizeOfObject,sizeof(SerialArray_Domain_Type));
             }
#endif
          if (New_Link != NULL)
             {
            ...
            // Put deleted object (New_Link) at front of linked list (Current_Link)!
             ...
             {
               printf ("ERROR: In SerialArray_Domain_Type::operator delete - attempt made to delete a NULL pointer! \n");
               APP_ABORT();
             }
#endif
        }
...
#if COMPILE_DEBUG_STATEMENTS
     if (APP_DEBUG > 1)
          printf ("Leaving SerialArray_Domain_Type::operator delete! \n");
#endif
