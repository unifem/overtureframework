...
/* $Id: pgshash.c,v 1.1.1.1 1999/06/09 02:04:07 dquinlan Exp $ */
...
int get_redirection(Hash_Table, PEptr, itemptr, gsTrace_ptrptr)
     HASH_TABLE_ENTRY *Hash_Table;
     int              *PEptr, *itemptr;
...
{ 	
  ...
  int item, PEIndex;
  itemlink *thisslot;
  GS_TRACE_STRUCT gsTrace;
  
  hash = &Hash_Table[*PEptr];
  item = *itemptr;
/*  gsTrace = **gsTrace_ptrptr;
  PEIndex = (gsTrace.CmplOwnerPEOrder)[*PEptr];*/
  thisslot = get_item_from_hash_table(hash, item);
...
void free_hash_table(Hash_Table, nproc)
     HASH_TABLE_ENTRY *Hash_Table;
     int nproc;
{ int p;
