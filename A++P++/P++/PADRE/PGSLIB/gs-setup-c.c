...
/* $Id: gs-setup-c.c,v 1.2 1999/11/22 22:54:18 painter Exp $ */
...
/* S_Buffers is accessed through double indirection, so that
  ...
  if (SendS.N_Send > 0) {
    ...
    pgslib_check_malloc_c((SendS.S_Buffers)[0], "malloc failed in gs_setup_n_duplicate");
...
/* In this case, the number of messages to be sent is N_CmplReceiverPEs, and the number
  ...
  if (SendS.N_Send > 0) {
    ...
    (SendS.S_Buffers)[0] = (SEND_BUFFER *)malloc(sizeof(SEND_BUFFER)*SendS.N_Send);  
    pgslib_check_malloc_c((SendS.S_Buffers)[0], "couldn't malloc SendS.S_Buffers[0] in pgslib_exchang_indices_c");
...
void pgslib_prep_supplement_c(N_SupplementPtr, pes, gsTrace_ptrptr)
	...
	fields in the trace.  The fields that get setup in 
	this routine are:
